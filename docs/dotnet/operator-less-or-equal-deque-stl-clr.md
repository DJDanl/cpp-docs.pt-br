...
Deque menor que ou igual comparação.  
...
## Comentários  
 A função do operador retorna `!(``right` `<` `left``)`.  Você usa para testar se `left` não está ordenado depois de `right` quando os dois deques elemento são comparados pelo elemento.  
