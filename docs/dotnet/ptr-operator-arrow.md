...
Este exemplo implementa uma classe CLR que usa um `com::ptr` encapsular seu membro privado `IXMLDOMDocument` objeto. O `WriteDocument` funções usos `operator->` para chamar o `get_firstChild` membro do objeto document.
