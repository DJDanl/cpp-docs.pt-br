...
## Comentários  
 A função do operador retorna `!(``left` `<` `right``)`.  Use\-a para testar se `left` não está ordenado antes de `right` quando os dois mapas são elemento comparado pelo elemento.  
