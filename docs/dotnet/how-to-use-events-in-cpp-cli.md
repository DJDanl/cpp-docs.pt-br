...
Um evento simples não pode ser especificado para substituir ou ocultar um evento de classe base.  Você deve definir todas as funções de acessador do evento e, em seguida, especificar a **`new`** `override` palavra-chave ou em cada função de acessador.
