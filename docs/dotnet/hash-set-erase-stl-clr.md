...
## Comentários  
 A primeira função de membro remove o elemento de sequência controlada apontada por `where`, e retorna um iterador que designa o primeiro elemento que permanece além de elemento removido, ou [hash\_set::end](../dotnet/hash-set-end-stl-clr.md)`()` se tal elemento existe.  Use\-a para remover um único elemento.  
