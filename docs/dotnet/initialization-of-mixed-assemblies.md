...
ms.locfileid: "70311843"
---
# <a name="initialization-of-mixed-assemblies"></a>Inicialização de assemblies mistos

Os desenvolvedores do Windows sempre devem ter cuidado com o bloqueio do carregador `DllMain`durante a execução do código durante. No entanto, há alguns problemas adicionais a serem considerados ao C++lidar com assemblies de modo misto/CLR.

O código no [DllMain](/windows/win32/Dlls/dllmain) não deve acessar o CLR (Common Language Runtime) do .net. Isso significa que `DllMain` não deve fazer chamadas para funções gerenciadas, direta ou indiretamente; nenhum código gerenciado deve ser declarado ou `DllMain`implementado em; e nenhuma coleta de lixo ou carregamento de biblioteca automático `DllMain` deve ocorrer em .
...
A `DllMain` função é um ponto de entrada definido pelo usuário para uma dll. A menos que o usuário especifique `DllMain` o contrário, é invocado toda vez que um processo ou thread é anexado ou desanexado da dll que o contém. Como essa invocação pode ocorrer enquanto o bloqueio do carregador é mantido, nenhuma função fornecida `DllMain` pelo usuário deve ser compilada para MSIL. Além disso, nenhuma função na árvore de chamada com `DllMain` raiz pode ser compilada para MSIL. Para resolver problemas aqui, o bloco de código que `DllMain` define deve ser modificado com `unmanaged`#pragma. O mesmo deve ser feito para cada função que `DllMain` chama.
...
Como alternativa, se `DllMain` não for necessário ou se não precisar ser executado no bloqueio de carregador, a implementação fornecida `DllMain` pelo usuário poderá ser removida, o que eliminará o problema.
...
Há várias funções fornecidas pelo usuário nas quais as bibliotecas dependem da inicialização durante a inicialização. Por exemplo, ao sobrecarregar C++ operadores globalmente em tais como os `new` operadores e `delete` , as versões fornecidas pelo usuário são usadas em qualquer lugar, incluindo na C++ inicialização e destruição da biblioteca padrão. Como resultado, C++ a biblioteca padrão e inicializadores estáticos fornecidos pelo usuário invocarão todas as versões fornecidas pelo usuário desses operadores.
...
As definições da função de localidade personalizada podem ser compiladas para código nativo usando `unmanaged` a diretiva #pragma.
...
Em casos de seleção, as implementações de função dentro de arquivos de cabeçalho podem complicar o diagnóstico. As funções embutidas e o código do modelo exigem que as funções sejam especificadas em um arquivo de cabeçalho.  O C++ idioma especifica a regra de uma definição, que força a semântica de todas as implementações de funções com o mesmo nome para serem semanticamente equivalentes. Consequentemente, C++ o vinculador não precisa fazer nenhuma consideração especial ao mesclar arquivos de objeto que têm implementações duplicadas de uma determinada função.
...
Como o mesmo cabeçalho pode ser incluído por C++ arquivos com **/CLR** habilitado e desabilitado, ou um #include pode ser encapsulado dentro `#pragma unmanaged` de um bloco, é possível ter versões MSIL e nativas de funções que fornecem implementações em conector. As implementações MSIL e nativa têm semânticas diferentes em relação à inicialização no bloqueio do carregador, o que viola efetivamente a regra de uma definição. Consequentemente, quando o vinculador escolher a maior implementação, ele poderá escolher a versão MSIL de uma função, mesmo que tenha sido compilado explicitamente para código nativo em outro lugar usando a diretiva #pragma não gerenciada. Para garantir que uma versão MSIL de um modelo ou função embutida nunca seja chamada sob o bloqueio do carregador, cada definição de cada função chamada sob o bloqueio do carregador deve `#pragma unmanaged` ser modificada com a diretiva. Se o arquivo de cabeçalho for de terceiros, a maneira mais fácil de fazer essa alteração é enviar por push e pop- `#pragma unmanaged` la sobre a diretiva de #include para o arquivo de cabeçalho incorreto. (Consulte [gerenciado, não gerenciado](../preprocessor/managed-unmanaged.md) por um exemplo.) No entanto, essa estratégia não funcionará para cabeçalhos que contêm outro código que deve chamar diretamente as APIs do .NET.
...
1. Entre `!dumpstack` na janela **imediata** para obter uma lista completa da pilha **/CLR** interna.

1. Procure a primeira instância (mais próxima à parte inferior da pilha) de _CorDllMain (se `DllMain` causar o problema) ou _VTableBootstrapThunkInitHelperStub ou GetTargetForVTableEntry (se um inicializador estático causar o problema). A entrada de pilha logo abaixo dessa chamada é a invocação da função implementada MSIL que tentou executar sob o bloqueio do carregador.
...
O exemplo a seguir mostra como evitar o bloqueio do carregador movendo o `DllMain` código de para o construtor de um objeto global.
