...
|[< do operador (Multimap) (STL/CLR)](#op_lt)|Determina se um `multimap` objeto é menor que outro `multimap` objeto.|
|[operador<= (Multimap) (STL/CLR)](#op_lteq)|Determina se um `multimap` objeto é menor ou igual a outro `multimap` objeto.|
|[Operator = = (Multimap) (STL/CLR)](#op_eq)|Determina se um `multimap` objeto é igual a outro `multimap` objeto.|
|[> do operador (Multimap) (STL/CLR)](#op_gt)|Determina se um `multimap` objeto é maior que outro `multimap` objeto.|
|[operador>= (Multimap) (STL/CLR)](#op_gteq)|Determina se um `multimap` objeto é maior ou igual a outro `multimap` objeto.|
...
Para qualquer elemento `X` que precede `Y` na sequência controlada, `key_comp()(Y, X)` é false. (Para o objeto delegado padrão, as chaves nunca diminuem em valor.) Diferentemente do mapa de classes de modelo [(STL/CLR)](../dotnet/map-stl-clr.md), um objeto de classe de modelo `multimap` não exige que as chaves para todos os elementos sejam exclusivas. (Duas ou mais chaves podem ter uma ordenação equivalente.)
...
A função Operator retornará true se, para a posição mais baixa `i` para a qual `!(right[i] < left[i])` também é verdadeira `left[i] < right[i]` . Caso contrário, ele retornará o `left->size() < right->size()` uso para testar se a *esquerda* é ordenada antes da *direita* quando os dois Multimaps são elementos comparados por elemento.
