...
## Comentários  
 A função do operador retorna `left``.first ==` `right``.first &&` `left``.second ==` `right``.second`.  Use\-a para testar se `left` é ordenado da mesma forma que `right` quando os dois pares são elemento comparado pelo elemento.  
