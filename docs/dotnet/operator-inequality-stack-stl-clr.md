...
## Comentários  
 A função do operador retorna `!(``left` `==` `right``)`.  Use\-a para testar se `left` não está ordenado da mesma forma que `right` quando as duas pilhas elemento são comparados pelo elemento.  
