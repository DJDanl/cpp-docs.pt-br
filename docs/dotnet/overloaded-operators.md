...
Na declaração de um tipo de referência, por exemplo, em vez de usando nativo `operator+` sintaxe, explicitamente se escreve o nome interno subjacente do operador – nesse caso, `op_Addition`. Além disso, a invocação de um operador deve ser chamado explicitamente por meio desse nome, impedindo, portanto, os dois principais benefícios de sobrecarga de operador: (a) a sintaxe intuitiva e (b) a capacidade de combinar novos tipos com os tipos existentes. Por exemplo:
...
Na nova sintaxe, as expectativas usual de um programador de C++ nativo são restauradas, tanto na declaração e uso dos operadores estáticos. Aqui está o `Vector` classe traduzida para a nova sintaxe:
