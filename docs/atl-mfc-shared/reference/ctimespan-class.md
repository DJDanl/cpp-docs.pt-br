 ...
 *timeSpanSrc*  
 ...
 `time`  
 Um **__time64_t** valor de tempo, que é o número de segundos em que o período de tempo. Em versões do Visual C++ 6.0 e versões anteriores, `time` era um valor de `time_t`. Visual C++ .NET ou posterior silenciosamente converte um `time_t` parâmetro **__time64_t**.  
