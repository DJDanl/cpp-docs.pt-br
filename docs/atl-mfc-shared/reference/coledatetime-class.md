---
title: Classe COleDateTime | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[COleDateTime::GetAsSystemTime](#getassystemtime)|Chame esse método para obter a hora no `COleDateTime` objeto como uma [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) estrutura de dados.|  
...
|[COleDateTime::GetDayOfWeek](#getdayofweek)|Retorna o dia da semana isso `COleDateTime` objeto representa (domingo = 1).|  
...
|[COleDateTime::SetDate](#setdate)|Define o valor desse `COleDateTime` objeto ao valor especificado somente Data.|  
|[COleDateTime::SetDateTime](#setdatetime)|Define o valor desse `COleDateTime` objeto para o valor de data/hora especificada.|  
|[COleDateTime::SetStatus](#setstatus)|Define o status (validade) deste `COleDateTime` objeto.|  
|[COleDateTime::SetTime](#settime)|Define o valor desse `COleDateTime` objeto ao valor especificado somente em tempo.|  
  
### <a name="public-operators"></a>Operadores públicos  

|Nome|Descrição|  
|----------|-----------------|  
|[COleDateTime::operator COleDateTime::operator = =<,></,>](#coledatetime_relational_operators)|Comparar dois `COleDateTime` valores.|  
|[COleDateTime::operator + COleDateTime::operator-](#operator_add_-)|Adicionar e subtrair `COleDateTime` valores.|  
|[+ = COleDateTime::operator, COleDateTime::operator =](#operator_add_eq_-_eq)|Adicionar e subtrair um `COleDateTime` valor desse `COleDateTime` objeto.|  
|[COleDateTime::operator =](#operator_eq)|Copia uma `COleDateTime` valor.|  
|[COleDateTime::operator data, data COleDateTime::operator *](#operator_date)|Converte um `COleDateTime` valor em uma `DATE` ou `DATE*`.|  
...
|[COleDateTime::m_status](#m_status)|Contém o status desse `COleDateTime` objeto.|  
...
> [!CAUTION]
>  Na tabela acima, observe que embora valores de dia se tornam negativas antes da meia-noite em 30 de dezembro de 1899, valores de hora do dia, não têm. Por exemplo, 6:00 AM sempre é representada por um valor fracionário 0,25, independentemente se o número inteiro que representa o dia é positivo (após 30 de dezembro de 1899) ou negativo (antes de 30 de dezembro de 1899). Isso significa que uma comparação de ponto flutuante simples erroneamente classifica uma `COleDateTime` que representa 6:00 em 29/12/1899 como **posteriormente** daquele representando 7:00 AM no mesmo dia.  
  
 O `COleDateTime` classe manipula datas de 1 de janeiro de 100 até 31 de dezembro de 9999. O `COleDateTime` classe usa o calendário gregoriano; ele não dá suporte a datas do calendário juliano. `COleDateTime`ignora o horário de verão. (Consulte [data e hora: suporte de automação](../../atl-mfc-shared/date-and-time-automation-support.md).)  
  
> [!NOTE]
>  Você pode usar o `%y` formato para recuperar um ano de dois dígitos apenas para datas começando em 1900. Se você usar o `%y` formato em uma data anterior a 1900, o código gera uma falha de ASSERÇÃO.  
 ...
 Se você criar um `COleDateTime` objeto usando uma data menor que 100, a data é aceitas, mas subsequentes chamadas para `GetYear`, `GetMonth`, `GetDay`, `GetHour`, `GetMinute`, e `GetSecond` falhará e retornará -1. Anteriormente, você poderia usar datas de dois dígitos, mas as datas devem estar 100 ou maior no MFC 4.2 e posterior.  
  
 Para evitar problemas, especifique uma data de quatro dígitos. Por exemplo:  
  
 [!code-cpp[NVC_ATLMFC_Utilities n º&1;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_1.cpp)]  
...
##  <a name="a-namecoledatetimerelationaloperatorsa--coledatetime-relational-operators"></a><a name="coledatetime_relational_operators"></a>Operadores relacionais COleDateTime  
...
### <a name="parameters"></a>Parâmetros  
 `dateSrc`  
 Existente `COleDateTime` o objeto a ser copiado para o novo `COleDateTime` objeto.  
  
 *varSrc*  
 Existente **VARIANT** estrutura de dados (possivelmente um `COleVariant` objeto) a ser convertido em um valor de data/hora ( `VT_DATE`) e copiados para o novo `COleDateTime` objeto.  
  
 `dtSrc`  
 Um valor de data/hora ( **data**) valor a ser copiado para o novo `COleDateTime` objeto.  
  
 `timeSrc`  
 A `time_t` ou **__time64_t** valor a ser convertido em um valor de data/hora e copiado para o novo `COleDateTime` objeto.  
  
 *systimeSrc*  
 A `SYSTEMTIME` estrutura a ser convertido em um valor de data/hora e copiado para o novo `COleDateTime` objeto.  
  
 `filetimeSrc`  
 A `FILETIME` estrutura a ser convertido em um valor de data/hora e copiado para o novo `COleDateTime` objeto. Observe que `FILETIME` usa o tempo Universal Coordenado (UTC), portanto, se você passar uma hora local na estrutura, os resultados serão incorretos. Consulte [arquivo vezes](http://msdn.microsoft.com/library/windows/desktop/ms724290) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] para obter mais informações.  
...
### <a name="remarks"></a>Comentários  
 Todos esses construtores criam novos `COleDateTime` objetos inicializados com o valor especificado. A tabela a seguir mostra os intervalos válidos para cada componente de data e hora:  
...
- `COleDateTime(`**)** Constrói uma `COleDateTime` objeto inicializado como 0 (meia-noite, 30 de dezembro de 1899).  
  
- `COleDateTime(``dateSrc` **)** Constrói uma `COleDateTime` objeto a partir de um existente `COleDateTime` objeto.  
  
- `COleDateTime(`*varSrc* **)** constrói uma `COleDateTime` objeto. Tenta converter um `VARIANT` estrutura ou [COleVariant](../../mfc/reference/colevariant-class.md) objeto em uma data/hora ( `VT_DATE`) valor. Se essa conversão for bem-sucedida, o valor convertido é copiado no novo `COleDateTime` objeto. Se não estiver, o valor da `COleDateTime` objeto é definido como 0 (meia-noite, 30 de dezembro de 1899) e seu status inválido.  
  
- `COleDateTime(``dtSrc` **)** Constrói uma `COleDateTime` de objeto um **data** valor.  
  
- `COleDateTime(``timeSrc` **)** Constrói uma `COleDateTime` do objeto de uma `time_t` valor.  
  
- `COleDateTime(`*systimeSrc* **)** constrói uma `COleDateTime` do objeto de uma `SYSTEMTIME` valor.  
  
- `COleDateTime(``filetimeSrc` **)** Constrói uma `COleDateTime` do objeto de uma `FILETIME` valor. . Observe que `FILETIME` usa o tempo Universal Coordenado (UTC), portanto, se você passar uma hora local na estrutura, os resultados serão incorretos. Consulte [arquivo vezes](http://msdn.microsoft.com/library/windows/desktop/ms724290) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] para obter mais informações.  
  
- `COleDateTime(``nYear`, `nMonth`, `nDay`, `nHour`, `nMin`, `nSec` **)** Constrói uma `COleDateTime` objeto dos valores numéricos especificados.  
  
- `COleDateTime(``wDosDate`, `wDosTime` **)** Constrói uma `COleDateTime` objeto a partir de valores de data e hora de MS-DOS especificados.  
  
 Para obter mais informações sobre o `time_t` tipo de dados, consulte o [tempo](../../c-runtime-library/reference/time-time32-time64.md) funcionar o *referência da biblioteca de tempo de execução*.  
  
 Para obter mais informações, consulte o [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) e [FILETIME](http://msdn.microsoft.com/library/windows/desktop/ms724284) estruturas no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&2;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_4.cpp)]  
...
- `VAR_TIMEVALUEONLY`Ignore a parte de data durante a análise.  
  
- `VAR_DATEVALUEONLY`Ignore a parte de hora durante a análise.  
 ...
 `lpszFormat`  
 Uma formatação de cadeia de caracteres semelhante de `printf` formatação de cadeia de caracteres. Cada uma formatação de código, precedido por uma porcentagem ( `%`) entre, é substituído pelo correspondente `COleDateTime` componente. Outros caracteres na cadeia de formatação são copiados inalterado de cadeia de caracteres retornada. Consulte a função de tempo de execução [strftime](../../c-runtime-library/reference/strftime-wcsftime-strftime-l-wcsftime-l.md) para obter mais informações. O valor e o significado dos códigos de formatação para `Format` são:  
...
### <a name="remarks"></a>Comentários  
 Se o status dessa `COleDateTime` o objeto é nulo, o valor de retorno é uma cadeia de caracteres vazia. Se o status for inválido, a cadeia de caracteres de retornada é especificada pelo recurso de cadeia de caracteres `ATL_IDS_DATETIME_INVALID`.  
  
 Segue uma breve descrição de três formas para esta função:  
  
 `Format`( `dwFlags`, `lcid`)  
 Este formulário formata o valor usando as especificações da linguagem (IDs de localidade) para data e hora. Usando os parâmetros padrão, este formulário imprimirá a data e hora, a menos que a parte de hora é 0 (meia-noite), caso em que ele imprimirá apenas a data ou a parte de data é 0 (30 de dezembro de 1899), no caso, ele imprimirá apenas a hora. Se o valor de data/hora é 0 (30 de dezembro de 1899, meia-noite), esse formulário com os parâmetros padrão serão impressos meia-noite.  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&3;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_5.cpp)]  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&4;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_6.cpp)]  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna **true** se bem sucedida; **false** se a conversão falhar, ou se o `COleDateTime` objeto **nulo** ou é inválido.  
  
### <a name="remarks"></a>Comentários  
 `GetAsSystemTime`armazena a hora resultante referenciado *sysTime* objeto. O `SYSTEMTIME` estrutura de dados inicializada por essa função terá seu **Wmilissegundos** membro definido como zero.  
...
### <a name="remarks"></a>Comentários  
 A **UDATE** estrutura representa uma data "descompactada".  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&5;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_7.cpp)]  
...
### <a name="return-value"></a>Valor de retorno  
 O dia do mês representado pelo valor desse `COleDateTime` objeto ou `COleDateTime::error` se o dia não pôde ser obtido.  
  
### <a name="remarks"></a>Comentários  
 Valores de retorno válidos variam entre 1 e 31.  
  
 Para obter informações sobre outras funções de membro que consultar o valor desse `COleDateTime` de objeto, consulte as seguintes funções de membro:  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&6;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_8.cpp)]  
...
### <a name="remarks"></a>Comentários  
 Valores de retorno válido variam entre 1 e 7, onde 1 = domingo, 2 = segunda-feira e assim por diante.  
...
### <a name="return-value"></a>Valor de retorno  
 O dia do ano que representa o valor desse `COleDateTime` objeto ou `COleDateTime::error` se o dia do ano não pôde ser obtido.  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&8;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_10.cpp)]  
...
### <a name="return-value"></a>Valor de retorno  
 A hora representada pelo valor desse `COleDateTime` objeto ou `COleDateTime::error` se a hora não pôde ser obtida.  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&9;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_11.cpp)]  
...
### <a name="return-value"></a>Valor de retorno  
 Mês representado pelo valor desse `COleDateTime` objeto ou `COleDateTime::error` se o mês não pôde ser obtido.  
...
> [!NOTE]
>  O `COleDateTime` classe não dá suporte a segundos intercalados.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna o status desse `COleDateTime` valor. Se você chamar **GetStatus** em uma `COleDateTime` objeto construído com o padrão, ele retornará válido. Se você chamar **GetStatus** em uma `COleDateTime` objeto inicializado com construtor definido como null, **GetStatus** retornará null. Consulte **comentários** para obter mais informações.  
  
### <a name="remarks"></a>Comentários  
 O valor de retorno é definido pelo **DateTimeStatus** enumeração de tipo, que é definido dentro de `COleDateTime` classe.  
...
- **COleDateTime::invalid** indica que esse `COleDateTime` objeto é inválido; isto é, seu valor pode estar incorreto.  
...
- **COleDateTime::null** indica que esse `COleDateTime` objeto é nulo, ou seja, se nenhum valor foi fornecido para este objeto. (Isso é "null" no sentido de banco de dados de "não ter nenhum valor," em vez do C++ **nulo**.)  
 ...
 O status de uma `COleDateTime` objeto é inválido nos seguintes casos:  
  
-   Se seu valor é definido de uma **VARIANT** ou `COleVariant` valor que não pôde ser convertido para um valor de data/hora.  
  
-   Se seu valor é definido de uma `time_t`, `SYSTEMTIME`, ou `FILETIME` valor que não pôde ser convertido para um valor de data/hora válida.  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&10;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_12.cpp)]  
...
### <a name="return-value"></a>Valor de retorno  
 O ano que representa o valor desse `COleDateTime` objeto ou `COleDateTime::error` se o ano não pôde ser obtido.  
  
### <a name="remarks"></a>Comentários  
 Valores de retorno válidos variam entre 100 e 9999, que inclui o século.  
...
##  <a name="a-namemdta--coledatetimemdt"></a><a name="m_dt"></a>COleDateTime::m_dt  
 Subjacente **data** estrutura para isso `COleDateTime` objeto.  
...
> [!CAUTION]
>  Alterando o valor de **data** objeto acessado pelo ponteiro retornado por essa função alterará o valor desse `COleDateTime` objeto. Ele não altera o status deste `COleDateTime` objeto.  
...
##  <a name="a-namemstatusa--coledatetimemstatus"></a><a name="m_status"></a>COleDateTime::m_status  
 Contém o status desse `COleDateTime` objeto.  
...
### <a name="remarks"></a>Comentários  
 O tipo desse membro de dados é o tipo enumerado **DateTimeStatus**, que é definido dentro de `COleDateTime` classe. Consulte [COleDateTime::GetStatus](#getstatus) para obter detalhes.  
...
- **operador = (** `dateSrc` **)** o valor e o status do operando são copiados para esse `COleDateTime` objeto.  
  
- **operador = (** *varSrc* **)** se a conversão do [VARIANT](http://msdn.microsoft.com/en-us/e305240e-9e11-4006-98cc-26f4932d2118) valor (ou [COleVariant](../../mfc/reference/colevariant-class.md) objeto) em uma data/hora ( `VT_DATE`) for bem-sucedida, o valor convertido é copiado para esse `COleDateTime` objeto e seu status é definido como válido. Se a conversão não for bem-sucedida, o valor desse objeto é definido como zero (30 de dezembro de 1899 meia-noite) e seu status inválido.  
  
- **operador = (** `dtSrc` **)** o **data** valor é copiado para esse `COleDateTime` objeto e seu status é definido como válido.  
  
- **operador = (** `timeSrc` **)** o `time_t` ou **__time64_t** valor é convertido e copiado para esse `COleDateTime` objeto. Se a conversão for bem-sucedida, o status desse objeto é definido como válido. Se bem-sucedida, ela será definida para inválido.  
  
- **operador = (** *systimeSrc* **)** o [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) valor é convertido e copiado para esse `COleDateTime` objeto. Se a conversão for bem-sucedida, o status desse objeto é definido como válido. Se bem-sucedida, ela será definida para inválido.  
  
- **operador = (** `udate` **)** o **UDATE** valor é convertido e copiado para esse `COleDateTime` objeto. Se a conversão for bem-sucedida, o status desse objeto é definido como válido. Se bem-sucedida, ela será definida para inválido. A **UDATE** estrutura representa uma data "descompactada". Consulte a função [VarDateFromUdate](http://msdn.microsoft.com/en-us/1c924ac5-b896-49e1-9ccf-825ac7a030c8) para obter mais detalhes.  
  
- **operador = (** `filetimeSrc` **)** o [FILETIME](http://msdn.microsoft.com/library/windows/desktop/ms724284) valor é convertido e copiado para esse `COleDateTime` objeto. Se a conversão for bem-sucedida, o status desse objeto é definido como válido. Caso contrário, ele é definido para inválido. `FILETIME`usa Tempo Universal Coordenado (UTC), portanto, se você passar uma hora UTC na estrutura, os resultados serão convertidos de hora UTC em hora local e serão armazenados como variante. Esse comportamento é o mesmo do Visual C++ 6.0 e Visual C++ .NET 2003 SP2. Consulte [arquivo vezes](http://msdn.microsoft.com/library/windows/desktop/ms724290) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)] para obter mais informações.  
  
 Para obter mais informações, consulte o [VARIANT](http://msdn.microsoft.com/en-us/e305240e-9e11-4006-98cc-26f4932d2118) entrada do [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Comentários  
 `COleDateTime`objetos representam horas absolutas. [COleDateTimeSpan](../../atl-mfc-shared/reference/coledatetimespan-class.md) objetos representam horas relativas. Os dois primeiros operadores permitem que você adicionar e subtrair um `COleDateTimeSpan` valor de uma `COleDateTime` valor. O terceiro operador permite que você subtrair um `COleDateTime` valor de outro para produzir uma `COleDateTimeSpan` valor.  
  
 Se qualquer um dos operandos for null, o status de resultante `COleDateTime` valor é null.  
  
 Se resultante `COleDateTime` valor fica fora dos limites de valores aceitáveis, o status do `COleDateTime` valor é inválido.  
...
##  <a name="a-nameoperatoraddeq-eqa--coledatetimeoperator---"></a><a name="operator_add_eq_-_eq"></a>+ = COleDateTime::operator-=  
 Adicionar e subtrair um **ColeDateTime** valor desse `COleDateTime` objeto.  
...
### <a name="remarks"></a>Comentários  
 Esses operadores permitem que você adicionar e subtrair um `COleDateTimeSpan` valor de e para isso `COleDateTime`. Se qualquer um dos operandos for null, o status de resultante `COleDateTime` valor é null.  
  
 Se resultante `COleDateTime` valor fica fora dos limites de valores aceitáveis, o status desse `COleDateTime` valor é definido para inválido.  
  
 Se qualquer um dos operandos for inválido e outros não for nulo, o status de resultante `COleDateTime` valor é inválido.  
...
### <a name="remarks"></a>Comentários  
 Se a cadeia de caracteres foi convertida com êxito em uma data/hora do valor, o valor desse `COleDateTime` objeto é definido como valor e seu status válido.  
  
> [!NOTE]
>  Valores de anos devem estar entre 100 e 9999, inclusive.  
  
 O `lpszDate` parâmetro pode assumir uma variedade de formatos. Por exemplo, as cadeias de caracteres a seguir contêm os formatos aceitáveis de data/hora:  
 ...
 Observe que a ID de localidade também afeta se o formato de cadeia de caracteres é aceitável para conversão em um valor de data/hora.  
  
 No caso de **VAR_DATEVALUEONLY**, o valor é definido como 0 hora ou meia-noite de tempo. No caso de **VAR_TIMEVALUEONLY**, a data de valor é definido como data 0, que significa que 30 de dezembro de 1899.  
  
 Se a cadeia de caracteres não pode ser convertida para um valor de data/hora ou se houve um estouro numérico, o status desse `COleDateTime` objeto é inválido.  
  
 Para obter mais informações sobre os limites e a implementação de `COleDateTime` valores, consulte o artigo [data e hora: suporte de automação de](../../atl-mfc-shared/date-and-time-automation-support.md).  
...
### <a name="return-value"></a>Valor de retorno  
 Zero se o valor desse `COleDateTime` objeto foi definido com êxito; caso contrário, 1. Esse valor de retorno se baseia o **DateTimeStatus** tipo enumerado. Para obter mais informações, consulte o [SetStatus](#setstatus) função de membro.  
  
### <a name="remarks"></a>Comentários  
 A data é definida com os valores especificados. O tempo é definido como hora 0, meia-noite.  
...
|`nDay`|0 – 31|  
 ...
 Se o dia do mês estourar, ela é convertida para o dia do mês seguinte e o mês e/ou ano é incrementado adequadamente. Um valor de dia zero indica o último dia do mês anterior. O comportamento é o mesmo que `SystemTimeToVariantTime`.  
...
|10000|1|1|1º de janeiro 10000 (data inválida)|  
 ...
 Para obter informações sobre funções de membro que consultar o valor desse `COleDateTime` de objeto, consulte as seguintes funções de membro:  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_ATLMFC_Utilities n º&11;](../../atl-mfc-shared/codesnippet/cpp/coledatetime-class_14.cpp)]  
  
##  <a name="a-namesetdatetimea--coledatetimesetdatetime"></a><a name="setdatetime"></a>COleDateTime::SetDateTime  
 Define a data e hora dessa `COleDateTime` objeto.  
...
|`nSec`|0 – 59|  
 ...
 Se o dia do mês estourar, ela é convertida para o dia do mês seguinte e o mês e/ou ano é incrementado adequadamente. Um valor de dia zero indica o último dia do mês anterior. O comportamento é o mesmo que [SystemTimeToVariantTime](http://msdn.microsoft.com/en-us/d9d69521-9b33-4fc5-8a1c-929f216db450).  
...
|10000|1|1|Inválido|  
 ...
 Para definir somente a data, consulte [COleDateTime::SetDate](#setdate). Para definir apenas a hora, consulte [COleDateTime::SetTime](#settime).  
...
### <a name="parameters"></a>Parâmetros  
 *status*  
 O novo valor de status para essa `COleDateTime` objeto.  
  
### <a name="remarks"></a>Comentários  
 O *status* o valor do parâmetro é definido pelo **DateTimeStatus** enumeração de tipo, que é definido dentro de `COleDateTime` classe. Consulte [COleDateTime::GetStatus](#getstatus) para obter detalhes.  
  
> [!CAUTION]
>  Essa função é para situações de programação avançadas. Essa função não alteram os dados nesse objeto. Ele geralmente será usado para definir o status como `null` ou **inválido**. Observe que o operador de atribuição ( [operador =](#eq)) e [SetDateTime](#setdatetime) definir o status do objeto com base em valores de origem.  
...
### <a name="remarks"></a>Comentários  
 A hora é definida com os valores especificados. A data é definida como data 0, que significa que 30 de dezembro de 1899.  
...
|`nSec`|0 – 59|  
 ...
 Se o tempo especificado pelos parâmetros de valor não é válido, o status desse objeto é definido como inválido e o valor desse objeto não será alterado.  
