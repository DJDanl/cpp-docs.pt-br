...
- **caractere** (para cadeias de caracteres ANSI).
...
*StringTraits*<br/>
Determina se a classe de cadeia de caracteres precisa de suporte a biblioteca CRT (tempo de execução) e onde os recursos de cadeia de caracteres estão localizados. Pode ser um dos seguintes:

- **StrTraitATL< wchar_t** &#124; **char** &#124; **TCHAR, ChTraitsCRT< wchar_t** &#124; **char** &#124; **TCHAR > >**

   A classe requer suporte a CRT e procura cadeias de caracteres de recurso no `m_hInstResource` módulo especificado por (um membro da classe de módulo do aplicativo).

- **StrTraitATL < wchar_t** &#124; **Char** &#124; &#124; TCHAR, ChTraitsOS < wchar_t Char TCHAR > > &#124;

   A classe não requer suporte a CRT e procura cadeias de caracteres de recurso no módulo `m_hInstResource` especificado por (um membro da classe de módulo do aplicativo).
...
|[CStringT::AppendFormat](#appendformat)|Anexa dados formatados a um objeto `CStringT` existente.|
...
|[CStringT:: Format](#format)|Formata a cadeia de `sprintf` caracteres como faz.|
...
`CStringT`herda da [classe CSimpleStringT](../../atl-mfc-shared/reference/csimplestringt-class.md). Recursos avançados, como manipulação de caracteres, ordenação e pesquisa, são implementados `CStringT`pelo.
...
Um `CStringT` objeto consiste em uma sequência de comprimento variável de caracteres. `CStringT`fornece funções e operadores usando sintaxe semelhante à do Basic. Os operadores de concatenação e comparação, juntamente com o gerenciamento de `CStringT` memória simplificado, tornam os objetos mais fáceis de usar do que as matrizes de caracteres comuns.
...
Usando diferentes combinações dos `BaseType` parâmetros e `StringTraits` , `CStringT` os objetos podem vir nos seguintes tipos, que foram predefinidos pelas bibliotecas do ATL.

Se estiver usando em um aplicativo ATL:

`CString`, `CStringA` e`CStringW` são exportados da dll do MFC (MFC90. DLL), nunca de DLLs de usuário. Isso é feito para evitar `CStringT` que ele seja definido de multiplicação.
...
Os seguintes tipos de cadeia de caracteres estão disponíveis em projetos em que ATL_CSTRING_NO_CRT não está definido:
...
|`PXSTR`|Um ponteiro para uma cadeia de caracteres ( **wchar_t** ou **Char**) com o mesmo tipo de caractere que `CStringT` o objeto.|
|`PYSTR`|Um ponteiro para uma cadeia de caracteres ( **wchar_t** ou **Char**) com o tipo de caractere oposto como `CStringT` o objeto.|
|`PCXSTR`|Um ponteiro para uma cadeia de caracteres **const** ( **wchar_t** ou **Char**) com o mesmo tipo de caractere que `CStringT` o objeto.|
|`PCYSTR`|Um ponteiro para uma cadeia de caracteres **const** ( **wchar_t** ou **Char**) com o tipo de caractere oposto como `CStringT` o objeto.|

> [!NOTE]
>  O código que usava anteriormente métodos não documentados `CString` de ( `AssignCopy`como) deve ser substituído por um código que usa os seguintes `GetBuffer` métodos documentados do `CStringT` (como `ReleaseBuffer`ou). Esses métodos são herdados `CSimpleStringT`de.
...
A função não estará disponível se _UNICODE for definido.
...
Anexa dados formatados a um objeto `CStringT` existente.
...
Zero se as cadeias de caracteres forem idênticas, `CStringT` < 0 se esse objeto for menor que *psz*ou > `CStringT` 0 se esse objeto for maior que *psz*.

### <a name="remarks"></a>Comentários

A função `_tcscoll`de texto genérico, que é definida em TCHAR. H, mapeia para `strcoll`, `wcscoll`ou `_mbscoll`, dependendo do conjunto de caracteres definido no momento da compilação. Cada função executa uma comparação de maiúsculas e minúsculas das cadeias de caracteres de acordo com a página de código em uso no momento. Para obter mais informações, consulte [strcoll, wcscoll, _mbscoll, _strcoll_l, _wcscoll_l, _mbscoll_l](../../c-runtime-library/reference/strcoll-wcscoll-mbscoll-strcoll-l-wcscoll-l-mbscoll-l.md).
...
Zero se as cadeias de caracteres forem idênticas (ignorando maiúsculas `CStringT` e minúsculas), < 0 se esse objeto for menor que *psz* ( `CStringT` ignorando maiúsculas/minúsculas) ou > 0 se esse objeto for maior que *psz* (ignorando maiúsculas e minúsculas)
...
A função `_tcscmp`de texto genérico, que é definida em TCHAR. H, mapeia para `strcmp`, `wcscmp`ou `_mbscmp`, dependendo do conjunto de caracteres definido no momento da compilação. Cada função executa uma comparação de maiúsculas e minúsculas das cadeias de caracteres e não é afetada pela localidade. Para obter mais informações, consulte [strcmp, wcscmp, _mbscmp](../../c-runtime-library/reference/strcmp-wcscmp-mbscmp.md).
...
A função `_tcsicmp`de texto genérico, que é definida em TCHAR. H, mapeia para `_stricmp` `_wcsicmp` ou `_mbsicmp`, dependendo do conjunto de caracteres definido no momento da compilação. Cada função executa uma comparação que não diferencia maiúsculas de minúsculas das cadeias de caracteres. A comparação depende do aspecto LC_CTYPE da localidade, mas não LC_COLLATE. Para obter mais informações, consulte [_stricmp, _wcsicmp, _mbsicmp, _stricmp_l, _wcsicmp_l, _mbsicmp_l](../../c-runtime-library/reference/stricmp-wcsicmp-mbsicmp-stricmp-l-wcsicmp-l-mbsicmp-l.md).
...
*pszSrc*<br/>
Uma cadeia de caracteres terminada em nulo a ser `CStringT` copiada para este objeto.

*pStringMgr*<br/>
Um ponteiro para o Gerenciador `CStringT` de memória do objeto. Para obter mais informações `IAtlStringMgr` sobre o e o `CStringT`gerenciamento de memória para o, consulte [Gerenciamento de memória com CStringT](../../atl-mfc-shared/memory-management-with-cstringt.md).

*strSrc*<br/>
Um objeto `CStringT` existente a ser copiado para `CStringT` este objeto. Para obter mais informações `CThisString` sobre `CThisSimpleString`o e o, consulte a seção comentários.

*varSrc*<br/>
Um objeto Variant a ser copiado para `CStringT` este objeto.
...
Como os construtores copiam os dados de entrada para um novo armazenamento alocado, você deve estar ciente de que as exceções de memória podem resultar. Observe que alguns desses construtores atuam como funções de conversão. Isso permite que você substitua, por exemplo, um LPTStr em que `CStringT` um objeto é esperado.

- `CStringT`( `LPCSTR` `lpsz` ): Constrói um Unicode `CStringT` a partir de uma cadeia de caracteres ANSI. Você também pode usar esse construtor para carregar um recurso de cadeia de caracteres, conforme mostrado no exemplo abaixo.
...
O operador `CSimpleStringT<>&()` Overload constrói um `CStringT` objeto de uma `CSimpleStringT` declaração.
...
A função é sobrecarregada para aceitar os dois caracteres únicos (semelhantes à função `strchr`de tempo de execução) e cadeias (semelhantes a `strstr`).
...
Grava dados formatados em `CStringT` um da mesma maneira que [sprintf_s](../../c-runtime-library/reference/sprintf-s-sprintf-s-l-swprintf-s-swprintf-s-l.md) formata dados em uma matriz de caracteres em estilo C.
...
A chamada falhará se o próprio objeto de cadeia de caracteres for oferecido como `Format`um parâmetro para. Por exemplo, o código a seguir causará resultados imprevisíveis:
...
A função requer uma definição de mensagem como entrada. A definição de mensagem é determinada por *pszFormat* ou do recurso de cadeia de caracteres identificado por *nFormatID*. A função copia o texto da mensagem formatada para `CStringT` o objeto, processando as sequências inseridas inseridas, se solicitado.
...
*pszFormat*<br/>
Aponta para a cadeia de caracteres de controle de formato. Ela será verificada em busca de inserções e formatadas adequadamente. A cadeia de caracteres de formato é semelhante às cadeias de formato de estilo de função `printf`em tempo de execução, exceto pelo fato de que os parâmetros a serem inseridos em uma ordem arbitrária.
...
A função requer uma definição de mensagem como entrada, determinada por *pszFormat*. A função copia o texto da mensagem formatada e uma lista variável de argumentos para `CStringT` o objeto, processando as sequências inseridas inseridas, se solicitado.
...
Grava uma cadeia de caracteres formatada e uma lista variável de argumentos `CStringT` em uma cadeia de caracteres da `vsprintf_s` mesma maneira que formata dados em uma matriz de caracteres de estilo C.
...
Extrai os caracteres `CStringT` *nCount* mais à esquerda desse objeto e retorna uma cópia da subcadeia de caracteres extraída.
...
Lê um recurso de cadeia de caracteres doWindows, identificado por NID `CStringT` , em um objeto existente.
...
Extrai uma subcadeia de caracteres de comprimento *nCount* a partir `CStringT` deste objeto, começando na posição *iFirst* (baseada em zero).
...
Essa função não estará disponível se _UNICODE for definido.
...
O operador de atribuição aceita `CStringT` outro objeto, um ponteiro de caractere ou um único caractere. Você deve estar ciente de que as exceções de memória podem ocorrer sempre que você usar esse operador porque um novo armazenamento pode ser alocado.

Para obter informações `CThisSimpleString`sobre o, consulte a seção comentários de [CStringT:: CStringT](#cstringt).
...
O operador aceita outro `CStringT` objeto, um ponteiro de caractere ou um único caractere. Você deve estar ciente de que as exceções de memória podem ocorrer sempre que você usar esse operador de concatenação porque um novo armazenamento pode ser `CStringT` alocado para caracteres adicionados a esse objeto.
...
##  <a name="operator_lt"></a>Operador CStringT::&lt;
...
##  <a name="operator_gt"></a>Operador CStringT::&gt;
...
##  <a name="operator_lt_eq"></a>Operador CStringT::&lt;=
...
##  <a name="operator_gt_eq"></a>Operador CStringT::&gt;=
...
Há duas versões do `Replace`. A primeira versão substitui uma ou mais cópias de uma subcadeia de caracteres usando outra subcadeia de caracteres. Ambas as subcadeias são terminadas em nulo. A segunda versão substitui uma ou mais cópias de um caractere usando outro caractere. Ambas as versões operam nos dados de caracteres `CStringT`armazenados no.
...
Para `CStringA`o `Replace` , funciona com caracteres ANSI ou multibyte (MBCS). Para `CStringW`o `Replace` , funciona com caracteres largos.
...
|Delas|Caracteres de byte único|
|Ambos|Indefinido|
...
A função é semelhante à função `strrchr`de tempo de execução.
...
Extrai os últimos (ou seja, os caracteres mais à direita) do `CStringT` nCount desse objeto e retorna uma cópia da subcadeia de caracteres extraída.
...
Se o primeiro caractere da cadeia de caracteres não estiver no conjunto de caracteres, `SpanIncluding` o retornará uma cadeia de caracteres vazia. Caso contrário, ele retorna uma sequência de caracteres consecutivos que estão no conjunto.
...
A `Tokenize` função localiza o próximo token na cadeia de caracteres de destino. O conjunto de caracteres em *pszTokens* Especifica possíveis delimitadores do token a ser encontrado. Em cada chamada para `Tokenize` a função começa em *isniciar*, o ignora os delimitadores à esquerda e retorna `CStringT` um objeto que contém o token atual, que é a cadeia de caracteres até o próximo caractere delimitador. O valor de *isniciar* é atualizado para ser a posição após o caractere delimitador final ou-1 se o final da cadeia de caracteres foi atingido. Mais tokens podem ser desfeitos do restante da cadeia de caracteres de destino por uma série `Tokenize`de chamadas para, usando isniciar para controlar onde na cadeia de caracteres o próximo token será lido. Quando não houver mais tokens, a função retornará uma cadeia de caracteres vazia e isniciar será definido como-1.

Diferentemente das funções de token do CRT, como [strtok_s, _strtok_s_l, wcstok_s, _wcstok_s_l, _mbstok_s, _mbstok_s_l](../../c-runtime-library/reference/strtok-s-strtok-s-l-wcstok-s-wcstok-s-l-mbstok-s-mbstok-s-l.md), `Tokenize` não modifica a cadeia de caracteres de destino.
...
*pszTargets*<br/>
Um ponteiro para uma cadeia de caracteres que contém os caracteres de destino a serem cortados. Todas as ocorrências à esquerda e à direita de caracteres em *pszTarget* serão aparadas `CStringT` do objeto.
