...
ms.locfileid: "70217752"
---
# <a name="_interlockedbittestandset-intrinsic-functions"></a>funções intrínsecas _interlockedbittestandset

**Seção específica da Microsoft**

Gere uma instrução para examinar o `b` bit do endereço `a` e retornar seu valor atual antes de defini-lo como 1.
...
O valor do bit na posição `b` antes de ser definido.
...
Em processadores ARM e ARM64, use o intrínsecos com `_acq` e `_rel` sufixos para semântica de aquisição e liberação, como no início e no final de uma seção crítica. O ARM intrínseco com um `_nf` sufixo ("sem limite") não atua como uma barreira de memória.
