...
> [!NOTE]
>  Como a ativação no local só se aplica a itens incorporados (não vinculados), aumentar o zoom se aplica somente objetos inseridos. Você verá os APIs no `COleServerDoc` e `COleServerItem` que são usados para aumentar o zoom. A razão para essa dicotomia é que apenas as funções que são válidas para itens incorporados e vinculados estão em `COleServerItem` (Isso permite que você tenha uma implementação comum) e funções que são válidas somente para objetos inseridos estão localizadas no `COleServerDoc` classe (da perspectiva do servidor, é o **documento** que é inserido).  
