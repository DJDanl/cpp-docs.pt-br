...
|-------------------|---------------|----------|  
|Banco de dados usando o mecanismo de banco de dados do Jet|"[ `database`];"|" `drive`:\\\ *path*\\\ *filename*. MDB"|  
...
|Importação de HTML|"Importação de HTML;"|" `drive`:\\\ *path*\ *filename*"|  
|Exportação de HTML|"Exportação HTML;"|" `drive`:\\\ *path*"|  
|Texto|"Texto";|"unidade:\\\path"|  
|ODBC|"ODBC; Banco de dados = `database`; UID= *user*; PWD = *senha*; DSN = *datasourcename;* LOGINTIMEOUT = *segundos;*" (Isso não pode ser uma cadeia de caracteres de conexão completa para todos os servidores; é apenas um exemplo. É muito importante não têm espaços entre os parâmetros.)|Nenhum|  
