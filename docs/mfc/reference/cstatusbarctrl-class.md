...
Esse controle (e, portanto, a classe `CStatusBarCtrl`) está disponível somente para programas em execução no Windows 95/98 e no Windows NT versão 3,51 e posterior.
...
##  <a name="create"></a>CStatusBarCtrl:: criar
...
Você constrói uma `CStatusBarCtrl` em duas etapas. Primeiro, chame o construtor e, em seguida, chame `Create`, que cria o controle da barra de status e o anexa ao objeto `CStatusBarCtrl`.
...
Por padrão, essa função de membro não faz nada. Substitua essa função de membro para implementar o desenho de um objeto de `CStatusBarCtrl` de desenho proprietário.
...
##  <a name="setsimple"></a>CStatusBarCtrl:: setsimples
