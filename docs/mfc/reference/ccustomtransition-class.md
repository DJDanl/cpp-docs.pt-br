---
title: Classe CCustomTransition | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|Nome|Descrição|  
|----------|-----------------|  
|[CCustomTransition::Create](#create)|Chama a biblioteca de transição para criar o objeto de transição encapsulado COM. (Substitui [CBaseTransition::Create](../../mfc/reference/cbasetransition-class.md#create).)|  
...
## <a name="remarks"></a>Comentários  
 A classe CCustomTransitions permite aos desenvolvedores implementar transições personalizadas. Ele foi criado e usado como uma transição padrão, mas seu construtor aceita como parâmetro, um ponteiro para um interpolador personalizado. Execute as seguintes etapas para usar transições personalizadas: 1. Derivar uma classe de CCustomInterpolator e implementar pelo menos InterpolateValue método. 2. Certifique-se de que o tempo de vida do objeto personalizado interpolador deve ser maior que a duração da animação onde ele é usado. 3. Instanciar (usando o operador new) um objeto CCustomTransition e passe um ponteiro para interpolador personalizado no construtor. 4. Chame CCustomTransition::SetInitialValue e CCustomTransition::SetInitialVelocity se esses parâmetros são necessários para a interpolação personalizada. 5. Passe o ponteiro para transição personalizada para o método AddTransition do objeto de animação, cujo valor deve ser animado com o algoritmo personalizado. 6. Quando o valor do objeto de animação deve alterar o API de animação do Windows chamará InterpolateValue (e outros métodos relevantes) em CCustomInterpolator.  
...
### <a name="remarks"></a>Comentários  
 Esse método também pode definir o valor inicial e velocidade inicial a ser aplicado a uma variável de animação, que é associada essa transição. Para essa finalidade, você precisa chamar SetInitialValue e SetInitialVelocity antes do framework cria o objeto de COM de transição encapsulado (isso acontece quando você chamar CAnimationController::AnimateGroup).  
