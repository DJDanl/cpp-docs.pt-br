...
|Nome|Descrição|
|----------|-----------------|
|[CProgressCtrl:: criar](#create)|Cria um controle de barra de progresso e o anexa a um `CProgressCtrl` objeto.|
...
Para obter mais informações sobre como usar o `CProgressCtrl` , consulte [controles](../../mfc/controls-mfc.md) e [usando CProgressCtrl](../../mfc/using-cprogressctrl.md).
...
Cria um controle de barra de progresso e o anexa a um `CProgressCtrl` objeto.
...
*Rect*<br/>
Especifica o tamanho e a posição do controle da barra de progresso. Pode ser um objeto [CRect](../../atl-mfc-shared/reference/crect-class.md) ou uma estrutura [Rect](/windows/win32/api/windef/ns-windef-rect) . Como o controle deve ser uma janela filho, as coordenadas especificadas são relativas à área do cliente do *pParentWnd*.
...
Você constrói um `CProgressCtrl` objeto em duas etapas. Primeiro, chame o construtor, que cria o `CProgressCtrl` objeto e, em seguida, chame `Create` , que cria o controle da barra de progresso.
...
Cria um controle (uma janela filho) e o associa ao `CProgressCtrl` objeto.
...
*clrBar*\
no Um valor de [COLORREF](/windows/win32/gdi/colorref) que especifica a nova cor da barra de indicador de progresso. Especifique CLR_DEFAULT para fazer com que a barra de progresso Use sua cor padrão.
...
*fMarqueeMode*\
no TRUE para ativar o modo de letreiro ou FALSE para ativar o modo de letreiro.

*nIntervalo*\
no Tempo em milissegundos entre as atualizações da animação do letreiro digital.
...
*IState*\
no O estado para definir a barra de progresso. Use um dos seguintes valores:
