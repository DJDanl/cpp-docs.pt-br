...
ms.locfileid: "37850282"
---
# <a name="cmfctoolbarimages-class"></a>Classe CMFCToolBarImages
As imagens em uma barra de ferramentas. O `CMFCToolBarImages` classe gerencia imagens de barra de ferramentas carregadas dos recursos do aplicativo ou de arquivos.  
...
|[CMFCToolBarImages::Clear](#clear)|Libera os recursos do sistema que foram alocados a este objeto.|  
|[CMFCToolBarImages::ConvertTo32Bits](#convertto32bits)|Converte sublinhado bitmaps para imagens de 32 bpp.|  
...
|[CMFCToolBarImages::EndDrawImage](#enddrawimage)|Libera os recursos do sistema depois que uma imagem da barra de ferramentas é desenhada.|  
|[CMFCToolBarImages::ExtractIcon](#extracticon)|Retorna o ícone que tem um índice de imagem especificada a partir das imagens da barra de ferramentas.|  
...
|[CMFCToolBarImages::GetBitsPerPixel](#getbitsperpixel)|Retorna a resolução atual das imagens de sublinhado.|  
...
|[CMFCToolBarImages::GetScale](#getscale)|Retorna a taxa de escala atual das imagens de sublinhado.|  
|[CMFCToolBarImages::GetTransparentColor](#gettransparentcolor)||  
|[CMFCToolBarImages::GrayImages](#grayimages)|Grays as imagens da barra de ferramentas para torná-las desabilitadas.|  
|[CMFCToolBarImages::Is32BitTransparencySupported](#is32bittransparencysupported)|Determina se o sistema operacional dá suporte a combinação alfa de 32 bits.|  
|[CMFCToolBarImages::IsPreMultiplyAutoCheck](#ispremultiplyautocheck)||  
|[CMFCToolBarImages::IsRTL](#isrtl)|Determina se o suporte a direita para esquerda (RTL) está habilitado.|  
|[CMFCToolBarImages::IsReadOnly](#isreadonly)|Determina se as imagens da barra de ferramentas são somente leitura.|  
|[CMFCToolBarImages::IsScaled](#isscaled)|Informa se imagens sublinhadas são dimensionadas ou não.|  
...
|[CMFCToolBarImages::IsValid](#isvalid)|Determina se este conjunto de imagens da barra de ferramentas contém uma imagem válida da barra de ferramentas.|  
...
|[CMFCToolBarImages::Mirror](#mirror)|Espelha horizontalmente todas as imagens da barra de ferramentas.|  
...
|[CMFCToolBarImages::PrepareDrawImage](#preparedrawimage)|Aloca os recursos que são necessárias para desenhar uma imagem da barra de ferramentas em um tamanho especificado.|  
...
|[CMFCToolBarImages::UpdateImage](#updateimage)|Atualiza uma imagem de barra de ferramentas definidas pelo usuário a partir de um bitmap.|  
...
## <a name="remarks"></a>Comentários  
 O bitmap completo das imagens da barra de ferramentas gerenciadas pelo `CMFCToolbarImages` consiste em uma ou mais imagens de pequena barra de ferramentas (botões) de tamanho fixo.  
  
## <a name="example"></a>Exemplo  
 O exemplo a seguir demonstra como configurar uma `CMFCToolBarImages` objeto usando vários métodos no `CMFCToolBarImages` classe. O exemplo mostra como definir o tamanho da imagem da barra de ferramentas, carregar uma imagem e definir a cor transparente da imagem. Este trecho de código faz parte do [amostra de demonstração do Visual Studio](../../visual-cpp-samples.md).  
...
### <a name="parameters"></a>Parâmetros  
 ...
 [in] *bAlphaBlend*  
 TRUE se este ícone é usado com combinação alfa; Caso contrário, FALSE.  
  
### <a name="return-value"></a>Valor de retorno  
 O índice baseado em zero da imagem da barra de ferramentas que foi adicionado se o método for bem-sucedida; Caso contrário,-1.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 [in] *bSetBitPerPixel*  
 TRUE se o `CMFCToolBarImages` objeto usa a intensidade de cor (bits por pixel) da nova imagem. FALSE se o `CMFCToolbarImages` objeto mantém a profundidade de cor atual.  
  
 [in] *imageList*  
 Uma referência a um `CMFCToolbarImages` objeto que contém a imagem a ser adicionada.  
  
 [in] *nIndex*  
 O índice na fonte de `CMFCToolbarImages` objeto de imagem a ser adicionada.  
  
### <a name="return-value"></a>Valor de retorno  
 O número de barra de ferramentas de imagens que o `CMFCToolBarImages` objeto mantém depois que o novo bitmap foi adicionado com êxito; -1 se a operação falhou.  
...
### <a name="remarks"></a>Comentários  
 Constrói um `CMFCToolBarImages` do objeto, inicializa o mecanismo de renderização e define o tamanho da imagem para seu valor padrão 16 x 15 pixels. Use [CMFCToolBarImages::SetImageSize](#setimagesize) para alterar o tamanho da imagem antes de adicionar imagens.  
...
### <a name="parameters"></a>Parâmetros  
 [in] *imageList*  
 A lista de imagens a ser usado como uma fonte para imagens da barra de ferramentas.  
...
### <a name="parameters"></a>Parâmetros  
 [in] *iImage*  
 Especifica o índice baseado em zero da imagem a ser excluída.  
  
### <a name="return-value"></a>Valor de retorno  
 TRUE se a imagem foi excluída com êxito; FALSE se o índice de imagem é inválido, o `CMFCToolbarImages` objeto é temporário, o `CMFCToolbarImages` objeto não contém imagens definidas pelo usuário, ou se algum outro erro ocorreu.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 [in] *x*  
 A coordenada X do lado esquerdo do retângulo em que a imagem deve ser desenhada.  
  
 [in] *y*  
 A coordenada Y da parte superior do retângulo em que a imagem deve ser desenhada.  
  
 [in] *iImageIndex*  
 O índice baseado em zero da imagem a ser exibida.  
  
 [in] *bHilite*  
 TRUE se a imagem precisar ser realçado; Caso contrário, FALSE.  
  
 [in] *bDesabilitado*  
 TRUE se a imagem deve ser desenhada no estilo desabilitado; Caso contrário, FALSE.  
  
 [in] *bIndeterminate*  
 TRUE se a imagem deve ser desenhada no estilo de estado indeterminado; Caso contrário, FALSE.  
  
 [in] *bShadow*  
 TRUE se a imagem deve ser desenhada com uma sombra; Caso contrário, FALSE.  
  
 [in] *bInactive*  
 TRUE se a imagem deve ser desenhada no estilo de estado inativo; Caso contrário, FALSE.  
  
 [in] *alphaSrc*  
 O valor de canal alfa (opacity). Um valor de 255 significa que a imagem é desenhado opaco. Um valor de 0 significa que a imagem é desenhada transparente. Esse valor é usado apenas para imagens coloridas de 32 bits e para imagens que são exibidos um estilo de vidro do Windows Vista.  
  
### <a name="return-value"></a>Valor de retorno  
 VERDADEIRO se a imagem especificada foi exibida com êxito; FALSE se o índice de imagem era inválido ou algum outro erro ocorreu.  
...
##  <a name="enddrawimage"></a>  CMFCToolBarImages::EndDrawImage  
 Libera recursos do sistema que [CMFCToolBarImages::PrepareDrawImage](#preparedrawimage) alocada depois que você desenhar uma imagem da barra de ferramentas chamando [CMFCToolBarImages::Draw](#draw).  
...
##  <a name="extracticon"></a>  CMFCToolBarImages::ExtractIcon  
 Retorna o ícone que tem um índice de imagem especificada a partir das imagens da barra de ferramentas.  
...
### <a name="parameters"></a>Parâmetros  
 [in] *nIndex*  
 O índice baseado em zero na lista de imagens no qual a imagem a ser extraída como um ícone está localizada.  
  
### <a name="return-value"></a>Valor de retorno  
 Um identificador para o ícone extraído, ou nulo se *nIndex* está fora do intervalo.  
...
### <a name="remarks"></a>Comentários  
 Use esse método para preencher um retângulo com uma cor que é a média das cores do sistema COLOR_BTNFACE e COLOR_BTNHIGHLIGHT. Se o sistema está usando cores de 256 ou menos, o retângulo será preenchido com um padrão de pontilhado dessas duas cores em vez disso.  
...
### <a name="remarks"></a>Comentários  
 O tamanho da imagem de origem é o tamanho das imagens que são armazenadas em do [CMFCToolbarImages](../../mfc/reference/cmfctoolbarimages-class.md) objeto. Você pode chamar [CMFCToolBarImages::SetImageSize](#setimagesize) para definir o tamanho do código-fonte. O valor padrão é 16 x 15 pixels.  
...
### <a name="remarks"></a>Comentários  
 As imagens da barra de ferramentas são armazenadas em uma linha em um único bitmap que é conhecido como um *poço de imagens*. Para encontrar uma imagem da barra de ferramentas corretamente a imagem, multiplique o índice da imagem pela largura das imagens da barra de ferramentas (consulte [CMFCToolBarImages::GetImageSize](#getimagesize)) para obter o deslocamento horizontal da imagem dentro da imagem também.  
...
### <a name="return-value"></a>Valor de retorno  
 Se o método foi bem-sucedido; um índice de imagem -1 se a imagem com a ID do recurso especificado não existe.  
...
##  <a name="grayimages"></a>  CMFCToolBarImages::GrayImages  
 Grays as imagens da barra de ferramentas para torná-las desabilitadas.  
...
### <a name="remarks"></a>Comentários  
 Esse método modifica as imagens da barra de ferramentas média entre os componentes vermelhos, verdes e azuis de cada pixel e multiplicando o resultado pelo *nGrayImageLuminancePercentage* dividido por 100. Se *nGrayImageLuminancePercentage* for zero ou negativo, o valor padrão de 130 é usado em vez disso.  
  
> [!NOTE]
>  Se você quiser desfazer a alteração, você deve recarregar as imagens da origem. Você pode fazer isso chamando [CMFCToolBarImages::Load](#load) ou [CMFCToolBarImages::UpdateImage](#updateimage) (somente para definidos pelo usuário imagens), ou chamando [CMFCToolBarImages::Clear](#clear)e adicionar as imagens novamente chamando [CMFCToolBarImages::AddIcon](#addicon) ou [CMFCToolBarImages::AddImage](#addimage).  
  
##  <a name="is32bittransparencysupported"></a>  CMFCToolBarImages::Is32BitTransparencySupported  
 Especifica se o sistema operacional dá suporte a combinação alfa de 32 bits.  
...
### <a name="remarks"></a>Comentários  
 Use esse método estático para determinar no tempo de execução se o sistema operacional dá suporte a combinação alfa de 32 bits. Esse recurso tem suporte no Windows 2000 e versões posteriores.  
...
##  <a name="isrtl"></a>  CMFCToolBarImages::IsRTL  
 Especifica se o suporte a direita para esquerda (RTL) está habilitado.  
...
### <a name="remarks"></a>Comentários  
 Suporte a RTL é usado quando o aplicativo for localizado para um idioma que é lido da direita para a esquerda, como árabe, hebraico, persa ou Urdu.  
...
### <a name="remarks"></a>Comentários  
 O `CMFCToolBarImages` objeto não é válido quando o identificador para um bitmap com imagens da barra de ferramentas for NULL.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 [in] *bAdicionar*  
 TRUE para adicionar o bitmap carregado para o bitmap existente, ou FALSE para substituir o bitmap existente.  
  
 [in] *lpszBmpFileName*  
 Um caminho para um arquivo de disco do qual carregar o bitmap.  
  
 [in] *nMaxFileSize*  
 Número máximo de bytes no arquivo de bitmap; ou 0 para carregar o bitmap, independentemente do tamanho do arquivo. Se o tamanho do arquivo exceder esse tamanho máximo, o método retornará FALSE e não carrega o bitmap.  
  
### <a name="return-value"></a>Valor de retorno  
 TRUE se o bitmap foi carregado com êxito; Caso contrário, FALSE.  
  
### <a name="remarks"></a>Comentários  
 Se o arquivo tem o atributo somente leitura, a lista de imagens está marcada como somente leitura.  
...
##  <a name="mirror"></a>  CMFCToolBarImages::Mirror  
 Substitui as imagens da barra de ferramentas com suas imagens de espelho horizontal.  
...
### <a name="remarks"></a>Comentários  
 Esse método é usado para dar suporte a sistemas de escrita da direita para esquerda.  
  
##  <a name="mirrorbitmap"></a>  CMFCToolBarImages::MirrorBitmap  
 Substitui um bitmap com sua imagem espelho horizontal.  
...
### <a name="parameters"></a>Parâmetros  
 [no, out] *hbmp*  
 Um identificador para o bitmap para espelhar.  
...
### <a name="remarks"></a>Comentários  
 Essa função é usada para dar suporte a sistemas de escrita da direita para esquerda.  
...
### <a name="remarks"></a>Comentários  
 Defina essa variável de membro como FALSE para habilitar truecolor transparência para imagens da barra de ferramentas.  
  
 O valor padrão é verdadeiro para compatibilidade com versões anteriores.  
  
##  <a name="preparedrawimage"></a>  CMFCToolBarImages::PrepareDrawImage  
 Aloca os recursos que são necessárias para desenhar uma imagem da barra de ferramentas em um tamanho especificado.  
...
### <a name="parameters"></a>Parâmetros  
 [in] *ds*  
 Uma referência ao `CAfxDrawState` estrutura, que armazena os recursos alocados entre estágios de processamento de imagem.  
  
 [in] *sizeImageDest*  
 Especifica o tamanho de uma imagem de destino.  
  
 [in] *bFadeInactive*  
 TRUE se você quiser atenuado inativas imagens a ser desenhado.  
...
### <a name="remarks"></a>Comentários  
 Depois de chamar esse método, você pode chamar [CMFCToolBarImages::Draw](#draw) qualquer número de vezes. Depois de concluído o desenho, você deve chamar [CMFCToolBarImages::EndDrawImage](#enddrawimage) para liberar os recursos alocados pelo `PrepareDrawImage`.  
...
### <a name="remarks"></a>Comentários  
 Chame esse método para armazenar as imagens definidas pelo usuário em um arquivo de disco. Se *lpszBmpFileName* for NULL, o método armazena o bitmap no arquivo do qual o bitmap foi carregado pelo [CMFCToolBarImages::Load](#load) método.  
...
### <a name="remarks"></a>Comentários  
 Use esse método para definir um valor de alfa personalizado para imagens desabilitados. O valor padrão é de 127, que faz com que as imagens de botão desabilitado para que sejam semitransparentes. Se você definir um valor de 0, desabilitadas imagens será completamente transparentes. Se você definir um valor de 255, imagens desabilitadas será completamente opacas.  
...
### <a name="remarks"></a>Comentários  
 Por padrão, o tamanho da imagem da barra de ferramentas é 16 x 15 pixels. Chame esse método se você quiser usar imagens da barra de ferramentas de um tamanho diferente.  
...
### <a name="remarks"></a>Comentários  
 Quando você ou o framework chama [CMFCToolBarImages::Draw](#draw), o método não desenhar qualquer pixel que corresponde à cor especificada pela *clrTransparent*.  
  
##  <a name="updateimage"></a>  CMFCToolBarImages::UpdateImage  
 Atualiza uma imagem de barra de ferramentas definidas pelo usuário a partir de um bitmap.  
...
### <a name="parameters"></a>Parâmetros  
 [in] *iImage*  
 O índice baseado em zero da imagem a ser atualizada.  
  
 [in] *hbmp*  
 Um identificador para o bitmap do qual atualizar a imagem.  
  
### <a name="return-value"></a>Valor de retorno  
 TRUE se a imagem foi atualizada com êxito; FALSE se a lista de imagens não é temporário ou definidas pelo usuário.  
  
##  <a name="convertto32bits"></a>  CMFCToolBarImages::ConvertTo32Bits  
 Converte sublinhado bitmaps para imagens de 32 bpp.  
...
### <a name="parameters"></a>Parâmetros  
 *clrTransparent*  
 Especifica a cor transparente de bitmaps de sublinhado.  
  
### <a name="remarks"></a>Comentários  
  
##  <a name="getbitsperpixel"></a>  CMFCToolBarImages::GetBitsPerPixel  
 Retorna a resolução atual das imagens de sublinhado.  
...
### <a name="return-value"></a>Valor de retorno  
 Um valor inteiro que representa a resolução atual das imagens de sublinhado, em bits por pixel (bpp).  
  
### <a name="remarks"></a>Comentários  
  
##  <a name="getscale"></a>  CMFCToolBarImages::GetScale  
 Retorna a taxa de escala atual das imagens de sublinhado.  
...
##  <a name="isscaled"></a>  CMFCToolBarImages::IsScaled  
 Informa se imagens sublinhadas são dimensionadas ou não.  
