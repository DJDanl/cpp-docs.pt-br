...
|[COleDataSource::D elayRenderFileData](#delayrenderfiledata)|Oferece dados em um formato especificado em um `CFile` ponteiro.|
...
- O tempo de espera de arrastar do Windows 3. x é armazenado no arquivo de WIN.INI, na seção [Windows}.
...
Se *lpStgMedium* ->  *TYMED* for TYMED_NULL, o `STGMEDIUM` deverá ser alocado e preenchido conforme especificado pelo *lpFormatEtc->TYMED*. Se não for TYMED_NULL, o `STGMEDIUM` deverá ser preenchido no local com os dados.
