...
Os parâmetros *lParam1* e *lParam2* correspondem ao `lParam` membro da estrutura [TVITEM](/windows/win32/api/commctrl/ns-commctrl-tvitemw) para os dois itens que estão sendo comparados. O parâmetro *lParamSort* corresponde ao `lParam` membro da `TV_SORTCB` estrutura.
