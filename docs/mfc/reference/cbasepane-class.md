...
|[CBasePane::CanAutoHide](#canautohide)|Determina se o painel oferece suporte ao modo ocultar automaticamente.|
...
|[CBasePane::DockPane](#dockpane)|Encaixa um painel em outro painel ou em uma janela do quadro.|
|[CBasePane::DockPaneUsingRTTI](#dockpaneusingrtti)|Encaixa o painel usando informações de tipo de tempo de execução.|
|[CBasePane::DockToFrameWindow](#docktoframewindow)|Encaixa um painel encaixáveis em um quadro.|
...
|`CBasePane::get_accChild`|Chamado pelo Framework para recuperar o endereço de uma `IDispatch` interface para o filho especificado. (Substitui [CWnd:: get_accChild](../../mfc/reference/cwnd-class.md#get_accchild).)|
...
|`CBasePane::get_accParent`|Chamado pelo Framework para recuperar a `IDispatch` interface para o pai do objeto. (Substitui [CWnd:: get_accParent](../../mfc/reference/cwnd-class.md#get_accparent).)|
...
|[CBasePane::IsFloating](#isfloating)|Determina se o painel está flutuante.|
...
|[CBasePane::IsTabbed](#istabbed)|Determina se o painel foi inserido no controle guia de uma janela com guias.|
...
|[CBasePane::PaneFromPoint](#panefrompoint)|Retorna o painel que contém o ponto determinado.|
...
Se você quiser criar uma classe de painel que dê suporte aos recursos de encaixe estendidos disponíveis no MFC, você deve derivar dele de `CBasePane` ou para a [Classe CPane](../../mfc/reference/cpane-class.md).

## <a name="customization-tips"></a>Dicas de personalização

As seguintes dicas de personalização pertencem ao `CBasePane Class` e às classes que herdam dele:
...
`CBasePane`implementa os seguintes métodos boolianos virtuais para refletir estes sinalizadores: [CBasePane::CanBeClosed](#canbeclosed), [CBasePane::CanAutoHide](#canautohide), [CBasePane::CanFloat](#canfloat). Você pode substituí-los em classes derivadas para personalizar seu comportamento.
...
- Se você quiser criar um painel redimensionável não flutuante, crie-o sem o estilo AFX_CBRS_FLOAT e chame [CFrameWnd::D ockcontrolbar](../../mfc/reference/cframewnd-class.md#dockcontrolbar).
...
- Se você quiser flutuar ou desencaixar um painel que está no modo de ocultação automática, deverá chamar [CDockablePane::](../../mfc/reference/cdockablepane-class.md#setautohidemode) autoocultarmode com false como o primeiro argumento antes de chamar [CBasePane:: FloatPane](#floatpane) ou [CBasePane:: UndockPane](#undockpane).

## <a name="example"></a>Exemplo

O exemplo a seguir demonstra como usar vários métodos na `CBasePane` classe. O exemplo demonstra como recuperar um painel da `CFrameWndEx` classe e como definir o modo de encaixe, o alinhamento do painel e o estilo do painel. O código é do [exemplo do Word pad](../../overview/visual-cpp-samples.md).
...
O tamanho da barra de controle, em pixels, `CSize` de um objeto.
...
Esse método verifica o sinalizador AFX_CBRS_RESIZE, que é especificado por padrão no `CBasePane::OnCreate`. Se esse sinalizador não for especificado, o Gerenciador de encaixe sinalizará o painel internamente como immovível, em vez de encaixar.
...
Somente objetos de determinados `CBasePane`tipos derivados, como a [Classe CDockablePane](../../mfc/reference/cdockablepane-class.md), podem ser convertidos em documentos com guias.
...
Cria uma janela da classe `lpszClassName`. Se você especificar WS_CAPTION, esse método limpará o bit do estilo WS_CAPTION e `CBasePane::m_bHasCaption` definirá como true, porque a biblioteca não oferece suporte a painéis com legendas.
...
Chame esse método para habilitar o alinhamento de encaixe no quadro principal. Você pode passar uma combinação de sinalizadores CBRS_ALIGN_ (para obter mais informações, consulte [CControlBar:: EnableDocking](../../mfc/reference/ccontrolbar-class.md#enabledocking)).

`EnableDocking`define o sinalizador `CBasePane::m_dwEnabledAlignment` interno e a estrutura verifica esse sinalizador quando um painel é encaixado.
...
Uma combinação de bit-a-ou de sinalizadores AFX_CBRS_.
...
DT_STANDARD se arrastar o painel for indicado na tela por um retângulo de arrastar. DT_IMMEDIATE se o conteúdo do painel for arrastado.

### <a name="remarks"></a>Comentários

A estrutura chama esse método para determinar o modo de encaixe atual do painel.

Se `CBasePane::m_dockMode` for indefinido (DT_UNDEFINED), o modo de encaixe será tirado do modo de encaixe global (`AFX_GLOBAL_DATA::m_dockModeGlobal`).

Ao definir *m_dockMode* ou substituição `GetDockingMode` , você pode controlar o modo de encaixe para cada painel.
...
Um ponteiro para `CDockingPanesRow` se o painel for encaixado ou nulo se estiver flutuante.
...
Uma combinação de estilos de barra de controle (incluindo estilos CBRS_) que foi definida pelo método [CBasePane::](#setpanestyle) SetPaneStyle no momento da criação.
...
Esse método não tem funcionalidade para a `CBasePane` classe.
...
O `CBasePane`objeto derivado que contém o ponto determinado, ou NULL se nenhum painel foi encontrado.
...
Inicialmente, o modo de encaixe é definido globalmente por [CDockingManager:: m_dockModeGlobal](../../mfc/reference/cdockingmanager-class.md#m_dockmodeglobal). Você pode definir o modo de encaixe para cada painel individualmente usando o `SetDockingMode` método.
...
*pWndInsertAfter*<br/>
no Identifica o `CWnd` objeto que vem antes desse `CWnd` objeto na ordem Z. Para obter mais informações, consulte [CWnd:: SetWindowPos](../../mfc/reference/cwnd-class.md#setwindowpos).
...
Esse método mostra ou oculta um painel. Use esse método em vez `ShowWindow` de porque esse método notifica os gerentes de encaixe relevantes sobre as alterações na visibilidade do painel.
