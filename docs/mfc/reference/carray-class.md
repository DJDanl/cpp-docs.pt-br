...
> [!NOTE]
> A maioria dos métodos que redimensionam um `CArray` objeto ou adiciona elementos a ele usa [memcpy_s](../../c-runtime-library/reference/memcpy-s-wmemcpy-s.md) para mover elementos. Isso é um problema porque `memcpy_s` o não é compatível com nenhum objeto que exija que o Construtor seja chamado. Se os itens no `CArray` não forem compatíveis com o `memcpy_s` , você deverá criar um novo `CArray` tamanho apropriado. Em seguida, você deve usar [CArray:: Copy](#copy) e [CArray:: SetAt](#setat) para popular a nova matriz porque esses métodos usam um operador de atribuição em vez de `memcpy_s` .
...
Determinadas funções de membro dessa classe chamam funções auxiliares globais que devem ser personalizadas para a maioria dos usos da `CArray` classe. Consulte o tópico [auxiliares de classe de coleção](../../mfc/reference/collection-class-helpers.md) na seção macros e globais do MFC.

A derivação de classe de matriz é como derivação de lista.

Para obter mais informações sobre como usar `CArray` o, consulte o artigo [coleções](../../mfc/collections.md).
...
Se [SetSize](#setsize) tiver sido usado com um `nGrowBy` valor maior que 1, a memória extra poderá ser alocada. No entanto, o limite superior aumentará apenas em 1.
...
A segunda versão insere todos os elementos de outra `CArray` coleção, começando na posição *nStartIndex* .
...
## <a name="carrayoperator-"></a><a name="operator_at"></a>Operador CArray::\[\]
...
[Auxiliares de classe de coleção](../../mfc/reference/collection-class-helpers.md)
