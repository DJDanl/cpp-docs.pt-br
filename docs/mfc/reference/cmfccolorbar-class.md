...
|[CMFCColorBar:: CreateControl](#createcontrol)|Cria uma janela de controle de barra de cores, anexa-a ao `CMFCColorBar` objeto e redimensiona o controle para conter a paleta de cores especificada.|
|[CMFCColorBar:: criar](#create)|Cria uma janela de controle de barra de cores e a anexa ao `CMFCColorBar` objeto.|
...
1. Defina todos os estilos e habilite ou desabilite os recursos do `CMFCColorBar` objeto durante a criação da [Classe CMFCColorMenuButton](../../mfc/reference/cmfccolormenubutton-class.md) . O `CMFCColorMenuButton` objeto cria dinamicamente o `CMFCColorBar` objeto depois que a estrutura chama o `CreatePopupMenu` método.
...
Esse método é chamado pelo Framework durante o processamento de WM_SIZE mensagem.
...
*src*<br/>
no Um `CMFCColorBar` objeto existente a ser copiado para o novo `CMFCColorBar` objeto.
...
*bSquareButtons*\
no TRUE para especificar que a forma dos botões em um controle de barra de cores é quadrado; caso contrário, FALSE. O valor padrão é TRUE.

*bCenterButtons*\
no TRUE para especificar que o conteúdo na face de um botão de controle de barra de cores está centralizado; caso contrário, FALSE. O valor padrão é TRUE.

### <a name="remarks"></a>Comentários

## <a name="cmfccolorbarcreate"></a><a name="create"></a> CMFCColorBar:: criar

Cria uma janela de controle de barra de cores e a anexa ao `CMFCColorBar` objeto.
...
*pPalette*<br/>
no Ponteiro para uma paleta de cores. O padrão é NULO.
...
Cria uma janela de controle de barra de cores, anexa-a ao `CMFCColorBar` objeto e redimensiona a janela de controle para conter a paleta de cores especificada.
...
*pPalette*<br/>
no Ponteiro para uma paleta de cores ou NULL. Se esse parâmetro for nulo, esse método calculará o tamanho do controle de barra de cores como se foram especificadas 20 cores. O padrão é NULO.
...
*arColors*\
no Uma matriz de cores.

*Palette*\
no Uma paleta de cores.
...
O rótulo padrão para esse botão é **mais cores...**
...
*bVertDock*\
no TRUE para executar o cálculo de um controle de barra de cores encaixado verticalmente; caso contrário, execute o cálculo para um controle encaixado horizontalmente.
...
Quando o usuário seleciona uma nova cor, a estrutura envia a ID de comando em uma WM_COMMAND mensagem para notificar o pai do `CMFCColorBar` objeto.
...
*nNumColumns*\
no Se o controle de barra de cores contiver cores de documento, o número de colunas a serem exibidas na grade de cores do documento. Caso contrário, esse valor não será usado.
...
*pPalette*\
no Um ponteiro para um objeto Palette ou NULL. Se esse parâmetro for nulo, esse método usará a paleta padrão do sistema operacional.
...
*pButton*\
no Ponteiro para um controle que reside em uma barra de ferramentas.
...
*Primário*\
no Ponteiro para o contexto do dispositivo do botão pai do controle da barra de cores atual.
...
*pWndList*\
no Ponteiro para objeto de controle de grade de propriedade.
...
Esse método envia a mensagem de WM_SETMESSAGESTRING para o proprietário do controle de barra de cores.
