...
|[CEdit::CharFromPos](#charfrompos)|Recupera os índices de linhas e de caracteres do caractere mais próximo para uma posição especificada.|  
|[CEdit::Clear](#clear)|Exclui (limpa) a seleção atual (se houver) na edição de controle.|  
|[CEdit::Copy](#copy)|Copia a seleção atual (se houver) no controle de edição para a área de transferência em **CF_TEXT** formato.|  
|[CEdit::Create](#create)|Cria o controle de edição do Windows e a anexa ao `CEdit` objeto.|  
|[CEdit::Cut](#cut)|Controlar o exclui (cortes) a seleção atual (se houver) na edição e copia o texto excluído na área de transferência em **CF_TEXT** formato.|  
|[CEdit::EmptyUndoBuffer](#emptyundobuffer)|Redefine (limpa) o sinalizador de desfazer de uma edição de controle.|  
...
|[CEdit::GetFirstVisibleLine](#getfirstvisibleline)|Determina a linha superior visível em um controle de edição.|  
...
|[CEdit::GetLimitText](#getlimittext)|Obtém a quantidade máxima de texto isso `CEdit` pode conter.|  
...
|[CEdit::GetPasswordChar](#getpasswordchar)|Recupera o caractere de senha exibido em um controle de edição quando o usuário insere o texto.|  
...
|[CEdit::SetHighlight](#sethighlight)|Realça um intervalo de texto que é exibido no atual de controle de edição.|  
...
|[CEdit::SetPasswordChar](#setpasswordchar)|Define ou remove um caractere de senha exibido em um controle de edição quando o usuário insere o texto.|  
...
|[CEdit::SetTabStops](#settabstops)|As paradas de tabulação em uma linha de vários conjuntos de controle de edição.|  
|[CEdit::ShowBalloonTip](#showballoontip)|Exibe uma dica de balão que está associada ao controle de edição atual.|  
...
## <a name="remarks"></a>Comentários  
 ...
 Você pode criar um controle de edição de um modelo de caixa de diálogo ou diretamente em seu código. Em ambos os casos, chame o construtor `CEdit` para construir o `CEdit` do objeto e, em seguida, chamar o [criar](#create) a função de membro para criar o Windows controle de edição e anexá-lo para o `CEdit` objeto.  
 ...
 `CEdit`herda a funcionalidade da `CWnd`. Para definir e recuperar o texto de um `CEdit` de objeto, use o `CWnd` funções de membro [SetWindowText](cwnd-class.md#setwindowtext) e [GetWindowText](cwnd-class.md#getwindowtext), que definir ou obter todo o conteúdo de um controle de edição, mesmo se ele é um controle de várias linhas. Linhas de texto em um controle de várias linhas são separadas por '\r\n' sequências de caracteres. Além disso, se um controle de edição é multilinha, obter e definir a parte do texto do controle chamando o `CEdit` funções de membro [GetLine](#getline), [SetSel](#setsel), [GetSel](#getsel), e [ReplaceSel](#replacesel).  
  
 Se você desejar tratar mensagens de notificação do Windows enviadas por um controle de edição para seu pai (geralmente uma classe derivada de `CDialog`), adicionar uma função de membro de entrada e o manipulador de mensagens do mapa de mensagem para a classe pai para cada mensagem.  
...
- **ON_EN_CHANGE** o usuário executou uma ação que pode ter alterado o texto em um controle de edição. Ao contrário de **EN_UPDATE** mensagem de notificação, essa mensagem de notificação é enviada depois que a exibição de atualizações do Windows.  
  
- **ON_EN_ERRSPACE** o controle de edição não é possível alocar memória suficiente para atender uma solicitação específica.  
  
- **ON_EN_HSCROLL** o usuário clica em barras de rolagem horizontal do controle de edição. A janela pai é notificada antes da tela seja atualizada.  
  
- **ON_EN_KILLFOCUS** o controle de edição perde o foco de entrada.  
  
- **ON_EN_MAXTEXT** a inserção atual excedeu o número especificado de caracteres de controle de edição e foi truncada. Também é enviado quando um controle de edição não tem o **ES_AUTOHSCROLL** estilo e o número de caracteres a ser inserido excede a largura do controle de edição. Também é enviado quando um controle de edição não tem o **ES_AUTOVSCROLL** estilo e o número total de linhas resultante de uma inserção de texto exceder a altura do controle de edição.  
  
- **ON_EN_SETFOCUS** enviadas quando um controle de edição recebe o foco de entrada.  
  
- **ON_EN_UPDATE** o controle de edição está prestes a exibir o texto alterado. Enviada depois que o controle tem o texto formatado, mas antes de ele filtra o texto para que o tamanho da janela pode ser alterado, se necessário.  
  
- **ON_EN_VSCROLL** o usuário clica em barras de rolagem vertical do controle de edição. A janela pai é notificada antes da tela seja atualizada.  
 ...
 Se você criar um `CEdit` objeto dentro de uma janela, você também precisa destruída. Se você criar o `CEdit` do objeto na pilha, ele será destruído automaticamente. Se você criar o `CEdit` objeto na pilha usando o **novo** função, você deve chamar **excluir** sobre o objeto a ser destruída quando o usuário termina o Windows controle de edição. Se você alocar toda a memória no `CEdit` de objeto, substituir o `CEdit` destruidor para descartar as alocações.  
  
 Para modificar alguns estilos em um controle de edição (como **ES_READONLY**) você deve enviar mensagens específicas para o controle em vez de usar [ModifyStyle](cwnd-class.md#modifystyle). Consulte [estilos de controle de edição](http://msdn.microsoft.com/library/windows/desktop/bb775464) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se a última operação de edição pode ser desfeita por uma chamada para o **desfazer** função de membro; 0 se ele não pode ser desfeito.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::Undo](#undo).  
...
##  <a name="charfrompos"></a>CEdit::CharFromPos  
 Chamar essa função para recuperar a linha de base zero e os índices de caracteres do caractere mais próximo ponto especificado neste `CEdit` controle  
...
### <a name="return-value"></a>Valor de retorno  
 O índice de caractere na ordem baixa **WORD**e o índice de linha na ordem alta **WORD**.  
...
##  <a name="copy"></a>CEdit::Copy  
 Chamar esta função para copiar a seleção atual (se houver) no controle de edição para a área de transferência em **CF_TEXT** formato.  
...
##  <a name="create"></a>CEdit::Create  
 Cria o controle de edição do Windows e a anexa ao `CEdit` objeto.  
...
### <a name="remarks"></a>Comentários  
 Você pode criar um `CEdit` objeto em duas etapas. Primeiro, chame o `CEdit` construtor e em seguida, chame **criar**, que cria o controle de edição do Windows e a anexa ao `CEdit` objeto.  
 ...
 Essas mensagens são tratadas por padrão, o [OnNcCreate](cwnd-class.md#onnccreate), [OnNcCalcSize](cwnd-class.md#onnccalcsize), [OnCreate](cwnd-class.md#oncreate), e [OnGetMinMaxInfo](cwnd-class.md#ongetminmaxinfo) funções de membro no `CWnd` classe base. Para estender a manipulação de mensagem padrão, derive uma classe de `CEdit`, adicione um mapa de mensagem para a nova classe e substituem as funções de membro acima do manipulador de mensagens. Substituir `OnCreate`, por exemplo, para executar inicialização necessária para a nova classe.  
  
 Aplicar o seguinte [estilos de janela](window-styles.md) para um controle de edição.  
...
##  <a name="cut"></a>CEdit::Cut  
 Chamar essa função para excluir (Recortar) a seleção atual (se houver) no controle de edição e copie o texto excluído na área de transferência em **CF_TEXT** formato.  
...
##  <a name="emptyundobuffer"></a>CEdit::EmptyUndoBuffer  
 Chame essa função para redefinir (Apagar) o sinalizador de desfazer de um controle de edição.  
...
### <a name="parameters"></a>Parâmetros  
 *bAddEOL*  
 Especifica se os caracteres de quebra de linha suave devem ser inseridos. Um valor de **TRUE** insere os caracteres; um valor de **FALSE** remove-los.  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se qualquer formatação ocorre; Caso contrário, 0.  
  
### <a name="remarks"></a>Comentários  
 Uma quebra de linha suave consiste em dois retornos de carro e alimentação de linhas inserido no final de uma linha que foi interrompida devido a quebra de texto. Uma quebra de linha rígido consiste em um retorno de carro e um avanço de linha. As linhas que terminam com uma quebra de linha definida não são afetadas por `FmtLines`.  
  
 Windows só responderá se o `CEdit` objeto é um controle de edição de várias linhas.  
  
 `FmtLines`só afeta o buffer retornado por [GetHandle](#gethandle) e o texto retornado pela [WM_GETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632627). Ele não tem impacto sobre a exibição do texto dentro do controle de edição.  
...
### <a name="parameters"></a>Parâmetros  
 [out] `lpszText`  
 Um ponteiro para uma cadeia de caracteres que contém o texto da indicação.  
  
 [in] `cchText`  
 O número de caracteres que pode ser recebido. Esse número inclui a terminação `NULL` caracteres.  
  
### <a name="return-value"></a>Valor de retorno  
 Para a primeira sobrecarga, `true` se o método for bem-sucedida; caso contrário `false`.  
  
 Para a segunda sobrecarga, uma [CString](../../atl-mfc-shared/using-cstring.md) que contém o texto da indicação se o método for bem-sucedida; caso contrário, a cadeia de caracteres vazia ("").  
  
### <a name="remarks"></a>Comentários  
 Esse método envia o [EM_GETCUEBANNER](http://msdn.microsoft.com/library/windows/desktop/bb761572) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Para obter mais informações, consulte o [Edit_GetCueBannerText](http://msdn.microsoft.com/library/windows/desktop/bb761695) macro.  
...
### <a name="remarks"></a>Comentários  
 O identificador é um identificador de memória local e podem ser usado por qualquer uma da **Local** lidar com funções de memória do Windows que usam um local de memória como um parâmetro.  
  
 **GetHandle** é processada apenas por controles de edição de várias linhas.  
  
 Chamar **GetHandle** para um controle de edição de várias linhas em uma caixa de diálogo somente se a caixa de diálogo foi criada com o **DS_LOCALEDIT** conjunto de sinalizadores de estilo. Se o **DS_LOCALEDIT** estilo não for definido, você ainda terá um valor de retorno diferente de zero, mas você não poderá usar o valor retornado.  
  
> [!NOTE]
> **GetHandle** não funcionará com o Windows 95/98. Se você chamar **GetHandle** no Windows 95/98, ela retornará **nulo**. **GetHandle** funcionará conforme documentado no Windows NT, versões 3.51 e posteriores.  
...
|Parâmetro|Descrição|  
|---------------|-----------------|  
|[out] `pichStart`|Índice de base zero do primeiro caractere no intervalo de texto que está realçado.|  
|[out] `pichEnd`|Índice de base zero do último caractere no intervalo de texto que está realçado.|  
  
### <a name="return-value"></a>Valor de retorno  
 `true`Se esse método for bem-sucedida; Caso contrário, `false`.  
  
### <a name="remarks"></a>Comentários  
 Esse método envia o [EM_GETHILITE](http://msdn.microsoft.com/library/windows/desktop/bb761578) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="getline"></a>CEdit::GetLine  
 Chamar essa função para recuperar uma linha de texto de um controle de edição e o coloca em `lpszBuffer`.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nMaxLength`  
 Especifica o número máximo de bytes que podem ser copiados para o buffer. `GetLine`coloca esse valor da primeira palavra de `lpszBuffer` antes de fazer a chamada para o Windows.  
  
### <a name="return-value"></a>Valor de retorno  
 O número de bytes copiados na verdade. O valor de retorno será 0 se o número da linha especificada por `nIndex` é maior que o número de linhas no controle de edição.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::GetLineCount](#getlinecount).  
...
### <a name="return-value"></a>Valor de retorno  
 Controle de edição de um inteiro que contém o número de linhas em várias linhas. Se nenhum texto tenha sido inserido no controle de edição, o valor de retorno é 1.  
...
##  <a name="getmargins"></a>CEdit::GetMargins  
 Chame essa função de membro para recuperar as margens esquerdas e direita deste controle de edição.  
...
### <a name="return-value"></a>Valor de retorno  
 A largura da margem esquerda na ordem baixa **WORD** e a largura da margem direita na ordem alta **WORD**.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEditView::GetEditCtrl](ceditview-class.md#geteditctrl).  
...
### <a name="remarks"></a>Comentários  
 O Windows mantém um sinalizador interno que indica se o conteúdo do controle de edição foram alterado. Este sinalizador será apagado quando o controle de edição é criada pela primeira vez e também pode ser eliminado chamando o [SetModify](#setmodify) função de membro.  
...
##  <a name="getpasswordchar"></a>CEdit::GetPasswordChar  
 Chame essa função para recuperar o caractere de senha é exibido em um controle de edição quando o usuário insere o texto.  
...
### <a name="remarks"></a>Comentários  
 Se você criar o controle de edição com o **ES_PASSWORD** estilo, a DLL que oferece suporte o controle determina o caractere de senha padrão. O manifesto ou [InitCommonControlsEx](http://msdn.microsoft.com/library/windows/desktop/bb775697) método determina qual DLL dá suporte a controle de edição. Se User32 dá suporte a controle de edição, o caractere de senha padrão é asterisco ('* ', U + 002A). Se a versão de comctl32.dll 6 oferece suporte a controle de edição, o caractere padrão é CÍRCULO preto ('●', U + 25CF). Para obter mais informações sobre o que dá suporte a DLL e versão controles comuns, consulte [Shell e versões de controles comuns](http://msdn.microsoft.com/library/windows/desktop/bb776779).  
  
 Esse método envia o [EM_GETPASSWORDCHAR](http://msdn.microsoft.com/library/windows/desktop/bb761594) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Comentários  
 O retângulo de formatação é o limitação retângulo do texto, que é independente do tamanho da janela do controle de edição.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::LimitText](#limittext).  
  
##  <a name="getsel"></a>CEdit::GetSel  
 Chame essa função para obter o início e término posições de caractere da seleção atual (se houver) em um controle de edição, usando o valor de retorno ou os parâmetros.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nEndChar`  
 Referência a um número inteiro que receberá a posição do primeiro caractere que não selecionado após o fim da seleção atual.  
  
### <a name="return-value"></a>Valor de retorno  
 A versão que retorna um `DWORD` retorna um valor que contém a posição inicial da palavra de ordem inferior e a posição do primeiro caractere que não selecionado após o final da seleção da palavra de ordem superior.  
...
### <a name="remarks"></a>Comentários  
 Essa função envia o [EM_HIDEBALLOONTIP](http://msdn.microsoft.com/library/windows/desktop/bb761604) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valor de retorno  
 O índice de linha especificada no `nLine` ou -1 se o número de linha especificado é maior que o número de linhas no controle de edição.  
...
### <a name="parameters"></a>Parâmetros  
 `nLine`  
 O índice de base zero de um caractere na linha cujo comprimento é a ser recuperado. O valor padrão é -1.  
  
### <a name="return-value"></a>Valor de retorno  
 Para controles de edição de linha única, o valor de retorno é o comprimento, em `TCHAR`s, do texto no controle de edição.  
  
 Para controles de edição de várias linhas, o valor de retorno é o comprimento, em `TCHAR`s, da linha especificada pelo `nLine` parâmetro. Para [!INCLUDE[vcpransi](../../atl-mfc-shared/reference/includes/vcpransi_md.md)] texto, o comprimento é o número de bytes na linha; texto Unicode, o comprimento é o número de caracteres na linha. O comprimento não inclui o caractere de retorno de carro no final da linha.  
  
 Se o `nLine` parâmetro for maior que o número de caracteres no controle, o valor de retorno é zero.  
  
 Se o `nLine` parâmetro é -1, o valor de retorno é o número de caracteres não selecionados em linhas que contêm caracteres selecionados. Por exemplo, se estende a seleção do quarto caracteres de uma linha por meio do oitavo caracteres do final da próxima linha, o valor de retorno é 10. Isto é, três caracteres na primeira linha e sete na próxima.  
  
 Para obter mais informações sobre o `TCHAR` de tipo, consulte o `TCHAR` linha na tabela [tipos de dados do Windows](http://msdn.microsoft.com/library/windows/desktop/aa383751).  
  
### <a name="remarks"></a>Comentários  
 Este método é suportado pelo [EM_LINELENGTH](http://msdn.microsoft.com/library/windows/desktop/bb761613) mensagem, que é descrita no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::LineIndex](#lineindex).  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nChars`  
 Especifica o número de posições de caractere para rolar horizontalmente. Esse valor é ignorado se o controle de edição foi o **ES_RIGHT** ou **ES_CENTER** estilo.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::GetFirstVisibleLine](#getfirstvisibleline).  
  
##  <a name="paste"></a>CEdit::Paste  
 Chamar essa função para inserir os dados da área de transferência para o `CEdit` no ponto de inserção.  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_MFC_CEdit n º 20](../../mfc/reference/codesnippet/cpp/cedit-class_19.cpp)]  
  
##  <a name="posfromchar"></a>CEdit::PosFromChar  
 Chamar essa função para obter a posição (canto superior esquerdo) de um determinado caractere dentro deste `CEdit` objeto.  
...
### <a name="remarks"></a>Comentários  
 O caractere for especificado, fornecendo o seu valor de índice baseado em zero. Se `nChar` é maior do que o índice do último caractere na `CEdit` do objeto, o valor de retorno Especifica as coordenadas de posição do caractere apenas após o último caractere na `CEdit` objeto.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::LineFromChar](#linefromchar).  
  
##  <a name="replacesel"></a>CEdit::ReplaceSel  
 Chamar essa função para substituir a seleção atual em um controle de edição com o texto especificado por `lpszNewText`.  
...
### <a name="remarks"></a>Comentários  
 ...
 Se não houver nenhuma seleção atual, o texto de substituição é inserido na posição do cursor atual.  
...
##  <a name="setcuebanner"></a>CEdit::SetCueBanner  
 Define o texto que é exibido como a indicação de texto, ou dica, em uma edição de controla quando o controle está vazio.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 [in] `fDrawWhenFocused`  
 Se `false`, a faixa de indicação não é desenhada quando o usuário clica no controle de edição e dá controle o foco.  
  
 Se `true`, a faixa de indicação é desenhada, mesmo quando o controle tem foco. A faixa de indicação desaparece quando o usuário começa a digitar no controle.  
...
### <a name="remarks"></a>Comentários  
 Esse método envia o [EM_SETCUEBANNER](http://msdn.microsoft.com/library/windows/desktop/bb761639) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Para obter mais informações, consulte o [Edit_SetCueBannerTextFocused](http://msdn.microsoft.com/library/windows/desktop/bb761703) macro.  
...
### <a name="remarks"></a>Comentários  
 ...
 Antes que um aplicativo defina um novo identificador de memória, ele deve usar o [GetHandle](#gethandle) a função de membro para obter o identificador para o buffer de memória atual e liberar essa memória usando o **LocalFree** função do Windows.  
  
 `SetHandle`Limpa o buffer de desfazer (o [CanUndo](#canundo) função de membro, em seguida, retorna 0) e o sinalizador interno de modificação (o [GetModify](#getmodify) função de membro, em seguida, retorna 0). A janela de controle de edição é redesenhada.  
...
##  <a name="sethighlight"></a>CEdit::SetHighlight  
 Realça um intervalo de texto que é exibido no atual de controle de edição.  
...
|Parâmetro|Descrição|  
|---------------|-----------------|  
|[in] `ichStart`|Índice de base zero do primeiro caractere no intervalo de texto para realçar.|  
|[in] `ichEnd`|Índice de base zero do último caractere no intervalo de texto para realçar.|  
  
### <a name="remarks"></a>Comentários  
 Esse método envia o [EM_SETHILITE](http://msdn.microsoft.com/library/windows/desktop/bb761643) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
##  <a name="setlimittext"></a>CEdit::SetLimitText  
 Chamar essa função de membro para definir o limite de texto para este `CEdit` objeto.  
...
### <a name="remarks"></a>Comentários  
 O limite de texto é a quantidade máxima de texto, em caracteres, o que pode aceitar o controle de edição.  
...
##  <a name="setmargins"></a>CEdit::SetMargins  
 Chame esse método para definir as margens esquerdas e direita deste controle de edição.  
...
### <a name="parameters"></a>Parâmetros  
 *nLeft*  
 A largura da margem esquerda nova, em pixels.  
  
 *nRight*  
 A largura da margem direita nova, em pixels.  
...
##  <a name="setpasswordchar"></a>CEdit::SetPasswordChar  
 Chame essa função para definir ou remover um caractere de senha exibido em um controle de edição quando o usuário digita o texto.  
...
### <a name="remarks"></a>Comentários  
 ...
 Essa função de membro não tem efeito em várias linhas controle de edição.  
 ...
 Se o controle de edição for criado com o [ES_PASSWORD](edit-styles.md) estilo, o caractere de senha padrão é definido como um asterisco ( **\***). Este estilo é removido se `SetPasswordChar` é chamado com *ch* definido como 0.  
...
### <a name="parameters"></a>Parâmetros  
 `bReadOnly`  
 Especifica se deve definir ou remover o estado somente leitura do controle de edição. Um valor de **TRUE** define o estado como somente leitura; um valor de **FALSE** define o estado de leitura/gravação.  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se a operação for bem-sucedida, ou 0 se um erro ocorre.  
  
### <a name="remarks"></a>Comentários  
 A configuração atual pode ser encontrada ao testar o [ES_READONLY](edit-styles.md) sinalizador no valor de retorno de [CWnd::GetStyle](cwnd-class.md#getstyle).  
...
### <a name="remarks"></a>Comentários  
 ...
 Use `SetRect` para definir a formatação de controle de edição do retângulo de várias linhas. O retângulo de formatação é o limitação retângulo do texto, que é independente do tamanho da janela do controle de edição. Quando o controle de edição é criada pela primeira vez, o retângulo a formatação é o mesmo que a área de cliente da janela do controle de edição. Usando o `SetRect` função de membro, um aplicativo pode fazer o retângulo formatação maior ou menor que a janela de controle de edição.  
  
 Se o controle de edição não tiver nenhuma barra de rolagem, texto será recortado, não encapsulados, se o retângulo formatação fica maior do que a janela. Se o controle de edição contiver uma borda, o formatação retângulo é reduzido pelo tamanho da borda. Se você ajustar o retângulo retornado pelo `GetRect` função de membro, você deve remover o tamanho da borda antes de você passar o retângulo para `SetRect`.  
...
### <a name="remarks"></a>Comentários  
 ...
 Quando o controle de edição é criada pela primeira vez, o retângulo a formatação é o mesmo que a área de cliente da janela do controle de edição. Chamando o `SetRectNP` função de membro, um aplicativo pode fazer o retângulo formatação maior ou menor que a janela de controle de edição.  
  
 Se o controle de edição não tiver nenhuma barra de rolagem, texto será recortado, não encapsulados, se o retângulo formatação fica maior do que a janela.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::SetRect](#setrect).  
...
### <a name="parameters"></a>Parâmetros  
 *dwSelection*  
 Especifica a posição inicial da palavra de ordem inferior e a posição final da palavra de ordem superior. Se a palavra de ordem inferior é 0 e a palavra de ordem alta é -1, todo o texto no controle de edição é selecionado. Se a palavra de ordem inferior é -1, qualquer seleção atual será removida.  
  
 *bNoScroll*  
 Indica se o cursor deve ser colocado na exibição. Se **FALSE**, o cursor é colocado na exibição. Se **TRUE**, o cursor não for rolado para a exibição.  
  
 `nStartChar`  
 Especifica a posição inicial. Se `nStartChar` é 0 e `nEndChar` é -1, todo o texto no controle de edição é selecionado. Se `nStartChar` é -1, qualquer seleção atual é removida.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEdit::GetSel](#getsel).  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nTabStops`  
 Especifica o número de paradas de tabulação contidos em `rgTabStops`. Esse número deve ser maior que 1.  
  
 `rgTabStops`  
 Aponta para uma matriz de inteiros sem sinal especificando a guia interrompe em unidades de diálogo. Uma unidade de diálogo é uma distância horizontal ou vertical. Uma unidade de diálogo horizontal é igual a um quarto da unidade de largura de base da caixa de diálogo atual e 1 unidade vertical da caixa de diálogo é igual a um oitavo da unidade de base altura da caixa de diálogo atual. As unidades de base da caixa de diálogo são calculadas com base na altura e largura da fonte atual do sistema. O **GetDialogBaseUnits** função Windows retorna a caixa de diálogo atual unidades base em pixels.  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se as guias foram definidas; Caso contrário, 0.  
  
### <a name="remarks"></a>Comentários  
 Quando o texto será copiado para um controle de edição de várias linhas, qualquer caractere de tabulação no texto fará com que o espaço a ser gerado até a próxima parada de tabulação.  
  
 Para definir paradas de tabulação para o tamanho padrão de 32 unidades de caixa de diálogo, chame a versão para essa função de membro. Para definir paradas de tabulação para um tamanho diferente de 32, chame a versão com o `cxEachStop` parâmetro. Para definir paradas de tabulação para uma matriz de tamanhos, use a versão com dois parâmetros.  
...
### <a name="example"></a>Exemplo  
  Consulte o exemplo para [CEditView::SetTabStops](ceditview-class.md#settabstops).  
  
##  <a name="showballoontip"></a>CEdit::ShowBalloonTip  
 Exibe uma dica de balão que está associada ao controle de edição atual.  
...
|[in] `ttiIcon`|Um `INT` que especifica o tipo de ícone a ser associado com a dica de balão. O valor padrão é `TTI_NONE`. Para obter mais informações, consulte o `ttiIcon` membro o [EDITBALLOONTIP](http://msdn.microsoft.com/library/windows/desktop/bb775466) estrutura.|  
  
### <a name="return-value"></a>Valor de retorno  
 `true`Se esse método for bem-sucedida; Caso contrário, `false`.  
  
### <a name="remarks"></a>Comentários  
 Essa função envia o [EM_SHOWBALLOONTIP](http://msdn.microsoft.com/library/windows/desktop/bb761668) mensagem, que é descrita a [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)]. Para obter mais informações, consulte o [Edit_ShowBalloonTip](http://msdn.microsoft.com/library/windows/desktop/bb761707) macro.  
...
### <a name="return-value"></a>Valor de retorno  
 Para um controle de edição de linha única, o valor de retorno sempre é diferente de zero. Para um controle de edição de várias linhas, o valor de retorno é diferente de zero se a operação de desfazer for bem-sucedida, ou 0 se a operação de desfazer falhar.  
