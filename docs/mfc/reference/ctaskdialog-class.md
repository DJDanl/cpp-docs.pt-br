...
|||
|-|-|
|[CTaskDialog:: AddCommandControl](#addcommandcontrol)|Adiciona um controle de botão de comando `CTaskDialog`ao.|
...
|[CTaskDialog::GetCommonButtonFlag](#getcommonbuttonflag)|Converte um botão padrão do Windows para o tipo de botão comum associado `CTaskDialog` à classe.|
|[CTaskDialog::GetCommonButtonId](#getcommonbuttonid)|Converte um dos tipos de botão comuns associados `CTaskDialog` à classe em um botão padrão do Windows.|
|[CTaskDialog::GetOptions](#getoptions)|Retorna os sinalizadores de opção para `CTaskDialog`isso.|
...
|[CTaskDialog:: IsSupported](#issupported)|Determina se o computador que está executando o aplicativo dá suporte `CTaskDialog`ao.|
...
|[CTaskDialog::NavigateTo](#navigateto)|Transfere o foco para `CTaskDialog`outro.|
...
|[CTaskDialog:: SetCommandControlOptions](#setcommandcontroloptions)|Atualiza um controle de botão de comando `CTaskDialog`no.|
...
|||
|-|-|
|`m_aButtons`|A matriz de controles de botão de comando `CTaskDialog`para o.|
|`m_aRadioButtons`|A matriz de controles de botão de opção `CTaskDialog`para o.|
...
|`m_hWnd`|Um identificador para a janela `CTaskDialog`do.|
...
|`m_nDefaultCommandControl`|A ID do controle de botão de comando que é selecionado quando `CTaskDialog` o é exibido.|
|`m_nDefaultRadioButton`|A ID do controle de botão de opção que é selecionado quando `CTaskDialog` o é exibido.|
...
|`m_strCollapse`|A cadeia de `CTaskDialog` caracteres exibe à direita da caixa expansão quando as informações expandidas estão ocultas.|
|`m_strContent`|A cadeia de caracteres de `CTaskDialog`conteúdo do.|
|`m_strExpand`|A cadeia de `CTaskDialog` caracteres exibe à direita da caixa expansão quando as informações expandidas são exibidas.|
|`m_strFooter`|O rodapé do `CTaskDialog`.|
|`m_strInformation`|As informações expandidas para `CTaskDialog`o.|
...
Adiciona um novo controle de botão de comando `CTaskDialog`ao.
...
*strCaption*<br/>
no A cadeia de caracteres `CTaskDialog` que o exibe para o usuário. Use essa cadeia de caracteres para explicar a finalidade do comando.
...
O `CTaskDialog Class` pode exibir um número ilimitado de controles de botão de comando. No entanto, `CTaskDialog` se um exibir qualquer controle de botão de comando, ele poderá exibir um máximo de seis botões. Se um `CTaskDialog` não tiver controles de botão de comando, ele poderá exibir um número ilimitado de botões.

Quando o usuário seleciona um controle de botão de comando `CTaskDialog` , o fecha. Se seu aplicativo exibir a caixa de diálogo usando [CTaskDialog::D omodal](#domodal), `DoModal` retornará o *nCommandControlID* do controle de botão de comando selecionado.
...
O `CTaskDialog` não exige que os parâmetros *nRadioButtonID* sejam exclusivos para cada botão de opção. No entanto, você poderá enfrentar um comportamento inesperado se não usar um identificador distinto para cada botão de opção.
...
*strContent*<br/>
no A cadeia de caracteres a ser usada para o `CTaskDialog`conteúdo do.
...
no O conjunto de opções a ser usado para `CTaskDialog`o.
...
O `CTaskDialog` é fechado quando o usuário seleciona um botão comum, um controle de link de comando ou `CTaskDialog`fecha o. O valor de retorno é o identificador que indica como o usuário fechou a caixa de diálogo.
...
O valor do botão comum `CTaskDialog` correspondente. Se não houver nenhum botão comum correspondente, esse método retornará 0.
...
*nFlag*<br/>
no O tipo de botão comum associado `CTaskDialog` à classe.
...
Retorna os sinalizadores de opção para `CTaskDialog`isso.
...
Você pode usar esse método para determinar a disponibilidade dos dois controles de botão de comando e os botões comuns `CTaskDialog` da classe *.
...
Determina se o computador que está executando o aplicativo dá suporte `CTaskDialog`ao.
...
TRUE se o computador oferecer suporte `CTaskDialog`a; Caso contrário, FALSE.

### <a name="remarks"></a>Comentários

Use essa função para determinar em tempo de execução se o computador que está executando seu aplicativo `CTaskDialog` der suporte à classe. Se o computador não oferecer suporte `CTaskDialog`ao, você deverá fornecer outro método de comunicação de informações para o usuário. Seu aplicativo falhará se tentar usar um `CTaskDialog` em um computador que não ofereça suporte à `CTaskDialog` classe.
...
Transfere o foco para `CTaskDialog`outro.
...
*lTime*<br/>
no Tempo em milissegundos desde que `CTaskDialog` o foi criado ou o temporizador foi redefinido.
...
Remove todos os controles de botão de comando `CTaskDialog`do.
...
Atualiza um controle de botão de comando `CTaskDialog`no.
...
Use este método para alterar se um controle de botão de comando está habilitado ou requer elevação após ele ter sido adicionado `CTaskDialog` à classe.
...
Você não poderá chamar esse método depois que a janela de exibição dessa instância `CTaskDialog` da classe for criada. Se você fizer isso, esse método lançará uma exceção.

Os botões indicados por *nButtonMask* substituem os botões comuns adicionados anteriormente `CTaskDialog`ao. Somente os botões indicados em *nButtonMask* estão disponíveis.
...
Quando o `CTaskDialog` é exibido pela primeira vez, ele não mostra as informações expandidas `strCollapsedLabel` e coloca ao lado do botão de expansão. Quando o usuário clica no botão de expansão, `CTaskDialog` o exibe *strExpandedInformation* e altera o rótulo para *strExpandedLabel*.
...
Esse método gera uma exceção com a [macro If](diagnostic-services.md#ensure) , se `CTaskDialog` o for exibido ou se o parâmetro de entrada for NULL.
...
O ícone de rodapé é exibido ao lado do texto do rodapé na parte `CTaskDialog`inferior do. Você pode alterar o ícone de rodapé com [CTaskDialog:: SetFooterIcon](#setfootericon).
...
*nOptionFlag*<br/>
no O conjunto de sinalizadores a serem usados para `CTaskDialog`o.
...
|TDF_NO_DEFAULT_RADIO_BUTTON|Indica que nenhum botão de opção está selecionado quando `CTaskDialog` o é exibido.|
|TDF_CAN_BE_MINIMIZED|Permite que o usuário minimize o `CTaskDialog`. Para dar suporte a essa opção `CTaskDialog` , o não pode ser modal. O MFC não oferece suporte a essa opção porque o MFC não oferece suporte `CTaskDialog`a modelos sem janela.|
...
Para exibir a barra de progresso, a opção TDF_SHOW_PROGRESS_BAR deve ser habilitada e TDF_SHOW_MARQUEE_PROGRESS_BAR não deve ser habilitado. Esse método define automaticamente TDF_SHOW_PROGRESS_BAR e limpa TDF_SHOW_MARQUEE_PROGRESS_BAR. Use [CTaskDialog:: SetOptions](#setoptions) para alterar manualmente as opções dessa instância da [classe CTaskDialog](../../mfc/reference/ctaskdialog-class.md).

Esse método gera uma exceção [com a macro](diagnostic-services.md#ensure) If If *nRangeMin* não é menor que *nRangeMax*. Esse método também lançará uma exceção se `CTaskDialog` o já estiver exibido e tiver uma barra de progresso do letreiro.
...
Esse método gera uma exceção [com a macro If, se a](diagnostic-services.md#ensure) `CTaskDialog` já estiver exibida, e tem uma barra de progresso de letreiro.
...
|||
|-|-|
|PBST_NORMAL|Depois que a barra de progresso é `CTaskDialog` preenchida, o não altera a cor da barra. Por padrão, a cor regular é verde.|
|PBST_ERROR|Depois que a barra de progresso é `CTaskDialog` preenchida, o altera a cor da barra para a cor do erro. Por padrão, isso é vermelho.|
|PBST_PAUSED|Depois que a barra de progresso é `CTaskDialog` preenchida, o altera a cor da barra para a cor pausada. Por padrão, isso é amarelo.|
...
*bChecked*<br/>
no TRUE para que a caixa de seleção de verificação seja `CTaskDialog` marcada quando o for exibido; FALSE para que a caixa de seleção de verificação não seja selecionada `CTaskDialog` quando o for exibido.
...
Esse método estático permite que você crie uma instância da `CTaskDialog` classe sem criar explicitamente um `CTaskDialog` objeto em seu código. Como não há nenhum `CTaskDialog` objeto, você não poderá chamar outros métodos `CTaskDialog` do se usar esse método para mostrar um `CTaskDialog` para o usuário.
...
*hwnd*<br/>
no Um identificador para a `m_hWnd` estrutura `CTaskDialog`do.
...
|TDN_TIMER|Tempo em milissegundos desde que `CTaskDialog` o foi criado ou o temporizador foi redefinido.|Não usado.|
...
[Passo a passo: adicionar um CTaskDialog a um aplicativo](../../mfc/walkthrough-adding-a-ctaskdialog-to-an-application.md)
