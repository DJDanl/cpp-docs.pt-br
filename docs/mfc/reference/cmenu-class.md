...
ms.locfileid: "69505518"
---
# <a name="cmenu-class"></a>Classe CMenu

Um encapsulamento das janelas `HMENU`.
...
|[CMenu:: Attach](#attach)|Anexa um identificador de menu do Windows a `CMenu` um objeto.|
...
|[CMenu::CreateMenu](#createmenu)|Cria um menu vazio e o anexa a um `CMenu` objeto.|
|[CMenu::CreatePopupMenu](#createpopupmenu)|Cria um menu pop-up vazio e o anexa a um `CMenu` objeto.|
|[CMenu::DeleteMenu](#deletemenu)|Exclui um item especificado do menu. Se o item de menu tiver um menu pop-up associado, destrói o identificador no menu pop-up e libera a memória usada por ele.|
|[CMenu::DeleteTempMap](#deletetempmap)|Exclui todos os `CMenu` objetos temporários criados `FromHandle` pela função de membro.|
|[CMenu::DestroyMenu](#destroymenu)|Destrói o menu anexado a um `CMenu` objeto e libera qualquer memória ocupada pelo menu.|
|[CMenu::Detach](#detach)|Desanexa um identificador de menu do Windows de `CMenu` um objeto e retorna o identificador.|
|[CMenu::DrawItem](#drawitem)|Chamado pelo Framework quando um aspecto visual de um menu desenhado pelo proprietário é alterado.|
|[CMenu::EnableMenuItem](#enablemenuitem)|Habilita, desabilita ou esmaece (cinza) um item de menu.|
|[CMenu::FromHandle](#fromhandle)|Retorna um ponteiro para um `CMenu` objeto dado a um identificador de menu do Windows.|
...
|[CMenu::GetSafeHmenu](#getsafehmenu)|Retorna o `m_hMenu` encapsulado por `CMenu` este objeto.|
...
|[CMenu::LoadMenu](#loadmenu)|Carrega um recurso de menu do arquivo executável e o anexa a um `CMenu` objeto.|
|[CMenu::LoadMenuIndirect](#loadmenuindirect)|Carrega um menu de um modelo de menu na memória e o anexa a um `CMenu` objeto.|
...
Crie um `CMenu` objeto no quadro de pilha como um local e, em `CMenu`seguida, chame as funções de membro para manipular o novo menu conforme necessário. Em seguida, chame [CWnd:: SetMenu](../../mfc/reference/cwnd-class.md#setmenu) para definir o menu para uma janela, seguido imediatamente por uma chamada para `CMenu` a função de membro [Detach](#detach) do objeto. A `CWnd::SetMenu` função membro define o menu da janela para o novo menu, faz com que a janela seja redesenhada para refletir a alteração do menu e também passa a propriedade do menu para a janela. A chamada para `Detach` desanexa o HMENU `CMenu` do objeto, de modo que, quando a variável `CMenu` local sair do escopo, o `CMenu` destruidor de objeto não tentará destruir um menu que não possui mais. O próprio menu é destruído automaticamente quando a janela é destruída.
...
- MF_UNCHECKED atua como uma alternância com MF_CHECKED para remover uma marca de seleção ao lado do item. Quando o aplicativo fornece bitmaps de marca de seleção (consulte `SetMenuItemBitmaps` a função membro), o bitmap "marca de seleção desativado" é exibido.
...
Essa função não deve ser chamada se um menu já estiver anexado ao `CMenu` objeto. O identificador de menu é armazenado no `m_hMenu` membro de dados.
...
O estado anterior do item: MF_CHECKED ou MF_UNCHECKED ou 0xFFFFFFFF se o item de menu não existir.
...
|------------|--------------------|
|MF_BYCOMMAND|Especifica que o parâmetro fornece a ID de comando do item de menu existente. Esse será o padrão se nem MF_BYCOMMAND nem MF_BYPOSITION for definido.|
|MF_BYPOSITION|Especifica que o parâmetro fornece a posição do item de menu existente. O primeiro item está na posição 0.|
...
### <a name="example"></a>Exemplo
  ...
  Consulte o exemplo de [ON_COMMAND_RANGE](message-map-macros-mfc.md#on_command_range).

##  <a name="cmenu"></a>  CMenu::CMenu

Cria um menu vazio e o anexa a um `CMenu` objeto.
...
Inicialmente, o menu está vazio. Os itens de menu podem ser adicionados usando `AppendMenu` a `InsertMenu` função de membro ou.
...
Cria um menu pop-up e o `CMenu` anexa ao objeto.
...
Inicialmente, o menu está vazio. Os itens de menu podem ser adicionados usando `AppendMenu` a `InsertMenu` função de membro ou. O aplicativo pode adicionar o menu pop-up a um menu ou menu pop-up existente. A `TrackPopupMenu` função de membro pode ser usada para exibir esse menu como um menu pop-up flutuante e para controlar as seleções no menu pop-up.
...
Chamado automaticamente pelo `CWinApp` manipulador de tempo ocioso, exclui todos os objetos `CMenu` temporários criados pela função de membro [FromHandle](#fromhandle) .
...
`DeleteTempMap`Desanexa o objeto de menu do Windows anexado a um `CMenu` objeto temporário antes de `CMenu` excluir o objeto.
...
O menu é desanexado do `CMenu` objeto antes de ser destruído. A função `DestroyMenu` do Windows é chamada automaticamente `CMenu` no destruidor.
...
Desanexa um menu do Windows de um `CMenu` objeto e retorna o identificador.
...
*nEnable*<br/>
Especifica a ação a ser tomada. Pode ser uma combinação de MF_DISABLED, MF_ENABLED ou MF_GRAYED, com MF_BYCOMMAND ou MF_BYPOSITION. Esses valores podem ser combinados usando-se o operador OR. Esses valores têm os seguintes significados:
...
O uso do valor MF_BYPOSITION exige que um aplicativo use o `CMenu`correto. Se o `CMenu` da barra de menus for usado, um item de menu de nível superior (um item na barra de menus) será afetado. Para definir o estado de um item em um menu pop-up ou pop-up aninhado por posição, um aplicativo deve especificar `CMenu` o do menu pop-up.

Quando um aplicativo especifica o sinalizador MF_BYCOMMAND, o Windows verifica todos os itens de menu pop-up subordinados `CMenu`ao; portanto, a menos que itens de menu duplicados `CMenu` estejam presentes, usar o da barra de menus é suficiente.
...
Retorna um ponteiro para um `CMenu` objeto dado um identificador do Windows a um menu.
...
Se um `CMenu` objeto ainda não estiver anexado ao objeto de menu do Windows, um `CMenu` objeto temporário será criado e anexado.

Esse objeto `CMenu` temporário só é válido até a próxima vez que o aplicativo tiver tempo ocioso em seu loop de eventos, quando todos os objetos temporários forem excluídos.
...
Recupera a ID da ajuda de contexto `CMenu`associada ao.
...
O ID da ajuda de contexto atualmente `CMenu` associado a se ele tiver um; caso contrário, será zero.
...
*fByPos*<br/>
Valor que especifica o significado `nIDItem`de. Por padrão, `ByPos` é false, o que indica que uItem é um identificador de item de menu. Se `ByPos` não estiver definido como false, isso indicará uma posição de item de menu.
...
Essa função de membro implementa o comportamento do da função [GetMenuItemInfo](/windows/win32/api/winuser/nf-winuser-getmenuiteminfow)do Win32, conforme descrito no SDK do Windows. Observe que, na implementação do MFC `GetMenuItemInfo`do, você não usa um identificador para um menu.
...
- MF_CHECKED atua como uma alternância com MF_UNCHECKED para posicionar a marca de seleção padrão ao lado do item. Quando o aplicativo fornece bitmaps de marca de seleção (consulte `SetMenuItemBitmaps` a função membro), o bitmap "marca de seleção ativa" é exibido.
...
- MF_UNCHECKED atua como uma alternância com MF_CHECKED para remover uma marca de seleção ao lado do item. Quando o aplicativo fornece bitmaps de marca de seleção (consulte `SetMenuItemBitmaps` a função membro), o bitmap "marca de seleção desativado" é exibido. Observe que o valor dessa constante é 0; um aplicativo não deve ser testado em relação a 0 para falha ao usar esse valor.
...
*rString*<br/>
Uma referência a um `CString` objeto que deve receber a cadeia de caracteres de menu copiada.
...
Retorna o HMENU encapsulado por `CMenu` este objeto ou um ponteiro`CMenu` nulo.
...
Um ponteiro para um `CMenu` objeto cujo `m_hMenu` membro contém um identificador para o menu pop-up se existir um menu pop-up na posição especificada; caso contrário, NULL. Se um `CMenu` objeto não existir, será criado um temporário. O `CMenu` ponteiro retornado não deve ser armazenado.
...
|nFlags|Interpretação de lpszNewItem|
|------------|-----------------------------------|
|MF_OWNERDRAW|Contém um valor de 32 bits fornecido pelo aplicativo que o aplicativo pode usar para manter dados adicionais associados ao item de menu. Esse valor de 32 bits está disponível para o aplicativo no `itemData` membro da estrutura fornecida pelas mensagens [WM_MEASUREITEM](/windows/win32/Controls/wm-measureitem) e [WM_DRAWITEM](/windows/win32/Controls/wm-drawitem) . Essas mensagens são enviadas quando o item de menu é exibido inicialmente ou é alterado.|
...
Quando *nIDNewItem* especifica um menu pop-up, ele se torna parte do menu em que é inserido. Se esse menu for destruído, o menu inserido também será destruído. Um menu inserido deve ser desanexado de um `CMenu` objeto para evitar conflitos.

Se a janela filho de MDI (interface de múltiplos documentos) ativa for maximizada e um aplicativo inserir um menu pop-up no menu do aplicativo MDI chamando essa função e especificando o sinalizador MF_BYPOSITION, o menu será inserido em uma posição mais distante à esquerda do que vista. Isso acontece porque o menu controle da janela filho MDI ativo é inserido na primeira posição da barra de menus da janela do quadro MDI. Para posicionar o menu corretamente, o aplicativo deve adicionar 1 ao valor de posição que, de outra forma, seria usado. Um aplicativo pode usar a mensagem WM_MDIGETACTIVE para determinar se a janela filho ativa no momento está maximizada.
...
*lpMenuItemInfo*<br/>
Consulte a descrição de lpmii `InsertMenuItem` no na SDK do Windows.

*fByPos*<br/>
Consulte a descrição de fByPosition `InsertMenuItem` no na SDK do Windows.
...
Carrega um recurso de menu do arquivo executável do aplicativo e o anexa ao `CMenu` objeto.
...
Carrega um recurso de um modelo de menu na memória e o `CMenu` anexa ao objeto.
...
Os `mtOption` sinalizadores devem incluir MF_END para o último item em uma lista pop-up e para o último item na lista principal. Consulte a `AppendMenu` função membro para outros sinalizadores. O `mtId` membro deve ser omitido da estrutura MENUITEMTEMPLATE quando MF_POPUP é especificado em `mtOption`.

O espaço alocado para a estrutura MENUITEMTEMPLATE deve ser grande o suficiente `mtString` para que contenha o nome do item de menu como uma cadeia de caracteres terminada em nulo.
...
Especifica o identificador HMENU do menu do Windows anexado ao `CMenu` objeto.
...
Por padrão, essa função de membro não faz nada. Substitua essa função de membro e preencha a `MEASUREITEMSTRUCT` estrutura para informar o Windows das dimensões do menu.
...
|MF_STRING|Contém um ponteiro longo para uma cadeia de caracteres terminada em nulo `CString`ou para um.|
|MF_SEPARATOR|O parâmetro *lpszNewItem* é ignorado (não é necessário).|
...
Sempre que um menu que reside em uma janela é alterado (quer a janela seja exibida ou não), o aplicativo deve chamar `CWnd::DrawMenuBar`. Para alterar os atributos dos itens de menu existentes, é muito mais rápido usar as `CheckMenuItem` funções `EnableMenuItem` de membro e.
...
Use esse operador para recuperar o identificador do `CMenu` objeto.
...
Se for bem-sucedido, o identificador do `CMenu` objeto; caso contrário, NULL.
...
Ele não destrói o identificador de um menu pop-up, portanto, o menu pode ser reutilizado. Antes de chamar essa função, o aplicativo pode chamar `GetSubMenu` a função de membro para recuperar o objeto `CMenu` pop-up para reutilização.
...
Associa uma ID `CMenu`de ajuda de contexto a.
...
*dwContextHelpId*<br/>
ID da ajuda de contexto a `CMenu`ser associada.
...
*lpMenuItemInfo*<br/>
Consulte a descrição de lpmii `SetMenuItemInfo` no na SDK do Windows.

*fByPos*<br/>
Consulte a descrição de fByPosition `SetMenuItemInfo` no na SDK do Windows.
...
*pWnd*<br/>
Identifica a janela que possui o menu pop-up. Esse parâmetro não pode ser nulo, mesmo que o sinalizador TPM_NONOTIFY seja especificado. Essa janela recebe todas as mensagens WM_COMMAND do menu. No Windows versões 3,1 e posteriores, a janela não recebe mensagens WM_COMMAND até `TrackPopupMenu` retornar. No Windows 3,0, a janela recebe mensagens WM_COMMAND antes `TrackPopupMenu` de retornar.
...
*pWnd*<br/>
Um ponteiro para a janela que possui o menu pop-up e recebe as mensagens do menu criado. Esta janela pode ser qualquer janela do aplicativo atual, mas não pode ser nula. Se você especificar TPM_NONOTIFY no parâmetro *fuFlags* , a função não enviará nenhuma mensagem para *pWnd*. A função deve retornar para a janela apontada por *pWnd* para receber a mensagem WM_COMMAND.
