...
Essas macros e funções estão disponíveis para todas as classes `CObject` derivadas das versões de depuração e de lançamento do MFC. No entanto, todos exceto DEBUG_NEW e VERIFY não fazem nada na versão de lançamento.
...
|[ASSERT_VALID](#assert_valid)|Testa a validade interna de um objeto chamando sua função `AssertValid` de membro; normalmente substituído de `CObject`.|
...
|[TRACE](#trace)|A `printf`funcionalidade do é semelhante na versão de depuração da biblioteca.|
...
|||
|-|-|
|[AfxDoForAllClasses](#afxdoforallclasses)|Executa uma função especificada em todas `CObject`as classes derivadas que dão suporte à verificação de tipo em tempo de execução.|
|[AfxDoForAllObjects](#afxdoforallobjects)|Executa uma função especificada em todos `CObject`os objetos derivados que foram alocados com o **novo**.|
...
Este exemplo de código causaria um aviso do compilador se _AFX_SECURE_NO_WARNINGS não estivesse definido.
...
`AfxDebugBreak`Não tem nenhum efeito nas versões de lançamento de um aplicativo MFC e deve ser removido. Essa função só deve ser usada em aplicativos MFC. Use a versão da API do `DebugBreak`Win32,, para causar uma interrupção em aplicativos não MFC.
...
O parâmetro *pObject* deve ser um ponteiro para um objeto e pode ser **const**. O objeto apontado para e a classe deve `CObject` dar suporte a informações de classe em tempo de execução. Por exemplo, para garantir que `pDocument` seja um ponteiro para um objeto `CMyDoc` da classe, ou qualquer um de seus derivados, você poderia codificar:

[!code-cpp[NVC_MFCDocView#194](../../mfc/codesnippet/cpp/diagnostic-services_3.cpp)]

O uso `ASSERT_KINDOF` da macro é exatamente o mesmo que a codificação:
...
*pObject*<br/>
Especifica um objeto de uma classe derivada de `CObject` que tem uma versão `AssertValid` de substituição da função membro.

### <a name="remarks"></a>Comentários

ASSERT_VALID chama a `AssertValid` função de membro do objeto passado como seu argumento.

Na versão de lançamento do MFC, o ASSERT_VALID não faz nada. Na versão de depuração, ele valida o ponteiro, verifica em relação a NULL e chama as funções de membro `AssertValid` do próprio objeto. Se qualquer um desses testes falhar, uma mensagem de alerta será exibida da mesma maneira que a [declaração](#assert).
...
Você pode usar o DEBUG_NEW em todos os lugares em seu programa que normalmente usaria o **novo** operador para alocar o armazenamento de heap.

No modo de depuração (quando o símbolo **_DEBUG** é definido), DEBUG_NEW controla o nome do arquivo e o número da linha para cada objeto que ele aloca. Em seguida, quando você usa a função de membro [CMemoryState::D umpallobjectssince](cmemorystate-structure.md#dumpallobjectssince) , cada objeto ALOCADO com DEBUG_NEW é mostrado com o nome de arquivo e o número de linha onde ele foi alocado.

Para usar o DEBUG_NEW, insira a diretiva a seguir em seus arquivos de origem:

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

Depois de inserir essa diretiva, o pré-processador inserirá DEBUG_NEW sempre que você usar **novo**, e o MFC fará o resto. Quando você compila uma versão de lançamento do seu programa, o DEBUG_NEW é resolvido para uma **nova** operação simples e as informações do nome de arquivo e de número de linha não são geradas.
...
A macro DEBUG_ONLY é equivalente à *expressão* ao redor `#ifdef _DEBUG` com `#endif`and.
...
ENSURE_VALID chama a macro ASSERT_VALID (que tem efeito apenas em compilações de depuração). Além disso, o ENSURE_VALID gera uma exceção se o ponteiro for nulo. O teste nulo é executado nas configurações de depuração e de versão.
...
`afxDump`é um objeto [CDumpContext](../../mfc/reference/cdumpcontext-class.md) predefinido que permite que você `CDumpContext` envie informações para a janela de saída do depurador ou para um terminal de depuração. Normalmente, você fornece `afxDump` como um parâmetro para `CObject::Dump`.

No Windows NT e em todas as versões do `afxDump` Windows, a saída é enviada para a janela de depuração C++ de saída do Visual quando você depura seu aplicativo.

Essa variável é definida somente na versão de depuração do MFC. Para obter mais informações `afxDump`sobre o, consulte [Debugging MFC Applications](/visualstudio/debugger/mfc-debugging-techniques).
...
`AfxDump`chama a função de `Dump` membro de um objeto e envia as informações para o local especificado `afxDump` pela variável. `AfxDump`está disponível somente na versão de depuração do MFC.

O código do programa não deve `AfxDump`chamar, mas, em vez `Dump` disso, deve chamar a função de membro do objeto apropriado.
...
Todos os blocos de memória alocados atualmente no heap são verificados, incluindo aqueles alocados por **novos** , mas não aqueles alocados por chamadas diretas para alocadores de memória subjacentes `GlobalAlloc` , como a função **malloc** ou a função do Windows. Se algum bloco for encontrado como corrompido, uma mensagem será impressa na saída do depurador.

Se você incluir a linha

[!code-cpp[NVC_MFCCObjectSample#14](../../mfc/codesnippet/cpp/diagnostic-services_1.cpp)]

em um módulo de programa, as próximas chamadas `AfxCheckMemory` para mostrar o nome de arquivo e o número de linha em que a memória foi alocada.

> [!NOTE]
>  Se o módulo contiver uma ou mais implementações de classes serializáveis, você deverá `#define` colocar a linha após a última chamada de macro IMPLEMENT_SERIAL.
...
- O AFX_STACK_DUMP_TARGET_TRACE Envia a saída por meio da macro de [rastreamento](#trace) . A macro de rastreamento gera saída somente em compilações de depuração; Ele não gera nenhuma saída nas compilações de versão. Além disso, o rastreamento pode ser redirecionado para outros destinos além do depurador.

- AFX_STACK_DUMP_TARGET_DEFAULT envia saída de despejo para o destino padrão. Para uma compilação de depuração, a saída vai para a macro de rastreamento. Em uma compilação de versão, a saída vai para a área de transferência.
...
- O AFX_STACK_DUMP_TARGET_BOTH envia a saída para a área de transferência e para a macro de rastreamento, simultaneamente.

- O AFX_STACK_DUMP_TARGET_ODS envia a saída diretamente para o depurador por meio da função `OutputDebugString()`do Win32. Essa opção gerará a saída do depurador em compilações de depuração e de versão quando um depurador é anexado ao processo. AFX_STACK_DUMP_TARGET_ODS sempre alcança o depurador (se estiver anexado) e não pode ser redirecionado.
...
Para obter mais informações `AfxEnableMemoryTracking`sobre o, consulte [Debugging MFC Applications](/visualstudio/debugger/mfc-debugging-techniques).
...
Chama a função de iteração especificada para `CObject`todas as classes derivadas de Serializable no espaço de memória do aplicativo.
...
*pfn*<br/>
Aponta para uma função de iteração a ser chamada para cada classe. Os argumentos de função são um ponteiro para `CRuntimeClass` um objeto e um ponteiro void para dados adicionais que o chamador fornece para a função.
...
As `CObject`classes derivadas de Serializable são classes derivadas usando a macro DECLARE_SERIAL. O ponteiro passado para `AfxDoForAllClasses` em *pContext* é passado para a função de iteração especificada sempre que é chamado.
...
Executa a função de iteração especificada para todos os objetos `CObject` derivados de que foram alocados com **New**.
...
*pfn*<br/>
Aponta para uma função de iteração a ser executada para cada objeto. Os argumentos de função são um ponteiro para `CObject` um e um ponteiro void para dados adicionais que o chamador fornece à função.
