...
### <a name="parameters"></a>Parâmetros  
 `lpDialogTemplate`  
 Pontos de memória que contém um modelo de caixa de diálogo usado para criar a caixa de diálogo. Este modelo é na forma de um [DLGTEMPLATE](http://msdn.microsoft.com/library/windows/desktop/ms645394) informações de estrutura e o controle, conforme descrito no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="example"></a>Exemplo  
 [!code-cpp[NVC_MFCControlLadenDialog º 63](../../mfc/codesnippet/cpp/cdialog-class_2.cpp)]  
...
### <a name="remarks"></a>Comentários  
 ...
 O **GetDialogBaseUnits** função Windows retorna informações sobre o tamanho da fonte do sistema, mas você pode especificar uma fonte diferente para cada caixa de diálogo se você usar o **DS_SETFONT** estilo no arquivo de definição de recurso. O `MapDialogRect` função do Windows usa a fonte apropriada para essa caixa de diálogo.  
