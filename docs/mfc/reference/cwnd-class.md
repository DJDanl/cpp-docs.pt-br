---
title: Classe CWnd | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CWnd::accHitTest](#acchittest)|Chamado pela estrutura para recuperar o elemento filho ou o objeto filho em um determinado ponto na tela.|  
|[CWnd::accLocation](#acclocation)|Chamado pela estrutura para recuperar o local da tela atual do objeto especificado.|  
|[CWnd::accNavigate](#accnavigate)|Chamado pela estrutura para percorrer a outro elemento de interface do usuário dentro de um contêiner e, se possível, recuperar o objeto.|  
...
|[CWnd::AnimateWindow](#animatewindow)|Anime o objeto de janela associada.|  
|[CWnd::ArrangeIconicWindows](#arrangeiconicwindows)|Organiza todas as janelas minimizadas filho (icônico).|  
|[CWnd::Attach](#attach)|Anexa um identificador para um `CWnd` objeto.|  
...
|[CWnd::BindDefaultProperty](#binddefaultproperty)|Associa a chamada propriedade padrão do objeto simples associadas, como marcada na biblioteca de tipos, a um cursor associado a um controle de fonte de dados.|  
|[CWnd::BindProperty](#bindproperty)|Associa uma propriedade associada de cursor em um controle ligado a dados a um controle de fonte de dados e registra esse relacionamento com o Gerenciador de ligação do MFC.|  
...
|[CWnd::CreateEx](#createex)|Cria uma janela de janelas sobrepostas, pop-up ou filho e anexa a um `CWnd` objeto.|  
...
|[CWnd::DrawAnimatedRects](#drawanimatedrects)|Desenha um retângulo de esboço e anima para indicar a abertura de um ícone ou a minimização ou a maximização de uma janela.|  
...
|[CWnd::EnableDynamicLayout](#enabledynamiclayout)|Permite que a posição e o tamanho das janelas filho para ajustar dinamicamente quando o usuário redimensiona a janela.|  
|[CWnd::EnableD2DSupport](#enabled2dsupport)|Habilita ou desabilita a janela `D2D` suporte. Chame esse método antes da janela principal é inicializada.|  
...
|[CWnd::EnableTrackingToolTips](#enabletrackingtooltips)|Habilita o controle de dica de ferramenta no modo de controle.|  
|[CWnd::EnableWindow](#enablewindow)|Habilita ou desabilita o mouse e teclado.|  
|[CWnd::EndModalLoop](#endmodalloop)|Termina o status de uma janela modal.|  
|[CWnd::EndModalState](#endmodalstate)|Chame essa função de membro para alterar uma janela de quadro da janela restrita sem janela restrita.|  
...
|[CWnd::FindWindow](#findwindow)|Retorna o identificador da janela, que é identificado por seu nome de janela e uma classe de janela.|  
|[CWnd::FindWindowEx](#findwindowex)|Retorna o identificador da janela, que é identificado por seu nome de janela e uma classe de janela.|  
...
|[CWnd::FromHandle](#fromhandle)|Retorna um ponteiro para um `CWnd` objeto quando recebe um identificador para uma janela. Se um `CWnd` objeto não está anexado ao identificador, um temporário `CWnd` objeto é criado e anexado.|  
|[CWnd::FromHandlePermanent](#fromhandlepermanent)|Retorna um ponteiro para um `CWnd` objeto quando recebe um identificador para uma janela. Se um `CWnd` objeto não está anexado ao identificador, um temporário `CWnd` objeto é criado e anexado.|  
|[CWnd::get_accChild](#get_accchild)|Chamado pela estrutura para recuperar o endereço de uma `IDispatch` interface para o filho especificado.|  
|[CWnd::get_accChildCount](#get_accchildcount)|Chamado pela estrutura para recuperar o número de filhos que pertence a este objeto.|  
|[CWnd::get_accDefaultAction](#get_accdefaultaction)|Chamado pela estrutura para recuperar uma cadeia de caracteres que descreve a ação padrão do objeto.|  
|[CWnd::get_accDescription](#get_accdescription)|Chamado pela estrutura para recuperar uma cadeia de caracteres que descreve a aparência visual do objeto especificado.|  
|[CWnd::get_accFocus](#get_accfocus)|Chamado pela estrutura para recuperar o objeto que tem o foco do teclado.|  
|[CWnd::get_accHelp](#get_acchelp)|Chamado pela estrutura para recuperar um objeto **ajuda** cadeia de caracteres de propriedade.|  
|[CWnd::get_accHelpTopic](#get_acchelptopic)|Chamado pela estrutura para recuperar o caminho completo do `WinHelp` arquivo associado ao objeto especificado e o identificador do tópico apropriado dentro desse arquivo.|  
|[CWnd::get_accKeyboardShortcut](#get_acckeyboardshortcut)|Chamado pela estrutura para recuperar a tecla de atalho do objeto especificado ou a chave de acesso.|  
|[CWnd::get_accName](#get_accname)|Chamado pela estrutura para recuperar o nome do objeto especificado.|  
|[CWnd::get_accParent](#get_accparent)|Chamado pela estrutura para recuperar o `IDispatch` interface do pai do objeto.|  
|[CWnd::get_accRole](#get_accrole)|Chamado pela estrutura para recuperar informações que descreve a função do objeto especificado.|  
|[CWnd::get_accSelection](#get_accselection)|Chamado pela estrutura para recuperar o filho selecionado deste objeto.|  
|[CWnd::get_accState](#get_accstate)|Chamado pela estrutura para recuperar o estado atual do objeto especificado.|  
...
|[CWnd::GetCheckedRadioButton](#getcheckedradiobutton)|Retorna a ID do botão de opção atualmente selecionado em um grupo de botões.|  
|[CWnd::GetClientRect](#getclientrect)|Obtém as dimensões do `CWnd` área do cliente.|  
...
|[CWnd::GetDCEx](#getdcex)|Recupera um contexto de exibição para a área do cliente e permite que o recorte durante o desenho.|  
...
|[CWnd::GetDlgItemInt](#getdlgitemint)|Converte o texto de um controle na caixa de diálogo determinado para um valor inteiro.|  
...
|[CWnd::GetIcon](#geticon)|Recupera o identificador a um ícone.|  
...
|[CWnd::GetLayeredWindowAttributes](#getlayeredwindowattributes)|Recupera a chave de cor opacidade e a transparência de uma janela em camadas.|  
...
|[CWnd::GetParentOwner](#getparentowner)|Retorna um ponteiro para a janela do pai de uma janela filho.|  
...
|[CWnd::GetSafeOwner](#getsafeowner)|Recupera o proprietário seguro para determinado intervalo.|  
|[CWnd::GetScrollBarCtrl](#getscrollbarctrl)|Retorna um irmão de controle barra de rolagem.|  
...
|[CWnd::GetUpdateRect](#getupdaterect)|Recupera as coordenadas do retângulo menor que engloba completamente o `CWnd` região de atualização.|  
...
|[CWnd::GetWindowPlacement](#getwindowplacement)|Recupera o normal (restaurada) e o estado Mostrar maximizadas e minimizadas posições de uma janela.|  
|[CWnd::GetWindowRect](#getwindowrect)|Obtém as coordenadas de tela de `CWnd`.|  
|[CWnd::GetWindowRgn](#getwindowrgn)|Recupera uma cópia da região de janela de uma janela.|  
|[CWnd::GetWindowText](#getwindowtext)|Retorna o título da janela de texto ou legenda (se houver).|  
...
|[CWnd::InvalidateRect](#invalidaterect)|Invalida a área do cliente dentro do retângulo determinada adicionando o retângulo para a região atual da atualização.|  
|[CWnd::InvalidateRgn](#invalidatergn)|Invalida a área do cliente dentro da região determinada pela adição de região para a região atual da atualização.|  
...
|[CWnd::IsDialogMessage](#isdialogmessage)|Determina se a mensagem determinada destina-se a caixa de diálogo sem janela restrita e, em caso afirmativo, processa.|  
|[CWnd::IsDlgButtonChecked](#isdlgbuttonchecked)|Determina se um controle de botão é verificado.|  
|[CWnd::IsDynamicLayoutEnabled](#isdynamiclayoutenabled)|Determina se o layout dinâmico está habilitado nesta janela. Se o layout dinâmico estiver habilitado, a posição e tamanho das janelas filho podem alterar quando o usuário redimensiona a janela pai.|  
...
|[CWnd::LockWindowUpdate](#lockwindowupdate)|Desativa ou reativa o desenho na janela determinada.|  
|[CWnd::MapWindowPoints](#mapwindowpoints)|Converte (maps) um conjunto de pontos no espaço de coordenadas do `CWnd` para o espaço de coordenadas de outra janela.|  
...
|[CWnd::NotifyWinEvent](#notifywinevent)|Sinaliza o sistema que ocorreu um evento predefinido.|  
|[CWnd::OnAmbientProperty](#onambientproperty)|Implementar os valores de propriedade de ambiente.|  
|[CWnd::OnDrawIconicThumbnailOrLivePreview](#ondrawiconicthumbnailorlivepreview)|Chamado pela estrutura quando ele precisa obter um bitmap a ser exibido em miniatura de guia do Windows 7 ou no cliente para a inspeção de aplicativo.|  
...
|[CWnd::PaintWindowlessControls](#paintwindowlesscontrols)|Desenha sem janelas controles no contêiner de controle.|  
|[CWnd::PostMessage](#postmessage)|Coloca uma mensagem na fila de aplicativos, em seguida, retornará sem aguardar a janela processar a mensagem.|  
|[CWnd::PreCreateWindow](#precreatewindow)|Chamado antes da criação da janela do Windows anexada a esta `CWnd` objeto.|  
|[CWnd::PreSubclassWindow](#presubclasswindow)|Permite que outros subclassificar necessária antes que [SubclassWindow](#subclasswindow) é chamado.|  
...
|[CWnd::PrintWindow](#printwindow)|Copia uma janela visual no contexto de dispositivo especificado, geralmente uma impressora DC.|  
...
|[CWnd::SendChildNotifyLastMsg](#sendchildnotifylastmsg)|Fornece uma mensagem de notificação para uma janela filho, na janela pai, para a janela filho possa lidar com uma tarefa.|  
|[CWnd::SendDlgItemMessage](#senddlgitemmessage)|Envia uma mensagem para o controle especificado.|  
|[CWnd::SendMessage](#sendmessage)|Envia uma mensagem para o `CWnd` do objeto e não retorna até que ele tenha processado a mensagem.|  
...
|[CWnd::SetCapture](#setcapture)|Faz com que o mouse subsequente de entrada sejam enviados para o `CWnd`.|  
|[CWnd::SetCaretPos](#setcaretpos)|Move o cursor para uma posição especificada.|  
|[CWnd::SetClipboardViewer](#setclipboardviewer)|Adiciona `CWnd` a cadeia do windows é notificado sempre que o conteúdo da área de transferência é alterado.|  
|[CWnd::SetDlgCtrlID](#setdlgctrlid)|Define a ID de janela ou um controle de janela (que pode ser qualquer janela filho, não apenas um controle em uma caixa de diálogo).|  
...
|[CWnd::SetFocus](#setfocus)|Declara o foco de entrada.|  
...
|[CWnd::SetIcon](#seticon)|Define o identificador como um ícone específico.|  
...
|[CWnd::SetRedraw](#setredraw)|Permite alterações no `CWnd` ser redesenhados ou impede alterações sendo redesenhadas.|  
...
|[CWnd::SetScrollRange](#setscrollrange)|Define os valores mínimo e máximo de posição para a barra de rolagem determinado.|  
...
|[CWnd::SetWindowPlacement](#setwindowplacement)|Define o estado de apresentação e o normal (restaurada), posições maximizadas e minimizadas para uma janela.|  
|[CWnd::SetWindowPos](#setwindowpos)|Altera o tamanho, posição e ordenação de filho, janelas pop-up e de nível superior.|  
|[CWnd::SetWindowRgn](#setwindowrgn)|Define a região de uma janela.|  
|[CWnd::SetWindowText](#setwindowtext)|Define o título da janela de texto ou legenda (se houver) para o texto especificado.|  
...
|[CWnd::SubclassDlgItem](#subclassdlgitem)|Anexa um controle do Windows para uma `CWnd` do objeto e torna rotear mensagens por meio de `CWnd`do mapa da mensagem.|  
|[CWnd::SubclassWindow](#subclasswindow)|Anexa uma janela para um `CWnd` do objeto e torna rotear mensagens por meio de `CWnd`do mapa da mensagem.|  
...
|[CWnd::UpdateLayeredWindow](#updatelayeredwindow)|Atualizações de posição, tamanho, forma, conteúdo e transparência de uma janela em camadas.|  
|[CWnd::UpdateWindow](#updatewindow)|Atualiza a área do cliente.|  
|[CWnd::ValidateRect](#validaterect)|Valida a área do cliente dentro do retângulo determinado removendo o retângulo da região de atualização atual.|  
...
|[CWnd::WindowFromPoint](#windowfrompoint)|Identifica a janela que contém um ponto específico.|  
...
|Nome|Descrição|  
|----------|-----------------|  
|[CWnd::Default](#default)|Chama o procedimento de janela padrão, que fornece processamento padrão para todas as mensagens de janela que um aplicativo não processar.|  
|[CWnd::DefWindowProc](#defwindowproc)|Chama o procedimento de janela padrão, que fornece processamento padrão para todas as mensagens de janela que um aplicativo não processar.|  
|[CWnd::DoDataExchange](#dodataexchange)|Para troca de dados de caixa de diálogo e validação. Chamado por `UpdateData`.|  
|[CWnd::GetCurrentMessage](#getcurrentmessage)|Retorna um ponteiro para a mensagem que esta janela está processando no momento. Só deve ser chamado quando um `On` *mensagem* função de membro de manipulador de mensagens.|  
|[CWnd::InitDynamicLayout](#initdynamiclayout)|Chamado pela estrutura para inicializar o layout dinâmico da janela.|  
|[CWnd::LoadDynamicLayoutResource](#loaddynamiclayoutresource)|Carrega as informações de layout dinâmico do arquivo de recurso.|  
...
|[CWnd::OnChar](#onchar)|Chamado quando um pressionamento de tecla é convertido para um caractere não são do sistema.|  
|[CWnd::OnCharToItem](#onchartoitem)|Chamado por uma caixa de lista filho com o [LBS_WANTKEYBOARDINPUT](list-box-styles.md) estilo em resposta a uma [WM_CHAR](#onchar) mensagem.|  
|[CWnd::OnChildActivate](#onchildactivate)|Chamado para várias janelas de filho MDI (interface) de documento sempre que o tamanho ou posição de `CWnd` alterações ou `CWnd` está ativado.|  
...
|[CWnd::OnColorizationColorChanged](#oncolorizationcolorchanged)|Chamado quando a política de processamento para a área do cliente não foi alterada.|  
...
|[CWnd::OnCompareItem](#oncompareitem)|Chamado determinar a posição relativa de um novo item em um filho classificados desenho proprietário caixa de combinação ou caixa de listagem.|  
...
|[CWnd::OnCtlColor](#onctlcolor)|Se chamado `CWnd` é o pai de um controle quando o controle está prestes a ser desenhada.|  
|[CWnd::OnDeadChar](#ondeadchar)|Chamado quando um pressionamento de tecla se traduz em um caractere de inatividade não é do sistema (como caracteres acentuados).|  
|[CWnd::OnDeleteItem](#ondeleteitem)|Chamado quando uma caixa de listagem do desenho proprietário filho ou caixa de combinação é destruída ou quando itens são removidos do controle.|  
|[CWnd::OnDestroy](#ondestroy)|Chamado quando `CWnd` está sendo destruída.|  
|[CWnd::OnDestroyClipboard](#ondestroyclipboard)|Chamado quando a área de transferência é esvaziada através de uma chamada para o Windows [EmptyClipboard](http://msdn.microsoft.com/library/windows/desktop/ms649037) função.|  
...
|[CWnd::OnDrawItem](#ondrawitem)|Chamado quando um aspecto visual de um controle de botão do desenho proprietário filho, controle de caixa de combinação, controle de caixa de listagem ou menu precisa ser desenhado.|  
...
|[CWnd::OnEnterIdle](#onenteridle)|Chamado para informar o procedimento de janela principal do aplicativo que um menu ou caixa de diálogo modal está entrando em um estado ocioso.|  
|[CWnd::OnEnterMenuLoop](#onentermenuloop)|Chamado quando um loop modal menu foi inserido.|  
...
|[CWnd::OnExitMenuLoop](#onexitmenuloop)|Chamado quando um loop modal menu foi encerrado.|  
...
|[CWnd::OnGetDlgCode](#ongetdlgcode)|Chamado para um controle para o controle pode processar a tecla de seta e a tecla TAB própria entrada.|  
|[CWnd::OnGetMinMaxInfo](#ongetminmaxinfo)|Chamado sempre que o Windows precisa saber a posição maximizada ou dimensões, ou o mínimo ou máximo controle do tamanho.|  
|[CWnd::OnHelpInfo](#onhelpinfo)|Chamado pela estrutura quando o usuário pressiona a tecla F1.|  
...
|[CWnd::OnKeyDown](#onkeydown)|Chamado quando uma tecla é pressionada.|  
|[CWnd::OnKeyUp](#onkeyup)|Chamado quando uma chave é liberada.|  
|[CWnd::OnKillFocus](#onkillfocus)|Chamado imediatamente antes `CWnd` perde o foco de entrada.|  
...
|[CWnd::OnMButtonUp](#onmbuttonup)|Chamado quando o usuário solta o botão do meio.|  
|[CWnd::OnMDIActivate](#onmdiactivate)|Chamado quando uma janela de filho MDI é ativada ou desativada.|  
|[CWnd::OnMeasureItem](#onmeasureitem)|Chamado para uma caixa de combinação do desenho proprietário filho, uma caixa de listagem ou um item de menu quando o controle é criado. `CWnd`informa ao Windows das dimensões do controle.|  
|[CWnd::OnMenuChar](#onmenuchar)|Chamado quando o usuário pressiona um caractere mnemônico de menu que não coincide com nenhum o mnemônico predefinido no menu atual.|  
|[CWnd::OnMenuDrag](#onmenudrag)|Chamado quando o usuário começa a arrastar um item de menu.|  
|[CWnd::OnMenuGetObject](#onmenugetobject)|Chamado quando o cursor do mouse entra em um item de menu ou move do centro do item na parte superior ou inferior do item.|  
...
|[CWnd::OnMouseActivate](#onmouseactivate)|Chamado quando o cursor estiver em uma janela ativa e o usuário pressiona o botão do mouse.|  
|[CWnd::OnMouseHover](#onmousehover)|Chamado quando o cursor focaliza a área cliente da janela para o período de tempo especificado em uma chamada anterior a [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265).|  
...
|[CWnd::OnMouseLeave](#onmouseleave)|Chamado quando o cursor sai da área do cliente na janela especificada em uma chamada anterior a [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265).|  
|[CWnd::OnMouseMove](#onmousemove)|Chamado quando o cursor do mouse se move.|  
|[CWnd::OnMouseWheel](#onmousewheel)|Chamado quando o usuário gira a roda do mouse. Usa o tratamento de mensagens do Windows NT 4.0.|  
|[CWnd::OnMove](#onmove)|Chamado após a posição do `CWnd` foi alterada.|  
|[CWnd::OnMoving](#onmoving)|Indica que um usuário está se movendo um `CWnd` objeto.|  
...
|[CWnd::OnNcCalcSize](#onnccalcsize)|Chamado quando o tamanho e a posição da área do cliente precisam ser calculado.|  
...
|[CWnd::OnNcMouseHover](#onncmousehover)|Chamado quando o cursor focaliza a área não-cliente da janela para o período de tempo especificado em uma chamada anterior a [TrackMouseEvent](http://msdn.microsoft.com/library/windows/desktop/ms646265).|  
...
|[CWnd::OnNcRButtonDblClk](#onncrbuttondblclk)|Chamado quando o usuário clica duas vezes no botão direito do mouse enquanto o cursor estiver em uma área não cliente de `CWnd`.|  
...
|[CWnd::OnNcRenderingChanged](#onncrenderingchanged)|Chamado quando a política de processamento para a área do cliente não foi alterada.|  
...
|[CWnd::OnNextMenu](#onnextmenu)|Chamado quando a tecla de seta para a direita ou esquerda é usada para alternar entre a barra de menu e o menu do sistema.|  
|[CWnd::OnNotify](#onnotify)|Chamado pela estrutura para informar uma janela pai de um evento ocorreu em um de seus controles ou que o controle precisa de informações.|  
...
|[CWnd::OnPaletteChanged](#onpalettechanged)|Chamado para permitir que o windows que usam uma paleta de cores para realizar suas paletas lógicas e atualize suas áreas de cliente.|  
|[CWnd::OnPaletteIsChanging](#onpaletteischanging)|Informa quando um aplicativo será utilize sua paleta lógica a outros aplicativos.|  
|[CWnd::OnParentNotify](#onparentnotify)|Chamado quando uma janela filho for criada ou destruída, ou quando o usuário clica em um botão do mouse enquanto o cursor estiver sobre a janela filho.|  
...
|[CWnd::OnRenderFormat](#onrenderformat)|Chamado para o proprietário da área de transferência quando um formato específico com renderização atrasada precisa ser processado.|  
...
|[CWnd::OnSpoolerStatus](#onspoolerstatus)|Chamada do Gerenciador de impressão sempre que um trabalho é adicionado ou removido da fila de Gerenciador de impressão.|  
|[CWnd::OnStyleChanged](#onstylechanged)|Indica que o [SetWindowLong](http://msdn.microsoft.com/library/windows/desktop/ms633591) função do Windows foi alterado um ou mais estilos da janela.|  
...
|[CWnd::OnSysDeadChar](#onsysdeadchar)|Chamado quando um pressionamento de tecla é convertido para um caractere inativo do sistema (como caracteres acentuados).|  
...
|[CWnd::OnTCard](#ontcard)|Chamado quando o usuário clica em um botão programáveis sendo.|  
...
|[CWnd::OnTouchInput](#ontouchinput)|Processo único de entrada de toque do Windows.|  
|[CWnd::OnTouchInputs](#ontouchinputs)|Entradas de processo do Windows touch.|  
...
|[CWnd::OnUnInitMenuPopup](#onuninitmenupopup)|Chamado quando um menu suspenso ou submenu foi destruída.|  
...
|[CWnd::OnVKeyToItem](#onvkeytoitem)|Chamado por uma caixa de listagem pertencente a `CWnd` em resposta a uma [WM_KEYDOWN](#onkeydown) mensagem.|  
...
|[CWnd::OnXButtonDblClk](#onxbuttondblclk)|Chamado quando o usuário clica duas vezes em XBUTTON1 ou XBUTTON2 enquanto o cursor estiver na área do cliente de uma janela.|  
|[CWnd::OnXButtonDown](#onxbuttondown)|Chamado quando o usuário pressiona XBUTTON1 ou XBUTTON2 enquanto o cursor estiver na área do cliente de uma janela.|  
|[CWnd::OnXButtonUp](#onxbuttonup)|Chamado quando o usuário libera XBUTTON1 ou XBUTTON2 enquanto o cursor estiver na área do cliente de uma janela.|  
...
|[CWnd::ResizeDynamicLayout](#resizedynamiclayout)|Chamado pela estrutura quando altera o tamanho de janela para ajustar o layout de janelas filho, se o layout dinâmico estiver habilitado para a janela.|  
...
|Nome|Descrição|  
|----------|-----------------|  
|[CWnd::m_hWnd](#m_hwnd)|Indica o `HWND` anexado a esse `CWnd`.|  
  
## <a name="remarks"></a>Comentários  
 Um `CWnd` objeto é diferente de uma janela do Windows, mas as duas são rigidamente vinculadas. A `CWnd` objeto for criado ou destruído pelo `CWnd` construtor e destruidor. A janela do Windows, por outro lado, é uma estrutura de dados interna do Windows criado por um **criar** função de membro e destruído pelo `CWnd` destruidor virtual. O [DestroyWindow](#destroywindow) função destrói a janela do Windows sem destruir o objeto.  
  
 O `CWnd` classe e o mecanismo de mapa de mensagem ocultar o **WndProc** função. Mensagens de notificação do Windows são roteadas automaticamente pelo mapa de mensagens para o próprio **na***mensagem* `CWnd` funções de membro. Você substitui um **na***mensagem* função de membro para manipular a mensagem específica de um membro de suas classes derivadas.  
  
 O `CWnd` classe também permite que você crie uma janela filho do Windows para o seu aplicativo. Derivar uma classe de `CWnd`, em seguida, adicionar variáveis de membro para a classe derivada para armazenar dados específicos do aplicativo. Implementar funções de membro de manipulador de mensagens e uma mensagem do mapa na classe derivada para especificar o que acontece quando as mensagens são direcionadas para a janela.  
  
 Você pode criar uma janela filho em duas etapas. Primeiro, chame o construtor `CWnd` para construir o `CWnd` do objeto, em seguida, chame o [criar](#create) a função de membro para criar a janela filho e anexá-lo a `CWnd` objeto.  
  
 Quando o usuário encerra sua janela filho, destruir o `CWnd` objeto ou chamar o `DestroyWindow` a função de membro para remover a janela e destruir suas estruturas de dados.  
  
 Na biblioteca Microsoft Foundation Class, mais classes são derivadas de `CWnd` para fornecer tipos específicos de janela. Muitas dessas classes, incluindo [CFrameWnd](../../mfc/reference/cframewnd-class.md), [CMDIFrameWnd](../../mfc/reference/cmdiframewnd-class.md), [CMDIChildWnd](../../mfc/reference/cmdichildwnd-class.md), [CView](../../mfc/reference/cview-class.md), e [CDialog](../../mfc/reference/cdialog-class.md), são projetados para derivação adicional. As classes de controle derivam de `CWnd`, como [CButton](../../mfc/reference/cbutton-class.md), pode ser usado diretamente ou pode ser usado para obter mais derivação de classes.  
  
 Para obter mais informações sobre como usar o `CWnd`, consulte [janelas com moldura](../../mfc/frame-windows.md) e [objetos de janela](../../mfc/window-objects.md).  
...
### <a name="parameters"></a>Parâmetros  
 `varChild`  
 Especifica se a ação padrão a ser chamado de objeto ou um dos elementos-filho do objeto. Esse parâmetro pode ser CHILDID_SELF (para realizar a ação padrão do objeto) ou uma ID de filho (para realizar a ação padrão de um dos elementos-filho do objeto).  
  
### <a name="return-value"></a>Valor de retorno  
 Retorna S_OK com êxito, um código de erro COM falha. Consulte **retornar valores** na [IAccessible:: accDoDefaultAction](http://msdn.microsoft.com/library/windows/desktop/dd318470) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="remarks"></a>Comentários  
 Essa função é parte do MFC [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) suporte.  
  
 Substituir essa função em sua `CWnd`-derivado da classe para executar a ação padrão do objeto. Para obter mais informações, consulte [IAccessible:: accDoDefaultAction](http://msdn.microsoft.com/library/windows/desktop/dd318470) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `pvarChild`  
 Recebe informações que identificam o objeto no ponto especificado por `xLeft` e `yTop`. Consulte *pvarID* na [IAccessible::accHitTest](http://msdn.microsoft.com/library/windows/desktop/dd318471) no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="return-value"></a>Valor de retorno  
 Retorna S_OK com êxito, um código de erro COM falha. Consulte **retornar valores** na **IAccessible::accHitTest** no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
### <a name="remarks"></a>Comentários  
 Essa função é parte do MFC [Active Accessibility](http://msdn.microsoft.com/library/windows/desktop/dd373592) suporte.  
  
 Substituir essa função em sua `CWnd`-classe derivada se você tiver elementos de interface do usuário nonwindowed (diferentes controles ActiveX sem janelas, que trata do MFC).  
 ...
 *pcyHeight*  
 ...
 `varChild`  
 Especifica se o local a ser recuperado do objeto ou em um dos elementos-filho do objeto. Esse parâmetro pode ser CHILDID_SELF (para obter informações sobre o objeto) ou uma ID de filho (para obter informações sobre o elemento do filho do objeto).  
  
### <a name="return-value"></a>Valor de retorno  
 Retorna S_OK com êxito, um código de erro COM falha. Consulte **retornar valores** na **IAccessible::accLocation** no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="a-nameaccnavigatea--cwndaccnavigate"></a><a name="accnavigate"></a>CWnd::accNavigate  
 Chamado pela estrutura para percorrer a outro elemento de interface do usuário dentro de um contêiner e, se possível, recuperar o objeto.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna S_OK com êxito, um código de erro COM falha. Consulte **retornar valores** na **IAccessible::accNavigate** no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna S_OK com êxito, um código de erro COM falha. Consulte **retornar valores** na **IAccessible:: accSelect** no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
##  <a name="a-nameanimatewindowa--cwndanimatewindow"></a><a name="animatewindow"></a>CWnd::AnimateWindow  
 Produz efeitos especiais quando mostrando ou ocultando windows.  
...
### <a name="parameters"></a>Parâmetros  
 *dwTime*  
 Especifica quanto tempo leva para executar a animação, em milissegundos. Normalmente, uma animação leva 200 milissegundos para executar.  
...
### <a name="remarks"></a>Comentários  
 Essa função membro emula a funcionalidade da função [AnimateWindow](http://msdn.microsoft.com/library/windows/desktop/ms632669), conforme descrito na [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="remarks"></a>Comentários  
 Essa função de membro também organiza os ícones na janela da área de trabalho, que ocupa a tela inteira. O [GetDesktopWindow](#getdesktopwindow) função de membro recupera um ponteiro para o objeto de janela da área de trabalho.  
  
 Para organizar janelas de filho MDI icônico em uma janela do cliente MDI, chame [CMDIFrameWnd::MDIIconArrange](../../mfc/reference/cmdiframewnd-class.md#mdiiconarrange).  
...
##  <a name="a-namebeginpainta--cwndbeginpaint"></a><a name="beginpaint"></a>CWnd::BeginPaint  
 Prepara `CWnd` para pintura e preenchimentos uma `PAINTSTRUCT` estrutura de dados com informações sobre a pintura.  
...
### <a name="parameters"></a>Parâmetros  
 `lpPaint`  
 Aponta para a [PAINTSTRUCT](../../mfc/reference/paintstruct-structure.md) estrutura de receber informações de pintura.  
...
##  <a name="a-namebinddefaultpropertya--cwndbinddefaultproperty"></a><a name="binddefaultproperty"></a>CWnd::BindDefaultProperty  
 Associa padrão simples associadas propriedade do objeto de chamada (como um controle de edição), como marcada na biblioteca de tipos, até o cursor subjacente que é definido pelas propriedades do controle de fonte de dados SQL, nome de usuário, senha e a fonte de dados.  
...
##  <a name="a-namebindpropertya--cwndbindproperty"></a><a name="bindproperty"></a>CWnd::BindProperty  
 Associa uma propriedade associada de cursor em um controle ligado a dados (como um controle de grade) a um controle de fonte de dados e registra esse relacionamento com o Gerenciador de ligação do MFC.  
...
##  <a name="a-namecalcwindowrecta--cwndcalcwindowrect"></a><a name="calcwindowrect"></a>CWnd::CalcWindowRect  
 Calcula o retângulo da janela que contém o retângulo do cliente especificado.  
...
### <a name="parameters"></a>Parâmetros  
 [in, out] `lpClientRect`  
 Ponteiro para uma estrutura de retângulo. Na entrada, essa estrutura contém o retângulo do cliente. Depois que o método for concluído, essa estrutura contém o retângulo da janela que contém o retângulo do cliente especificado.  
...
> [!NOTE]
>  Usando a função de membro não tem nenhum efeito em dicas de ferramenta gerenciados pelo seu código. Ele afeta somente o controle da dica de ferramenta gerenciado pelo [CWnd::EnableToolTips](#enabletooltips).  
...
### <a name="remarks"></a>Comentários  
 Geralmente chamada de [CDialog::OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog) center em caixas de diálogo em relação a janela principal do aplicativo. Por padrão, a função centers janelas filho em relação à sua janela pai e janelas pop-up em relação a seu proprietário. Se a janela pop-up não é de propriedade, ela é centralizada em relação à tela. Para centralizar uma janela em relação a uma janela específica que não seja o proprietário ou pai, o `pAlternateOwner` parâmetro pode ser definido como uma janela válido. Para forçar a centralização em relação à tela, passe o valor retornado por [CWnd::GetDesktopWindow](#getdesktopwindow) como `pAlternateOwner`.  
...
##  <a name="a-namechangeclipboardchaina--cwndchangeclipboardchain"></a><a name="changeclipboardchain"></a>CWnd::ChangeClipboardChain  
 Remove `CWnd` da cadeia de visualizadores de área de transferência e faz a janela especificada pelo `hWndNext` descendente do `CWnd` ancestral da cadeia.  
...
### <a name="parameters"></a>Parâmetros  
 `hWndNext`  
 Identifica a janela seguinte `CWnd` da cadeia do Visualizador da área de transferência.  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se for bem-sucedida; Caso contrário, 0.  
  
##  <a name="a-namecheckdlgbuttona--cwndcheckdlgbutton"></a><a name="checkdlgbutton"></a>CWnd::CheckDlgButton  
 (Uma marca de seleção ao lado de lugares) de marca ou desmarca (remove uma marca de seleção de) um botão ou altera o estado de um botão de três estados.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nCheck`  
 Especifica a ação a ser tomada. Se `nCheck` é diferente de zero, o `CheckDlgButton` função de membro coloca uma marca de seleção ao lado do botão; se for 0, a marca de seleção será removida. Botões de três estados, se `nCheck` for 2, o estado do botão é indeterminado.  
...
##  <a name="a-namecheckradiobuttona--cwndcheckradiobutton"></a><a name="checkradiobutton"></a>CWnd::CheckRadioButton  
 Seleciona (adiciona uma marca de seleção para) um botão de opção especificado em um grupo e limpa (remover uma marca de seleção) todos os outros botões no grupo.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nIDCheckButton`  
 Especifica o identificador inteiro do botão de opção a ser verificada.  
  
### <a name="remarks"></a>Comentários  
 O `CheckRadioButton` função envia um [BM_SETCHECK](http://msdn.microsoft.com/library/windows/desktop/bb775989) mensagem para o botão especificado.  
...
##  <a name="a-namechildwindowfrompointa--cwndchildwindowfrompoint"></a><a name="childwindowfrompoint"></a>CWnd::ChildWindowFromPoint  
 Determina que, se houver, das janelas filho pertencentes a `CWnd` contém o ponto especificado.  
...
|**CWP_SKIPDISABLED**|Ignorar janelas filho desabilitado|  
|**CWP_SKIPTRANSPARENT**|Ignorar janelas filho transparente|  
  
### <a name="return-value"></a>Valor de retorno  
 Identifica a janela filho que contém o ponto. É **nulo** se determinado ponto está fora da área do cliente. Se o ponto estiver dentro da área do cliente, mas não está contido em qualquer janela filho, `CWnd` é retornado.  
  
 Essa função de membro retornará uma janela filho desabilitados ou ocultos que contém o ponto especificado.  
  
 Mais de uma janela pode conter ponto específico. No entanto, essa função retorna apenas o `CWnd`* da primeira janela encontrada que contém o ponto.  
...
```  
Criar BOOL virtual (LPCTSTR lpszClassName,  
...
```  
virtual CreateAccessibleProxy HRESULT (WPARAM wParam,  
    LPARAM lParam,  
    LRESULT* pResult);
```  
...
BOOL CreateControl (clsid REFCLSID,  
    LPCTSTR pszWindowName,  
    DWORD dwStyle,  
    RECT const < / rect,  
    CWnd* pParentWnd, UINT nID, CFile* pPersist = NULL,  
...
BOOL CreateControl (clsid REFCLSID,  
    LPCTSTR pszWindowName,  
    DWORD dwStyle,  
    PONTO const* ppt, tamanho const* psize,  
    CWnd* pParentWnd, UINT nID, CFile* pPersist = NULL,  
    BOOL bStorage = FALSE,  
    BSTR bstrLicKey = NULL);
```  
...
```  
virtual LRESULT DefWindowProc (mensagem UINT,  
    WPARAM wParam,  
    LPARAM lParam);
```  
...
```  
estático DeleteTempMap() PASCAL void;
```  
...
```  
Int (LPTSTR lpPathSpec, DlgDirList  
    int nIDListBox,  
    int nIDStaticPath,  
    UINT nFileType);
```  
...
```  
Int (LPTSTR lpPathSpec, DlgDirListComboBox  
    int nIDComboBox,  
    int nIDStaticPath,  
    UINT nFileType);
```  
...
```  
void EnableDynamicLayout (BOOL bAtivar = TRUE);
```  
...
```  
void EnableD2DSupport (BOOL bAtivar = TRUE,  
...
```  
EndModalLoop virtual void (int Nresultado);
```  
...
```  
estático CWnd * PASCAL FindWindow (LPCTSTR lpszClassName,  
    LPCTSTR lpszWindowName);
```  
...
```  
estático CWnd * FindWindowEx (HWND hwndParent,  
    HwndChildAfter HWND  
    LPCTSTR lpszClass,  
    LPCTSTR lpszWindow);
```  
...
```  
estático CWnd * PASCAL FromHandle(HWND hWnd);
```  
...
```  
estático CWnd * PASCAL FromHandlePermanent(HWND hWnd);
```  
...
```  
virtual get_accChild HRESULT (VARIANTE varChild,  
...
```  
virtual get_accChildCount HRESULT (long * pcountChildren);
```  
...
```  
virtual get_accDefaultAction HRESULT (VARIANTE varChild,  
...
```  
virtual get_accDescription HRESULT (VARIANTE varChild,  
    BSTR* pszDescription);
```  
...
```  
virtual get_accFocus HRESULT (VARIANT * pvarChild);
```  
...
```  
virtual get_accHelp HRESULT (VARIANTE varChild,  
    BSTR* pszHelp);
```  
...
```  
virtual get_accHelpTopic HRESULT (BSTR* pszHelpFile, varChild de VARIANTE longos* pidTopic);
```  
...
```  
virtual get_accKeyboardShortcut HRESULT (VARIANTE varChild,  
...
```  
virtual get_accName HRESULT (VARIANTE varChild,  
    BSTR* pszName);
```  
...
```  
virtual get_accParent HRESULT (IDispatch * * ppdispParent);
```  
...
```  
virtual get_accRole HRESULT (VARIANTE varChild,  
    Variant* pvarRole);
```  
...
```  
virtual get_accSelection HRESULT (VARIANT * pvarChildren);
```  
...
```  
virtual get_accState HRESULT (VARIANTE varChild,  
    Variant* pvarState);
```  
...
```  
virtual get_accValue HRESULT (VARIANTE varChild,  
    BSTR* pszValue);
```  
...
```  
estático CWnd * PASCAL GetActiveWindow();
```  
...
```  
estático CWnd * PASCAL GetCapture();
```  
...
```  
GetCaretPos() do PASCAL CPoint estático;
```  
...
```  
estático CWnd * PASCAL GetClipboardOwner();
```  
...
```  
estático CWnd * PASCAL GetClipboardViewer();
```  
...
```  
estático CWnd * PASCAL GetDesktopWindow();
```  
...
```  
estático CWnd * PASCAL GetForegroundWindow();
```  
...
```  
estático CWnd * PASCAL GetOpenClipboardWindow();
```  
...
```  
estático CWnd* GetSafeOwner (CWnd* pParent = NULL,  
    HWND* pWndTop = NULL);
```  
...
```  
Int (LPTSTR lpszStringBuf, GetWindowText  
    nMaxCount int) const;  
...
```  
Help virtual void (DWORD_PTR dwData,  
    UINT nCmd = 0x000F);
```  
...
void AFX_CDECL InvokeHelper (DISPID dwDispID,  
    WFlags WORD,  
    VARTYPE vtRet,  
    void* pvRet, BYTE const* pbParamInfo,  
 ...);
```  
...
```  
afx_msg anular AoAtivar (UINT nState,  
    CWnd * pWndOther,  
    BMinimized BOOL);
```  
...
```  
afx_msg void OnActivateApp (BOOL bScripts Ativos,  
    DWORD dwThreadID);
```  
...
```  
virtual OnAmbientProperty BOOL (COleControlSite* pSite DISPID dispid, VARIANTE* pvar);
```  
...
```  
afx_msg void OnAppCommand (CWnd * Apropriei,  
    NCmd UINT,  
    NDevice UINT,  
    UINT nKey);
```  
...
```  
afx_msg void OnCaptureChanged (CWnd * Apropriei);
```  
...
```  
afx_msg void OnChangeUIState (nação UINT,  
    UINT nUIElement);
```  
...
```  
virtual OnChildNotify BOOL (mensagem UINT,  
    WPARAM wParam,  
    LPARAM lParam,  
    LRESULT* pResult);
```  
...
```  
virtual OnCommand BOOL (WPARAM wParam,  
    LPARAM lParam);
```  
...
```  
afx_msg anular OnContextMenu (CWnd * Apropriei,  
    CPoint pos);
```  
...
```  
afx_msg OnCopyData BOOL (CWnd* Apropriei, COPYDATASTRUCT* pCopyDataStruct);
```  
...
```  
afx_msg HBRUSH OnCtlColor (CDC* pDC, CWnd* Apropriei,  
    UINT nCtlColor);
```  
...
```  
afx_msg void OnDevModeChange (lpDeviceName LPTSTR);
```  
...
```  
virtual OnDrawIconicThumbnailOrLivePreview void (CDC < / dc,  
    CRect rect,  
...
```  
afx_msg void OnEnable (BOOL bAtivar);
```  
...
```  
afx_msg OnEraseBkgnd(CDC* pDC) BOOL;
```  
...
```  
afx_msg OnHelpInfo(HELPINFO* lpHelpInfo) BOOL;
```  
...
```  
afx_msg anular OnKeyDown (nChar UINT,  
    NRepCnt UINT,  
    UINT nFlags);
```  
...
```  
afx_msg anular OnKeyUp (nChar UINT,  
    NRepCnt UINT,  
    UINT nFlags);
```  
...
```  
afx_msg OnMenuChar LRESULT (nChar UINT,  
    NFlags UINT,  
    CMenu* pMenu);
```  
...
```  
afx_msg OnMenuGetObject(MENUGETOBJECTINFO* pMenuGetObjectInfo) UINT;
```  
...
```  
afx_msg anular OnMouseMove (UINT nFlags,  
    Ponto de CPoint);
```  
...
```  
afx_msg void OnMoving (UINT Xplorando,  
    LPRECT lpRect);
```  
...
```  
afx_msg OnNcActivate(BOOL bActive) BOOL;
```  
...
```  
afx_msg OnNcCreate(LPCREATESTRUCT lpCreateStruct) BOOL;
```  
...
```  
afx_msg OnNcHitTest(CPoint point) LRESULT;
```  
...
```  
void OnNcXButtonDblClk (nHitTest curto,   
    NButton UINT,   
    Ponto de CPoint);
```  
...
```  
afx_msg void OnNcXButtonDown (nHitTest curto,   
    NButton UINT,   
    Ponto de CPoint);
```  
...
```  
afx_msg void OnNcXButtonUp (nHitTest curto,   
    NButton UINT,   
    Ponto de CPoint);
```  
...
```  
virtual OnNotify BOOL (WPARAM wParam,  
    LPARAM lParam,  
    LRESULT* pResult);
```  
...
```  
afx_msg OnNotifyFormat UINT (CWnd * Apropriei,   
    UINT nCommand);
```  
...
```  
afx_msg void OnParentNotify (mensagem UINT,  
    LPARAM lParam);
```  
...
```  
afx_msg OnQueryEndSession() BOOL;
```  
...
```  
afx_msg OnQueryNewPalette() BOOL;
```  
...
```  
afx_msg OnSetCursor BOOL (CWnd * Apropriei,  
    NHitTest UINT,  
    Mensagem UINT);
```  
...
```  
afx_msg anular OnSettingChange (UINT uFlags,  
    LPCTSTR lpszSection);
```  
...
```  
afx_msg void OnShowWindow (BOOL bMostrar,  
    UINT nStatus);
```  
...
```  
afx_msg void OnSizing (UINT Xplorando,  
    LPRECT lpRect);
```  
...
```  
afx_msg anular OnTimer (UINT_PTR nIDEvent);
```  
...
```  
virtual INT_PTR OnToolHitTest (CPoint ponto,  
    TOOLINFO* pTI) const;  
```  
...
```  
virtual OnTouchInput BOOL (CPoint pt,  
    int nInputNumber,  
    int nInputsCount,  
    PTOUCHINPUT pInput);
```  
...
```  
virtual OnTouchInputs BOOL (UINT nInputsCount,  
    PTOUCHINPUT pInputs);
```  
...
```  
afx_msg void OnUpdateUIState (nação UINT,  
    UINT nUIElement);
```  
...
```  
virtual OnWndMsg BOOL (mensagem UINT,  
    WPARAM wParam,  
    LPARAM lParam,  
    LRESULT* pResult);
```  
...
```  
Operador BOOL! = (CWnd const < wnd /) const;  
```  
...
```  
BOOL PostMessage (mensagem UINT,  
    WPARAM wParam = 0,  
    LPARAM lParam = 0);
```  
...
```  
virtual PreCreateWindow(CREATESTRUCT& cs); BOOL
```  
...
```  
PreSubclassWindow() void virtual;
```  
...
```  
PreTranslateMessage(MSG* pMsg) BOOL virtual;
```  
...
BOOL RedrawWindow (LPCRECT lpRectUpdate = NULL,  
    ...
    UINT sinalizadores = RDW_INVALIDATE | RDW_UPDATENOW | RDW_ERASE);
```  
...
```  
BOOL ReflectChildNotify (mensagem UINT,  
    WPARAM wParam,  
    LPARAM lParam,  
    LRESULT* pResult);
```  
...
```  
estático BOOL PASCAL ReflectLastMsg (HWND hWndChild,  
...
### <a name="remarks"></a>Comentários  
 O `ScreenToClient` função membro substitui as coordenadas de tela fornecidas na `lpPoint` ou `lpRect` com coordenadas do cliente. As novas coordenadas são relativas ao canto superior esquerdo do `CWnd` área do cliente.  
...
##  <a name="a-namescrollwindowa--cwndscrollwindow"></a><a name="scrollwindow"></a>CWnd::ScrollWindow  
 Rola o conteúdo da área do cliente do atual `CWnd` objeto.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `lpRect`  
 Aponta para um [CRect](../../atl-mfc-shared/reference/crect-class.md) objeto ou [estrutura RECT](../../mfc/reference/rect-structure1.md) que especifica a parte da área do cliente a ser rolado. Se `lpRect` é **nulo**, toda a área cliente é rolada. O cursor é reposicionado se o retângulo de cursor interceptar o retângulo de rolagem.  
  
 `lpClipRect`  
 Aponta para um `CRect` objeto ou `RECT` estrutura que especifica o retângulo de recorte para rolar. Somente os bits dentro desse retângulo serão rolados. Bits fora desse retângulo não são afetadas, mesmo se eles estiverem no `lpRect` retângulo. Se `lpClipRect` é **nulo**, sem recorte é executada no retângulo rolagem.  
  
### <a name="remarks"></a>Comentários  
 Se o cursor estiver no `CWnd` sendo rolado, `ScrollWindow` automaticamente oculta o cursor para impedir que ele seja apagado e restaura o cursor após a rolagem. A posição do cursor será ajustada.  
  
 A área revelados pelo `ScrollWindow` função de membro não é redesenhada, mas é combinada em atual `CWnd` região de atualização do objeto. O aplicativo eventualmente receberá um [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) mensagem notificando que a região requer repintura. Para redesenhar a área descoberta ao mesmo tempo a rolagem é feita, chame o [UpdateWindow](#updatewindow) a função de membro imediatamente depois de chamar `ScrollWindow`.  
  
 Se `lpRect` é **nulo**, as posições de todas as janelas filho na janela são separadas pelo valor especificado por `xAmount` e `yAmount`e as áreas (não pintadas) inválidas no `CWnd` também são deslocados. `ScrollWindow`é mais rápido quando `lpRect` é **nulo**.  
 ...
 *dy*  
 ...
 `lpRectScroll`  
 Aponta para um [estrutura RECT](../../mfc/reference/rect-structure1.md) que especifica a parte da área do cliente a ser rolado. Se esse parâmetro for **nulo**, toda a área cliente é rolada.  
  
 `lpRectClip`  
 Aponta para um `RECT` estrutura que especifica o retângulo de recorte para rolar. Essa estrutura tem precedência sobre o retângulo apontado por `lpRectScroll`. Somente os bits dentro desse retângulo serão rolados. Bits fora desse retângulo não são afetadas, mesmo se eles estiverem no `lpRectScroll` retângulo. Se esse parâmetro for **nulo**, sem recorte é executada no retângulo rolagem.  
...
- **SW_SCROLLCHILDREN** rola todas as janelas filho que fazem interseção com o retângulo apontado por `lpRectScroll` pelo número de pixels especificado em `dx` e *dy*. O Windows envia um [WM_MOVE](http://msdn.microsoft.com/library/windows/desktop/ms632631) mensagem para todas as janelas filho que se cruzam `lpRectScroll`, mesmo que eles não serão movidos. O cursor é reposicionado quando uma janela filho é rolada e o retângulo de cursor intercepta o retângulo de rolagem.  
  
### <a name="return-value"></a>Valor de retorno  
 O valor de retorno é **SIMPLEREGION** (região retangular invalidado), **COMPLEXREGION** (região invalidado não retangular; sobrepostos retângulos), ou **NULLREGION** (nenhuma região invalidado), se a função for bem-sucedida; caso contrário, o valor de retorno é **erro**.  
...
##  <a name="a-namesendchildnotifylastmsga--cwndsendchildnotifylastmsg"></a><a name="sendchildnotifylastmsg"></a>CWnd::SendChildNotifyLastMsg  
 Essa função de membro é chamada pela estrutura para fornecer uma mensagem de notificação para uma janela filho, na janela pai, para a janela filho possa lidar com uma tarefa.  
...
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se a janela filho tratou a mensagem enviada ao seu pai; Caso contrário, 0.  
...
### <a name="parameters"></a>Parâmetros  
 `nID`  
 Especifica o identificador do controle de caixa de diálogo que irá receber a mensagem.  
...
### <a name="remarks"></a>Comentários  
 ...
 Usando `SendDlgItemMessage` é idêntica à obtenção de um `CWnd`* para o controle de determinado e chamar o [SendMessage](#sendmessage) função de membro.  
...
### <a name="remarks"></a>Comentários  
 O **SendMessage** função de membro chama o procedimento de janela diretamente e não retorna até que esse procedimento de janela processou a mensagem. Isso está em contraste com o [PostMessage](#postmessage) função de membro, que coloca a mensagem na fila de mensagens da janela e retorna imediatamente.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `bDeep`  
 Especifica o nível no qual pesquisar. Se **TRUE**, recursivamente pesquisar todos os filhos; se **FALSE**, pesquisar apenas os filhos imediatos.  
  
 `bOnlyPerm`  
 Especifica se a mensagem será recebida pelo windows temporários. Se **TRUE**, windows temporários podem receber a mensagem; se **FALSE**, somente o windows permanentes a mensagem. Para obter mais informações sobre windows temporários consulte [técnico Observação 3](../../mfc/tn003-mapping-of-windows-handles-to-objects.md).  
...
### <a name="remarks"></a>Comentários  
 ...
 Se `bDeep` e `bOnlyPerm` são **TRUE**, a pesquisa continua abaixo windows temporários. Nesse caso, apenas o windows permanente encontrado durante a pesquisa a mensagem. Se `bDeep` é **FALSE**, a mensagem é enviada somente para os filhos imediatos da janela.  
...
### <a name="remarks"></a>Comentários  
 O `SetActiveWindow` função de membro deve ser usada com cuidado, pois ele permite que um aplicativo arbitrariamente assumir a janela ativa e o foco de entrada. Normalmente, o Windows se encarrega de ativação todos os.  
...
### <a name="remarks"></a>Comentários  
 Quando `CWnd` não exige todas as entradas do mouse, o aplicativo deve chamar o [ReleaseCapture](http://msdn.microsoft.com/library/windows/desktop/ms646261) função para que outras janelas podem receber entrada do mouse.  
...
### <a name="remarks"></a>Comentários  
 O `SetCaretPos` função de membro Move o cursor somente se ele pertence a uma janela na tarefa atual. `SetCaretPos`Move o cursor se o cursor está oculto ou não.  
...
### <a name="return-value"></a>Valor de retorno  
 Um identificador para a próxima janela na cadeia de área de armazenamento se for bem-sucedido. Aplicativos devem salvar esse identificador (ele pode ser armazenado como uma variável de membro) e usá-lo ao responder a mensagens de cadeia de área de armazenamento.  
  
### <a name="remarks"></a>Comentários  
 Uma janela que é parte da cadeia da área de armazenamento deve responder a [WM_DRAWCLIPBOARD](#ondrawclipboard), [WM_CHANGECBCHAIN](#onchangecbchain), e [WM_DESTROY](#ondestroy) mensagens e passar a mensagem para a próxima janela na cadeia.  
  
 Essa função de membro envia um `WM_DRAWCLIPBOARD` mensagem na janela. Como o identificador para a próxima janela na cadeia de área de armazenamento ainda não foi retornado, o aplicativo não deve passar `WM_DRAWCLIPBOARD` mensagens recebidas durante a chamada para `SetClipboardViewer`.  
...
##  <a name="a-namesetdlgctrlida--cwndsetdlgctrlid"></a><a name="setdlgctrlid"></a>CWnd::SetDlgCtrlID  
 Define a ID de janela ou controle de janela para um novo valor.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `bSigned`  
 Especifica se o valor inteiro assinado ou não assinado. Se esse parâmetro for **TRUE**, `nValue` está assinado. Se esse parâmetro for **TRUE** e `nValue` é menor que 0, um sinal de menos símbolo é colocado antes do primeiro dígito na cadeia de caracteres. Se esse parâmetro for **FALSE**, `nValue` não está assinado.  
...
### <a name="remarks"></a>Comentários  
 Entrada do teclado é direcionada para a janela e várias dicas visuais são alteradas para o usuário. A janela de primeiro plano é a janela com a qual o usuário está trabalhando no momento. A janela de primeiro plano aplica-se somente para janelas de nível superior (caixas de janelas ou caixas de diálogo de quadro).  
...
### <a name="remarks"></a>Comentários  
 O foco de entrada direciona todas as entradas de teclado subsequentes a esta janela. Perde qualquer janela que anteriormente tinha o foco de entrada.  
  
 O `SetFocus` função de membro envia um [WM_KILLFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646282) mensagem para a janela que perde o foco de entrada e um [WM_SETFOCUS](http://msdn.microsoft.com/library/windows/desktop/ms646283) mensagem à janela que recebe o foco de entrada. Ele também ativa a janela ou seu pai.  
  
 Se a janela atual está ativa, mas não tem o foco (ou seja, nenhuma janela tiver o foco), qualquer tecla pressionada produzirá as mensagens [WM_SYSCHAR](#onsyschar), [WM_SYSKEYDOWN](#onsyskeydown), ou [WM_SYSKEYUP](#onsyskeyup).  
...
### <a name="remarks"></a>Comentários  
 Esse método não tem nenhum efeito a menos que a janela processa o `WM_SETFONT` mensagem. Muitas classes do MFC que derivam de `CWnd` processar essa mensagem porque eles são anexados a uma classe de janela predefinida que inclui um manipulador de mensagens para o `WM_SETFONT` mensagem. Para usar esse método, as classes que derivam de `CWnd` deve definir um manipulador de método para o `WM_SETFONT` mensagem.  
  
##  <a name="a-nameseticona--cwndseticon"></a><a name="seticon"></a>CWnd::SetIcon  
 Chame essa função de membro para definir o identificador com um ícone específico, conforme identificado pelo `hIcon`.  
...
### <a name="parameters"></a>Parâmetros  
 `crKey`  
 Ponteiro para uma **COLORREF** valor que especifica a chave de cor de transparência a ser usado ao criar a janela em camadas. Todos os pixels pintados pela janela essa cor será transparentes. Para gerar um **COLORREF**, use a macro RGB.  
  
 `bAlpha`  
 Valor alfa usado para descrever a opacidade da janela em camadas. Para obter mais informações, consulte o **SourceConstantAlpha** membro do [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) estrutura. Quando `bAlpha` é 0, a janela é completamente transparente. Quando `bAlpha` é 255, a janela é opaca.  
  
 `dwFlags`  
 Especifica uma ação. Esse parâmetro pode ser um ou mais dos valores a seguir. Para obter uma lista dos valores possíveis, consulte [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540).  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se a função for bem-sucedida; Caso contrário, 0.  
  
### <a name="remarks"></a>Comentários  
 Essa função membro emula a funcionalidade da função [SetLayeredWindowAttributes](http://msdn.microsoft.com/library/windows/desktop/ms633540), conforme descrito na [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parâmetros  
 `pMenu`  
 Identifica o novo menu. Se esse parâmetro for **nulo**, menu atual é removido.  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se o menu for alterado; Caso contrário, 0.  
  
### <a name="remarks"></a>Comentários  
 Faz com que a janela seja redesenhado para refletir a alteração do menu.  
...
### <a name="parameters"></a>Parâmetros  
 *pOwnerWnd*  
 Identifica o novo proprietário do objeto window. Se esse parâmetro for **nulo**, o objeto de janela tem sem proprietário.  
...
### <a name="remarks"></a>Comentários  
 ...
 Costuma ser útil estabelecer conexões entre objetos de janela que não estão relacionados à hierarquia de janela. Por exemplo, [CToolBar](../../mfc/reference/ctoolbar-class.md) envia notificações para seu proprietário, em vez de seu pai. Isso permite que a barra de ferramentas para se tornar o filho de uma janela (por exemplo, uma janela de aplicativo de contêiner OLE) ao enviar notificações para outra janela (por exemplo, a janela de quadro in-loco). Além disso, quando uma janela de servidor é desativada ou ativada no local durante a edição, qualquer pertencente a janela do quadro de janela é ocultada ou exibida. Essa propriedade é definida explicitamente com uma chamada para `SetOwner`.  
...
### <a name="remarks"></a>Comentários  
 Se a janela filho estiver visível, o Windows realiza o redesenho apropriado e redesenho.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `vtProp`  
 Especifica o tipo da propriedade a ser definida. Para obter valores possíveis, consulte a seção comentários para [COleDispatchDriver::InvokeHelper](../../mfc/reference/coledispatchdriver-class.md#invokehelper).  
...
> [!NOTE]
>  Essa função deve ser chamada somente em uma `CWnd` objeto que representa um controle OLE.  
...
##  <a name="a-namesetredrawa--cwndsetredraw"></a><a name="setredraw"></a>CWnd::SetRedraw  
 Um aplicativo chama `SetRedraw` para permitir alterações para ser redesenhado ou para impedir que alterações sendo redesenhadas.  
...
### <a name="parameters"></a>Parâmetros  
 `bRedraw`  
 Especifica o estado do sinalizador redesenho. Se esse parâmetro for **TRUE**, o sinalizador de redesenho é definido; se **FALSE**, o sinalizador está desmarcado.  
  
### <a name="remarks"></a>Comentários  
 Essa função membro define ou limpa o sinalizador de redesenho. Enquanto o sinalizador de redesenho estiver desmarcado, o conteúdo não será atualizado após cada alteração e não será repintado até que o redesenho sinalizador é definido. Por exemplo, um aplicativo que precisa adicionar vários itens a uma caixa de listagem pode limpar o sinalizador de redesenho, adicione os itens e, em seguida, defina o sinalizador de redesenho. Finalmente, o aplicativo pode chamar o [Invalidate](#invalidate) ou [InvalidateRect](#invalidaterect) a função de membro para fazer com que a caixa de listagem seja redesenhada.  
...
- **SB_CTL** contém os parâmetros para um controle de barra de rolagem. O `m_hWnd` membro de dados deve ser a alça de controle de barra de rolagem.  
...
### <a name="remarks"></a>Comentários  
 ...
 O Windows MFC mensagem manipuladores que indicam a posição da barra de rolagem, [CWnd::OnHScroll](#onhscroll) e [CWnd::OnVScroll](#onvscroll), fornecer apenas 16 bits de dados de posição. [GetScrollInfo](#getscrollinfo) e `SetScrollInfo` fornecem 32 bits de dados de posição da barra de rolagem. Assim, um aplicativo pode chamar `GetScrollInfo` ao processar uma `CWnd::OnHScroll` ou `CWnd::OnVScroll` para obter dados de posição da barra de rolagem de 32 bits.  
  
> [!NOTE]
> [CWnd::GetScrollInfo](#getscrollinfo) permite que aplicativos usem posições da barra de rolagem de 32 bits.  
...
### <a name="parameters"></a>Parâmetros  
 `nBar`  
 Especifica a barra de rolagem seja definido. Esse parâmetro pode ser um destes procedimentos:  
...
### <a name="remarks"></a>Comentários  
 Definindo `bRedraw` para **FALSE** é útil sempre que a barra de rolagem será redesenhada por uma chamada subsequente para outra função.  
...
### <a name="parameters"></a>Parâmetros  
 `nBar`  
 Especifica a barra de rolagem seja definido. Esse parâmetro pode ser qualquer um dos seguintes valores:  
...
### <a name="remarks"></a>Comentários  
 ...
 Se a chamada para `SetScrollRange` imediatamente após uma chamada para o [SetScrollPos](#setscrollpos) função de membro, o `bRedraw` parâmetro o `SetScrollPos` função de membro deve ser 0 para impedir que a barra de rolagem que está sendo desenhado duas vezes.  
...
### <a name="parameters"></a>Parâmetros  
 `nIDEvent`  
 Especifica um identificador de timer diferente de zero. Se o identificador de timer é exclusivo, esse mesmo valor é retornado por `SetTimer`. Caso contrário, `SetTimer` determina um novo valor exclusivo e retorna que. Para um timer de janela (que tem uma função de retorno nulo), o valor deve ser exclusivo somente para os demais temporizadores do windows que estão associados com a janela atual. Para um timer de retorno de chamada, o valor deve ser exclusivo para todos os temporizadores em todos os processos. Portanto, quando você cria um temporizador de retorno de chamada, é mais provável que o valor retornado pode diferir do valor especificado.  
  
 `nElapse`  
 Especifica o valor de tempo limite ou intervalo, em milissegundos.  
  
 `lpfnTimer`  
 Especifica o endereço do que a fornecida pelo aplicativo `TimerProc` função de retorno de chamada que processa o [WM_TIMER](http://msdn.microsoft.com/library/windows/desktop/ms644902) mensagens. Se esse parâmetro for `NULL`, o `WM_TIMER` mensagens são colocadas na fila de mensagem do aplicativo e tratadas pelo `CWnd` objeto.  
  
### <a name="return-value"></a>Valor de retorno  
 O identificador de timer do novo temporizador se a função for bem-sucedida. Esse valor pode ou não ser igual ao valor passado por meio de `nIDEvent` parâmetro. Um aplicativo sempre deve passar o valor de retorno para o [KillTimer](#killtimer) a função de membro para eliminar o temporizador. Diferente de zero se for bem-sucedida; Caso contrário, 0.  
  
### <a name="remarks"></a>Comentários  
 Um valor de intervalo for especificado, e sempre que o intervalo expira, o sistema lançará um `WM_TIMER` mensagem para a fila de mensagens de instalação do aplicativo de instalação ou passa a mensagem para um aplicativo definido `TimerProc` função de retorno de chamada.  
  
 O `lpfnTimer` função de retorno de chamada não precisa ser nomeada `TimerProc`, mas isso deve ser declarado como estático e definidos da seguinte maneira.  
...
### <a name="remarks"></a>Comentários  
 Se uma janela filho não tiver um identificador de contexto de Ajuda, ele herda o identificador da janela pai. Da mesma forma, se uma janela de propriedade não tem um identificador de contexto de Ajuda, ele herda o identificador da janela do proprietário. Essa herança de identificadores de contexto de Ajuda permite que um aplicativo defina apenas um identificador para uma caixa de diálogo e todos os seus controles.  
...
##  <a name="a-namesetwindowplacementa--cwndsetwindowplacement"></a><a name="setwindowplacement"></a>CWnd::SetWindowPlacement  
 Define o estado de apresentação e o normal (restaurada), posições maximizadas e minimizadas para uma janela.  
...
### <a name="parameters"></a>Parâmetros  
 `lpwndpl`  
 Aponta para um [WINDOWPLACEMENT](../../mfc/reference/windowplacement-structure.md) estrutura que especifica o novo show state e posições.  
...
- **wndTopMost** coloca a janela windows acima de tudo não superior. A janela mantém sua posição superior, mesmo quando ele está desativado.  
  
- **wndNoTopMost** reposiciona a janela na parte superior de todas as janelas não são mais altas (isto é, por trás de todas as janelas de nível superior). Esse sinalizador não tem efeito se a janela já é uma janela não são mais altas.  
...
- `SWP_NOACTIVATE`Não ative a janela. Se este sinalizador não for definido, a janela é ativada e movida para o início de nível superior ou o grupo não são mais altas (dependendo da configuração do `pWndInsertAfter` parâmetro).  
...
- **SWP_NOREDRAW** não atualiza as alterações. Se esse sinalizador estiver definido, nenhuma redesenho de qualquer tipo ocorre. Isso se aplica a qualquer parte da janela pai revelada como resultado da janela movida, a área não cliente (incluindo as título e barras de rolagem) e a área do cliente. Quando esse sinalizador estiver definido, o aplicativo deve invalidar explicitamente ou redesenhar as partes da janela e janela pai que precisa ser redesenhada.  
...
### <a name="remarks"></a>Comentários  
 ...
 Todas as coordenadas para janelas filho são coordenadas do cliente (em relação ao canto superior esquerdo da área de cliente da janela pai).  
  
 Uma janela pode ser movida para o topo da ordem Z definindo o `pWndInsertAfter` parâmetro **< / wndTopMost** e garantir que o `SWP_NOZORDER` não sinalizador definido ou definindo uma janela de ordem Z para que ele fique acima quaisquer janelas existentes. Quando uma janela não superior é feita primeira, suas propriedade windows também são feitas na extremidade superior. Seus proprietários não são alterados.  
  
 Uma janela de nível superior não está mais acima se ele é reposicionado para a parte inferior ( **< / wndBottom**) da ordem Z ou depois de qualquer janela não superior. Quando uma janela de nível superior é feita sem primeiro, todos os seus proprietários e suas janelas de propriedade também são feitos janelas não ativas.  
...
-   Essa janela não é a janela ativa.  
 ...
 Um aplicativo não pode ativar uma janela inativa sem colocá-lo também na parte superior da ordem Z. Aplicativos podem alterar a ordem Z de uma janela ativada sem restrições.  
  
 Uma janela não superior pode possuir uma janela de nível superior, mas não vice-versa. Qualquer janela (por exemplo, uma caixa de diálogo) pertencente a uma janela de nível superior é feita uma janela de nível superior para garantir que todas as propriedade do windows permanecer acima de seu proprietário.  
  
 Com o Windows versões 3.1 e posteriores, windows pode ser movido para o topo da ordem Z e bloqueado existe, definindo seus **WS_EX_TOPMOST** estilos. Primeira janela mantém sua posição superior, mesmo quando desativado. Por exemplo, selecionando o comando WinHelp sempre visível faz com que a janela da Ajuda mais alta e então permanece visível quando você retornar ao seu aplicativo.  
  
 Para criar uma janela de nível superior, chame `SetWindowPos` com o `pWndInsertAfter` parâmetro igual a **< / wndTopMost**, ou defina o **WS_EX_TOPMOST** ao criar a janela de estilo.  
 ...
 Se `SetWindowPos` é chamado quando o `pWndInsertAfter` parâmetro é **< / wndBottom** e `CWnd` é uma janela de nível superior, ela perde seu status de nível mais alto ( **WS_EX_TOPMOST** estiver desmarcada), e o sistema coloca a janela na parte inferior da ordem Z.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `bRedraw`  
 Se **TRUE**, o sistema operacional redesenha a janela depois de definir a região; caso contrário, não. Normalmente, definir `bRedraw` para **TRUE** se a janela está visível. Se definido como **TRUE**, o sistema envia a `WM_WINDOWPOSCHANGING` e `WM_WINDOWPOSCHANGED` mensagens na janela.  
  
### <a name="return-value"></a>Valor de retorno  
 Se a função obtiver êxito, o valor de retorno é diferente de zero. Se a função falhar, o valor retornado será zero.  
...
### <a name="remarks"></a>Comentários  
 Se a janela for um controle, o texto dentro do controle é definido.  
  
 Essa função faz com que uma [WM_SETTEXT](http://msdn.microsoft.com/library/windows/desktop/ms632644) mensagem a ser enviada para essa janela.  
...
### <a name="remarks"></a>Comentários  
 ...
 O `ShowCaret` função de membro mostra apenas o cursor se ele tem uma forma atual e não foi ocultado duas ou mais vezes consecutivas. Se o cursor não pertence a esta janela, o cursor não é mostrado.  
  
 Ocultar o cursor é cumulativo. Se o [HideCaret](#hidecaret) função de membro foi chamada cinco vezes consecutivas, `ShowCaret` deve ser chamado cinco vezes para mostrar o cursor.  
...
- **SB_VERT** Especifica que a janela é uma barra de rolagem vertical.  
 ...
 `bShow`  
 Especifica se o Windows mostra ou oculta a barra de rolagem. Se esse parâmetro for **TRUE**, a barra de rolagem é mostrada; caso contrário, a barra de rolagem é oculto.  
...
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se a janela tiver sido anteriormente visível; 0 se o `CWnd` anteriormente oculto.  
  
### <a name="remarks"></a>Comentários  
 `ShowWindow`deve ser chamado somente uma vez por aplicativo para a janela principal com [CWinApp::m_nCmdShow](../../mfc/reference/cwinapp-class.md#m_ncmdshow). As chamadas subsequentes para `ShowWindow` deve usar um dos valores listados acima, em vez de especificado pelo `CWinApp::m_nCmdShow`.  
...
##  <a name="a-namesubclassdlgitema--cwndsubclassdlgitem"></a><a name="subclassdlgitem"></a>CWnd::SubclassDlgItem  
 Chame essa função de membro "dinamicamente subclasse" um controle criado a partir de um modelo de caixa de diálogo e anexá-lo a este `CWnd` objeto.  
...
### <a name="remarks"></a>Comentários  
 Quando um controle é dinamicamente subclasse, encaminhe mensagens do windows por meio de `CWnd`do mapa da mensagem e chamam manipuladores de mensagens o `CWnd`da classe primeiro. As mensagens que são passadas para a classe base serão passadas para o manipulador de mensagens padrão no controle.  
...
### <a name="remarks"></a>Comentários  
 Quando uma janela está dinamicamente subclasse, encaminhe mensagens do windows por meio de `CWnd`do mapa da mensagem e chamam manipuladores de mensagens o `CWnd`da classe primeiro. As mensagens que são passadas para a classe base serão passadas para o manipulador de mensagem padrão na janela.  
...
> [!NOTE]
>  A janela não já deve estar anexada a um objeto do MFC quando essa função é chamada.  
...
### <a name="remarks"></a>Comentários  
 Apenas uma janela por vez pode ser bloqueada usando `LockWindowUpdate`. Consulte [CWnd::LockWindowUpdate](#lockwindowupdate) ou a função do Win32 [LockWindowUpdate](http://msdn.microsoft.com/library/windows/desktop/dd145034) para obter mais informações sobre o bloqueio do windows.  
  
##  <a name="a-nameunsubclasswindowa--cwndunsubclasswindow"></a><a name="unsubclasswindow"></a>CWnd::UnsubclassWindow  
 Chame essa função de membro para definir **WndProc** de volta para seu valor original e desanexar a janela identificada por `HWND` do **CWnd** objeto.  
...
##  <a name="a-nameupdatedataa--cwndupdatedata"></a><a name="updatedata"></a>CWnd::UpdateData  
 Chame essa função de membro para inicializar os dados em uma caixa de diálogo, ou para recuperar e validar os dados da caixa de diálogo.  
...
### <a name="remarks"></a>Comentários  
 O framework chama automaticamente `UpdateData` com `bSaveAndValidate` definida como **FALSE** quando a caixa de diálogo modal é criada na implementação do padrão de [CDialog::OnInitDialog](../../mfc/reference/cdialog-class.md#oninitdialog). A chamada ocorre antes que a caixa de diálogo está visível. A implementação padrão de [CDialog::OnOK](../../mfc/reference/cdialog-class.md#onok) chama essa função de membro com `bSaveAndValidate` definida como **TRUE** para recuperar os dados e se for bem-sucedido, fechará a caixa de diálogo. (Se você clicar no botão Cancelar na caixa de diálogo, a caixa de diálogo é fechada sem os dados que estão sendo recuperados).  
  
##  <a name="a-nameupdatedialogcontrolsa--cwndupdatedialogcontrols"></a><a name="updatedialogcontrols"></a>CWnd::UpdateDialogControls  
 Chame essa função de membro para atualizar o estado da caixa de diálogo botões e outros controles em uma caixa de diálogo ou janela que usa o [ON_UPDATE_COMMAND_UI](http://msdn.microsoft.com/library/c4de3c21-2d2e-4b89-a4ce-d0c0e2d9edc4) mecanismo de retorno de chamada.  
...
### <a name="parameters"></a>Parâmetros  
 `pTarget`  
 Aponta para a janela do quadro principal do aplicativo e é usado para rotear mensagens de atualização *.*  
  
 `bDisableIfNoHndler`  
 Sinalizador que indica se um controle que não tenha nenhum manipulador de atualização deve ser exibido automaticamente como desabilitado.  
  
### <a name="remarks"></a>Comentários  
 Se um controle filho não tiver um manipulador e `bDisableIfNoHndler` é **TRUE**, em seguida, o controle filho será desabilitado.  
  
 O framework chama essa função de membro para controles em barras de ferramentas ou barras de caixa de diálogo como parte do aplicativo de processamento ocioso.  
  
##  <a name="a-nameupdatelayeredwindowa--cwndupdatelayeredwindow"></a><a name="updatelayeredwindow"></a>CWnd::UpdateLayeredWindow  
 Atualizações de posição, tamanho, forma, conteúdo e transparência de uma janela em camadas.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `pptDst`  
 Um ponteiro para um **ponto** estrutura especificando a nova posição de tela da janela em camadas. Se a posição atual não está mudando, `pptDst` podem ser **nulo**.  
  
 *psize*  
 Ponteiro para uma **tamanho** estrutura que especifica o novo tamanho da janela em camadas. Se não estiver alterando o tamanho da janela, *psize* podem ser **nulo**.  
  
 If `pDCSrc` is **NULL**, *psize* must be **NULL**.  
  
 `pDCSrc`  
 Um ponteiro para um controlador de domínio para a superfície que define a janela em camadas. Se não estiver alterando a forma e o contexto visual da janela, `pDCSrc` podem ser **nulo**.  
  
 `pptSrc`  
 Ponteiro para uma **ponto** estrutura que especifica o local da camada no contexto do dispositivo.  
 ...
 *pblend*  
 Ponteiro para uma [BLENDFUNCTION](http://msdn.microsoft.com/library/windows/desktop/dd183393) estrutura que especifica o valor de transparência a ser usado ao criar a janela em camadas.  
  
 `dwFlags`  
 Especifica uma ação. Esse parâmetro pode ser um ou mais dos valores a seguir. Para obter uma lista dos valores possíveis, consulte [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556).  
  
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se a função for bem-sucedida; Caso contrário, 0.  
  
### <a name="remarks"></a>Comentários  
 Essa função membro emula a funcionalidade da função [UpdateLayeredWindow](http://msdn.microsoft.com/library/windows/desktop/ms633556), conforme descrito na [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
  
##  <a name="a-nameupdatewindowa--cwndupdatewindow"></a><a name="updatewindow"></a>CWnd::UpdateWindow  
 Atualiza a área do cliente enviando um [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) mensagem se a região de atualização não está vazia.  
...
### <a name="remarks"></a>Comentários  
 O `UpdateWindow` função de membro envia um `WM_PAINT` mensagem diretamente, ignorando a fila de aplicativos. Se a região de atualização estiver vazia, `WM_PAINT` não são enviadas.  
...
##  <a name="a-namevalidaterecta--cwndvalidaterect"></a><a name="validaterect"></a>CWnd::ValidateRect  
 Valida a área do cliente dentro do retângulo determinado removendo o retângulo da região de atualização da janela.  
...
### <a name="parameters"></a>Parâmetros  
 `lpRect`  
 Aponta para um [CRect](../../atl-mfc-shared/reference/crect-class.md) objeto ou [estrutura RECT](../../mfc/reference/rect-structure1.md) que contém as coordenadas do retângulo do cliente a ser removido da região de atualização. Se `lpRect` é **nulo**, toda a janela é validada.  
  
### <a name="remarks"></a>Comentários  
 O [BeginPaint](#beginpaint) função de membro valida automaticamente toda a área cliente. Nem o `ValidateRect` nem [ValidateRgn](#validatergn) função de membro deve ser chamada se uma parte da região de atualização precisa ser validada antes de [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) próxima geração.  
  
 Windows continua a gerar `WM_PAINT` mensagens até que a região de atualização atual é validada.  
...
### <a name="remarks"></a>Comentários  
 Determinada região deve ter sido criado anteriormente por uma função de região. As coordenadas de região devem para ser coordenadas do cliente.  
  
 O [BeginPaint](#beginpaint) função de membro valida automaticamente toda a área cliente. Nem o [ValidateRect](#validaterect) nem `ValidateRgn` função de membro deve ser chamada se uma parte da região de atualização deve ser validada antes do próximo [WM_PAINT](http://msdn.microsoft.com/library/windows/desktop/dd145213) mensagem é gerada.  
...
### <a name="return-value"></a>Valor de retorno  
 Um ponteiro para o objeto de janela no qual se encontra o ponto. É **nulo** se nenhuma janela existe em determinado ponto. O ponteiro retornado pode ser temporário e não deve ser armazenado para uso posterior.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `nCmd`  
 Especifica o tipo de ajuda solicitado. Para obter uma lista de valores possíveis e como eles afetam o `dwData` parâmetro, consulte o [WinHelp](http://msdn.microsoft.com/library/windows/desktop/bb762267) Windows funcionar no [!INCLUDE[winSDK](../../atl/includes/winsdk_md.md)].  
...
### <a name="parameters"></a>Parâmetros  
 `bRegister`  
 `TRUE`indica o registro Windows touch suporte; `FALSE` caso contrário.  
...
##  <a name="a-nameresizedynamiclayouta--cwndresizedynamiclayout"></a><a name="resizedynamiclayout"></a>CWnd::ResizeDynamicLayout  
 Chamado pela estrutura quando altera o tamanho de janela para ajustar o layout de janelas filho, se o layout dinâmico estiver habilitado para a janela.  
