---
title: Classe CDataRecoveryHandler | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
O `CDataRecoveryHandler` salve documentos e restaurá-las, se um aplicativo é encerrado inesperadamente.  
...
|[CDataRecoveryHandler::GetDocumentListName](#getdocumentlistname)|Recupera o nome do documento de uma `CDocument` objeto.|  
...
|[CDataRecoveryHandler::QueryRestoreAutosavedDocuments](#queryrestoreautosaveddocuments)|Exibe uma caixa de diálogo para o usuário para cada documento que o `CDataRecoveryHandler` salva automaticamente. A caixa de diálogo determina se o usuário deseja restaurar o documento salva automaticamente.|  
...
|[CDataRecoveryHandler::UpdateDocumentInfo](#updatedocumentinfo)|Atualiza as informações de um documento porque o usuário salvou.|  
...
|m_dwRestartManagerSupportFlags|Fornece sinalizadores que indicam que suporte o Gerenciador de reinicialização do aplicativo.|  
|m_lstAutosavesToDelete|Uma lista de salva automaticamente arquivos que não foram excluídos quando os documentos originais foram fechados. Quando o aplicativo for fechado, as tentativas de Gerenciador de reinicialização excluindo os arquivos.|  
...
|m_mapDocNameToRestoreBool|Um mapa dos nomes de documento para um parâmetro booleano que indica se deve restaurar o documento salva automaticamente.|  
...
## <a name="remarks"></a>Comentários  
 ...
 Porque o `CDataRecoveryHandler` classe intimamente está conectada ao Gerenciador de reinicialização, `CDataRecoveryHandler` depende do parâmetro global `m_dwRestartManagerSupportFlags`. Esse parâmetro determina quais permissões o Gerenciador de reinicialização e como ele interage com o aplicativo. Para incorporar o Gerenciador de reinicialização de um aplicativo existente, você precisa atribuir `m_dwRestartManagerSupportFlags` o valor apropriado no construtor do aplicativo principal. Para obter mais informações sobre como usar o Gerenciador de reinicialização, consulte [como: adicionar suporte do Gerenciador de reinicialização](../../mfc/how-to-add-restart-manager-support.md).  
...
### <a name="remarks"></a>Comentários  
 Esse método retorna `TRUE` se não houver nenhum documento deve ser salvo. Ele também retorna `TRUE` sem salvar todos os documentos se recuperando o `CWinApp` ou `CDocManager` para o aplicativo gera um erro.  
...
|[in] `bResetModifiedFlag`|`TRUE`indica que o `CDataRecoveryHandler` considera `pDocument` a ser modificada; `FALSE` indica que a estrutura considera `pDocument` ser modificados. Consulte a seção de comentários para obter mais informações sobre o efeito desse sinalizador.|  
  
### <a name="return-value"></a>Valor de retorno  
 `TRUE`Se os sinalizadores adequados estão definidos e `pDocument` é válido `CDocument` objeto.  
  
### <a name="remarks"></a>Comentários  
 Cada `CDocument` objeto tem um sinalizador que indica se ele foi alterado desde a última gravação. Use [CDocument::IsModified](../../mfc/reference/cdocument-class.md#ismodified) para determinar o estado desse sinalizador. Se um `CDocument` não foi alterado desde a última gravação, `AutosaveDocumentInfo` exclui todos os arquivos salva automaticamente para esse documento. Se um documento foi alterado desde o último salvamento, fechá-lo solicita ao usuário para salvar o documento antes de fechar.  
  
> [!NOTE]
>  Usando `bResetModifiedFlag` alterar o estado do documento para inalterado pode causar o usuário perder dados não salvos. Se a estrutura considera um documento sem modificações, fechá-lo não pede ao usuário para salvar.  
  
 Esse método lançará uma exceção com o [ASSERT](http://msdn.microsoft.com/library/1e70902d-d58c-4e7b-9f69-2aeb6cbe476c) macro se `pDocument` não é válido `CDocument` objeto.  
...
|[in] `nAutosaveInterval`|O tempo entre salve. Esse parâmetro é em milissegundos.|  
...
### <a name="remarks"></a>Comentários  
 Esse método verifica se `pDocument` já está na lista de documentos antes de adicionar o documento. Se `pDocument` já está na lista, esse método exclui o arquivo salva automaticamente associado `pDocument`.  
...
### <a name="remarks"></a>Comentários  
 O nome de cada documento tem um mapeamento um para um com um nome de arquivo de salvamento automático.  
...
### <a name="return-value"></a>Valor de retorno  
 O número de milissegundos entre autosave tentativas.  
...
### <a name="return-value"></a>Valor de retorno  
 O local onde são armazenados os documentos salva automaticamente.  
...
##  <a name="a-namegetdocumentlistnamea--cdatarecoveryhandlergetdocumentlistname"></a><a name="getdocumentlistname"></a>CDataRecoveryHandler::GetDocumentListName  
 Recupera o nome do documento de uma `CDocument` objeto.  
...
### <a name="remarks"></a>Comentários  
 Por padrão, o título recuperado de um documento é o título normal com **[Recuperado]** anexado a ele. O título recuperado é exibido ao usuário quando o `CDataRecoveryHandler` consulta o usuário para restaurar os documentos salva automaticamente.  
...
### <a name="return-value"></a>Valor de retorno  
 `TRUE`indica que o Gerenciador de reinicialização causou o aplicativo Sair; `FALSE` indica que não funcionou.  
...
### <a name="remarks"></a>Comentários  
 O processo de inicialização carrega o caminho para armazenar arquivos de salvamento automático do registro. Se o `Initialize` método não é possível localizar esse diretório ou se o caminho é `NULL`, `Initialize` falhará e retornará `FALSE`.  
 ...
 O `Initialize` método também inicia um cronômetro para monitorar quando ocorre o salvamento automático Avançar. Use [CDataRecoveryHandler::SetAutosaveInterval](#setautosaveinterval) para alterar o intervalo de salvamento automático depois que seu aplicativo inicializa o `CDataRecoveryHandler`.  
...
##  <a name="a-namequeryrestoreautosaveddocumentsa--cdatarecoveryhandlerqueryrestoreautosaveddocuments"></a><a name="queryrestoreautosaveddocuments"></a>CDataRecoveryHandler::QueryRestoreAutosavedDocuments  
 Exibe uma caixa de diálogo para o usuário para cada documento que o `CDataRecoveryHandler` salva automaticamente. A caixa de diálogo determina se o usuário deseja restaurar o documento salva automaticamente.  
...
### <a name="remarks"></a>Comentários  
 Se seu aplicativo for Unicode, esse método exibe uma [CTaskDialog](../../mfc/reference/ctaskdialog-class.md) para o usuário. Caso contrário, a estrutura usa [AfxMessageBox](../../mfc/reference/cstring-formatting-and-message-box-display.md#afxmessagebox) para consultar o usuário.  
  
 Depois de `QueryRestoreAutosavedDocuments` reúne todas as respostas do usuário, ele armazena as informações na variável membro `m_mapDocNameToRestoreBool`. Esse método não restaura os documentos salva automaticamente.  
...
### <a name="remarks"></a>Comentários  
 Essa função carrega as informações do documento aberto do registro e o armazena na variável membro `m_mapDocNameToAutosaveName`.  
...
|-|-|  
|Parâmetro|Descrição|  
|[in] `pDocument`|Um ponteiro para o documento para remover.|  
...
### <a name="remarks"></a>Comentários  
 Este método abre o salvamento mais recente dos documentos anteriormente abertos. Se um documento não foi salvo ou salva automaticamente, `ReopenPreviousDocuments` abre um documento em branco baseado no modelo para esse tipo de arquivo.  
 ...
 Se não há documentos armazenados na lista de documentos anteriormente abertos, `ReopenPreviousDocuments` não faz nada e retorna `FALSE`.  
...
### <a name="remarks"></a>Comentários  
 Esse método chama [CDataRecoveryHandler::QueryRestoreAutosavedDocuments](#queryrestoreautosaveddocuments) para determinar quais documentos que o usuário deseja restaurar. Se um usuário decidir não restaurar um documento salva automaticamente, `RestoreAutosavedDocuments` exclui o arquivo de salvamento automático. Caso contrário, `RestoreAutosavedDocuments` substitui o documento aberto com a versão salva automaticamente.  
...
### <a name="return-value"></a>Valor de retorno  
 `TRUE`Se não há documentos abertos para salvar ou se eles foram salvos com êxito. `FALSE`Se houver documentos para salvar o registro, mas não foram salvas porque ocorreu um erro.  
  
### <a name="remarks"></a>Comentários  
 As chamadas de Gerenciador de reinicialização `SaveOpenDocumentList` quando o aplicativo é encerrado inesperadamente ou quando ele sai de uma atualização. Quando o aplicativo for reiniciado, ele usa [CDataRecoveryHandler::ReadOpenDocumentList](#readopendocumentlist) para recuperar a lista de documentos abertos.  
...
|-|-|  
|Parâmetro|Descrição|  
|[in] `strAutosavePath`|O caminho onde estão armazenados os arquivos de salvamento automático.|  
...
|-|-|  
|Parâmetro|Descrição|  
|[in] `bShutdownByRestartManager`|`TRUE`para indicar que o Gerenciador de reinicialização causou o aplicativo Sair; `FALSE` para indicar que o aplicativo seja encerrado por outro motivo.|  
  
### <a name="remarks"></a>Comentários  
 A estrutura se comporta de maneira diversa dependendo se a saída anterior era esperada ou se ela foi iniciada pelo Gerenciador de reinicialização.  
...
##  <a name="a-nameupdatedocumentinfoa--cdatarecoveryhandlerupdatedocumentinfo"></a><a name="updatedocumentinfo"></a>CDataRecoveryHandler::UpdateDocumentInfo  
 Atualiza as informações de um documento porque o usuário salvou.  
...
### <a name="return-value"></a>Valor de retorno  
 `TRUE`Se esse método excluído o documento salva automaticamente e atualizadas as informações do documento; `FALSE` se ocorreu um erro.  
  
### <a name="remarks"></a>Comentários  
 Quando um usuário salva um documento, o aplicativo remove o arquivo salva automaticamente porque ele não é mais necessária. `UpdateDocumentInfo`Exclui o arquivo salva automaticamente chamando [CDataRecoveryHandler::RemoveDocumentInfo](#removedocumentinfo). `UpdateDocumentInfo`em seguida, adiciona as informações de `pDocument` à lista de atualmente documentos abertos porque `RemoveDocumentInfo` exclui essas informações, mas o salvo documento ainda está aberto.  
