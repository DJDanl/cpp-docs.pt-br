...
3.  Chame [CMFCDesktopAlertWnd::Create](#create) usando a ID do recurso de modelo de caixa de diálogo e um ponteiro para as informações de classe de tempo de execução da classe derivada.  
  
4.  A caixa de diálogo personalizada para lidar com todas as notificações provenientes os controles hospedados de programa ou programar os controles hospedados para lidar com essas notificações diretamente.  
