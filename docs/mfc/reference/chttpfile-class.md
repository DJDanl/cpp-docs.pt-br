---
title: Classe CHttpFile | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|Nome|Descrição|  
|----------|-----------------|  
|[CHttpFile::AddRequestHeaders](#addrequestheaders)|Adiciona cabeçalhos à solicitação enviada para um servidor HTTP.|  
|[CHttpFile::EndRequest](#endrequest)|Termina uma solicitação enviada para um servidor HTTP com o [SendRequestEx](#sendrequestex) função de membro.|  
...
## <a name="remarks"></a>Comentários  
 ...
 Para saber mais sobre como `CHttpFile` funciona com as outras classes do MFC Internet, consulte o artigo [Internet Programando com WinInet](../../mfc/win32-internet-extensions-wininet.md).  
...
### <a name="parameters"></a>Parâmetros  
 `pstrHeaders`  
 Um ponteiro para uma cadeia de caracteres que contém o cabeçalho ou cabeçalhos para acrescentar à solicitação. Cada cabeçalho deve ser terminado por um par de CR/LF.  
  
 `dwFlags`  
 Modifica a semântica dos cabeçalhos de novo. Pode ser uma das seguintes opções:  
  
- `HTTP_ADDREQ_FLAG_COALESCE`Mescla os cabeçalhos de mesmo nome, usando o sinalizador para adicionar o cabeçalho da primeira encontrado para o cabeçalho subsequente. Por exemplo, "aceitar: texto / *" seguido por "aceitar: áudio /\*" resulta na formação de um único cabeçalho "aceitar: texto /\*, áudio /\*". É responsabilidade do aplicativo de chamada para garantir um esquema consistente com relação aos dados recebidos pelo solicitações enviadas com cabeçalhos agrupados ou separados.  
...
- `HTTP_ADDREQ_FLAG_ADD_IF_NEW`Apenas adiciona o cabeçalho se ela ainda não existir. Se houver um, um erro será retornado.  
  
- `HTTP_ADDREQ_FLAG_ADD`Usado com substituição. Adiciona o cabeçalho se ela não existir.  
  
 `dwHeadersLen`  
 O comprimento, em caracteres, de `pstrHeaders`. Se isso for-1l, então `pstrHeaders` deve para ser terminada em zero e o comprimento é computado.  
  
 `str`  
 Uma referência a um [CString](../../atl-mfc-shared/reference/cstringt-class.md) objeto que contém o cabeçalho de solicitação ou cabeçalhos a ser adicionado.  
...
### <a name="remarks"></a>Comentários  
 `AddRequestHeaders`acrescenta cabeçalhos adicionais, formato livre para o identificador de solicitação HTTP. Ele destina para uso por clientes sofisticados que precisam de controle detalhado sobre a solicitação exata enviada para o servidor HTTP.  
...
##  <a name="chttpfile"></a>CHttpFile::CHttpFile  
 Essa função de membro é chamada para construir uma `CHttpFile` objeto.  
...
##  <a name="endrequest"></a>CHttpFile::EndRequest  
 Chame essa função de membro para encerrar uma solicitação enviada para um servidor HTTP com o [SendRequestEx](#sendrequestex) função de membro.  
...
### <a name="return-value"></a>Valor de retorno  
 A [CString](../../atl-mfc-shared/reference/cstringt-class.md) objeto que contém uma URL de referência o recurso associado a esse arquivo.  
  
### <a name="remarks"></a>Comentários  
 Use essa função de membro somente após uma chamada bem-sucedida para [SendRequest](#sendrequest) ou em um `CHttpFile` objeto criado com êxito pelo [OpenURL](../../mfc/reference/cinternetsession-class.md#openurl).  
...
### <a name="return-value"></a>Valor de retorno  
 A [CString](../../atl-mfc-shared/reference/cstringt-class.md) objeto que contém o nome do objeto.  
  
### <a name="remarks"></a>Comentários  
 Use essa função de membro somente após uma chamada bem-sucedida para [SendRequest](#sendrequest) ou em um `CHttpFile` objeto criado com êxito pelo [OpenURL](../../mfc/reference/cinternetsession-class.md#openurl).  
  
##  <a name="getverb"></a>CHttpFile::GetVerb  
 Chame essa função de membro para obter o HTTP verbo (ou método) associado a esse `CHttpFile`.  
...
### <a name="return-value"></a>Valor de retorno  
 A [CString](../../atl-mfc-shared/reference/cstringt-class.md) objeto que contém o nome do verbo HTTP (ou método).  
...
### <a name="parameters"></a>Parâmetros  
 `dwInfoLevel`  
 Uma combinação de atributo para consulta e os seguintes sinalizadores que especificam o tipo de informação solicitada:  
...
- **HTTP_QUERY_FLAG_SYSTEMTIME** para esses cabeçalhos cujo valor é uma cadeia de caracteres de data/hora, como "Last-Modified-Time," Este sinalizador retorna o valor do cabeçalho como Win32 padrão [SYSTEMTIME](http://msdn.microsoft.com/library/windows/desktop/ms724950) estrutura que não exige que o aplicativo para analisar os dados. Se você usar esse sinalizador, você talvez queira usar o `SYSTEMTIME` substituir da função.  
...
- **HTTP_QUERY_FLAG_NUMBER** para esses cabeçalhos cujo valor é um número, como o código de status, este sinalizador retorna os dados como um número de 32 bits.  
 ...
 `lpdwBufferLength`  
 Na entrada, isso aponta para um valor que contém o comprimento do buffer de dados, no número de caracteres ou bytes. Consulte o **comentários** seção para obter mais informações sobre esse parâmetro.  
  
 `lpdwIndex`  
 Um ponteiro para um índice baseado em zero do cabeçalho. Pode ser **nulo**. Use esse sinalizador para enumerar vários cabeçalhos com o mesmo nome. Na entrada, `lpdwIndex` indica o índice do cabeçalho especificado para retornar. Na saída, `lpdwIndex` indica o índice do próximo cabeçalho. Se o próximo índice não for encontrado, **ERROR_HTTP_HEADER_NOT_FOUND** é retornado.  
  
 `str`  
 Uma referência para o [CString](../../atl-mfc-shared/reference/cstringt-class.md) objeto receber as informações retornadas.  
...
- `DWORD`(para **STATUS_CODE**, **CONTENT_LENGTH**, etc.)  
 ...
 Quando uma cadeia de caracteres é gravada no buffer e a função de membro for bem-sucedida, `lpdwBufferLength` contém o comprimento da cadeia de caracteres menos 1 para encerramento do **nulo** caracteres.  
...
##  <a name="queryinfostatuscode"></a>CHttpFile::QueryInfoStatusCode  
 Chame essa função de membro para obter o código de status associado a uma solicitação HTTP e o coloca no fornecido `dwStatusCode` parâmetro.  
...
### <a name="parameters"></a>Parâmetros  
 `dwStatusCode`  
 Uma referência a um código de status. Códigos de status indicam o êxito ou falha do evento solicitado. Consulte **comentários** para uma seleção de descrições de código de status.  
...
### <a name="remarks"></a>Comentários  
 ...
 Códigos de status HTTP se enquadram em grupos indicando o êxito ou falha da solicitação. As tabelas a seguir descrevem os grupos de códigos de status e os códigos de status HTTP mais comuns.  
...
|404|Solicitado URL não encontrado|  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `lpOptional`  
 Quaisquer dados opcionais para enviar imediatamente após os cabeçalhos de solicitação. Isso geralmente é usado para **POST** e **colocar** operações. Isso pode ser **nulo** se não houver dados opcionais a serem enviados.  
  
 *dwOptionalLen*  
 O comprimento de `lpOptional`.  
  
 `strHeaders`  
 Uma cadeia de caracteres que contém o nome dos cabeçalhos de solicitação que está sendo enviada.  
...
### <a name="return-value"></a>Valor de retorno  
 Diferente de zero se for bem-sucedido. Se a chamada falhar, determine a causa da falha examinando o lançada [CInternetException](../../mfc/reference/cinternetexception-class.md) objeto.  
  
### <a name="remarks"></a>Comentários  
 Essa função permite que seu aplicativo para enviar dados usando o [gravar](../../mfc/reference/cinternetfile-class.md#write) e [WriteString](../../mfc/reference/cinternetfile-class.md#writestring) métodos de `CInternetFile`. Você deve saber o comprimento dos dados para enviar antes de chamar qualquer substituição dessa função. A primeira substituição permite que você especifique o comprimento dos dados que você deseja enviar. A segunda substituição aceita ponteiros para **INTERNET_BUFFERS** estruturas que podem ser usadas para descrever o buffer detalhadamente.  
...
### <a name="example"></a>Exemplo  
 Este fragmento de código envia o conteúdo de uma cadeia de caracteres para uma DLL chamada MFCISAPI. DLL do servidor de LOCALHOST. Embora este exemplo usa apenas uma chamada para `WriteString`, usando várias chamadas para enviar dados em blocos é aceitável.  
