...
|[COleControl::D oClick](#doclick)|Implementação do método de `DoClick` de estoque.|
|[COleControl::D oPropExchange](#dopropexchange)|Serializa as propriedades de um objeto `COleControl`.|
...
|[COleControl::FireClick](#fireclick)|Dispara o evento de `Click` de estoque.|
|[COleControl::FireDblClick](#firedblclick)|Dispara o evento de `DblClick` de estoque.|
|[COleControl::FireError](#fireerror)|Dispara o evento de `Error` de estoque.|
|[COleControl:: FireEvent](#fireevent)|Dispara um evento personalizado.|
|[COleControl::FireKeyDown](#firekeydown)|Dispara o evento de `KeyDown` de estoque.|
|[COleControl::FireKeyPress](#firekeypress)|Dispara o evento de `KeyPress` de estoque.|
|[COleControl::FireKeyUp](#firekeyup)|Dispara o evento de `KeyUp` de estoque.|
|[COleControl::FireMouseDown](#firemousedown)|Dispara o evento de `MouseDown` de estoque.|
|[COleControl::FireMouseMove](#firemousemove)|Dispara o evento de `MouseMove` de estoque.|
|[COleControl::FireMouseUp](#firemouseup)|Dispara o evento de `MouseUp` de estoque.|
|[COleControl::FireReadyStateChange](#firereadystatechange)|Dispara um evento quando o estado pronto do controle é alterado.|
|[COleControl::GetActivationPolicy](#getactivationpolicy)|Altera o comportamento de ativação padrão de um controle que dá suporte à interface `IPointerInactive`.|
...
|[COleControl::GetFontTextMetrics](#getfonttextmetrics)|Retorna as métricas de um objeto `CFontHolder`.|
...
|[COleControl::InternalGetFont](#internalgetfont)|Retorna um objeto `CFontHolder` para a propriedade Font de stock.|
...
|[COleControl:: fechamento](#onclose)|Notifica o controle que `IOleControl::Close` foi chamado.|
...
|[COleControl::ResetStockProps](#resetstockprops)|Inicializa `COleControl` Propriedades de estoque para seus valores padrão.|
...
|[COleControl::SerializeStockProps](#serializestockprops)|Serializa ou inicializa as propriedades `COleControl` stock.|
...
|[COleControl:: WindowProc](#windowproc)|Fornece um procedimento do Windows para um objeto `COleControl`.|
...
Quando a ativação sem janela é usada, o contêiner (que tem uma janela) é responsável por fornecer serviços que de outra forma teriam sido fornecidos pela janela do próprio controle. Por exemplo, se seu controle precisar consultar o foco do teclado, consultar a captura do mouse ou obter um contexto do dispositivo, essas operações serão gerenciadas pelo contêiner. As funções de membro de operação sem janela do `COleControl` invocam essas operações no contêiner.

Quando a ativação sem janela está habilitada, o contêiner Delega mensagens de entrada para a interface de `IOleInPlaceObjectWindowless` do controle (uma extensão de [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject) para suporte sem janelas). a implementação de `COleControl`dessa interface enviará essas mensagens por meio do mapa de mensagens do controle, depois de ajustar as coordenadas do mouse adequadamente. Você pode processar essas mensagens como mensagens de janela comuns, adicionando as entradas correspondentes ao mapa de mensagens.

Em um controle sem janela, você sempre deve usar as funções de membro `COleControl` em vez das funções de membro `CWnd` correspondentes ou suas funções de API do Windows relacionadas.
...
Solicita permissão da interface `IPropertyNotifySink` para alterar um valor de propriedade associada fornecido pelo controle.
...
A função de membro substituível `COleControl::OnClick` será chamada e um evento de clique de ação será disparado, se houver suporte do controle.

Essa função é suportada pela classe base `COleControl` como um método de estoque, chamado DoClick. Para obter mais informações, consulte o artigo [controles ActiveX: métodos](../../mfc/mfc-activex-controls-methods.md).
...
Chame essa função para lidar corretamente com a pintura de um controle OLE não ativo. Essa função só deve ser usada se o controle OLE subclasses de um controle do Windows e deve ser chamado na função `OnDraw` do seu controle.
...
> [!NOTE]
>  Constantes Variant adicionais foram definidas para todos os tipos de variante, com exceção de VTS_FONT e VTS_PICTURE, que fornecem um ponteiro para a constante de dados Variant. Essas constantes são nomeadas usando a Convenção de`constantname` **VTS_P** . Por exemplo, VTS_PCOLOR é um ponteiro para uma constante VTS_COLOR.
...
Quando a interface de `IPointerInactive` está habilitada, o contêiner delegará WM_SETCURSOR e WM_MOUSEMOVE mensagens a ele. a implementação de `COleControl`dessa interface enviará essas mensagens por meio do mapa de mensagens do controle, depois de ajustar as coordenadas do mouse adequadamente.

Sempre que o contêiner recebe uma mensagem de WM_SETCURSOR ou WM_MOUSEMOVE com o ponteiro do mouse sobre um objeto inativo que oferece suporte a `IPointerInactive`, ele deve chamar `GetActivationPolicy` na interface e retornar os sinalizadores da enumeração POINTERINACTIVE.

Você pode processar essas mensagens assim como mensagens de janela comuns, adicionando as entradas correspondentes ao mapa de mensagens. Em seus manipuladores, evite usar a variável de membro `m_hWnd` (ou qualquer função de membro que o use) sem primeiro verificar se seu valor é não nulo.
...
A habilitação da interface de `IPointerInactive` normalmente significa que você deseja que o controle seja capaz de processar mensagens do mouse em todos os momentos. Para obter esse comportamento em um contêiner que não dá suporte à interface `IPointerInactive`, você precisará ter o controle sempre ativado quando visível, o que significa que o controle deve ter o sinalizador OLEMISC_ACTIVATEWHENVISIBLE entre seus diversos sinalizadores. No entanto, para evitar que esse sinalizador entre em vigor em um contêiner que dá suporte a `IPointerInactive`, você também pode especificar o sinalizador de OLEMISC_IGNOREACTIVATEWHENVISIBLE:

[!code-cpp[NVC_MFCAxCtl#10](../../mfc/reference/codesnippet/cpp/colecontrol-class_2.cpp)]

##  <a name="getambientproperty"></a>COleControl:: getambienteproperty
...
Se você usar `GetAmbientProperty` para recuperar as propriedades de ScaleUnits e DisplayName de ambiente, defina *vtProp* como VT_BSTR e *pvProp* como `CString*`. Se você estiver recuperando a propriedade de fonte de ambiente, defina *vtProp* como VT_FONT e *pvProp* como LPFONTDISP *.
...
##  <a name="getappearance"></a>COleControl:: getaparentance
...
Determina se o objeto de `COleControl` tem a captura do mouse.
...
Caso contrário, retorna o objeto `CWnd` que tem a captura do mouse (o mesmo que `CWnd::GetCapture`).
...
*lpRect*<br/>
Ponteiro para uma estrutura de `RECT` que contém as dimensões da área do cliente do controle sem janela; ou seja, o tamanho do controle menos bordas de janela, quadros, barras de rolagem e assim por diante. O parâmetro *lpRect* indica o tamanho do retângulo do cliente do controle, não sua posição.
...
O ponteiro retornado aponta para uma instância de `IOleClientSite`. A interface `IOleClientSite`, implementada por contêineres, é a exibição do objeto de seu contexto: onde está ancorada no documento, onde ele obtém seu armazenamento, a interface do usuário e outros recursos.
...
|`clipPaintDC`|Se não estiver definido, desabilita a chamada para `IntersectClipRect` feita por `COleControl` e ganha uma vantagem de velocidade pequena. Se você estiver usando a ativação sem janela, o sinalizador não terá nenhum efeito.|
|`pointerInactive`|Se definido, fornece a interação com o mouse enquanto o controle está inativo habilitando a implementação de `COleControl`da interface `IPointerInactive`, que está desabilitada por padrão.|
...
##  <a name="getcontrolsize"></a>COleControl:: getcontrolaize
...
Ponteiro para o contexto do dispositivo de vídeo para o contêiner `CWnd` área do cliente se for bem-sucedido; caso contrário, o valor de retorno será NULL. O contexto do dispositivo de vídeo pode ser usado em funções GDI subsequentes para desenhar na área do cliente da janela do contêiner.

### <a name="remarks"></a>Comentários

A função de membro [ReleaseDC](#releasedc) deve ser chamada para liberar o contexto após a pintura. Ao chamar `GetDC`, os objetos passam o retângulo que desejam desenhar em suas próprias coordenadas de cliente. o `GetDC` converte esses em coordenadas da área do cliente do contêiner. O objeto não deve solicitar um retângulo de desenho desejado maior do que seu próprio retângulo de área de cliente, o tamanho que pode ser recuperado com [GetClientRect](#getclientrect). Isso impede que os objetos desenham inadvertidamente onde não deveriam.

##  <a name="getenabled"></a>COleControl:: getabilited
...
Determina se o objeto de `COleControl` tem o foco.
...
Caso contrário, retorna o objeto `CWnd` que tem o foco (o mesmo que `CWnd::GetFocus`).
...
Essa fonte pode ser selecionada com a função [COleControl:: SelectFontObject](#selectfontobject) . `GetFontTextMetrics` inicializará a estrutura de `TEXTMETRIC` apontada por *lptm* com informações de métricas válidas sobre a fonte do `fontHolder`se tiver êxito ou preencher a estrutura com zeros, se não for bem-sucedida. Você deve usar essa função em vez de [GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ao pintar seu controle porque controles, como qualquer objeto OLE inserido, podem ser necessários para serem renderizados em um metarquivo.

A estrutura de `TEXTMETRIC` para a fonte padrão é atualizada quando a função [SelectFontObject](#selectfontobject) é chamada. Você deve chamar `GetFontTextMetrics` somente depois de selecionar a propriedade Font de stock para garantir que as informações fornecidas por ele sejam válidas.
...
##  <a name="getreadystate"></a>COleControl:: getpreparestate
...
A função `GetStockTextMetrics` inicializará a estrutura de `TEXTMETRIC` apontada por *lptm* com informações de métricas válidas se for bem-sucedida ou preencherá a estrutura com zeros, se não for bem-sucedida. Use essa função em vez de [GetTextMetrics](/windows/win32/api/wingdi/nf-wingdi-gettextmetrics) ao pintar seu controle porque controles, como qualquer objeto OLE incorporado, podem ser necessários para serem renderizados em um metarquivo.

A estrutura de `TEXTMETRIC` para a fonte padrão é atualizada quando a função `SelectStockFont` é chamada. Você deve chamar essa função somente depois de selecionar a fonte stock para garantir que as informações fornecidas por ele sejam válidas.
...
Ponteiro para a interface de `IDropTarget` do objeto. Como ele não tem uma janela, um objeto sem janela não pode registrar uma interface `IDropTarget`. No entanto, para participar de arrastar e soltar, um objeto sem janela ainda pode implementar a interface e retorná-la em `GetWindowlessDropTarget`.

### <a name="remarks"></a>Comentários

Normalmente, isso exigiria que a janela do controle fosse registrada como um destino de soltura. Mas como o controle não tem nenhuma janela própria, o contêiner usará sua própria janela como um destino de soltar. O controle simplesmente precisa fornecer uma implementação da interface `IDropTarget` à qual o contêiner pode delegar chamadas no momento apropriado. Por exemplo:
...
Isso pode ser usado para redesenhar controles sem janelas dentro do contêiner. A região invalidada, juntamente com todas as outras áreas na região de atualização, é marcada para pintura quando a próxima mensagem de [WM_PAINT](/windows/win32/gdi/wm-paint) é enviada.
...
Ao converter um formulário que usa controles do VBX para um que usa controles OLE, o código de carregamento especial para os controles OLE pode ser necessário. Por exemplo, se você estiver carregando uma instância do seu controle OLE, poderá ter uma chamada para [PX_Font](persistence-of-ole-controls.md#px_font) no seu `DoPropExchange`:
...
Outro caso seria se o controle VBX salvasse dados binários proprietários (em seu manipulador de VBM_SAVEPROPERTY mensagem) e o controle OLE salva seus dados binários em um formato diferente. Se desejar que seu controle OLE seja compatível com versões anteriores com o controle VBX, você poderá ler os formatos antigo e novo usando a função `IsConvertingVBX`, distinguindo se o controle VBX ou o controle OLE estava sendo carregado.

Na função de `DoPropExchange` do seu controle, você pode verificar essa condição e, se for verdadeiro, executar código de carga específico para essa conversão (como os exemplos anteriores). Se o controle não estiver sendo convertido, você poderá executar o código de carga normal. Essa capacidade só é aplicável a controles sendo convertidos de contrapartes do VBX.
...
Substitua essa função se desejar uma notificação depois que essa propriedade for alterada. A implementação padrão chama `InvalidateControl`.
...
##  <a name="onborderstylechanged"></a>COleControl:: onestilodabordachanged
...
##  <a name="onclose"></a>COleControl:: fechamento

Chamado pelo Framework quando o contêiner chamou a função de `IOleControl::Close` do controle.
...
Por padrão, `OnClose` salva o objeto de controle se ele foi modificado e o *dwSaveOption* é OLECLOSE_SAVEIFDIRTY ou OLECLOSE_PROMPTSAVE.

##  <a name="ondoverb"></a>COleControl::OnDoVerb

Chamado pelo Framework quando o contêiner chama a função membro `IOleObject::DoVerb`.
...
A implementação padrão usa o ON_OLEVERB e ON_STDOLEVERB entradas de mapa de mensagens para determinar a função apropriada a ser invocada.
...
`OnDraw` normalmente é chamado para exibição de tela, passando um contexto de dispositivo de tela como *PDC*. O parâmetro *rcBounds* identifica o retângulo no contexto do dispositivo de destino (em relação ao seu modo de mapeamento atual). O parâmetro *rcInvalid* é o retângulo real inválido. Em alguns casos, isso será uma área menor do que *rcBounds*.
...
Isso tem o mesmo efeito que invocar o verbo de OLEIVERB_UIACTIVATE do controle.

Essa função normalmente é usada como a função de manipulador para uma entrada de mapa de mensagem ON_OLEVERB. Isso torna um verbo de "edição" disponível no menu "objeto" do controle. Por exemplo:
...
Chamado pelo Framework quando o contêiner chama a função membro `IOleObject::EnumVerbs`.
...
A implementação padrão enumera as entradas de ON_OLEVERB no mapa de mensagens.
...
A implementação padrão chama `COleControl::InvalidateControl`. Se o controle estiver subclassendo um controle do Windows, a implementação padrão também enviará uma mensagem de WM_SETFONT para a janela do controle.
...
Chamado pelo Framework depois que o contêiner chama `IOleControl::FreezeEvents`.
...
Chamado pelo Framework quando o contêiner chama a função membro `IViewObject::GetColorSet`.
...
Chamado pelo Framework em resposta à solicitação de `IViewObjectEx::GetNaturalExtent` de um contêiner.
...
*pExtentInfo*<br/>
Aponta para a estrutura de `DVEXTENTINFO` que especifica os dados de dimensionamento. A estrutura de `DVEXTENTINFO` é:
...
O membro da estrutura `dwExtentMode` pode ter um destes dois valores:
...
Substitua essa função para retornar o tamanho de exibição do objeto mais próximo do tamanho e do modo de extensão propostos na estrutura de `DVEXTENTINFO`. A implementação padrão retorna FALSE e não faz ajustes no tamanho.
...
Substitua essa função se o seu controle tiver uma propriedade com um conjunto de possíveis valores que podem ser representados por cadeias de caracteres. Para cada elemento adicionado ao *pStringArray*, você deve adicionar um elemento "cookie" correspondente a *pCookieArray.* Esses valores de "cookie" podem ser passados posteriormente pela estrutura para a função `COleControl::OnGetPredefinedValue`.

##  <a name="ongetpredefinedvalue"></a>COleControl::OnGetPredefinedValue

Chamado pelo Framework para obter o valor correspondente a uma das cadeias de caracteres predefinidas retornadas anteriormente por uma substituição de `COleControl::OnGetPredefinedStrings`.
...
*lpvarOut*<br/>
Ponteiro para uma estrutura de `VARIANT` por meio da qual um valor de propriedade será retornado.
...
Chamado pelo Framework em resposta à solicitação de `IViewObjectEx::GetRect` de um contêiner.
...
- DVASPECT_CONTENT retângulo delimitador do objeto inteiro. O canto superior esquerdo na origem e no tamanho do objeto é igual à extensão retornada pelo `GetViewExtent` *.*
...
O tamanho do objeto é convertido por `OnGetViewRect` em um retângulo a partir de uma posição específica (o padrão é o canto superior esquerdo da exibição). Substitua essa função se o seu controle usar desenho de duas passagens, e suas partes opacas e transparentes tiverem dimensões diferentes.

##  <a name="ongetviewstatus"></a>COleControl::OnGetViewStatus

Chamado pelo Framework em resposta à solicitação de `IViewObjectEx::GetViewStatus` de um contêiner.
...
|VIEWSTATUS_SOLIDBKGND|O objeto tem um plano de fundo sólido (que consiste em uma cor sólida, não um padrão de pincel). Esse bit só é significativo se VIEWSTATUS_OPAQUE estiver definido e se aplicar somente a aspectos relacionados a conteúdo e não a DVASPECT_ICON ou DVASPECT_DOCPRINT.|
...
|VIEWSTATUS_DVASPECTTRANSPARENT|O objeto dá suporte a DVASPECT_TRANSPARENT. Todos os métodos de `IViewObjectEx` que usam um aspecto de desenho como um parâmetro podem ser chamados com esse aspecto.|
...
Chamado pelo contêiner do objeto inativo sob o ponteiro do mouse sobre o recebimento de uma mensagem de WM_MOUSEMOVE.
...
Chamado pelo contêiner do objeto inativo sob o ponteiro do mouse sobre o recebimento de uma mensagem de WM_SETCURSOR.
...
*lpclsid*<br/>
Ponteiro para uma estrutura de `CLSID` por meio da qual uma ID de classe será retornada.
...
##  <a name="onmnemonic"></a>COleControl:: onmnemônico
...
Você também pode usar essa função para fazer com que a exibição das páginas de propriedades do seu controle. Faça uma chamada para a função `OnProperties`, passando o identificador do pai do controle no parâmetro *hwndParent* . Nesse caso, os valores dos parâmetros *lpMsg* e *lpRect* são ignorados.

##  <a name="onqueryhitpoint"></a>COleControl::OnQueryHitPoint

Chamado pelo Framework em resposta à solicitação de `IViewObjectEx::QueryHitPoint` de um contêiner.
...
*pRectBounds*<br/>
Ponteiro para uma estrutura de `RECT` especificando o retângulo delimitador da área do cliente do controle OLE.

*ptlLoc*<br/>
Ponteiro para a estrutura de `POINT` especificando o ponto a ser verificado em busca de uma visita. O ponto é especificado em coordenadas da área do cliente OLE.
...
- HITRESULT_CLOSE *ptlLoc* está dentro ou fora do objeto OLE, mas é quase o suficiente para o objeto a ser considerado dentro. Objetos pequenos, finos ou detalhados podem usar esse valor. Mesmo que um ponto esteja fora do retângulo delimitador de um objeto, ele ainda pode ser fechado (isso é necessário para pressionar objetos pequenos).
...
Chamado pelo Framework em resposta à solicitação de `IViewObjectEx::QueryHitRect` de um contêiner.
...
*prcLoc*<br/>
Ponteiro para a estrutura de `RECT` que especifica o retângulo a ser verificado em busca de um pressionamento (sobreposição com o retângulo do objeto), em relação ao canto superior esquerdo do objeto.
...
Para obter mais informações, consulte a estrutura de `FORMATETC` no SDK do Windows.
...
Por padrão, `OnSetClientSite` verifica se as propriedades do caminho de dados estão carregadas e, se estiverem, chamadas `DoDataPathPropExchange`.
...
Se os dados estiverem no formato de conjunto de propriedades persistentes, a implementação padrão modificará o estado do controle de acordo. Caso contrário, a implementação padrão não fará nada. Se *bRelease* for true, será feita uma chamada para `ReleaseStgMedium`; caso contrário, não.
...
*lpSizeL*<br/>
Um ponteiro para a estrutura de `SIZEL` que usa inteiros longos para representar a largura e a altura do controle, expresso em unidades HIMETRIC.
...
A implementação padrão manipula o redimensionamento da extensão do controle. Se o controle estiver ativo no local, será feita uma chamada para o `OnPosRectChanged` do contêiner.
...
*lpRectPos*<br/>
Um ponteiro para uma estrutura de `RECT` que indica a nova posição e o tamanho do controle em relação ao contêiner.

*lpRectClip*<br/>
Um ponteiro para uma estrutura de `RECT` que indica uma área retangular na qual o controle deve ser recortado.
...
Chamado pelo Framework em resposta à solicitação de `IOleInPlaceObjectWindowless::OnWindowMessage` de um contêiner.
...
Processa mensagens da janela para controles sem janela. os `OnWindowlessMessage` de `COleControl`devem ser usados para mensagens de janela que não sejam mensagens de mouse e de teclado. `COleControl` fornece [SetCapture](#setcapture) e [SetFocus](#setfocus) especificamente para obter a captura do mouse e o foco do teclado para objetos OLE sem janela.

Como os objetos sem janela não têm uma janela, eles precisam de um mecanismo para permitir que o contêiner envie mensagens para eles. Um objeto OLE sem janela obtém mensagens de seu contêiner, por meio do método `OnWindowMessage` na interface `IOleInPlaceObjectWindowless` (uma extensão de [IOleInPlaceObject](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceobject) para suporte sem janelas). `OnWindowMessage` não pega um parâmetro `HWND`.
...
- HTERROR no plano de fundo da tela ou em uma linha divisória entre janelas (o mesmo que HTNOWHERE, exceto que a função `DefWndProc` Windows produz um aviso sonoro do sistema para indicar um erro).
...
Chame essa função depois de exibir qualquer caixa de diálogo modal. Você deve chamar essa função para que o contêiner possa habilitar qualquer janela de nível superior desabilitada pelo `PreModalDialog`. Essa função deve ser emparelhada com uma chamada para `PreModalDialog`.
...
##  <a name="refresh"></a>COleControl:: atualizar
...
Essa função é suportada pela classe base `COleControl` como um método de estoque, chamado atualização. Isso permite que os usuários do seu controle OLE repinte o controle em um horário específico. Para obter mais informações sobre esse método, consulte o artigo [controles ActiveX: métodos](../../mfc/mfc-activex-controls-methods.md).
...
Inicializa o estado do `COleControl` Propriedades de ações para seus valores padrão.
...
As propriedades são: aparência, BackColor, BorderStyle, legenda, habilitado, fonte, ForeColor, hWnd e texto. Para obter uma descrição das propriedades de estoque, consulte [controles ActiveX: Adicionando propriedades de estoque](../../mfc/mfc-activex-controls-adding-stock-properties.md).

Você pode melhorar o desempenho de inicialização binária do controle usando `ResetStockProps` e `ResetVersion` para substituir `COleControl::OnResetState`. Veja o exemplo abaixo. Para obter mais informações sobre como otimizar a inicialização, consulte [controles ActiveX: otimização](../../mfc/mfc-activex-controls-optimization.md).
...
*lpClipRect*<br/>
Aponta para um objeto `CRect` ou `RECT` estrutura que especifica o retângulo a ser recortado. Somente os pixels dentro do retângulo são rolados. Os bits fora do retângulo não são afetados mesmo se estiverem no retângulo *lpRect* . Se *lpClipRect* for nulo, nenhum recorte será executado no retângulo de rolagem.
...
Um ponteiro para o objeto de `CFont` selecionado anteriormente. Você deve usar [CDC:: SelectObject](../../mfc/reference/cdc-class.md#selectobject) para selecionar essa fonte de volta ao contexto do dispositivo quando terminar.
...
Você pode melhorar o desempenho de persistência binária de um controle usando `SerializeExtent`, `SerializeStockProps`e `SerializeVersion` para substituir `COleControl::Serialize`. Veja o exemplo abaixo. Para obter mais informações sobre como otimizar a inicialização, consulte [controles ActiveX: otimização](../../mfc/mfc-activex-controls-optimization.md).
...
Serializa ou inicializa o estado das `COleControl` Propriedades de estoque: aparência, BackColor, BorderStyle, Caption, habilitado, fonte, ForeColor e texto.
...
O número de versão do controle. Se o arquivo especificado estiver sendo carregado, `SerializeVersion` retornará a versão carregada a partir desse arquivo. Caso contrário, ele retornará a versão atualmente carregada.
...
##  <a name="setappearance"></a>COleControl:: setaparecimento
...
Para obter mais informações sobre as propriedades de ações, consulte [controles ActiveX: Propriedades](../../mfc/mfc-activex-controls-properties.md).
...
Um ponteiro para o objeto de janela `CWnd` que recebeu a entrada do mouse anteriormente.

### <a name="remarks"></a>Comentários

Se o controle estiver ativado e sem janela, essa função fará com que a janela do contêiner do controle assuma a posse da captura do mouse, em nome do controle. Caso contrário, essa função faz com que o próprio controle assuma a posse da captura do mouse (o mesmo que `CWnd::SetCapture`).

##  <a name="setcontrolsize"></a>COleControl:: setcontrolaize
...
##  <a name="setenabled"></a>COleControl:: sethabilitado
...
Um ponteiro para o objeto de janela `CWnd` que anteriormente tinha o foco de entrada, ou NULL se não houver tal janela.

### <a name="remarks"></a>Comentários

Se o controle estiver ativado e sem janela, essa função fará com que a janela do contêiner do controle assuma a posse do foco de entrada, em nome do controle. O foco de entrada direciona a entrada do teclado para a janela do contêiner e o Contêiner distribui todas as mensagens de teclado subsequentes para o objeto OLE que chama `SetFocus`. Qualquer janela que anteriormente tinha o foco de entrada a perde.

Se o controle não for sem janela, essa função fará com que o próprio controle assuma a posse do foco de entrada (o mesmo que `CWnd::SetFocus`).
...
Se o controle estiver aberto, ele será redimensionado; caso contrário, a função de `OnPosRectChanged` do contêiner será chamada.
...
*lpptlHimetric*<br/>
Ponteiro para uma estrutura de `POINTL` que contém coordenadas em unidades HIMETRIC.

*lpptfContainer*<br/>
Ponteiro para uma estrutura de `POINTF` que contém coordenadas no tamanho da unidade do contêiner.
...
- XFORMCOORDS_HIMETRICTOCONTAINER transformar unidades HIMETRIC em unidades do contêiner.
