...
## <a name="remarks"></a>Comentários  
 ...
 `CMultiLock` os objetos são mais úteis quando um thread tem um grande número de `CEvent` objetos que ele pode responder. Criar uma matriz que contém todos os `CEvent` ponteiros e chamada `Lock`. Isso fará com que o thread de espera até que um dos eventos é sinalizado.  
