...
### <a name="radio-button-and-check-box-styles"></a>Estilos de caixa de seleção e o botão de opção  
...
|Estilo|Descrição|  
|-----------|-----------------|  
|`CBS_AUTOHSCROLL`|Rola o texto no controle de edição para a direita automaticamente quando o usuário digita um caractere no final da linha. Se esse estilo não for definido, é permitido somente texto que cabe dentro do limite retangular.|  
...
|`CBS_DROPDOWN`|Semelhante ao `CBS_SIMPLE`, exceto que a caixa de lista não é exibida, a menos que o usuário seleciona um ícone ao lado de controle de edição.|  
...
|`CBS_HASSTRINGS`|Uma caixa de combinação do desenho proprietário contém itens compostos por cadeias de. A caixa de combinação mantém a memória e os ponteiros para as cadeias de caracteres para o aplicativo pode usar o `GetText` a função de membro para recuperar o texto para um determinado item.|  
|`CBS_LOWERCASE`|Converte em minúsculas todo o texto no campo de seleção e a lista.|  
|`CBS_NOINTEGRALHEIGHT`|Especifica o tamanho da caixa de combinação será exatamente o tamanho especificado pelo aplicativo quando ele criado na caixa de combinação. Normalmente, o Windows dimensiona uma caixa de combinação para que a caixa de combinação não exibe itens parciais.|  
...
|Estilo|Descrição|  
|-----------|-----------------|  
|`ES_AUTOHSCROLL`|Rola automaticamente o texto à direita, 10 caracteres quando o usuário digita um caractere no final da linha. Quando o usuário pressiona a tecla ENTER, o controle rola todo o texto para a posição 0.|  
|`ES_AUTOVSCROLL`|Rola automaticamente o texto de uma página para cima quando o usuário pressiona ENTER na última linha.|  
|`ES_CENTER`|Centraliza o texto em um controle de edição de linha única ou multilinha.|  
|`ES_LEFT`|Alinha à esquerda o texto em um controle de edição de linha única ou multilinha.|  
|`ES_LOWERCASE`|Converte todos os caracteres para letras minúsculas quando eles são digitados no controle de edição.|  
|`ES_MULTILINE`|Designa um controle de edição de várias linhas. (O padrão é a única linha.) Se o `ES_AUTOVSCROLL` estilo for especificado, o controle de edição mostra tantas linhas quanto possível e rola verticalmente quando o usuário pressiona a tecla ENTER. Se `ES_AUTOVSCROLL` não é fornecido, o controle de edição mostra tantas linhas quanto possível e avisos sonoros se ENTER é pressionada quando não há mais linhas podem ser exibidas. Se o `ES_AUTOHSCROLL` estilo for especificado, o controle de edição de várias linhas automaticamente rola horizontalmente quando o cursor vai além da borda direita do controle. Para iniciar uma nova linha, o usuário deve pressionar ENTER. Se `ES_AUTOHSCROLL` não é fornecido, o controle automaticamente quebra de palavras para o início da próxima linha quando necessário; uma nova linha é iniciada se ENTER for pressionada. A posição do wordwrap é determinada pelo tamanho da janela. Se o tamanho da janela for alterado, as alterações na posição wordwrap e o texto é exibida novamente. Controles de edição de várias linhas podem ter barras de rolagem. Um controle de edição com barras de rolagem processa mensagens sua própria barra de rolagem. Editar controles sem rolagem de barras de rolagem conforme descrito acima e processar as mensagens de rolagem enviadas pela janela pai.|  
...
|`LBS_NOREDRAW`|Exibição de caixa de listagem não é atualizada quando as alterações são feitas. Esse estilo pode ser alterado a qualquer momento, enviando um `WM_SETREDRAW` mensagem.|  
...
|`LBS_OWNERDRAWFIXED`|O proprietário da caixa de listagem é responsável por desenhar seu conteúdo. os itens na caixa de lista são da mesma altura.|  
...
|Estilo|Descrição|  
|-----------|-----------------|  
|`MB_ABORTRETRYIGNORE`|A caixa de mensagem contém três pushbuttons: anular, repetir e ignorar.|  
...
|Estilo|Descrição|  
|-----------|-----------------|  
|`MB_APPLMODAL`|O usuário deve responder à caixa de mensagem antes de continuar o trabalho na janela atual. No entanto, o usuário pode mover para o de outros aplicativos do windows e funcionam nessas janelas. O padrão é `MB_APPLMODAL` se nem `MB_SYSTEMMODAL` nem `MB_TASKMODAL` for especificado.|  
|`MB_SYSTEMMODAL`|Todos os aplicativos são suspensos até que o usuário responda à caixa de mensagem. Caixas de mensagem modal do sistema são usadas para notificar o usuário de sério, potencialmente prejudiciais erros que exigem atenção imediata e devem ser usados com moderação.|  
|`MB_TASKMODAL`|Semelhante ao `MB_APPLMODAL`, mas não são úteis dentro de um aplicativo de classe do Microsoft Foundation. Esse sinalizador é reservado para um aplicativo de chamada ou uma biblioteca que não tem um identificador de janela disponível.|  
...
|`MB_ICONINFORMATION`|Um ícone que consiste em um "I" em um círculo é exibido na caixa de mensagem.|  
|`MB_ICONQUESTION`|Um ícone de ponto de interrogação aparece na caixa de mensagem.|  
|`MB_ICONSTOP`|Um ícone de sinal de parada é exibida na caixa de mensagem.|  
### <a name="message-box-default-buttons"></a>Botões de padrão da caixa de mensagem  
...
|Estilo|Descrição|  
|-----------|-----------------|  
|`SBS_BOTTOMALIGN`|Usado com o `SBS_HORZ` estilo. A parte inferior da barra de rolagem está alinhada com a borda inferior do retângulo especificado no `Create` função de membro. A barra de rolagem tem a altura padrão das barras de rolagem do sistema.|  
|`SBS_HORZ`|Designa uma barra de rolagem horizontal. Se nem o `SBS_BOTTOMALIGN` nem `SBS_TOPALIGN` estilo for especificado, a barra de rolagem tem a altura, largura e posição fornecida no `Create` função de membro.|  
|`SBS_LEFTALIGN`|Usado com o `SBS_VERT` estilo. A borda esquerda da barra de rolagem está alinhada com a borda esquerda do retângulo especificado no `Create` função de membro. A barra de rolagem tem a largura padrão de barras de rolagem do sistema.|  
|`SBS_RIGHTALIGN`|Usado com o `SBS_VERT` estilo. A borda direita da barra de rolagem está alinhada com a borda direita do retângulo especificado no `Create` função de membro. A barra de rolagem tem a largura padrão de barras de rolagem do sistema.|  
|`SBS_SIZEBOX`|Designa uma caixa de tamanho. Se nem o `SBS_SIZEBOXBOTTOMRIGHTALIGN` nem `SBS_SIZEBOXTOPLEFTALIGN` estilo for especificado, a caixa de tamanho tem a altura, largura e posição fornecida no `Create` função de membro.|  
|`SBS_SIZEBOXBOTTOMRIGHTALIGN`|Usado com o `SBS_SIZEBOX` estilo. O canto inferior direito da caixa de tamanho é alinhado com o canto inferior direito do retângulo especificado no `Create` função de membro. A caixa de tamanho tem o tamanho padrão para caixas de tamanho do sistema.|  
|`SBS_SIZEBOXTOPLEFTALIGN`|Usado com o `SBS_SIZEBOX` estilo. O canto superior esquerdo da caixa de tamanho é alinhado com o canto superior esquerdo do retângulo especificado no `Create` função de membro. A caixa de tamanho tem o tamanho padrão para caixas de tamanho do sistema.|  
...
|`SS_BLACKFRAME`|Especifica uma caixa com um quadro desenhado com a mesma cor quadros de janela. O padrão é preto.|  
|`SS_BLACKRECT`|Especifica um retângulo preenchido com a cor usada para desenhar os quadros de janela. O padrão é preto.|  
|`SS_CENTER`|Designa um retângulo simple e exibe o texto determinado centralizado no retângulo. O texto é formatado antes que ele seja exibido. Palavras que seriam estenderá após o término de uma linha são quebradas automaticamente para o início da próxima linha centralizado.|  
|`SS_CENTERIMAGE`|Especifica que, se o ícone ou bitmap for menor que a área cliente do controle estático, o restante da área do cliente é preenchido com a cor do pixel no canto superior esquerdo do bitmap ou ícone. Se o controle estático contém uma única linha de texto, o texto está centralizado verticalmente na área cliente do controle.|  
|`SS_ENDELLIPSIS`|ou `SS_PATHELLIPSIS` substitui parte de cadeia de caracteres especificada com reticências, se necessário, para que o resultado caiba no retângulo especificado. <br /><br />Você pode especificar `SS_END_ELLIPSIS` para substituir caracteres do final da cadeia de caracteres, ou `SS_PATHELLIPSIS` para substituir caracteres no meio da cadeia. Se a cadeia de caracteres contém a barra invertida (\\) caracteres, `SS_PATHELLIPSIS` preserva quanto o texto após a última barra invertida possível.|  
|`SS_ENHMETAFILE`|Especifica um metarquivo avançado deve ser exibida no controle estático. O texto especificado é o nome de um metarquivo. Um controle estático de metarquivo avançado tem um tamanho fixo; o metarquivo é dimensionado para ajustar a área do cliente do controle estático.|  
...
|`SS_GRAYFRAME`|Especifica uma caixa com um quadro desenhado com a mesma cor do plano de fundo de tela (desktop). O padrão é cinza.|  
...
|`SS_LEFT`|Designa um retângulo simple e exibe o texto determinado liberação esquerdo do retângulo. O texto é formatado antes que ele seja exibido. Palavras que seriam estenderá após o término de uma linha são quebradas automaticamente para o início da próxima linha esquerda de liberação.|  
|`SS_LEFTNOWORDWRAP`|Designa um retângulo simple e exibe o texto determinado liberação esquerdo do retângulo. Guias são expandidas, mas palavras não são encapsuladas. Texto que ultrapassa o final de uma linha é recortado.|  
|`SS_NOPREFIX`|A menos que esse estilo for especificado, o Windows interpretará quaisquer caracteres no texto do controle accelerator caracteres de prefixo e comercial (&). Nesse caso, o e comercial é removido e o próximo caractere na cadeia de caracteres é sublinhado. Se for um controle estático conter o texto em que esse recurso não se for desejado, `SS_NOPREFIX` podem ser adicionados. Esse estilo de controle estático pode ser incluído com nenhum dos controles estáticos definidos. Você pode combinar `SS_NOPREFIX` com outros estilos usando o operador OR bit a bit. Isso geralmente é usado quando nomes de arquivos ou outras cadeias de caracteres que podem conter um e comercial precisam ser exibido em um controle estático em uma caixa de diálogo.|  
|`SS_NOTIFY`|Envia a janela pai `STN_CLICKED`, `STN_DBLCLK`, `STN_DISABLE`, e `STN_ENABLE` mensagens de notificação quando o usuário clica ou clicar duas vezes no controle.|  
...
|`SS_REALSIZEIMAGE`|Impede que um controle estático ícone ou bitmap (ou seja, os controles estáticos que têm o `SS_ICON` ou `SS_BITMAP` estilo) que está sendo redimensionada porque ela está carregada ou desenhada. Se o ícone ou bitmap for maior que a área de destino, a imagem é recortada.|  
|`SS_RIGHT`|Designa um retângulo simple e exibe o texto determinado liberação à direita do retângulo. O texto é formatado antes que ele seja exibido. Palavras que seriam estenderá após o término de uma linha são quebradas automaticamente para o início da próxima linha do direito de liberação.|  
|`SS_RIGHTJUST`|Especifica que o canto inferior direito de um controle estático com o `SS_BITMAP` ou `SS_ICON` estilo é permanecer fixo quando o controle é redimensionado. Somente os lados superior e esquerdos são ajustados para acomodar um ícone ou bitmap de novo.|  
|`SS_SIMPLE`|Designa um retângulo simple e exibe uma única linha de texto liberação esquerdo do retângulo. A linha de texto não pode ser reduzida ou alterada de alguma forma. (Caixa de diálogo ou janela pai do controle não deve processar o `WM_CTLCOLOR` mensagem.)|  
...
|`WS_CAPTION`|Cria uma janela que tem uma barra de título (implica o `WS_BORDER` estilo). Não pode ser usado com o `WS_DLGFRAME` estilo.|  
|`WS_CHILD`|Cria uma janela filho. Não pode ser usado com o `WS_POPUP` estilo.|  
|`WS_CHILDWINDOW`|Mesmo que o `WS_CHILD` estilo.|  
...
|`WS_CLIPSIBLINGS`|Recorta janelas filho em relação a outros; ou seja, quando uma janela filho específico recebe uma mensagem de pintura, o `WS_CLIPSIBLINGS` estilo Recorta todas as outras janelas de sobreposto filho fora da região da janela filho a serem atualizadas. (Se `WS_CLIPSIBLINGS` não for fornecido e sobreposição de windows do filho, quando você desenhar dentro da área cliente de uma janela filho, é possível desenhar dentro da área cliente de uma janela filho vizinhos.) Para uso com o `WS_CHILD` apenas de estilo.|  
...
|`WS_ICONIC`|Cria uma janela que é inicialmente minimizada. Mesmo que o `WS_MINIMIZE` estilo.|  
|`WS_MAXIMIZE`|Cria uma janela de tamanho máximo.|  
|`WS_MAXIMIZEBOX`|Cria uma janela com um botão de maximização.|  
|`WS_MINIMIZE`|Cria uma janela que é inicialmente minimizada. Para uso com o `WS_OVERLAPPED` apenas de estilo.|  
|`WS_MINIMIZEBOX`|Cria uma janela que tenha um botão de minimização.|  
...
|`WS_POPUPWINDOW`|Cria uma janela pop-up com o `WS_BORDER`, `WS_POPUP`, e `WS_SYSMENU` estilos. O `WS_CAPTION` estilo deve ser combinado com a `WS_POPUPWINDOW` estilo para tornar visível o menu de controle.|  
|`WS_SIZEBOX`|Cria uma janela que tem uma borda de dimensionamento. Mesmo que o `WS_THICKFRAME` estilo.|  
|`WS_SYSMENU`|Cria uma janela que tem uma caixa de menu de controle em sua barra de título. Usado somente para windows com barras de título.|  
...
|`WS_THICKFRAME`|Cria uma janela com um quadro grosso que pode ser usado para dimensionar a janela.|  
...
|Estilo|Descrição|  
|-----------|-----------------|  
|`WS_EX_ACCEPTFILES`|Especifica que uma janela criada com este estilo aceita arrastar e soltar arquivos.|  
...
|`WS_EX_CONTEXTHELP`|Inclui um ponto de interrogação na barra de título da janela. Quando o usuário clica no ponto de interrogação, o cursor muda para um ponto de interrogação com um ponteiro. Se o usuário clica em uma janela filha, a filha receberá um `WM_HELP` mensagem.|  
|`WS_EX_CONTROLPARENT`|Permite que o usuário navegue entre as janelas filho da janela usando a tecla TAB.|  
|`WS_EX_DLGMODALFRAME`|Designa uma janela com uma borda dupla (opcionalmente) pode ser criada com um título de barra ao especificar o `WS_CAPTION` estilo sinalizador no `dwStyle` parâmetro.|  
|`WS_EX_LAYERED`|A janela é uma [em camadas janela](http://msdn.microsoft.com/library/ms632599.aspx#layered). Este estilo não pode ser usado se a janela tem uma [classe estilo](http://msdn.microsoft.com/library/ms633574.aspx#class_styles) do `CS_OWNDC` ou `CS_CLASSDC`. No entanto, [!INCLUDE[win8_first](../../mfc/reference/includes/win8_first_md.md)] dá suporte a `WS_EX_LAYERED` estilo de janelas filho, em que versões anteriores do Windows suportam apenas para janelas de nível superior.|  
|`WS_EX_LEFT`|Fornece propriedades genéricas alinhado à esquerda da janela. Esse é o padrão.|  
|`WS_EX_LEFTSCROLLBAR`|Insere uma barra de rolagem vertical para a esquerda da área cliente.|  
|`WS_EX_LTRREADING`|Exibe o texto da janela esquerda para direita ler as propriedades de ordem. Esse é o padrão.|  
|`WS_EX_MDICHILD`|Cria uma janela de filho MDI.|  
|`WS_EX_NOPARENTNOTIFY`|Especifica que uma janela filho criada com esse estilo não enviará o `WM_PARENTNOTIFY` mensagem para sua janela pai quando a janela filho é criada ou destruída.|  
...
|`WS_EX_RIGHTSCROLLBAR`|Insere uma barra de rolagem vertical (se houver) à direita da área do cliente. Esse é o padrão.|  
|`WS_EX_RTLREADING`|Exibe o texto da janela usando leitura da direita para esquerda solicitar propriedades.|  
|`WS_EX_STATICEDGE`|Cria uma janela com um estilo de borda tridimensional que se destina a ser usado para itens que não aceitam entrada do usuário.|  
...
|`WS_EX_TOPMOST`|Especifica que uma janela criada com este estilo deve ser colocado nontopmost acima de todos os windows e permanecer acima delas, mesmo quando a janela é desativada. Um aplicativo pode usar o `SetWindowPos` a função de membro para adicionar ou remover esse atributo.|  
|`WS_EX_TRANSPARENT`|Especifica que uma janela criada com este estilo é ser transparente. Ou seja, todas as janelas que estão sob a janela não são obscurecidas pela janela. Uma janela criada com este estilo recebe `WM_PAINT` mensagens somente após a atualização de todas as janelas irmãs abaixo dela.|  
|`WS_EX_WINDOWEDGE`|Especifica que uma janela tem uma borda com uma borda aumentada.|  
    
## <a name="see-also"></a>Consulte também  
  
[Visão geral da classe MFC](../../mfc/class-library-overview.md)  
...
[Lista de estilos de caixa (Windows)](https://msdn.microsoft.com/library/windows/desktop/bb775149) 
