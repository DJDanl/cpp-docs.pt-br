...
ms.locfileid: "69511424"
---
# <a name="styles-used-by-mfc"></a>Estilos usados pelo MFC

Use os seguintes sinalizadores de estilo para especificar a aparência e o comportamento do controle e da janela ao criar o objeto MFC correspondente. Na maioria dos casos, esses estilos são definidos no parâmetro *dwStyle* da função de `Create` classe.
...
|BS_AUTO3STATE|Cria um botão de caixa de seleção com três Estados: BST_CHECKED, BST_INDETERMINATE e BST_UNCHECKED. Clicar no botão envia uma notificação BN_CLICKED para a janela do proprietário e altera o estado do botão. O botão Estados do ciclo na ordem de BST_CHECKED, BST_INDETERMINATE e BST_UNCHECKED. Por padrão, o texto associado é exibido à direita da caixa de seleção. Para exibir o texto à esquerda da caixa de seleção, use o estilo BS_LEFTTEXT ou BS_RIGHTBUTTON.|
...
|BS_AUTORADIOBUTTON|Cria um botão de opção com dois Estados: BST_CHECKED e BST_UNCHECKED. Botões de opção geralmente são usados em grupos, com cada grupo com um máximo de uma opção marcada por vez. Clicar no botão envia uma notificação BN_CLICKED para a janela do proprietário, define o estado do botão de opção clicado para BST_CHECKED e define os Estados de todos os outros botões de opção no grupo de botões como BST_UNCHECKED. Por padrão, o texto associado é exibido à direita do botão de opção. Para exibir texto à esquerda do botão de opção, use o estilo BS_LEFTTEXT ou BS_RIGHTBUTTON.|
...
|BS_OWNERDRAW|Cria um botão de desenho proprietário. A estrutura chama o `DrawItem` método quando um aspecto visual do botão é alterado. Esse estilo deve ser definido quando você usa a `CBitmapButton` classe.|
...
|BS_NOTIFY|Habilita um botão para enviar mensagens de notificação BN_DBLCLK, BN_KILLFOCUS e BN_SETFOCUS para sua janela pai. Observe que os botões enviam a notificação BN_CLICKED independentemente de esse estilo ser especificado.|
...
|CBS_HASSTRINGS|Uma caixa de combinação de desenho proprietário contém itens que consistem em cadeias de caracteres. A caixa de combinação mantém a memória e os ponteiros para as cadeias de caracteres `GetText` para que o aplicativo possa usar a função de membro para recuperar o texto de um determinado item.|
...
|CBS_OEMCONVERT|O texto inserido no controle de edição de caixa de combinação é convertido do conjunto de caracteres ANSI para o conjunto de caracteres OEM e, em seguida, de volta para ANSI. Isso garante a conversão adequada de caracteres quando o aplicativo `AnsiToOem` chama a função do Windows para converter uma cadeia de caracteres ANSI na caixa de combinação para caracteres OEM. Esse estilo é mais útil para caixas de combinação que contêm nomes de Filee aplica-se somente a caixas de combinação criadas com os estilos CBS_SIMPLE ou CBS_DROPDOWN.|
...
|ES_OEMCONVERT|O texto inserido no controle de edição é convertido do conjunto de caracteres ANSI para o conjunto de caracteres OEM e, em seguida, de volta para ANSI. Isso garante a conversão adequada de caracteres quando o aplicativo `AnsiToOem` chama a função do Windows para converter uma cadeia de caracteres ANSI no controle de edição para caracteres OEM. Esse estilo é mais útil para controles de edição que contêm nomes de File.|
|ES_PASSWORD|Exibe todos os caracteres como um asterisco __\*__ (), pois eles são digitados no controle de edição. Um aplicativo pode usar a `SetPasswordChar` função de membro para alterar o caractere que é exibido.|
...
|Estilo|Descrição|
|-----------|-----------------|
|FWS_ADDTOTITLE|Especifica as informações a serem acrescentadas ao final de um título de janela do quadro. Por exemplo, "Microsoft Draw-sorteio no Documento1". Você pode especificar as cadeias de caracteres exibidas na guia cadeias de caracteres do modelo de documento no assistente de aplicativo. Se você precisar desativar essa opção, substitua a função de `CWnd::PreCreateWindow` membro.|
|FWS_PREFIXTITLE|Mostra o nome do documento antes do nome do aplicativo em um título da janela do quadro. Por exemplo, "Document-WordPad". Você pode especificar as cadeias de caracteres exibidas na guia cadeias de caracteres do modelo de documento no assistente de aplicativo. Se você precisar desativar essa opção, substitua a função de `CWnd::PreCreateWindow` membro.|
...
|SS_ENDELLIPSIS|ou SS_PATHELLIPSIS substitui parte da cadeia de caracteres fornecida por reticências, se necessário, para que o resultado caiba no retângulo especificado. <br /><br />Você pode especificar SS_END_ELLIPSIS para substituir os caracteres no final da cadeia de caracteres ou SS_PATHELLIPSIS para substituir os caracteres no meio da cadeia de caracteres. Se a cadeia de caracteres contiver\\caracteres de barra invertida (), SS_PATHELLIPSIS preservará a maior parte do texto após a última barra invertida possível.|
...
|SS_REALSIZEIMAGE|Impede um controle de bitmap ou ícone estático (ou seja, controles estáticos que têm o estilo SS_ICON ou SS_BITMAP) sejam redimensionados à medida que são carregados ou desenhados. Se o ícone ou bitmap for maior que a área de destino, a imagem será recortada.|
...
|SS_RIGHTJUST|Especifica que o canto inferior direito de um controle estático com o estilo SS_BITMAP ou SS_ICON é mantido fixo quando o controle é redimensionado. Somente os lados superior e esquerdo são ajustados para acomodar um novo bitmap ou ícone.|
|SS_SIMPLE|Designa um retângulo simples e exibe uma única linha de texto liberado para a esquerda no retângulo. A linha de texto não pode ser reduzida nem alterada de forma alguma. (A janela pai do controle ou a caixa de diálogo não deve processar a mensagem WM_CTLCOLOR.)|
...
|WS_CLIPSIBLINGS|Corta janelas filhas em relação umas às outras; ou seja, quando uma janela filho específica recebe uma mensagem de pintura, o estilo WS_CLIPSIBLINGS corta todas as outras janelas filho sobrepostas da região da janela filho a ser atualizada. (Se WS_CLIPSIBLINGS não for fornecido e a sobreposição de janelas filho, quando você desenhar dentro da área do cliente de uma janela filho, será possível desenhar dentro da área do cliente de uma janela filho vizinha.) Para uso somente com o estilo WS_CHILD.|
...
|WS_MINIMIZE|Cria uma janela que é inicialmente minimizada. Para uso somente com o estilo WS_OVERLAPPED.|
...
|WS_POPUPWINDOW|Cria uma janela pop-up com os estilos WS_BORDER, WS_POPUP e WS_SYSMENU. O estilo WS_CAPTION deve ser combinado com o estilo WS_POPUPWINDOW para tornar o menu de controle visível.|
...
|WS_TABSTOP|Especifica um dos vários controles pelos quais o usuário pode mover usando a tecla TAB. A tecla TAB move o usuário para o próximo controle especificado pelo estilo WS_TABSTOP.|
...
|WS_EX_DLGMODALFRAME|Designa uma janela com uma borda dupla que pode (opcionalmente) ser criada com uma barra de título quando você especifica o sinalizador de estilo WS_CAPTION no parâmetro *dwStyle* .|
|WS_EX_LAYERED|A janela é uma [janela em camadas](/windows/win32/winmsg/window-features). Esse estilo não poderá ser usado se a janela tiver um [estilo de classe](/windows/win32/winmsg/about-window-classes) de CS_OWNDC ou CS_CLASSDC. No entanto, o Microsoft Windows 8 oferece suporte ao estilo WS_EX_LAYERED para janelas filhas, em que versões anteriores do Windows dão suporte apenas para as janelas de nível superior.|
...
|WS_EX_TOPMOST|Especifica que uma janela criada com esse estilo deve ser colocada acima de todas as janelas não mais altas e permanecer acima delas mesmo quando a janela for desativada. Um aplicativo pode usar a `SetWindowPos` função membro para adicionar ou remover este atributo.|
