...
|[CHtmlView::GetHtmlDocument](#gethtmldocument)|Recupera o documento ativo do HTML.|
...
|[CHtmlView::GetParentBrowser](#getparentbrowser)|Recupera um ponteiro para o `IDispatch` interface. Para obter mais informações, consulte [Implementando a IDispatch Interface](/previous-versions/windows/desktop/automat/implementing-the-idispatch-interface).|
...
|[CHtmlView::GetRegisterAsBrowser](#getregisterasbrowser)|Indica se o controle WebBrowser é registrado como um navegador de nível superior para resolução de nome de destino.|
|[CHtmlView::GetRegisterAsDropTarget](#getregisterasdroptarget)|Indica se o controle WebBrowser é registrado como um destino de soltar para navegação.|
...
|[CHtmlView::GoForward](#goforward)|Navega para o próximo item na lista do histórico.|
...
|[CHtmlView::OnDocumentComplete](#ondocumentcomplete)|Chamado para notificar um aplicativo que um documento atingiu o estado READYSTATE_COMPLETE.|
|[CHtmlView::OnDocWindowActivate](#ondocwindowactivate)|Chamada da implementação do Internet Explorer ou MSHTML [IOleInPlaceActiveObject::OnDocWindowActivate](/windows/desktop/api/oleidl/nf-oleidl-ioleinplaceactiveobject-ondocwindowactivate), que notifica o objeto ativo no local quando a janela de documento do contêiner é ativada ou desativada.|
|[CHtmlView::OnDownloadBegin](#ondownloadbegin)|Chamado para notificar um aplicativo que uma operação de navegação está começando.|
|[CHtmlView::OnDownloadComplete](#ondownloadcomplete)|Chamado quando terminar, uma operação de navegação foi interrompido ou falhou.|
|[CHtmlView::OnEnableModeless](#onenablemodeless)|Chamado para habilitar ou desabilitar as caixas de diálogo sem janela restrita quando o contêiner cria ou destrói uma caixa de diálogo modal.|
...
|[CHtmlView::OnFrameWindowActivate](#onframewindowactivate)|Chamado a partir [IOleInPlaceActiveObject::OnFrameWindowActivate](/windows/desktop/api/oleidl/nf-oleidl-ioleinplaceactiveobject-onframewindowactivate) para notificar o objeto quando do nível superior do contêiner a janela do quadro é ativada ou desativada.|
|[CHtmlView::OnFullScreen](#onfullscreen)|Chamado quando a propriedade FullScreen é alterada.|
|[CHtmlView::OnGetDropTarget](#ongetdroptarget)|Chamado pelo Internet Explorer ou MSHTML quando ele está sendo usado como um destino de soltar para permitir que o host fornecer uma alternativa [IDropTarget](/windows/desktop/api/oleidl/nn-oleidl-idroptarget).|
|[CHtmlView::OnGetExternal](#ongetexternal)|Chamado pelo Internet Explorer ou o MSHTML para obter o host `IDispatch` interface.|
...
|[CHtmlView::OnNavigateComplete2](#onnavigatecomplete2)|Chamado depois que a navegação a um hiperlink é concluída (no elemento de uma janela ou um conjunto de quadros).|
|[CHtmlView::OnNavigateError](#onnavigateerror)|Chamado pelo framework se falhar de navegação para um hiperlink.|
|[CHtmlView::OnNewWindow2](#onnewwindow2)|Chamado quando uma nova janela deve ser criado para exibir um recurso.|
...
|[CHtmlView::OnResizeBorder](#onresizeborder)|Chamada da implementação do Internet Explorer ou MSHTML [IOleInPlaceActiveObject::ResizeBorder](/windows/desktop/api/oleidl/nf-oleidl-ioleinplaceactiveobject-resizeborder), que o objeto que ele precisa para redimensionar o seu espaço de borda de alertas.|
|[CHtmlView::OnShowContextMenu](#onshowcontextmenu)|Chamado a partir do Internet Explorer ou MSHTML quando ele está prestes a mostrar o menu de contexto.|
...
|[CHtmlView::SetRegisterAsBrowser](#setregisterasbrowser)|Define um valor que indica se o controle WebBrowser é registrado como um navegador de nível superior para resolução de nome de destino.|
|[CHtmlView::SetRegisterAsDropTarget](#setregisterasdroptarget)|Define um valor que indica se o controle WebBrowser é registrado como um destino de soltar para navegação.|
...
O controle WebBrowser é uma janela na qual o usuário pode procurar sites na World Wide Web, bem como pastas no sistema de arquivos local e em uma rede. O controle WebBrowser suporta hyperlink, navegação de localizador de recurso uniforme (URL) e mantém uma lista de histórico.

## <a name="using-the-chtmlview-class-in-an-mfc-application"></a>Usando a classe CHtmlView em um aplicativo MFC

No padrão do framework aplicativo MFC (SDI ou MDI com base), o objeto de exibição normalmente é derivado de um conjunto especializado de classes. Essas classes, todas derivadas de `CView`, oferecem funções especializadas além daquelas já fornecidas pelo `CView`.

Baseando a classe de exibição do aplicativo em `CHtmlView` fornece a exibição com o controle WebBrowser. Isso efetivamente torna o aplicativo em um navegador da web. O método preferencial de criação de um aplicativo de estilo de navegador da web é usar o Assistente de aplicativo do MFC e especificar `CHtmlView` como a classe de exibição. Para obter mais informações sobre como implementar e usar o controle WebBrowser dentro de aplicativos MFC, consulte [criando um aplicativo de estilo de navegador da Web](../../mfc/reference/creating-a-web-browser-style-mfc-application.md).

> [!NOTE]
>  O controle WebBrowser ActiveX (e, portanto, `CHtmlView`) está disponível somente para programas em execução em versões do Windows NT 4.0 ou posteriores, no qual o Internet Explorer 4.0 ou posterior foi instalado.

`CHtmlView` é projetado para aplicativos que acessam a Web (e/ou documentos HTML). O seguinte `CHtmlView` funções de membro se aplicam a apenas o aplicativo do Internet Explorer. Essas funções terá êxito no controle WebBrowser, mas eles terão nenhum efeito visível.
...
*lpszClassName*<br/>
Aponta para uma cadeia de caracteres terminada em nulo que nomeia a classe Windows. O nome de classe pode ser qualquer nome registrado com o [AfxRegisterWndClass](../../mfc/reference/application-information-and-management.md#afxregisterwndclass) função global ou o `RegisterClass` função do Windows. Se for NULL, usa o padrão predefinido [CFrameWnd](../../mfc/reference/cframewnd-class.md) atributos.
...
*pVarIn*<br/>
Ponteiro para um `VARIANT` estrutura que contém os argumentos de entrada. Pode ser NULL.

*pVarOut*<br/>
Ponteiro para um `VARIANT` estrutura que receberá a saída do comando. Pode ser NULL.

### <a name="return-value"></a>Valor de retorno

Um valor padrão de HRESULT. Para obter uma lista completa de valores possíveis, consulte [IOleCommandTarget::Exec](/windows/desktop/api/docobj/nf-docobj-iolecommandtarget-exec) no SDK do Windows.

### <a name="remarks"></a>Comentários

`ExecFormsCommand` implementa o comportamento de [IOleCommandTarget::Exec](/windows/desktop/api/docobj/nf-docobj-iolecommandtarget-exec) método.
...
Ver [IWebBrowser2::ExecWB](/previous-versions//aa752117\(v=vs.85\)) no Windows SDK.
...
Aplica-se ao Internet Explorer. Se você usar essa chamada com um controle WebBrowser, não será retornado nenhum erro, mas ele irá ignorar essa chamada.
...
Um ponteiro para o `IDispatch` interface do objeto de documento ativo. Para obter mais informações, consulte [Implementando a IDispatch Interface](/previous-versions/windows/desktop/automat/implementing-the-idispatch-interface).

### <a name="remarks"></a>Comentários

Aplica-se ao navegador da Web e o Internet Explorer.

##  <a name="getbusy"></a>  CHtmlView::GetBusy

Chame essa função de membro para determinar se o controle WebBrowser está envolvido em uma operação de download ou a navegação.
...
Um [CString](../../atl-mfc-shared/reference/cstringt-class.md) objeto que contém o caminho e o nome do arquivo exibido no momento. Se nenhum caminho e nome de arquivo existirem, `GetFullName` retornará um `CString`.
...
Se o recurso for uma página HTML na World Wide Web, o nome é o título da página. Se o recurso for uma pasta ou arquivo na rede ou no computador local, o nome é o UNC ou caminho completo da pasta ou arquivo.
...
Se o recurso for uma pasta ou arquivo na rede ou no computador local, o nome é o UNC ou caminho completo da pasta ou arquivo.
...
Diferente de zero se a barra de menu está visível; Caso contrário, zero.
...
Diferente de zero se o navegador da web está offline no momento; Caso contrário, zero.
...
Um [READYSTATE](/previous-versions//aa768362\(v=vs.85\)) de valor, conforme descrito no SDK do Windows.
...
Chame essa função de membro para determinar se o objeto WebBrowser é registrado como um navegador de nível superior para resolução de nome de destino.
...
Diferente de zero se o navegador é registrado como um navegador de nível superior; Caso contrário, zero.
...
Chame essa função de membro para determinar se o controle WebBrowser é registrado como um destino de soltar para navegação.
...
Diferente de zero se o navegador é registrado como um destino de soltar; Caso contrário, zero.
...
Chame essa função de membro para determinar se as caixas de diálogo podem ser exibidas no controle WebBrowser.
...
Quando o navegador da web está em modo de teatro, a janela principal do navegador ocupa a tela inteira, uma barra de ferramentas com um conjunto mínimo de ferramentas de navegação é exibida e a barra de status aparece no canto superior direito da tela.
...
Um valor que indica se a barra de ferramentas está visível. Diferente de zero se a barra de ferramentas está visível; Caso contrário, zero.

##  <a name="gettop"></a>  CHtmlView::GetTop

Chame essa função de membro para recuperar a coordenada de tela da borda superior da janela de principal do controle WebBrowser.
...
Navega encaminhar um item na lista do histórico.
...
Navega para a atual inicial ou a página de início especificada na caixa de diálogo Opções da Internet do Internet Explorer ou a caixa de diálogo Propriedades da Internet, acessado a partir do painel de controle.
...
*URL*<br/>
Uma cadeia de caracteres alocada pelo chamador que contém a URL para navegar até, ou o caminho completo do arquivo a ser exibido.

*dwFlags*<br/>
Os sinalizadores de uma variável que especifica se deve adicionar o recurso à lista do histórico, devem ser lido para ou do cache de gravação e se deseja exibir o recurso em uma nova janela. A variável pode ser uma combinação dos valores definidos pela [BrowserNavConstants](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa768360\(v=vs.85\)) enumeração.

*lpszTargetFrameName*<br/>
Um ponteiro para uma cadeia de caracteres que contém o nome do quadro no qual exibir o recurso.

*lpszHeaders*<br/>
Um ponteiro para um valor que especifica os cabeçalhos HTTP para enviar para o servidor. Esses cabeçalhos são adicionados aos cabeçalhos padrão do Internet Explorer. Os cabeçalhos podem especificar essas coisas como a ação necessária do servidor, o tipo de dados que está sendo passados para o servidor ou um código de status. Esse parâmetro será ignorado se *URL* não é uma URL HTTP.
...
*dwPostDataLen*<br/>
Dados a serem enviados com a transação de HTTP POST. Por exemplo, a transação de POST é usada para enviar os dados coletados por um formulário HTML. Se esse parâmetro não especifica quaisquer dados de postagem, `Navigate` emite uma transação HTTP GET. Esse parâmetro será ignorado se *URL* não é uma URL HTTP.
...
*pIDL*<br/>
Um ponteiro para um [ITEMIDLIST](/windows/desktop/api/shtypes/ns-shtypes-_itemidlist) estrutura.
...
*lpvPostData*<br/>
Dados a serem enviados com a transação de HTTP POST. Por exemplo, a transação de POST é usada para enviar os dados coletados por um formulário HTML. Se esse parâmetro não especifica quaisquer dados de postagem, `Navigate2` emite uma transação HTTP GET. Esse parâmetro será ignorado se *URL* não é uma URL HTTP ou HTTPS.

*dwPostDataLen*<br/>
Comprimento em bytes dos dados apontados pela *lpvPostData* parâmetro.
...
Essa função membro estende o `Navigate` função de membro com o suporte à navegação em pastas especiais, como área de trabalho e meu computador, são representadas pelo parâmetro *pIDL*.
...
Essa função membro é chamada pelo framework para fazer com que um evento seja acionado antes que ocorra uma navegação no navegador da web.
...
*lpszURL*<br/>
Ponteiro para uma cadeia de caracteres que contém a URL para navegar para.
...
*baPostedData*<br/>
Uma referência a um `CByteArray` objeto que contém os dados para enviar para o servidor se a transação de HTTP POST que está sendo usada.

*lpszHeaders*<br/>
Um ponteiro para uma cadeia de caracteres que contém os cabeçalhos HTTP adicionais para enviar para o servidor (somente para URLs de HTTP). Os cabeçalhos podem especificar essas coisas como a ação necessária do servidor, o tipo de dados que está sendo passados para o servidor ou um código de status.
...
Essa função membro é chamada pelo framework para notificar um aplicativo que o estado ativado de um comando de navegador da web foi alterado.
...
Essa função membro é chamada pelo framework para notificar um aplicativo que um documento atingiu o estado READYSTATE_COMPLETE.
...
*lpszURL*<br/>
Um ponteiro para uma cadeia de caracteres que é avaliada para a URL, UNC do arquivo o nome, ou um PIDL (um ponteiro para uma lista de identificadores de item) que foi acessado.

### <a name="remarks"></a>Comentários

Não é a cada quadro acionará esse evento, mas cada quadro que é acionado uma [OnDownloadBegin](#ondownloadbegin) evento será disparado um correspondente `OnDocumentComplete` eventos.

A URL indicada por *lpszURL* pode ser diferente da URL que o navegador disse para navegar para, pois essa URL é a URL canonizada e qualificada. Por exemplo, se um aplicativo especifica uma URL de "www.microsoft.com" em uma chamada para [Navigate](#navigate) ou [Navigate2](#navigate2), a URL passada `OnNavigateComplete2` será "<http://www.microsoft.com/>". Além disso, se o servidor redirecionado do navegador para uma URL diferente, a URL redirecionada será refletida aqui.

##  <a name="ondocwindowactivate"></a>  CHtmlView::OnDocWindowActivate

Chamada da implementação do Internet Explorer ou MSHTML do `IOleInPlaceActiveObject::OnDocWindowActivate`, que notifica o objeto ativo no local quando a janela de documento do contêiner é ativada ou desativada.
...
Substituir `OnDocWindowActivate` reagir ao `OnDocWindowActivate` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::OnDocWindowActivate](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753261\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="ondownloadbegin"></a>  CHtmlView::OnDownloadBegin

Essa função membro é chamada pelo framework para começar a baixar um documento.
...
Este evento é acionado logo após o [OnBeforeNavigate2](#onbeforenavigate2) evento, a menos que a navegação é cancelada. Qualquer animação ou a indicação "ocupada" que precisa exibir o contêiner deve estar conectada a este evento.

##  <a name="ondownloadcomplete"></a>  CHtmlView::OnDownloadComplete

Essa função membro é chamada pelo framework para indicar que uma operação de navegação terminada, foi interrompido ou falhou.
...
*fEnable*<br/>
Indica se as caixas de diálogo sem janela restrita do host estão habilitadas ou desabilitadas. Se esse valor for diferente de zero, caixas de diálogo sem janela restrita estão habilitadas. Se esse valor for zero, caixas de diálogo sem janela restrita estão desabilitadas.
...
Habilita ou desabilita as caixas de diálogo sem janela restrita quando o contêiner cria ou destrói uma caixa de diálogo modal. Substituir `OnEnableModeless` reagir ao `EnableModeless` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::EnableModeless](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753253\(v=vs.85\)) no SDK do Windows para obter mais informações.
...
*pDataObject*<br/>
Endereço do [IDataObject](/windows/desktop/api/objidl/nn-objidl-idataobject) interface fornecida pelo Internet Explorer ou MSHTML.

*ppDataObject*<br/>
O endereço que recebe o `IDataObject` ponteiro de interface fornecido pelo host. O conteúdo desse parâmetro sempre deve ser inicializado como NULL, mesmo se o método falha.

### <a name="return-value"></a>Valor de retorno

S_OK se o objeto de dados é substituído, S_FALSE se o objeto de dados não for substituído ou um código de erro definido por OLE se ocorrer um erro.

### <a name="remarks"></a>Comentários

Substituir `OnFilterDataObject` reagir ao `FilterDataObject` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::FilterDataObject](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753254\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="onframewindowactivate"></a>  CHtmlView::OnFrameWindowActivate

Chamado a partir [IOleInPlaceActiveObject::OnFrameWindowActivate](/windows/desktop/api/oleidl/nf-oleidl-ioleinplaceactiveobject-onframewindowactivate) para notificar o objeto quando do nível superior do contêiner a janela do quadro é ativada ou desativada.
...
Substituir `OnFrameWindowActivate` reagir ao `OnFrameWindowActivate` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::OnFrameWindowActivate](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753262\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="onfullscreen"></a>  CHtmlView::OnFullScreen

Essa função membro é chamada pelo framework quando o [FullScreen](/previous-versions/aa752119\(v=vs.85\)) propriedade foi alterada.
...
Chamado pelo Internet Explorer ou MSHTML quando ele está sendo usado como um destino de soltar para permitir que o host fornecer uma alternativa `IDropTarget`.
...
*pDropTarget*<br/>
[IDropTarget](/windows/desktop/api/oleidl/nn-oleidl-idroptarget) Internet Explorer ou MSHTML propõe a usar.
...
Ver [IDocHostUIHandler::GetDropTarget](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753255\(v=vs.85\)) no SDK do Windows para obter uma lista de códigos de retorno.

### <a name="remarks"></a>Comentários

Substituir `OnGetDropTarget` reagir ao `GetDropTarget` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::GetDropTarget](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753255\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="ongetexternal"></a>  CHtmlView::OnGetExternal

Chamado pelo Internet Explorer ou o MSHTML para obter o host `IDispatch` interface.
...
*lppDispatch*<br/>
Um ponteiro para o endereço que recebe o `IDispatch` ponteiro de interface do aplicativo host. Se o host expõe uma interface de automação, ele pode fornecer uma referência para o Internet Explorer ou MSHTML por meio desse parâmetro. O conteúdo desse parâmetro sempre deve ser inicializado como NULL, mesmo se o método falha.
...
Substituir `OnGetExternal` reagir ao `GetExternal` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::GetExternal](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753256\(v=vs.85\)) no SDK do Windows para obter mais informações.
...
*pInfo*<br/>
Endereço de uma [DOCHOSTUIINFO](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa770044\(v=vs.85\)) estrutura que recebe os recursos de interface do usuário do host.
...
Substituir `OnGetHostInfo` reagir ao `GetHostInfo` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::GetHostInfo](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753257\(v=vs.85\)) no SDK do Windows para obter mais informações.
...
*pchKey*<br/>
Endereço de um `LPOLESTR` que recebe a cadeia de subchaves de registro em que o host armazena suas opções padrão. Essa subchave será na chave HKEY_CURRENT_USER. Alocar essa memória usando [CoTaskMemAlloc](/windows/desktop/api/combaseapi/nf-combaseapi-cotaskmemalloc). O aplicativo de chamada é responsável por liberar essa memória usando [CoTaskMemFree](/windows/desktop/api/combaseapi/nf-combaseapi-cotaskmemfree). Esse parâmetro sempre deve ser inicializado como NULL, mesmo se o método falha.
...
Substituir `OnGetOptionKeyPath` reagir ao `GetOptionKeyPath` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::GetOptionKeyPath](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753258\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="onhideui"></a>  CHtmlView::OnHideUI

Essa função membro é chamada pelo framework quando o Internet Explorer ou MSHTML remove seus menus e barras de ferramentas.
...
Substituir `OnHideUI` reagir ao `HideUI` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::HideUI](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753259\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="onmenubar"></a>  CHtmlView::OnMenuBar

Essa função membro é chamada pelo framework quando o [barra de menus](/previous-versions/aa752131\(v=vs.85\)) propriedade foi alterada.
...
Essa função membro é chamada pelo framework depois que a navegação a um hiperlink é concluída (no elemento de uma janela ou um conjunto de quadros).
...
Chamado pelo framework se falhar de navegação para um hiperlink.
...
*lpszFrame*<br/>
O nome do quadro no qual o recurso deve ser exibida ou nulo se nenhum quadro nomeado foi direcionado para o recurso.

*dwError*<br/>
Um código de status erro, se disponível. Para obter uma lista dos possíveis códigos de status HTTP e HRESULT, consulte [códigos de Status de evento NavigateError.](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa768365\(v=vs.85\)).

*pbCancel*<br/>
Especifica se deve cancelar a navegação para uma página de erro ou qualquer recurso adicional de pesquisa automática. Se TRUE (padrão), continue com a navegação para uma página de erro ou uma pesquisa automática; Se for FALSE, cancele a navegação para uma página de erro ou uma pesquisa automática.
...
Essa função membro é chamada pelo framework quando uma nova janela deve ser criada para exibir um recurso.
...
*ppDisp*<br/>
Um ponteiro para um ponteiro de interface que, opcionalmente, recebe o `IDispatch` ponteiro de interface de um novo objeto WebBrowser ou o Internet Explorer.
...
Essa função membro é chamada pelo framework para notificar um aplicativo que o progresso de uma operação de download foi atualizado.
...
*nProgress*<br/>
Quantidade de progresso total para mostrar ou -1 quando o andamento for concluído.
...
Essa função membro é chamada pelo framework para notificar um aplicativo que [PutProperty](#putproperty) alterou o valor de uma propriedade.
...
Essa função membro é chamada pelo framework para notificar um aplicativo que o aplicativo do Internet Explorer está pronto para sair.
...
Chamada da implementação do Internet Explorer ou MSHTML [IOleInPlaceActiveObject::ResizeBorder](/windows/desktop/api/oleidl/nf-oleidl-ioleinplaceactiveobject-resizeborder), que o objeto que ele precisa para redimensionar o seu espaço de borda de alertas.
...
*pUIWindow*<br/>
Um ponteiro para a interface para o objeto de janela de quadro ou documento cuja borda foi alterado.

*fFrameWindow*<br/>
TRUE se a janela do quadro está chamando [IOleInPlaceActiveObject::ResizeBorder](/windows/desktop/api/oleidl/nf-oleidl-ioleinplaceactiveobject-resizeborder), caso contrário, FALSE.
...
Substituir `OnResizeBorder` reagir ao `ResizeBorder` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::ResizeBorder](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753263\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="onshowcontextmenu"></a>  CHtmlView::OnShowContextMenu

Chamado a partir do Internet Explorer ou MSHTML quando ele está prestes a mostrar o menu de contexto.
...
*pcmdtReserved*<br/>
[IOleCommandTarget](/windows/desktop/api/docobj/nn-docobj-iolecommandtarget) interface usada para consultar o status do comando e executar comandos nesse objeto.

*pdispReserved*<br/>
Interface IDispatch do objeto coordenadas da tela. Isso permite que um host diferenciar objetos específicos para fornecer contexto mais específico.

### <a name="return-value"></a>Valor de retorno

Ver [IDocHostUIHandler::ShowContextMenu](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753264\(v=vs.85\)) no SDK do Windows para obter uma lista de valores.

### <a name="remarks"></a>Comentários

Substituir `OnShowContextMenu` reagir ao `ShowContextMenu` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::ShowContextMenu](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753264\(v=vs.85\)) no SDK do Windows para obter mais informações.
...
*pActiveObject*<br/>
[IOleInPlaceActiveObject](/windows/desktop/api/oleidl/nn-oleidl-ioleinplaceactiveobject) interface do objeto ativo no momento.

*pCommandTarget*<br/>
[IOleCommandTarget](/windows/desktop/api/docobj/nn-docobj-iolecommandtarget) interface do objeto.

*pFrame*<br/>
[IOleInPlaceFrame](/windows/desktop/api/oleidl/nn-oleidl-ioleinplaceframe) interface do objeto. Isso é necessário para menus e barras de ferramentas.

*pDoc*<br/>
[IOleInPlaceUIWindow](/windows/desktop/api/oleidl/nn-oleidl-ioleinplaceuiwindow) interface para o objeto. Isso é necessário para barras de ferramentas.

### <a name="return-value"></a>Valor de retorno

Ver [IDocHostUIHandler::ShowUI](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753265\(v=vs.85\)) no SDK do Windows para obter uma lista de valores.

### <a name="remarks"></a>Comentários

Substituir `OnShowUI` reagir ao `ShowUI` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::ShowUI](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753265\(v=vs.85\)) no SDK do Windows para obter mais informações.

##  <a name="onstatusbar"></a>  CHtmlView::OnStatusBar

Essa função membro é chamada pelo framework quando o [StatusBar](/previous-versions//aa768270\(v=vs.85\)) propriedade foi alterada.
...
Essa função membro é chamada pelo framework para notificar um aplicativo que o texto da barra de status associado ao controle WebBrowser foi alterado.
...
Essa função membro é chamada pelo framework quando o [TheaterMode](/previous-versions/aa768273\(v=vs.85\)) propriedade foi alterada.
...
Essa função membro é chamada pelo framework para notificar um aplicativo se o título de um documento no controle WebBrowser ficar disponível ou alterações.
...
Para HTML, o título pode ser alterado; enquanto ainda está baixando o HTML, a URL do documento é definida como o título. Depois que o título real (se houver) é analisado do HTML, o título é alterado para refletir o título real.

##  <a name="ontoolbar"></a>  CHtmlView::OnToolBar

Essa função membro é chamada pelo framework quando o [barra de ferramentas](/previous-versions/aa768274\(v=vs.85\)) propriedade foi alterada.
...
*lpMsg*<br/>
Aponta para a mensagem que talvez precise ser traduzido.
...
Substituir `OnTranslateAccelerator` reagir ao `TranslateAccelerator` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::TranslateAccelerator](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753266\(v=vs.85\)) no SDK do Windows para obter mais informações.
...
*pchURLIn*<br/>
Endereço de uma cadeia de caracteres fornecida pelo Internet Explorer ou MSHTML que representa a URL a ser traduzido.

*ppchURLOut*<br/>
Endereço de um ponteiro de cadeia de caracteres que recebe o endereço da URL traduzido. O host aloca o buffer usando o alocador de memória da tarefa. O conteúdo desse parâmetro sempre deve ser inicializado como NULL, mesmo se a URL não é convertida ou o método falhará.

### <a name="return-value"></a>Valor de retorno

`S_OK` Se a URL foi convertido, S_FALSE se a URL não foi convertida, ou um código de erro definido por OLE se ocorreu um erro.

### <a name="remarks"></a>Comentários

Substituir `OnTranslateUrl` reagir ao `TranslateUrl` notificação do controle de navegador da Web de Microsoft. Ver [IDocHostUIHandler::TranslateUrl](/previous-versions/windows/internet-explorer/ie-developer/platform-apis/aa753267\(v=vs.85\)) no SDK do Windows para obter mais informações.
...
O host deve atualizar o estado dos botões da barra de ferramentas. Esse método é chamado, independentemente do valor de retorno `ShowUI`. Substituir `OnUpdateUI` reagir ao `UpdateUI` notificação do controle de navegador da Web de Microsoft.

##  <a name="onvisible"></a>  CHtmlView::OnVisible

Essa função membro é chamada pelo framework quando a janela de WebBrowser deve ser mostrada ou ocultada.
...
Isso permite que a janela de host de controle do objeto a se comportar da mesma maneira que comporta-se a janela do Internet Explorer.
...
*dwCommandID*<br/>
O identificador do comando que está sendo consultado para.

*pbSupported*<br/>
Um ponteiro para um BOOLIANO que especifica se o comando (identificado por *dwCommandID*) tem suporte. Se for TRUE, o comando é suportado; Caso contrário, FALSE.

*pbEnabled*<br/>
Um ponteiro para um BOOLIANO que especifica se o comando (identificado por *dwCommandID*) está habilitado. Se for TRUE, o comando é suportado; Caso contrário, FALSE.

*pbChecked*<br/>
Um ponteiro para um BOOLIANO que especifica se o comando (identificado por *dwCommandID*) é verificada. Se for TRUE, o comando é suportado; Caso contrário, FALSE.

### <a name="return-value"></a>Valor de retorno

Um valor padrão de HRESULT. Para obter uma lista completa de valores possíveis, consulte [IOleCommandTarget::QueryStatus](/windows/desktop/api/docobj/nf-docobj-iolecommandtarget-querystatus) no SDK do Windows.

### <a name="remarks"></a>Comentários

`QueryFormsCommand` implementa o comportamento de [IOleCommandTarget::QueryStatus](/windows/desktop/api/docobj/nf-docobj-iolecommandtarget-querystatus) método.

##  <a name="querystatuswb"></a>  CHtmlView::QueryStatusWB

Chame essa função de membro para consultar o status do comando.
...
*cmdID*<br/>
O [OLECMDID](/windows/desktop/api/docobj/ne-docobj-olecmdid) valor do comando para o qual o chamador precisa de informações de status.

### <a name="return-value"></a>Valor de retorno

O endereço do [OLECMDF](/windows/desktop/api/docobj/ne-docobj-olecmdf) valor que recebe o status do comando.

### <a name="remarks"></a>Comentários

`QueryStatusWB` implementa o comportamento de [IOleCommandTarget::QueryStatus](/windows/desktop/api/docobj/nf-docobj-iolecommandtarget-querystatus) método.
...
*nLevel*<br/>
O endereço da variável especificando o nível de atualização. As variáveis de possíveis são definidas no [RefreshConstants](/previous-versions/aa768363\(v=vs.85\)), no SDK do Windows.

### <a name="remarks"></a>Comentários

Diferentemente [Refresh](#refresh), `Refresh2` contém um parâmetro que especifica o nível de atualização.
...
Chame essa função de membro para definir o Internet Explorer para qualquer um dos modos de janela normal ou de tela inteira.
...
Chame essa função de membro para definir um valor que indica se o controle WebBrowser é registrado como um navegador de nível superior para resolução de nome de destino.
...
*bNewValue*<br/>
Determina se o Internet Explorer é registrado como um navegador de nível superior. Se for diferente de zero, o navegador da web está registrado como um navegador de nível superior; Se for zero, não é um navegador de nível superior. O valor padrão é zero.
...
Chame essa função de membro para definir um valor que indica se o controle WebBrowser é registrado como um destino de soltar para navegação.
...
*bNewValue*<br/>
Determina se o controle WebBrowser está registrado como um destino de soltar para navegação. Se for diferente de zero, o objeto é registrado como um destino de soltar; Se for zero, não é um destino de soltar.
...
Chame essa função de membro para cancelar qualquer navegação pendente ou operação de download e parar quaisquer elementos de página dinâmica, como sons em segundo plano e animações.
