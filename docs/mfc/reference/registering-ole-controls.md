...
`AfxOleRegisterTypeLib`é normalmente chamado em uma implementação de DLL de controle `DllRegisterServer`de. Da mesma `AfxOleUnregisterTypeLib` forma, é `DllUnregisterServer`chamado por. `AfxOleRegisterControlClass`, `AfxOleRegisterPropertyPageClass`e `AfxOleUnregisterClass` sãonormalmentechamadospelafunçãomembrodafábricadeclassesoudapáginadepropriedades`UpdateRegistry` de um controle.
...
O exemplo acima demonstra como `AfxOleRegisterControlClass` o é chamado com o sinalizador para insertável e o sinalizador para o modelo de apartamento or em conjunto para criar o sexto parâmetro:
...
> [!NOTE]
>  Nas versões do MFC anteriores ao MFC 4,2, o parâmetro **int** *nRegFlags* não estava disponível. Observe também que o `afxRegInsertable` sinalizador não é uma opção válida para páginas de propriedades e causará uma declaração no MFC, se estiver definido
