...
## <a name="prerequisites"></a>Pré-requisitos  
 Este passo a passo pressupõe que você definiu [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] usar **configurações gerais de desenvolvimento**. Se você estiver usando configurações diferentes, alguns dos elementos da interface do usuário (UI) mencionados nas instruções talvez não sejam exibidos. Para obter informações sobre como alterar as configurações, consulte [como: redefinir suas configurações](http://msdn.microsoft.com/c95c51be-e609-4769-abba-65e6beedec76).  
