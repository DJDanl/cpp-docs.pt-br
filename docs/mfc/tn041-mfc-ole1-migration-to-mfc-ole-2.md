...
Em MFC/OLE1, `COleClientItem::GetBounds` e `SetBounds` foram usadas para consultar e manipular a extensão de um item (o `left` e `top` membros foram sempre zero). Em MFC/OLE 2 isso é mais diretamente suportado por `COleClientItem::GetExtent` e `SetExtent`, que lidam com uma **tamanho** ou `CSize` em vez disso.
...
- Aplicar zoom para true comportamento WYSIWYG em relação ao contêiner.
