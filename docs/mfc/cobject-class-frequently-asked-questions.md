---
title: 'Classe CObject: Perguntas frequentes | Microsoft Docs'
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- cpp-windows
ms.tgt_pltfrm: 
ms.topic: article
f1_keywords:
- CObject
dev_langs:
- C++
helpviewer_keywords:
- CObject class [MFC], FAQ
ms.assetid: 809a8b99-a2f8-4e16-8b4b-023c94f4125c
caps.latest.revision: 
author: mikeblome
ms.author: mblome
manager: ghogen
ms.workload:
- cplusplus
ms.openlocfilehash: ecad3239510519e1c262e41bf9198358c4442912
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 12/21/2017
---
# <a name="cobject-class-frequently-asked-questions"></a>Classe CObject: perguntas frequentes
Esta seção aborda as perguntas na classe `CObject`.  
  
## <a name="what-do-you-want-to-know-more-about"></a>O que você deseja saber mais sobre  
  
-   [Preciso derivar novas classes de CObject](do-i-have-to-derive-new-classes-from-cobject-q.md)  
  
-   [Quanto custa o mim para derivar uma classe de CObject](what-does-it-cost-me-to-derive-a-class-from-cobject-q.md)  
  
## <a name="see-also"></a>Consulte também  
 [Usando CObject](../mfc/using-cobject.md)
