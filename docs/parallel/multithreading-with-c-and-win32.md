...
As bibliotecas CRT fornecem as seguintes funções para a criação e o encerramento de thread: [_beginthread, _beginthreadex](../c-runtime-library/reference/beginthread-beginthreadex.md), [_endthread e _endthreadex](../c-runtime-library/reference/endthread-endthreadex.md).

As `_beginthread` funções `_beginthreadex` e criam um novo thread e retornam um identificador de thread se a operação for bem-sucedida. O thread será encerrado automaticamente se concluir a execução. Ou, ele pode ser encerrado com uma chamada `_endthread` para `_endthreadex`ou.

> [!NOTE]
> Se você chamar as rotinas de tempo de execução de C de um programa criado com LIBCMT. lib, deverá iniciar seus `_beginthread` threads com a função ou `_beginthreadex` . Não use as funções `ExitThread` do Win32 e `CreateThread`o. O `SuspendThread` uso do pode levar a um deadlock quando mais de um thread é bloqueado aguardando o thread suspenso concluir seu acesso a uma estrutura de dados de tempo de execução C.

###  <a name="_core_the__beginthread_function"></a>As funções _beginthread e _beginthreadex

As `_beginthread` funções `_beginthreadex` e criam um novo thread. Um thread compartilha o código e os segmentos de dados de um processo com outros threads no processo, mas tem seus próprios valores de registro exclusivos, espaço de pilha e endereço de instrução atual. O sistema dá tempo de CPU a cada thread, para que todos os threads em um processo possam ser executados simultaneamente.
...
`_beginthread`e `_beginthreadex` retorne um identificador para o novo thread, se for bem-sucedido, ou um código de erro se houvesse um erro.

###  <a name="_core_the__endthread_function"></a>As funções _endthread e _endthreadex

A função [_endthread](../c-runtime-library/reference/endthread-endthreadex.md) encerra um thread criado pelo `_beginthread` (e, da mesma forma, `_endthreadex` encerra um thread criado `_beginthreadex`pelo). Os threads são encerrados automaticamente quando são concluídos. `_endthread`e `_endthreadex` são úteis para o encerramento condicional de dentro de um thread. Um thread dedicado ao processamento de comunicações, por exemplo, pode ser fechado se não for possível obter o controle da porta de comunicação.
...
> [!NOTE]
> Para obter uma discussão semelhante do ponto de vista do MFC, [consulte Multithreading: Dicas](multithreading-programming-tips.md) de programação [e multithreading: Quando usar as classes](multithreading-when-to-use-the-synchronization-classes.md)de sincronização.
...
O programa Bounce. c mostrado no [programa multithread c de exemplo](sample-multithread-c-program.md) usa um mutex `ScreenMutex` chamado para coordenar atualizações da tela. Cada vez que um dos threads de exibição está pronto para gravar na tela, ele `WaitForSingleObject` chama o identificador para `ScreenMutex` e constante infinita para indicar que `WaitForSingleObject` a chamada deve bloquear o mutex e não atingir o tempo limite. Se `ScreenMutex` estiver claro, a função Wait definirá o mutex para que outros threads não possam interferir na exibição e continuar executando o thread. Caso contrário, o thread será bloqueado até que o mutex seja apagado. Quando o thread conclui a atualização de exibição, ele libera o mutex chamando `ReleaseMutex`.

Telas de tela e dados estáticos são apenas dois dos recursos que exigem gerenciamento cuidadoso. Por exemplo, seu programa pode ter vários threads acessando o mesmo arquivo. Como outro thread pode ter movido o ponteiro do arquivo, cada thread deve redefinir o ponteiro do arquivo antes da leitura ou gravação. Além disso, cada thread deve garantir que não seja preempção entre o tempo que ele posiciona o ponteiro e a hora em que ele acessa o arquivo. Esses threads devem usar um semáforo para coordenar o acesso ao arquivo digitando cada acesso de `WaitForSingleObject` arquivo `ReleaseMutex` com chamadas e. O exemplo de código a seguir ilustra essa técnica:
...
O primeiro argumento na `_beginthread` chamada é um ponteiro para a `BounceProc` função, que executa os threads. O segundo argumento especifica o tamanho de pilha padrão para o thread. O último argumento é um número de identificação que é passado `BounceProc`para. `BounceProc`usa o número de ID para propagar o gerador de números aleatórios e selecionar o atributo de cor e o caractere de exibição do thread.

Os threads que fazem chamadas para a biblioteca de tempo de execução C ou para a API do Win32 devem permitir espaço suficiente de pilha para as funções de biblioteca e API que eles chamam. A função `printf` C requer mais de 500 bytes de espaço de pilha, e você deve ter de 2K bytes de espaço de pilha disponível ao chamar rotinas de API do Win32.
...
Há vários problemas que você pode encontrar ao criar, vincular ou executar um programa multithread C. Alguns dos problemas mais comuns são descritos na tabela a seguir. (Para uma discussão semelhante do ponto de vista do MFC, consulte [multithreading: Dicas](multithreading-programming-tips.md)de programação.)
