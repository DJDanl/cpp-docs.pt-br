...
- Quando possível, envolva todas as chamadas para [Concurrency:: Task:: Get](reference/task-class.md#get) e [Concurrency:: Task:: aguardar](reference/task-class.md#wait) com um `try`/`catch` bloquear para lidar com erros dos quais você pode se recuperar. O tempo de execução encerra o aplicativo se uma tarefa gera uma exceção e essa exceção não é detectada pela tarefa, uma de suas continuações ou o aplicativo principal.
...
O exemplo a seguir mostra a estrutura básica de uma função de trabalho que gera uma exceção. O exemplo usa um objeto `task_group` para imprimir os valores de dois objetos `point` em paralelo. A função de trabalho `print_point` imprime os valores de um objeto `point` no console. A função trabalho gera uma exceção se o valor de entrada é `NULL`. O tempo de execução armazena essa exceção e a empacota para o contexto que chama `task_group::wait`.
...
O mecanismo de tratamento de exceção é o mesmo para exceções que são geradas pelo tempo de execução como exceções que são geradas por funções de trabalho. Por exemplo, a função [Concurrency:: Receive](reference/concurrency-namespace-functions.md#receive) gera `operation_timed_out` quando não recebe uma mensagem no período de tempo especificado. Se `receive` gera uma exceção em uma função de trabalho que você passa para um grupo de tarefas, o tempo de execução armazena essa exceção e a empacota para o contexto que chama `task_group::wait`, `structured_task_group::wait`, `task_group::run_and_wait`ou `structured_task_group::run_and_wait`.

O exemplo a seguir usa o algoritmo [Concurrency::p arallel_invoke](reference/concurrency-namespace-functions.md#parallel_invoke) para executar duas tarefas em paralelo. A primeira tarefa aguarda cinco segundos e, em seguida, envia uma mensagem para um buffer de mensagens. A segunda tarefa usa a função `receive` para aguardar três segundos para receber uma mensagem do mesmo buffer de mensagem. A função `receive` gera `operation_timed_out` se ela não receber a mensagem no período de tempo.
...
O exemplo a seguir usa o algoritmo `parallel_for` para imprimir números no console. Ele lançará uma exceção se o valor de entrada for menor que um valor mínimo ou maior que um valor máximo. Neste exemplo, várias funções de trabalho podem gerar uma exceção.
...
O exemplo a seguir mostra uma maneira de tratar exceções em uma classe derivada de [Concurrency:: Agent](../../parallel/concrt/reference/agent-class.md). Este exemplo define a classe `points_agent`. O método `points_agent::run` lê `point` objetos do buffer de mensagens e os imprime no console do. O método `run` gera uma exceção se receber um ponteiro de `NULL`.

O método `run` envolve todo o trabalho em um bloco de `catch` -`try`. O bloco de `catch` armazena a exceção em um buffer de mensagens. O aplicativo verifica se o agente encontrou um erro ao ler a partir desse buffer após a conclusão do agente.
