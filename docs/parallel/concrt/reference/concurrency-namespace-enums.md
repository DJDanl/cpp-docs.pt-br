...
||||  
|-|-|-|  
|[Enumeração Agents_EventType](#agents_eventtype)|[Enumeração ConcRT_EventType](#concrt_eventtype)|[Enumeração Concrt_TraceFlags](#concrt_traceflags)|  
|[Enumeração CriticalRegionType](#criticalregiontype)|[Enumeração DynamicProgressFeedbackType](#dynamicprogressfeedbacktype)|[Enumeração PolicyElementKey](#policyelementkey)|  
|[Enumeração SchedulerType](#schedulertype)|[Enumeração SchedulingProtocolType](#schedulingprotocoltype)|[Enumeração SwitchingProxyState](#switchingproxystate)|  
|[Enumeração WinRTInitializationType](#winrtinitializationtype)|[Enumeração agent_status](#agent_status)|[Enumeração join_type](#join_type)|  
|[Enumeração message_status](#message_status)|[Enumeração task_group_status](#task_group_status)|  
  
##  <a name="a-nameagentstatusa--agentstatus-enumeration"></a><a name="agent_status"></a>Enumeração agent_status  
...
##  <a name="a-nameagentseventtypea--agentseventtype-enumeration"></a><a name="agents_eventtype"></a>Enumeração Agents_EventType  
...
##  <a name="a-nameconcrteventtypea--concrteventtype-enumeration"></a><a name="concrt_eventtype"></a>Enumeração ConcRT_EventType  
...
##  <a name="a-nameconcrttraceflagsa--concrttraceflags-enumeration"></a><a name="concrt_traceflags"></a>Enumeração Concrt_TraceFlags  
...
##  <a name="a-namecriticalregiontypea--criticalregiontype-enumeration"></a><a name="criticalregiontype"></a>Enumeração CriticalRegionType  
...
##  <a name="a-namedynamicprogressfeedbacktypea--dynamicprogressfeedbacktype-enumeration"></a><a name="dynamicprogressfeedbacktype"></a>Enumeração DynamicProgressFeedbackType  
 Usado pelo `DynamicProgressFeedback` baseado em políticas para descrever se recursos para o Agendador serão balanceados acordo com informações estatísticas coletadas do Agendador ou apenas em processadores virtuais sai no estado ocioso por meio de chamadas para o `Activate` e `Deactivate` métodos o `IVirtualProcessorRoot` interface. Para obter mais informações sobre políticas de Agendador disponíveis, consulte [enumeração PolicyElementKey](concurrency-namespace-enums.md).  
...
##  <a name="a-namejointypea--jointype-enumeration"></a><a name="join_type"></a>Enumeração join_type  
...
##  <a name="a-namemessagestatusa--messagestatus-enumeration"></a><a name="message_status"></a>Enumeração message_status  
...
##  <a name="a-namepolicyelementkeya--policyelementkey-enumeration"></a><a name="policyelementkey"></a>Enumeração PolicyElementKey  
...
|`SchedulerKind`|O tipo de threads que utiliza o Agendador para contextos de execução subjacente. Para obter mais informações, consulte [enumeração SchedulerType](#schedulertype).<br /><br /> Valores válidos: membro o `SchedulerType` enumeração, por exemplo,`ThreadScheduler`<br /><br /> Valor padrão: `ThreadScheduler`. Isso se traduz em threads de Win32 em todos os sistemas operacionais.|  
|`SchedulingProtocol`|Descreve qual algoritmo de agendamento será usado pelo Agendador. Para obter mais informações, consulte [enumeração SchedulingProtocolType](#schedulingprotocoltype).<br /><br /> Valores válidos: membro do `SchedulingProtocolType` enumeração, `EnhanceScheduleGroupLocality` ou`EnhanceForwardProgress`<br /><br /> Valor padrão:`EnhanceScheduleGroupLocality`|  
...
##  <a name="a-nameschedulertypea--schedulertype-enumeration"></a><a name="schedulertype"></a>Enumeração SchedulerType  
 Usado pelo `SchedulerKind` diretiva descrever o tipo de threads que o Agendador deve utilizar para contextos de execução subjacente. Para obter mais informações sobre políticas de Agendador disponíveis, consulte [enumeração PolicyElementKey](concurrency-namespace-enums.md).  
...
##  <a name="a-nameschedulingprotocoltypea--schedulingprotocoltype-enumeration"></a><a name="schedulingprotocoltype"></a>Enumeração SchedulingProtocolType  
 Usado pelo `SchedulingProtocol` diretiva descrever qual algoritmo de agendamento será utilizado para o Agendador. Para obter mais informações sobre políticas de Agendador disponíveis, consulte [enumeração PolicyElementKey](concurrency-namespace-enums.md).  
...
##  <a name="a-nameswitchingproxystatea--switchingproxystate-enumeration"></a><a name="switchingproxystate"></a>Enumeração SwitchingProxyState  
...
##  <a name="a-nametaskgroupstatusa--taskgroupstatus-enumeration"></a><a name="task_group_status"></a>Enumeração task_group_status  
...
##  <a name="a-namewinrtinitializationtypea--winrtinitializationtype-enumeration"></a><a name="winrtinitializationtype"></a>Enumeração WinRTInitializationType  
 Usado pelo `WinRTInitialization` diretiva descrever se e como o tempo de execução do Windows será inicializado em threads de Agendador para um aplicativo que é executado em sistemas operacionais com a versão do Windows 8 ou superior. Para obter mais informações sobre políticas de Agendador disponíveis, consulte [enumeração PolicyElementKey](concurrency-namespace-enums.md).  
