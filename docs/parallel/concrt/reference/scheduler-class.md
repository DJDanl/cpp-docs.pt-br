...
|Nome|Descrição|  
|----------|-----------------|  
|[Construtor de Agendador](#ctor)|Um objeto de `Scheduler` classe pode ser apenas criados usando métodos de fábrica, ou implicitamente.|  
...
|Nome|Descrição|  
|----------|-----------------|  
|[Método Attach](#attach)|Anexa o Agendador para o contexto de chamada. Depois que este método retorna, o contexto de chamada é gerenciado pelo Agendador e o Agendador atual se torna o Agendador.|  
|[Método Create](#create)|Cria um novo Agendador cujo comportamento é descrito pelo `_Policy` parâmetro, coloca uma referência inicial no Agendador e retorna um ponteiro para ele.|  
|[Método CreateScheduleGroup](#createschedulegroup)|Sobrecarregado. Cria um novo grupo de agenda no Agendador. A versão que aceita o parâmetro `_Placement` faz com que tarefas dentro do grupo de agenda recém-criada para ser mais adequado para a execução no local especificado pelo parâmetro.|  
|[Método GetNumberOfVirtualProcessors](#getnumberofvirtualprocessors)|Retorna o número atual de processadores virtuais para o Agendador.|  
|[Método GetPolicy](#getpolicy)|Retorna uma cópia da política criada com o Agendador.|  
|[Método ID](#id)|Retorna um identificador exclusivo para o Agendador.|  
|[Método IsAvailableLocation](#isavailablelocation)|Determina se um determinado local está disponível no Agendador.|  
|[Método Reference](#reference)|Incrementa a contagem de referência do Agendador.|  
...
|[Método de liberação](#release)|Contagem de referência do programador diminui.|  
|[Método ResetDefaultSchedulerPolicy](#resetdefaultschedulerpolicy)|Redefine a política de agendador padrão para o padrão de tempo de execução. Na próxima vez que um agendador padrão é criado, ele usará as configurações de política do tempo de execução padrão.|  
|[Método ScheduleTask](#scheduletask)|Sobrecarregado. Agenda uma tarefa leve no Agendador. A tarefa leve será colocada em um grupo de agendamento determinado pelo tempo de execução. A versão que aceita o parâmetro `_Placement` faz com que a tarefa a ser mais adequado para a execução no local especificado.|  
...
##  <a name="a-nameattacha-attach"></a><a name="attach"></a>Anexar 
...
##  <a name="a-namectora-scheduler"></a><a name="ctor"></a>Agendador 
