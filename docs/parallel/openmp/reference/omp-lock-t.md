---
title: "omp_lock_t | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "omp_lock_t"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "omp_lock_t OpenMP data type"
ms.assetid: 51b80629-4ffc-4b8a-95c7-1af048f1f286
caps.latest.revision: 7
caps.handback.revision: 7
author: "mikeblome"
ms.author: "mblome"
manager: "ghogen"
---
# omp_lock_t
[!INCLUDE[vs2017banner](../../../assembler/inline/includes/vs2017banner.md)]

Um tipo que mantém o status de um bloqueio, se o bloqueio está disponível ou se um segmento possui um bloqueio.  
  
 As seguintes funções de uso  **omp\_lock\_t**:  
  
-   [omp\_init\_lock](../../../parallel/openmp/reference/omp-init-lock.md)  
  
-   [omp\_destroy\_lock](../../../parallel/openmp/reference/omp-destroy-lock.md)  
  
-   [omp\_set\_lock](../../../parallel/openmp/reference/omp-set-lock.md)  
  
-   [omp\_unset\_lock](../../../parallel/openmp/reference/omp-unset-lock.md)  
  
-   [omp\_test\_lock](../../../parallel/openmp/reference/omp-test-lock.md)  
  
 Para obter mais informações, consulte [3.2 Lock Functions](../../../parallel/openmp/3-2-lock-functions.md).  
  
## Exemplo  
 Consulte [omp\_init\_lock](../../../parallel/openmp/reference/omp-init-lock.md) para obter um exemplo do uso de  **omp\_lock\_t**.  
  
## Consulte também  
 [Data Types](../../../parallel/openmp/reference/openmp-data-types.md)