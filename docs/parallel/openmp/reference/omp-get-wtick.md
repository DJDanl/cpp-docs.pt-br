---
title: omp_get_wtick | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- devlang-cpp
ms.tgt_pltfrm: 
ms.topic: article
f1_keywords:
- omp_get_wtick
dev_langs:
- C++
helpviewer_keywords:
- omp_get_wtick OpenMP function
ms.assetid: 8ca950cd-4952-4081-bc0b-8544dd27ad30
caps.latest.revision: 7
author: mikeblome
ms.author: mblome
manager: ghogen
translation.priority.ht:
- cs-cz
- de-de
- es-es
- fr-fr
- it-it
- ja-jp
- ko-kr
- pl-pl
- pt-br
- ru-ru
- tr-tr
- zh-cn
- zh-tw
translationtype: Machine Translation
ms.sourcegitcommit: 3168772cbb7e8127523bc2fc2da5cc9b4f59beb8
ms.openlocfilehash: 4a32939173b00cdbc0e1caa8426935a6a1a4b8a4
ms.lasthandoff: 02/25/2017

---
# <a name="ompgetwtick"></a>omp_get_wtick
Retorna o número de segundos entre tiques do relógio de processador.  
  
## <a name="syntax"></a>Sintaxe  
  
```  
double omp_get_wtick( );  
```  
  
## <a name="remarks"></a>Comentários  
 Para obter mais informações, consulte [3.3.2 função omp_get_wtick](../../../parallel/openmp/3-3-2-omp-get-wtick-function.md).  
  
## <a name="example"></a>Exemplo  
 Consulte [omp_get_wtime](../../../parallel/openmp/reference/omp-get-wtime.md) para obter um exemplo de como usar `omp_get_wtick`.  
  
## <a name="see-also"></a>Consulte também  
 [Funções](../../../parallel/openmp/reference/openmp-functions.md)
