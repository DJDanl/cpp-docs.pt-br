...
## <a name="step-1-converting-the-project-file"></a><a name="convert_project_file"></a> Etapa 1. Conversão do arquivo de projeto.
...
![A solução de&#43;&#43; do Spy](../porting/media/spyxxsolution.PNG "A solução de&#43;&#43; do Spy")

Temos dois projetos, um com um grande número de arquivos de C++ e o outro com uma DLL que é escrita em C.

## <a name="step-2-header-file-problems"></a><a name="header_file_problems"></a> Etapa 2. Problemas de arquivos de cabeçalho
...
A maneira mais fácil de localizar um símbolo nos arquivos de inclusão disponíveis é usar **Localizar em arquivos** (**Ctrl** + **Shift** + **F**) e especificar **Visual C++ diretórios de inclusão**. Nós o encontramos em ntverp.h. Substituímos a inclusão de verstamp.h por ntverp.h e esse erro desapareceu.

## <a name="step-3-linker-outputfile-setting"></a><a name="linker_output_settings"></a>Etapa 3. Configuração OutputFile do vinculador
...
O MSBuild reclama que a propriedade **link. arquivo_de_saída** não corresponde aos valores **TargetPath** e **TargetName** , emitindo MSB8012.

```Output
warning MSB8012: TargetPath(...\spyxx\spyxxhk\.\..\Debug\SpyxxHk.dll) does not match the Linker's OutputFile property value (...\spyxx\Debug\SpyHk55.dll). This may cause your project to build incorrectly. To correct this, please make sure that $(OutDir), $(TargetName) and $(TargetExt) property values match the value specified in %(Link.OutputFile).warning MSB8012: TargetName(SpyxxHk) does not match the Linker's OutputFile property value (SpyHk55). This may cause your project to build incorrectly. To correct this, please make sure that $(OutDir), $(TargetName) and $(TargetExt) property values match the value specified in %(Link.OutputFile).
```

**Link.OutputFile** é a saída de build (por exemplo, EXE, DLL) e normalmente é construído de `$(TargetDir)$(TargetName)$(TargetExt)`, fornecendo o caminho, o nome do arquivo e a extensão. Esse é um erro comum ao migrar projetos da ferramenta de build do Visual C++ antiga (vcbuild.exe) para a nova ferramenta de build (MSBuild.exe). Visto que a alteração da ferramenta de build ocorreu no Visual Studio 2010, você poderá encontrar esse problema sempre que migrar um projeto de versão anterior a 2010 para uma versão 2010 ou posterior. O problema básico é que o assistente de migração de projeto não atualiza o valor de **link. OutputType** , pois nem sempre é possível determinar o que seu valor deve ter com base nas outras configurações do projeto. Portanto, você geralmente precisa defini-lo manualmente. Para obter mais detalhes, consulte esta [postagem](https://devblogs.microsoft.com/cppblog/visual-studio-2010-c-project-upgrade-guide/) no blog do Visual C++.

Nesse caso, a propriedade **Link.OutputFile** no projeto convertido foi definida como .\Debug\Spyxx.exe e .\Release\Spyxx.exe para o projeto Spy++, dependendo da configuração. A melhor opção é simplesmente substituir esses valores codificados por `$(TargetDir)$(TargetName)$(TargetExt)` para **Todas as Configurações**. Se isso não funcionar, você poderá personalizar a partir daí ou alterar as propriedades na seção **geral** em que esses valores estão definidos (as propriedades são **diretório de saída**, **nome de destino**e **extensão de destino**. Lembre-se de que se a propriedade que você está exibindo utiliza macros, você pode escolher **Editar** na lista suspensa para exibir uma caixa de diálogo que mostra a cadeia de caracteres final com as substituições de macro feitas. Você pode exibir todas as macros disponíveis e seus valores atuais, escolhendo o botão **Macros**.

## <a name="step-4-updating-the-target-windows-version"></a><a name="updating_winver"></a>Etapa 4. Atualizando a Versão de Destino do Windows
...
Definiremos WINVER para o Windows 7. É mais fácil ler o código posteriormente se você usar a macro para o Windows 7 (_WIN32_WINNT_WIN7), em vez do próprio valor (0x0601).

```cpp
#define WINVER _WINNT_WIN32_WIN7 // Minimum targeted Windows version is Windows 7
```

## <a name="step-5-linker-errors"></a><a name="linker_errors"></a>Etapa 5. Erros de Vinculador
...
O ponto de entrada para uma DLL não deve ser exportado. O ponto de entrada é destinado apenas para ser chamado pelo carregador de quando a DLL é carregada pela primeira vez na memória, portanto, ele não deve estar na tabela de exportação, a qual é para outros chamadores. Precisamos apenas garantir que ele não tenha a **`__declspec(dllexport)`** diretiva anexada a ele. Em spyxxhk.c, precisamos removê-lo de dois locais, a declaração e a definição de `DLLEntryPoint`. Nunca fez sentido usar esta diretiva, mas versões anteriores do compilador e vinculador não sinalizaram isso como um problema. As versões mais recentes do vinculador dão um aviso.
...
## <a name="step-6-more-outdated-header-files"></a><a name="outdated_header_files"></a>Etapa 6. Mais arquivos de cabeçalho desatualizados

Agora começamos a trabalhar no principal projeto executável, Spyxx.

Não foi possível encontrar um par de outros arquivos de inclusão: ctl3d.h e penwin.h. Embora possa ser útil Pesquisar a Internet para tentar identificar o que incluía o cabeçalho, às vezes, as informações não são úteis. Descobrimos que ctl3d.h fazia parte do Kit de desenvolvimento do Exchange e dava suporte para um determinado estilo dos controles no Windows 95, enquanto penwin.h está relacionado a Window Pen Computing, uma API obsoleta. Nesse caso, simplesmente comentamos a linha `#include` e lidamos com os símbolos indefinidos como fizemos com verstamp.h. Tudo que se relaciona aos Controles 3D ou Pen Computing foi removido do projeto.
...
## <a name="step-7-updating-old-iostreams-code"></a><a name="updating_iostreams_code"></a>Etapa 7. Atualizando o código antigo de iostreams
...
Com essa alteração, temos problemas com `ostrstream`, que não é mais usado. A substituição adequada é ostringstream. Tentamos adicionar um **`typedef`** para `ostrstream` para evitar modificar o código muito, pelo menos como um início.

```cpp
typedef std::basic_ostringstream<TCHAR> ostrstream;
```

Atualmente, o projeto é criado usando MBCS (conjunto de caracteres de vários bytes), portanto, **`char`** é o tipo de dados de caractere apropriado. No entanto, para permitir uma atualização mais fácil do código para Unicode UTF-16, nós atualizamos isso para `TCHAR` , que **`char`** é resolvido para ou **`wchar_t`** dependendo se a propriedade do **conjunto de caracteres** nas configurações do projeto está definida como MBCS ou Unicode.

Alguns outros trechos de código precisam ser atualizados.  Substituímos a classe base `ios` por `ios_base` e substituímos ostream por basic_ostream \<T> . Adicionamos duas definições de tipo adicionais e esta seção é compilada.
...
Este é o próximo erro.

```Output
error C2039: 'freeze': is not a member of 'std::basic_stringbuf<char,std::char_traits<char>,std::allocator<char>>'
```

O próximo problema é que o `basic_stringbuf` não tem um `freeze` método. O método `freeze` é usado para evitar um vazamento de memória no `ostream` antigo. Não precisamos disso agora que estamos usando o novo `ostringstream` . Podemos excluir a chamada para `freeze`.

```cpp
//rdbuf()->freeze(0);
```

Os próximos dois erros ocorreram em linhas adjacentes. A primeira reclamações sobre como usar `ends` , que é o `iostream` manipulador de e/s da biblioteca antiga que adiciona um terminador nulo a uma cadeia de caracteres. O segundo desses erros explica que a saída do `str` método não pode ser atribuída a um ponteiro não const.
...
Usando a nova biblioteca de fluxo, `ends` não é necessário porque a cadeia de caracteres sempre é terminada em nulo, portanto, essa linha pode ser removida. Para o segundo problema, o problema é que agora `str()` não retorna um ponteiro para a matriz de caracteres de uma cadeia de caracteres; ele retorna o `std::string` tipo. A solução para o segundo problema é alterar o tipo para `LPCSTR` e usar o método `c_str()` para solicitar o ponteiro.
...
## <a name="step-8-the-compilers-more-strict-conversions"></a><a name="stricter_conversions"></a>Etapa 8. As conversões mais restritas do compilador
...
Como há cerca de dez ocorrências dessa função em diferentes classes derivadas de CWnd, é útil usar **ir para definição** (teclado: **F12**) e **ir para a declaração** (teclado: **Ctrl** + **F12**) quando o cursor estiver na função no editor para localizá-los e navegar para eles na janela **Localizar símbolo** . **Ir Para Definição** é geralmente o mais útil dos dois. **Ir Para Declaração** localizará declarações que não sejam a declaração de classe definidora, assim como declarações de classe friend ou referências de encaminhamento.

## <a name="step-9-mfc-changes"></a><a name="mfc_changes"></a>Etapa 9. Alterações do MFC
...
## <a name="step-10-addressing-compiler-warnings"></a><a name="compiler_warnings"></a>Etapa 10. Tratando dos avisos do compilador
...
Esse código foi escrito antes que o tipo bool interno tivesse suporte no Visual C++. Nesse código, BOOL era um **`typedef`** para **`int`** . O tipo **`int`** é um **`signed`** tipo, e a representação de bit de a **`signed int`** é usar o primeiro bit como um bit de sinal, de modo que um campo de bits do tipo int poderia ser interpretado como representando 0 ou-1, provavelmente não é o que pretendia.
...
Você pode se perguntar se usar o tipo padrão **`bool`** por enquanto seria útil. Muitos dos padrões de código antigos, como o tipo BOOL, foram inventados para resolver problemas que foram resolvidos mais tarde no C++ padrão. portanto, alterar de BOOL para o **`bool`** tipo interno é apenas um exemplo de tal alteração que você considerar depois de colocar seu código inicialmente em execução na nova versão.
...
Outros avisos que recebemos foram úteis para limpeza de código geral. Há várias conversões implícitas de **`int`** ou **`unsigned int`** para Word (que é um typedef para **`unsigned short`** ). Elas envolvem uma possível perda de dados. Adicionamos uma conversão para WORD nesses casos.
...
LNK1181 ocorre porque uma versão desatualizada da biblioteca estática do mfc está incluída na entrada do vinculador. Isso não é mais necessário porque podemos vincular o MFC dinamicamente, portanto, precisamos apenas remover todas as bibliotecas estáticas do MFC da propriedade **Input** na seção do **vinculador** das propriedades do projeto. Este projeto também está usando a opção `/NODEFAULTLIB` e, em vez disso, ele lista todas as dependências de biblioteca.
...
Alguns locais no código já estavam usando TCHAR, aparentemente em antecipação à possibilidade de eventualmente dar suporte a Unicode. Alguns não estavam. Pesquisamos instâncias de CHAR, que é uma **`typedef`** para **`char`** e substituídas a maioria delas por TCHAR. Além disso, procuramos por `sizeof(CHAR)`. Quando alteramos de CHAR para TCHAT, geralmente precisamos alterar para `sizeof(TCHAR)`, já que isso foi utilizado frequentemente para determinar o número de caracteres em uma cadeia de caracteres. Usar o tipo errado aqui não produz um erro do compilador, portanto, vale a pena prestar um pouco de atenção a este caso.
...
A \_ macro T tem o efeito de fazer com que uma cadeia de caracteres literal seja compilada como uma cadeia de caracteres **`char`** ou uma **`wchar_t`** cadeia de caracteres, dependendo da configuração de MBCS ou Unicode. Para substituir todas as cadeias de caracteres por \_ T no Visual Studio, primeiro abra a caixa **substituição rápida** (teclado: **Ctrl** + **F**) ou **substitua em arquivos** (teclado: **Ctrl** + **Shift** + **H**) e, em seguida, escolha a caixa de seleção **usar expressões regulares** . Digite `((\".*?\")|('.+?'))` como o texto de pesquisa e `_T($1)` como o texto de substituição. Se você já tiver a \_ macro T em algumas cadeias de caracteres, esse procedimento irá adicioná-la novamente e também poderá encontrar casos em que você não deseja que o \_ T, como quando você usa `#include` , portanto, é melhor usar **substituir avançar** em vez de **substituir tudo**.
...
Outro erro comum que você verá na conversão para Unicode é isso.
...
Embora a `_tcscpy` função tenha sido usada, que é a função STRCPY TCHAR para copiar uma cadeia de caracteres, o buffer que foi alocado era um **`char`** buffer. Isso é facilmente alterado para TCHAR.
...
Em muitos casos foi necessário usar a versão genérica (macro) de uma API Win32 como `GetClassName` (em vez de `GetClassNameA`). Na instrução switch do manipulador de mensagens, algumas mensagens são MBCS ou Unicode específicas, nesses casos, tivemos que alterar o código para chamar explicitamente a versão MBCS, porque substituímos as funções nomeadas genericamente por funções específicas a e **w** , e adicionamos uma macro para o nome genérico que é resolvido para o nome correto **de** a ou **w** **com base** no fato de o Unicode ser definido.  Em muitas partes do código, quando mudamos para definir \_ o Unicode, a versão W agora é escolhida mesmo quando a **A** versão a é desejada.
...
Como uma verificação em nosso trabalho, devemos redefinir o **conjunto de caracteres** para **usar um conjunto de caracteres multibyte** e garantir que o código ainda seja compilado usando MBCS, bem como Unicode. Obviamente, um teste completo deve ser executado no aplicativo recompilado depois de todas essas alterações.
...
## <a name="step-12-porting-to-use-the-secure-crt"></a><a name="porting_to_secure_crt"></a>Etapa 12. Portabilidade para usar o CRT Seguro

Portar o código para usar as versões seguras (as versões com o sufixo **_S** ) de funções de CRT é a próxima. Nesse caso, a estratégia geral é substituir a função pela versão **_S** e, em seguida, adicionar os parâmetros de tamanho de buffer adicionais necessários. Em muitos casos isso é fácil, já que o tamanho é conhecido. Em outros casos, em que o tamanho não está disponível imediatamente, é necessário adicionar parâmetros adicionais à função que está usando a função CRT ou talvez examinar o uso do buffer de destino e ver quais são os limites de tamanho apropriados.
...
Quando você define isso, sempre que o buffer é uma matriz e não um ponteiro bruto, seu tamanho é inferido do tipo de matriz e isso é usado como o parâmetro de tamanho, sem que seja necessário fornecê-lo. Que ajuda a reduzir a complexidade de reescrever o código. Você ainda precisa substituir o nome da função pela versão **_S** , mas isso geralmente pode ser feito por uma operação de pesquisa e substituição.
...
## <a name="step-13-zcforscope--is-deprecated"></a><a name="deprecated_forscope"></a>Etapa 13. /Zc:forScope- foi preterido
