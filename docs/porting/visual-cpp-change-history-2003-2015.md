...
-   **Formatação de Infinito e NaN** Nas versões anteriores, infinitos e NaNs eram formatados usando um conjunto de cadeias de caracteres de sentinela específico do MSVC.  
...
-   **steady_clock** A implementação \<chrono> de [steady_clock](../standard-library/steady-clock-struct.md) foi alterada para atender aos requisitos de constância e monotonicidade do Padrão C++. O steady_clock agora baseia-se em [QueryPerformanceCounter](https://msdn.microsoft.com/library/windows/desktop/ms644904.aspx) e high_resolution_clock agora é um typedef para steady_clock. Como resultado, no Visual Studio steady_clock::time_point agora é um typedef para chrono::time_point<steady_clock>, no entanto, esse não é, necessariamente, o caso de outras implementações.  
  
-   **Alocadores and const** Agora exigimos que as comparações de igualdade/desigualdade de alocador aceitem argumentos const em ambos os lados.  Se seu alocadores definem esses operadores da seguinte maneira:  
...
-   **Elementos const** O padrão C++ sempre proibiu contêineres de elementos const (como vector\<const T> ou set\<const T>). O Visual Studio 2013 e versões anteriores aceitavam esses contêineres. Na versão atual, esses contêineres falham ao compilar.  
  
-   **std::allocator::deallocate** No Visual Studio 2013 e versões anteriores, o std::allocator::deallocate(p, n) ignorava o argumento passado para n.  O padrão C++ sempre exigiu que n fosse igual ao valor passado como o primeiro argumento para a invocação de alocar que retornava p. No entanto, na versão atual, o valor de n é inspecionado. O código que passa argumentos para n que diferem do que o padrão exige pode falhar em tempo de execução.  
  
-   **hash_map e hash_set** Os arquivos de cabeçalho não padrão hash_map e hash_set estão preteridos no Visual Studio 2015 e serão removidos em uma versão futura. Em vez disso, use o unordered_map e o unordered_set.  
...
-   **Características de tipo** Os nomes antigos para características de tipo de uma versão anterior do padrão de rascunho C++ foram removidos. Elas foram alteradas no C++11 e foram atualizadas para os valores de C++11 no Visual Studio 2015. A tabela a seguir mostra os nomes antigos e os novos.  
...
## <a name="compiler-conformance-improvements-in-visual-studio-2015"></a>Aprimoramentos de conformidade do compilador no Visual Studio 2015  
 ...
 Algumas vezes, os aprimoramentos contínuos no Visual Studio 2015 para conformidade do compilador podem alterar a maneira pela qual o compilador compreende seu código-fonte existente. Quando isso acontecer, você poderá encontrar erros novos ou diferentes durante o build ou até diferenças de comportamento no código que foi compilado anteriormente e parecia executar corretamente.  
...
-   **std::is_convertable agora detecta a autoatribuição** (biblioteca padrão)  
    ...
    ```  
     ...
     Nas versões anteriores do compilador, as asserções estáticas na parte inferior deste exemplo passam porque `std::is_convertable<>::value` foi definido incorretamente como `true`. Agora `std::is_convertable<>::value` está definido corretamente como `false`, fazendo com que as asserções estáticas falhem.  
...
-   **Preterimento do suporte a código ATL atribuído** (Nível 1 (/ W1) ativado por padrão)  
     ...
     As versões anteriores do compilador ofereciam suporte ao código ATL atribuído. Como um passo seguinte da remoção do suporte ao código ATL atribuído que [começou no Visual Studio 2008](https://msdn.microsoft.com/library/bb384632\(v=vs.90\).aspx), o código ATL atribuído foi preterido. Agora o compilador emite o aviso do compilador C4467 para ajudar a identificar esse tipo de código preterido.  
...
-   O Padrão C++ não permite a especialização explícita em uma classe. Embora o compilador do Microsoft Visual C++ permita isso em alguns casos, em casos como no exemplo a seguir, um erro será gerado porque o compilador não considera a segunda função como sendo uma especialização da primeira.  
...
-   Antes de colocar o compilador em conformidade com o ISO C++11, o seguinte código seria compilado e faria com que x fosse resolvido para o tipo int:  
...
-   O nome da pesquisa foi alterada. O código a seguir é resolvido de forma diferente no compilador C++ no Visual Studio 2012 e no Visual Studio 2013:  
    ...
    ```  
     ...
     No Visual Studio 2012, o E1 na expressão E1::b foi resolvido para ::E1 no escopo global. No Visual Studio 2013, o E1 na expressão E1::b resolve-se na definição typedef E2 em main() e tem o tipo ::E2.  
...
-   No Visual Studio 2013, o resultado de sizeof(S2) no x64 é 48, mas em versões anteriores, ele era avaliado como 32. Para fazer isso ser avaliado como 32 no compilador C++ no Visual Studio 2013 para x64, adicione uma classe base fictícia que tenha uma função virtual:  
    ...
    ```  
     ...
     Para encontrar locais no seu código que uma versão anterior tentou otimizar, use um compilador dessa versão juntamente com a opção do compilador /W3 e ative o Aviso 4370. Por exemplo:  
    ...
    ```  
     ...
     O resultado de sizeof(S) é 24. No entanto, isso pode ser reduzido para 16 se você usar a solução alternativa que acabou de ser mencionada para x64:  
...
-   Se o seu código existente usa enumerações com escopo simuladas da versão anterior — enumerações sem escopo tradicionais envolvidas em namespaces — será necessário alterá-lo. Por exemplo, se você fez referência ao tipo std::future_status::future_status, agora você terá que declarar std::future_status. No entanto, a maior parte do código não é afetada, por exemplo, std::future_status::ready ainda é compilado.  
  
-   operador explícito bool() é mais estrito que o operador unspecified-bool-type(). operador explícito bool() permite conversões explícitas para bool — por exemplo, dado shared_ptr\<X> sp, ambos static_cast\<bool>(sp) e bool b(sp) são válidos — e "conversões contextuais" para bool testáveis por booliano — por exemplo, if (sp), !sp, sp e qualquer outro. No entanto, o operador explícito bool() proíbe conversões implícitas para bool, portanto, você não pode declarar bool b = sp. E para um tipo de retorno booleano determinado, você não pode declarar return sp.  
...
-   Além de palavras-chave comuns, os cabeçalhos da Biblioteca Padrão do C++ agora proíbem a criação de macros das palavras-chave "override" e "final" sensíveis ao contexto.  
...
-   A Biblioteca Padrão do C++ não oferece suporte para /clr:oldSyntax.  
...
-   A especificação C++11 para common_type<> teve consequências inesperadas e indesejadas. Particularmente, ela faz common_type\<int, int>::type retornar int&&. Portanto, o compilador implementa a publicação 2141 da Resolução proposta para o grupo de trabalho de bibliotecas, que faz common_type\<int, int="">::type retornar int.  
     ...
     Como um efeito colateral dessa alteração, o caso de identidade não funciona mais (common_type\<T> nem sempre resulta no tipo T). Isso segue a resolução proposta, mas interrompe qualquer código que depende do comportamento anterior.  
...
-   As assinaturas são alteradas para acomodar manipuladores de mensagens. As listas de parâmetros das seguintes funções foram alteradas para usar manipuladores de mensagens ON_WM_* recém-adicionados:  
    ...
    -   CWnd::OnDisplayChange alterado para (UINT, int, int) em vez de (WPARAM, LPARAM) para que a nova macro ON_WM_DISPLAYCHANGE possa ser usada no mapa de mensagens.  
  
    -   CFrameWnd::OnDDEInitiate alterado para (CWnd*, UINT, UNIT) em vez de (WPARAM, LPARAM) para que a nova macro ON_WM_DDE_INITIATE possa ser usada no mapa de mensagens.  
  
    -   CFrameWnd::OnDDEExecute alterado para (CWnd*, HANDLE) em vez de (WPARAM, LPARAM) para que a nova macro ON_WM_DDE_EXECUTE possa ser usada no mapa de mensagens.  
  
    -   CFrameWnd::OnDDETerminate alterado para (CWnd*) como o parâmetro, em vez de (WPARAM, LPARAM) para que a nova macro ON_WM_DDE_TERMINATE possa ser usada no mapa de mensagens.  
...
-   \#ifdefs nos arquivos de cabeçalho MFC foram removidos. Vários #ifdefs nos arquivos de cabeçalho MFC relacionados a versões sem suporte do Windows (WINVER &lt; 0x0501) foram removidos.  
...
-   No código que é compilado usando /clr, a palavra-chave de classe enum define um enum de C++11 e não um enum de CLR (Common Language Runtime). Para definir um enum de CLR, você deve ser explícito sobre sua acessibilidade.  
...
-   Quando o novo sinalizador de vinculador /HIGHENTROPYVA for especificado, o Windows 8 geralmente faz com que as alocações de memória retornem um endereço de 64 bits.                 (Antes do Windows 8, essas alocações retornavam, com mais frequência, endereços que eram menores que 2 GB).  Isso pode expor bugs de truncamento de ponteiro no código existente. Por padrão, essa opção está ativada.  Para desabilitar esse comportamento, especifique /HIGHENTROPYVA:NO.  
...
-   Outra alteração significativa entre os padrões C++98/03 e C++11: quando A é implicitamente conversível para B e B é implicitamente conversível para C, mas A não é implicitamente conversível para C, O C++98/03 and Visual C++ 2010 permitia que pair\<A, X> fosse convertido (implícita ou explicitamente) para pair\<C, X>. (O tipo X não é de interesse aqui e isso não é específico ao primeiro tipo no par.) Como o C++11 e o compilador C++ no Visual Studio 2012 detectam que A não é implicitamente conversível para C, eles removem a conversão de par da resolução de sobrecarga. Essa é uma alteração positiva para muitos cenários. Por exemplo, a sobrecarga de func(const pair\<int, int>&) e func(const pair\<string, string>&) e chamar func() com pair\<const char *, const char \*> compilará com essa alteração. No entanto, essa alteração interrompe o código que baseava-se em conversões de par agressivas. Esse código geralmente pode ser corrigido executando explicitamente uma parte da conversão — por exemplo, ao passar make_pair(static_cast\<B>(a), x) para uma função que espera pair\<C, X>.  
...
-   O C++11 17.6.4.3.1 [macro.names]/2 proíbe palavras-chave em macros quando os cabeçalhos da Biblioteca Padrão do C++ estão incluídos. Os cabeçalhos agora emitem erros do compilador se detectam palavras-chave em macros. (A definição de _ALLOW_KEYWORD_MACROS permite que esse código seja compilado, mas nós não recomendamos esse uso). Como uma exceção, o new em macro é permitido por padrão, porque os cabeçalhos se defendem de forma abrangente usando #pragma push_macro("new")/#undef new/#pragma pop_macro("new"). A definição de _ENFORCE_BAN_OF_MACRO_NEW faz exatamente o que seu nome sugere.  
  
-   Para implementar várias otimizações e verificações de depuração, a implementação da Biblioteca Padrão do C++ interrompe intencionalmente a compatibilidade binária entre as versões do Visual Studio (2005, 2008, 2010, 2012). Quando a Biblioteca Padrão do C++ é usada, isso proíbe a mistura de arquivos de objeto e das bibliotecas estáticas que são compiladas usando versões diferentes em um binário (EXE ou DLL) e proíbe a passagem dos objetos da Biblioteca Padrão do C++ entre binários que são compilados usando versões diferentes. A mistura de arquivos-objetos e bibliotecas estáticas usando a Biblioteca Padrão do C++ que foi compilada usando o Visual C++ 2010 com aquelas que foram compiladas usando o compilador C++ no Visual Studio 2012, emite erros de vinculador sobre a incompatibilidade de _MSC_VER, em que _MSC_VER é a macro que contém a versão principal do compilador (1700 para o Visual C++ no Visual Studio 2012). Essa verificação não detecta combinação DLL e não pode detectar uma combinação que envolva Visual C++ 2008 ou anterior.  
...
-   Os operadores operator\<(), operator>(), operator\<=(), and operator>=() estavam disponíveis anteriormente para as famílias de contêineres std::unordered_map e stdext::hash_map, embora suas implementações não eram realmente úteis. Esses operadores não padrão foram removidos do Visual C++ no Visual Studio 2012. Além disso, a implementação do operator==() e operator!=() para a família std::unordered_map foi estendida para cobrir a família stdext::hash_map. (É recomendável que você evite o uso da família stdext::hash_map no novo código.)  
  
-   O C++11 22.4.1.4 [locale.codecvt] especifica que codecvt::length() e codecvt::do_length() devem obter parâmetros modificáveis stateT&, mas o Visual C++ 2010 obteve const stateT&. O compilador C++ no Visual Studio 2012 usa stateT& conforme designado pelo padrão. Essa diferença é significativa para qualquer pessoa que esteja tentando substituir a função virtual do_length().  
...
-   A assinatura de CFileDialog::SetDefExt foi alterada para usar LPCTSTR. Portanto, as compilações de Unicode foram afetadas.  
...
-   Foi adicionado um parâmetro aos construtores CMFCRibbonStatusBarPane. (É um parâmetro padrão e, portanto, não causa interrupção de fonte).  
  
-   Um parâmetro foi adicionado para o construtor CMFCRibbonCommandsListBox. (É um parâmetro padrão e, portanto, não causa interrupção de fonte).  
  
-   A API AFXTrackMouse foi removida (e o timer proc relacionado). Use a API do Win32 TrackMouseEvent no lugar.  
  
-   Um parâmetro para o construtor CFolderPickerDialog foi adicionado. (É um parâmetro padrão e, portanto, não causa interrupção de fonte).  
  
-   O tamanho da estrutura CFileStatus foi alterado: O membro m_attribute foi alterado de BYTE para DWORD (para corresponder ao valor que é retornado de GetFileAttributes).  
  
-   CRichEditCtrl e CRichEditView usam MSFTEDIT_CLASS (controle RichEdit 4.1) em vez de RICHEDIT_CLASS (controle RichEdit 3.0) em compilações de Unicode.  
  
-   AFX_GLOBAL_DATA::IsWindowsThemingDrawParentBackground foi removido porque ele é sempre TRUE no Windows Vista, Windows 7 e Windows 8.  
  
-   AFX_GLOBAL_DATA::IsWindowsLayerSupportAvailable foi removido porque ele é sempre TRUE no Windows Vista, Windows 7 e Windows 8.  
...
-   AFX_GLOBAL_DATA::DwmIsCompositionEnabled foi renomeado para IsDwmCompositionEnabled para eliminar a colisão de nomes.  
...
-   Vários objetos globais estáticos CString foram removidos para eliminar vazamentos de memória em algumas situações (substituídos por #defines) e as seguintes variáveis de membro de classe:  
...
-   CPropertyPage::GetParentSheet foi adicionado e, na classe CPropertyPage, chame-o em vez de chamar GetParent para obter a janela de folha pai correta, que pode ser uma janela pai ou avô para CPropertyPage. Você pode ter que alterar o código para chamar GetParentSheet em vez de GetParent.  
...
-   O método RemoveButtonByIndex foi adicionado para a classe CMFCOutlookBarPane.  
...
-   Métodos não utilizados de CDockablePane foram removidos: SetCaptionStyle, IsDrawCaption, IsHideDisabledButtons, GetRecentSiblingPaneInfo e CanAdjustLayout.  
  
-   Variáveis de membro estático CDockablePane foram removidas de m_bCaptionText e m_bHideDisabledButtons.  
  
-   Um método DeleteString de substituição foi adicionado para CMFCFontComboBox.  
  
-   Os métodos não utilizados de CPane foram removidos: GetMinLength e IsLastPaneOnLastRow.  
...
-   No cabeçalho \<algorithm>, as funções checked_* e unchecked_\* foram removidas. E, no cabeçalho \<iterator>>, a checked_iteratorclass foi removida e a classe unchecked_array_iterator foi adicionada.  
  
-   O construtor CComPtr::CComPtr(int) foi removido. Esse construtor permitia que um objeto CComPtr fosse construído da macro NULL, mas era desnecessário e permitia construções absurdas de inteiros diferentes de zero.  
  
     Um CComPtr ainda pode ser construído de NULL, que é definida como 0, mas falhará se for construído de um valor inteiro diferente de 0 literal. Em vez disso, use nullptr.  
...
-   A partir do Visual Studio 2008, o assistente de novo projeto não tem uma opção para criar um projeto do SQL Server do C++. Os projetos do SQL Server criados usando uma versão anterior do Visual Studio ainda serão compilados e funcionarão corretamente.  
...
-   As seguintes funções foram removidas. Use as funções de API do Windows GetVersion ou GetVersionEx no lugar delas:  
...
-   Classe CWnd e classe CFrameWnd: o método CWnd::GetMenuBarInfo foi removido.  
     ...
     O método CFrameWnd::GetMenuBarInfo agora é um método não virtual. Para obter mais informações, consulte Função de GetMenuBarInfo no SDK do Windows.  
  
-   Suporte a ISAPI do MFC: o MFC não oferece mais suporte ao build de aplicativos com a ISAPI (interface ISAPI). Se você deseja criar um aplicativo ISAPI, chame diretamente as extensões ISAPI.  
...
-   Agora, muitas funções validam seus parâmetros, interrompendo a execução se determinados parâmetros forem inválidos. Isso pode interromper o código que passa parâmetros inválidos e depende da função ignorá-los ou apenas retornar um código de erro. Consulte Validação de Parâmetro.  
...
-   O especificador de formato %n não tem mais suporte por padrão em qualquer uma das famílias ou funções printf porque é inerentemente inseguro. O comportamento padrão se %n for encontrado será invocar o manipulador de parâmetro inválido. Para habilitar o suporte a %n, use _set_printf_count_output (e também veja _get_printf_count_output).  
...
-   time_t agora é um valor de 64 bits (a menos que _USE_32BIT_TIME_T for definido).  
  
-   As funções _spawn, _wspawn agora deixam errno inalterado com o êxito, conforme especificado pelo Padrão C.  
...
-   A inicialização de um typedef não é permitida pelo padrão e agora gera um erro do compilador ([Erro do Compilador C2513](../error-messages/compiler-errors-2/compiler-error-c2513.md)).  
...
-   A especialização explícita de função de membro fora da classe não será válida se a função já foi especializada explicitamente por meio de uma especialização de modelo de classe ([Erro do Compilador C2910](../error-messages/compiler-errors-2/compiler-error-c2910.md)).  
