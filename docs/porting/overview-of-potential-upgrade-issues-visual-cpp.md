...
- Se o arquivo de origem estiver em um local não padrão, você adicionou o caminho aos diretórios de inclusão do projeto?
...
- Os tipos dos argumentos no site de chamada realmente coincidem com uma sobrecarga existente da função? Verifique os tipos subjacentes para quaisquer tipos na assinatura da função e no código que chama a função são o que você espera que eles sejam.
...
A tabela a seguir mostra as bibliotecas cujo conteúdo foi alterado começando com o Visual Studio 2015. Para atualizar, é necessário adicionar os novos nomes da biblioteca na segunda coluna às bibliotecas na primeira coluna. Algumas dessas bibliotecas são bibliotecas de importação, mas isso não deve importar.
...
Você pode pressionar **F12** **(Go to Definition)** para ver onde o tipo em questão é definido.
...
Foram feitas várias alterações no runtime de C ao longo dos anos. Muitas versões seguras de funções foram adicionadas e algumas foram removidas. Além disso, como descrito anteriormente neste artigo, a implementação do CRT pela Microsoft foi refatorada no Visual Studio 2015 em novos binários e arquivos .lib associados.
...
Ao longo dos anos, foram introduzidas versões seguras de funções de runtime de C. Embora as versões antigas e não seguras ainda estejam disponíveis, é recomendável alterar o código para usar as versões seguras. O compilador emitirá um aviso para o uso das versões não seguras. Você pode optar por desabilitar ou ignorar esses avisos. Para desativar o aviso para todos os projetos da sua solução, abra **o View** > **Property Manager,** selecione todos os projetos para os quais deseja desativar o aviso e clique com o botão direito do mouse sobre os itens selecionados e escolha **Propriedades**. Na caixa de diálogo **Páginas de Propriedades** em **Propriedades de Configuração** > **C/C++** > **Avançado**, selecione **Desabilitar Avisos Específicos**. Clique na seta para dado e clique em **Editar**. Digite 4996 na caixa de texto. (Não inclua o prefixo 'C'.) Para obter mais informações, consulte [Porting para usar o Secure CRT](porting-guide-spy-increment.md#porting_to_secure_crt).
...
Antes de o Unicode ser padronizado, muitos programas usavam o MBCS (Conjunto de Caracteres Multibyte) para representar os caracteres que não estavam incluídos no conjunto de caracteres ASCII. Em projetos do MFC mais antigos, o MBCS era a configuração padrão e ao atualizar tal programa, você verá avisos que recomendam usar o Unicode em vez disso. Você poderá optar por desabilitar ou ignorar o aviso se decidir que a conversão para Unicode não vale a pena o custo de desenvolvimento. Para desabilitá-lo para todos os projetos da solução, abra **o View** > **Property Manager,** selecione todos os projetos para os quais deseja desativar o aviso e clique com o botão direito do mouse sobre os itens selecionados e escolha **Propriedades**. Na caixa de diálogo **Páginas de Propriedades**, selecione **Propriedades de Configuração** > **C/C++** > **Avançado**. Na propriedade **Desabilitar Avisos Específicos**, abra a seta suspensa e escolha **Editar**. Digite 4996 na caixa de texto. (Não inclua o prefixo 'C'.) Escolha **OK** para salvar a propriedade e escolha **OK** para salvar suas alterações.

Para obter mais informações, consulte [Porting from MBCS to Unicode](porting-guide-spy-increment.md#porting_to_unicode) (Portabilidade de MBCS para Unicode). Para obter informações gerais sobre MBCS vs. Unicode, consulte [Texto e Cordas no Visual C++](../text/text-and-strings-in-visual-cpp.md) e [Internacionalização](../c-runtime-library/internationalization.md) .
