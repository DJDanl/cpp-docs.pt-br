---
title: Símbolos predefinidos MFC
ms.date: 02/14/2019
helpviewer_keywords:
- MFC symbols
- symbols [C++], MFC
...
ms.locfileid: "88846297"
---
# <a name="mfc-predefined-symbols"></a>Símbolos predefinidos MFC

Os projetos MFC sempre incluem vários arquivos de cabeçalho que dão suporte ao Windows. Eles são adicionados por meio `#include` de instruções no `StdAfx.h` arquivo:
...
Os arquivos de cabeçalho incluem valores de ID de símbolo para valores comuns do MFC. Esses símbolos só estão disponíveis quando você está trabalhando em um projeto do MFC. O `AFX_` prefixo é seguido pelos prefixos de nome de símbolo padrão.
