---
title: Extensões específicas de C++/CLI
ms.date: 10/12/2018
ms.topic: reference
ms.assetid: 81e3fd32-6c8f-490b-9190-a6dc291545c8
ms.openlocfilehash: 34c9d48c10480d8afcd301b20909773f7585724d
ms.sourcegitcommit: 6052185696adca270bc9bdbec45a626dd89cdcdd
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 10/31/2018
ms.locfileid: "50630926"
---
# <a name="extensions-that-are-specific-to-ccli"></a>Extensões específicas de C++/CLI

Os seguintes recursos de idioma se aplicam somente a C + + c++ CLI:

[__identifier (C++/CLI)](../windows/identifier-cpp-cli.md)

[Conversões C-Style com /clr (C + + / CLI)](../windows/c-style-casts-with-clr-cpp-cli.md)

[interior_ptr (C++/CLI)](../windows/interior-ptr-cpp-cli.md)

[pin_ptr (C++/CLI)](../windows/pin-ptr-cpp-cli.md)

[Encaminhamento de tipo (C++/CLI)](../windows/type-forwarding-cpp-cli.md)

[Listas de argumentos variáveis (...) (C++/CLI)](../windows/variable-argument-lists-dot-dot-dot-cpp-cli.md)

## <a name="see-also"></a>Consulte também

[Extensões de componentes para .NET e UWP](../windows/component-extensions-for-runtime-platforms.md)