...
Se a cadeia de caracteres de comando contém um ou mais parâmetros, como \[db_column] ou \[bindto], **db_command** gera um conjunto de linhas e um mapa de acessador para atender a essas variáveis associadas. Ver [db_accessor](../windows/db-accessor.md) para obter mais informações.  
...
Se você especificar um valor para *ligações*, **db_command** analisa o valor associado e não analisará o \[ *bindtype*] parâmetro. Esse uso permite que você use a sintaxe do provedor OLE DB. Para desabilitar a análise, sem a associação de parâmetros, especifique **associações = ""**.  
