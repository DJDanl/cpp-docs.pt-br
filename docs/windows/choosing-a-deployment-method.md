...
- Implantação central usando pacotes redistribuíveis, que instalam bibliotecas do Visual C++ como DLLs compartilhadas em %windir%\system32\\. (A instalação nesta pasta requer direitos de administrador.) Você pode criar um script ou programa de configuração que execute o pacote redistributível antes de instalar seu aplicativo no computador de destino. Os pacotes redistribuíveis estão disponíveis para as plataformas x86, x64 e ARM (VCRedist_x86.exe, VCRedist_x64.exe ou VCRedist_arm.exe). O Visual Studio inclui esses pacotes em %ProgramFiles(x86)%\Microsoft Visual Studio `version`\VC\Redist\\`locale ID`\\. Baixe-os também no [Centro de Download da Microsoft](https://www.microsoft.com/download). (Use a caixa de pesquisa no Centro de Download para pesquisar o "Pacote Redistribuível do Visual C++ *versão do Visual Studio e atualização*" que corresponde ao seu aplicativo. Por exemplo, se você usou a atualização 3 do Visual Studio 2015 para construir seu aplicativo, procure por "Visual C++ Redistributable Package 2015 update 3".) Para obter informações sobre como usar um pacote redisível, consulte [Passo a Passo: Implantando um aplicativo Visual C++ usando o pacote visual C++ Redistributable .](deploying-visual-cpp-application-by-using-the-vcpp-redistributable-package.md)

- Implantação central usando módulos de mesclagem, em que cada um instala uma biblioteca específica do Visual C++ como uma DLL compartilhada em %windir%\system32\\. (A instalação nesta pasta requer direitos de administrador.) Os módulos de mesclagem tornam-se parte do arquivo de instalador .msi para o seu aplicativo. Os módulos de mesclagem redistribuíveis do Visual C++ são incluídos no Visual Studio, em \Arquivos de Programas (x86)\Common Files\Merge Modules\\. Para obter mais informações, confira [Redistribuindo componentes usando módulos de mesclagem](redistributing-components-by-using-merge-modules.md).
