...
Um subconjunto C++ de regras de verificação principal está incluído no conjunto de regras Microsoft Native recomendado que é executado por padrão quando a análise de código está habilitada. Para habilitar outras regras de verificação principais, clique na lista suspensa e escolha quais conjuntos de regras você deseja incluir:

![Menu suspenso para C++ conjuntos de regras de verificação principais adicionais](media/cppcorecheck_codeanalysis_extensions.png)
...
1. Desabilite o uso do cabeçalho pré-compilado (**cabeçalhos C/C++\Precompiled**). Isso é necessário porque o mecanismo de extensões pode tentar ler suas informações internas do cabeçalho pré-compilado (PCH); Se o PCH for compilado com opções de projeto padrão, ele não será compatível.
