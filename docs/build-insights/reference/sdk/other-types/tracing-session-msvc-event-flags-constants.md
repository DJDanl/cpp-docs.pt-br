...
description: O SDK de informações de compilação do C++ TRACING_SESSION_MSVC_EVENT_FLAGS referência de constantes.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
As `TRACING_SESSION_MSVC_EVENT_FLAGS` constantes são usadas para descrever quais eventos MSVC coletar durante um rastreamento. Use-os para inicializar o campo da estrutura de [TRACING_SESSION_OPTIONS](tracing-session-options-struct.md) `MsvcEventFlags` .
...
| `TRACING_SESSION_MSVC_EVENT_FLAGS_BACKEND_FUNCTIONS` | [FORCE_INLINEE](../event-table.md#force-inlinee)<br/>[FUNCIONAMENTO](../event-table.md#function) |
| `TRACING_SESSION_MSVC_EVENT_FLAGS_ALL` | Esse sinalizador ativa todos os eventos. |
