...
description: O C++ Build Insights SDK TRACING_SESSION_MSVC_EVENT_FLAGS referência de constantes.
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
As `TRACING_SESSION_MSVC_EVENT_FLAGS` constantes são usadas para descrever quais eventos MSVC devem ser coletados durante um rastreamento. Use-os para inicializar o `MsvcEventFlags` campo da estrutura [TRACING_SESSION_OPTIONS.](tracing-session-options-struct.md)
...
| `TRACING_SESSION_MSVC_EVENT_FLAGS_BACKEND_FUNCTIONS` | [FORCE_INLINEE](../event-table.md#force-inlinee)<br/>[Função](../event-table.md#function) |
| `TRACING_SESSION_MSVC_EVENT_FLAGS_ALL` | Esta bandeira liga todos os eventos. |
