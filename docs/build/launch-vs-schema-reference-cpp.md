...
|`projectTarget`|string|Especifica o destino opcional invocado durante a compilação `project` . Isso `projectTarget` já deve existir e corresponder ao nome na lista suspensa de **destino de depuração** .|
...
|`inheritEnvironments`|matriz|Especifica um conjunto de variáveis de ambiente herdadas de várias fontes. Você pode definir algumas variáveis em arquivos como *CMakeSettings.jsno* ou *CppProperties.js* e disponibilizá-los para o contexto de depuração.  **Visual Studio 16,4:**: especificar variáveis de ambiente por destino usando a `env.VARIABLE_NAME` sintaxe. Para remover a definição de uma variável, defina-a como "NULL".|
