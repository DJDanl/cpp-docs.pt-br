...
### <a name="assumptions"></a>Suposições  
 As imagens executáveis do Windows baseado em ARM usam o formato PE (executável portátil). Para obter mais informações, consulte [Microsoft PE e COFF especificação](http://go.microsoft.com/fwlink/p/?linkid=84140). As informações de tratamento de exceção são armazenadas nas seções .pdata e .xdata da imagem.  
...
|-----------------|----------|-------------|  
|0|0-31|*Função RVA iniciar* é RVA 32 bits do início da função. Se a função contiver código de posição, o bit baixo deste endereço deverá ser configurado.|  
|1|0-1|*Sinalizador* é um campo de 2 bits que indica como interpretar os bits restantes 30 da segunda palavra. pData. Se *sinalizador* for 0, o restante de bits formulário um *RVA de informações de exceção* (com dois bits baixos implicitamente 0). Se *sinalizador* é diferente de zero, em seguida, o restante de bits formulário um *desenrolar dados* estrutura.|  
|1|2-31|*Informações de exceção RVA* ou *empacotadas dados Desenrolados*.<br /><br /> *Exceção informações RVA* é o endereço da estrutura de informações de exceção de comprimento variável, armazenado na seção .xdata. Esses dados devem ser alinhados para 4 bytes.<br /><br /> *Empacotadas desenrolar dados* é uma descrição compactada as operações necessárias para desenrolar a partir de uma função, supondo que uma forma canônica. Nesse caso, nenhum registro .xdata é necessário.|  
...
|-----------------|----------|-------------|  
|0|0-31|*Função RVA iniciar* é RVA 32 bits do início da função. Se a função contiver código de posição, o bit baixo deste endereço deverá ser configurado.|  
|1|0-1|*Sinalizador* é um campo de 2 bits que tem estes significados:<br /><br /> -00 = compactado desenrolar dados não usados; os bits restantes do ponto de registro .xdata.<br />-01 = compactado liberar dados.<br />-10 = compactado desenrolar a data em que a função deve para ter nenhum prólogo. É útil para descrever fragmentos de função que são descontínuos com o início da função.<br />-11 = reservado.|  
|1|2-12|*Função comprimento* é um campo de bits 11 que fornece o comprimento da função inteira em bytes dividido por 2. Se a função tiver mais de 4K bytes, um registro completo de .xdata deverá ser usado no lugar.|  
|1|13-14|*RET* é um campo de 2 bits que indica como a função retorna:<br /><br /> -00 = retornam via pop {pc} (o *L* bit de sinalizador deve ser definido como 1 neste caso).<br />-01 = retornam usando uma ramificação de 16 bits.<br />-10 = retornam usando uma ramificação de 32 bits.<br />-11 não = nenhuma epílogo afinal. É útil para descrever um fragmento de função descontínuo que pode conter somente um prólogo, mas cujo epílogo está em outro lugar.|  
|1|15|*H* é um sinalizador de bit 1 que indica se a função "homes" parâmetro inteiro registra (r0-r3) enviando-os no início da função e desaloca 16 bytes da pilha antes de retornar. (0 = não hospeda registros, 1 = hospeda registros.)|  
|1|16-18|*Reg* é salvo de um campo de 3 bits que indica o índice do último registro não volátil. Se o *R* bit for 0, somente os registros de número inteiro estão sendo salvas e devem para estar no intervalo de rN r4, onde N é igual a 4 + *Reg*. Se o *R* bit for 1, em seguida, registra somente ponto flutuante está sendo salvas e deve para estar no intervalo de dN d8, onde N é igual a 8 + *Reg*. A combinação especial de *R* = 1 e *Reg* = 7 indica que não há registros são salvos.|  
|1|19|*R* um sinalizador de bit 1 que indica se os registros não volátil salvos são inteiro registra (0) ou registros de ponto flutuantes (1). Se *R* é definido como 1 e o *Reg* é definido como 7, sem registros não volátil foram enviados.|  
|1|20|*L* um sinalizador de bit 1 que indica se a função salva/restaurações LR, juntamente com outros registros indicado pelo *Reg* campo. (0 = não salva/restaura, 1 = salva/restaura.)|  
|1|21|*C* um sinalizador de bit 1 que indica se a função inclui instruções adicionais para configurar uma cadeia de quadro para rápido percorrer a pilha (1) ou não (0). Se este bit estiver configurado, r11 será implicitamente adicionado à lista de registros não voláteis de inteiros salvos. (Consulte restrições abaixo se o *C* sinalizador é usado.)|  
|1|22-31|*Ajuste de pilha* é um campo de 10 bits que indica o número de bytes da pilha que está alocado para essa função, dividida por 4. No entanto, apenas valores entre 0x000-0x3F3 podem ser codificados diretamente. As funções que alocam mais de 4044 bytes de pilha devem usar um registro completo de .xdata. Se o *pilha ajustar* campo é 0x3F4 ou maior, em seguida, 4 bits baixos têm significado especial:<br /><br /> -Bits 0-1 indicam o número de palavras de ajuste de pilha (1-4) menos 1.<br />-2 bit é definido como 1 se o prólogo combinado esse ajuste em sua operação de envio.<br />-3 bit é definido como 1 se o epílogo combinado esse ajuste em sua operação pop.|  
  
 Devido a possíveis redundâncias nas codificações acima, estas restrições se aplicam:  
  
-   Se o *C* sinalizador é definido como 1:  
  
    -   O *L* sinalizador também deve ser definido como 1, porque o encadeamento de quadro necessário r11 e LR.  
  
    -   r11 não deve ser incluído no conjunto de registros descrito por *Reg*. Ou seja, se r4 r11 é enviados por push, *Reg* só deve descrever r4-r10, porque o *C* sinalizador implica r11.  
...
-   Se o *Ret* campo é definido como 0, o *L* sinalizador deve ser definido como 1.  
 ...
 Para fins da discussão abaixo, dois sinalizadores pseudo são derivados do *pilha ajustar*:  
  
-   *PF* ou "prólogo dobra" indica que *pilha ajustar* é 0x3F4 ou 2 maiores e bit é definido.  
  
-   *EF* ou "epílogo dobra" indica que *pilha ajustar* é 0x3F4 ou 3 maiores e bit é definido.  
...
|5|*Ajuste de pilha* ! = 0 e PF = = 0|16/32|`sub sp,sp,#xx`|00-7F/E8-EB|  
 ...
 Instrução 1 está sempre presente se a *H* bit é definido como 1.  
  
 Para configurar o encadeamento de quadro, instrução 3a ou 3b está presente se a *C* bit é definido. Ele será um `mov` de 16 bits se nenhum registro diferente de r11 e LR for enviado por push, caso contrário, será um `add` de 32 bits.  
  
 Se um ajuste não dobrado for especificado, a instrução 5 será o ajuste de pilha explícito.  
  
 As instruções 2 e 4 são configuradas com base na necessidade de um envio por push. Esta tabela resume quais registros são salvos com base no *C*, *L*, *R*, e *PF* campos. Em todos os casos, *N* é igual a *Reg* + 4, *E* é igual a *Reg* + 8, e *S* é igual a (~*Pilha ajustar*) e 3.  
  
|C|L|R|PF|Registros de inteiro enviados por push|Registros VFP enviados por push|  
|-------|-------|-------|--------|------------------------------|--------------------------|  
|0|0|0|0|r R4*N*|nenhum|  
|0|0|0|1|r*S*- r*N*|nenhum|  
|0|0|1|0|nenhum|d D8*E*|  
|0|0|1|1|r*S*-r3|d D8*E*|  
|0|1|0|0|r R4*N*, LR|nenhum|  
|0|1|0|1|r*S*- r*N*, LR|nenhum|  
|0|1|1|0|LR|d D8*E*|  
|0|1|1|1|r*S*-r3, LR|d D8*E*|  
|1|0|0|0|r R4*N*, r11|nenhum|  
|1|0|0|1|r*S*- r*N*, r11|nenhum|  
|1|0|1|0|r11|d D8*E*|  
|1|0|1|1|r*S*-r3, r11|d D8*E*|  
|1|1|0|0|r R4*N*, r11, LR|nenhum|  
...
|10b|*RET*= = 2|32|`b     address`|  
 ...
 A instrução 6 será o ajuste de pilha explícito se um ajuste não dobrado for especificado. Porque *PF* é independente do *EF*, é possível ter 5 presente sem instrução 6 da instrução, ou vice-versa.  
  
 Instruções 7 e 8 usam a mesma lógica como prólogo para determinar quais registros são restaurados da pilha, mas com estas duas alterações: primeiro, *EF* é usado no lugar de *PF*; segundo, se *Ret*  = 0, então LR é substituído com um computador na lista de registro e de epílogo encerrado imediatamente.  
  
 Se *H* for definida, instrução 9a ou 9b está presente. Instrução 9a é usado quando *L* é 0, para indicar que não é o LR na pilha. Nesse caso, a pilha é ajustada manualmente e *Ret* deve ser 1 ou 2 para especificar um retorno explícito. Instrução 9b é usado quando *L* é 1 para indicar uma extremidade de início para epílogo e para retornar e ajustar a pilha ao mesmo tempo.  
  
 Se o epílogo não já tiver terminado, em seguida, qualquer instrução 10a ou 10b estiver presente, para indicar uma ramificação de 16 ou 32 bits, com base no valor de *Ret*.  
...
1.  Um cabeçalho com 1 ou 2 palavras que descreve o tamanho geral da estrutura .xdata e fornece dados de função-chave. A segunda palavra está presente somente se o *epílogo contagem* e *código palavras* campos são definidos como 0. Os campos estão divididos nesta tabela:  
    ...
    |0|18-19|*Drivers* é um campo de 2 bits que descreve a versão de xdata restante. Apenas a versão 0 está definida atualmente; os valores de 1 a 3 são reservados.|  
    |0|20|*X* é um campo de bits 1 que indica a presença (1) ou ausência (0) de dados de exceção.|  
    |0|21|*E* é um campo de bits 1 que indica que informações que descrevem um único epílogo são empacotadas em cabeçalho (1) em vez de exigir o escopo adicional palavras posterior (0).|  
    |0|22|*F* é um campo de bits 1 que indica que este registro descreve um fragmento de função (1) ou uma função completa (0). Um fragmento implica que não há prólogo e que todo o processamento do prólogo deve ser ignorado.|  
    |0|23-27|*Contagem de epílogo* é um campo de bits 5 que tem dois significados, dependendo do estado do *E* bits:<br /><br /> -Se *E* for 0, esse campo é uma contagem do número total de escopos de exceção descrito na seção 3. Se existirem mais de 31 escopos em função, em seguida, esse campo e o *código palavras* campo deve ser definido como 0 para indicar que uma palavra de extensão é necessária.<br />-Se *E* for 1, este campo especifica o índice do primeiro código de desenrolamento que descreve o epílogo apenas.|  
    |0|28-31|*Código palavras* é um campo de 4 bits que especifica o número de palavras de 32 bits necessários para conter todos os códigos de desenrolamento na seção 4. Se mais de 15 palavras são necessárias para mais de 63 bytes de código de desenrolamento, esse campo e o *epílogo contagem* campo deve ser definido como 0 para indicar que uma palavra de extensão é necessária.|  
    |1|0-15|*Estendido epílogo contagem* é um campo de 16 bits que fornece mais espaço para um número muito grande de epilogues de codificação. A palavra de extensão que contém esse campo está presente somente se o *epílogo contagem* e *código palavras* campos da primeira palavra de cabeçalho são definidos como 0.|  
    |1|16-23|*Estendido código palavras* é um campo de 8 bits que fornece mais espaço para um número muito grande de palavras de código de desenrolamento de codificação. A palavra de extensão que contém esse campo está presente somente se o *epílogo contagem* e *código palavras* campos da primeira palavra de cabeçalho são definidos como 0.|  
    |1|24-31|Reservado|  
  
2.  Depois que os dados de exceção (se o *E* bit no cabeçalho foi definido como 0) é uma lista de informações sobre escopos de epílogo, que são empacotadas uma a uma palavra e armazenados em ordem crescente de deslocamento inicial. Cada escopo contém estes campos:  
  
    |Bits|Finalidade|  
    |----------|-------------|  
    |0-17|*Deslocamento de início de epílogo* é um campo de 18 bits que descreve o deslocamento de epílogo, em bytes, dividido por 2, em relação ao início da função.|  
    |18-19|*Res* é um campo de 2 bits reservado para expansão futura. Seu valor deve ser 0.|  
    |20-23|*Condição* é um campo de 4 bits que fornece a condição sob a qual o epílogo é executado. Para epílogos incondicionais, ele deve ser configurado como 0xE, que indica "sempre". (Um epílogo deve ser inteiramente condicional ou inteiramente incondicional, e no modo Posição 2, o epílogo começa com a primeira instrução após o opcode de TI.)|  
    |24-31|*Índice do começo epílogo* é um campo de 8 bits que indica o índice de bytes do primeiro código de desenrolamento que descreve este epílogo.|  
...
|FF||||-|end|  
 ...
 Isso mostra o intervalo de valores hexadecimais para cada byte de um código de desenrolamento *código*, juntamente com o tamanho de opcode *Opsize* e a interpretação de instrução original correspondente. As células vazias indicam códigos de desenrolamento mais curtos. Em instruções que tenham valores grandes que abrangem vários bytes, os bits mais significativos são armazenados primeiro. O *Opsize* campo mostra o tamanho de opcode implícita associado a cada operação Thumb-2. As entradas duplicadas aparentes na tabela com diferentes codificações são usadas para distinguir entre tamanhos de opcode diferentes.  
...
-   Epílogos somente; prólogo e, possivelmente, epílogos adicionais em outros fragmentos.  
 ...
 O terceiro e quarto casos, a presença de um prólogo pseudo for especificada ou definindo o *sinalizador* campo do registro. pData compact para 2 ou definindo o *F* sinalizador no cabeçalho .xdata para 1. Em qualquer dos casos, a verificação do desenrolamento parcial de um prólogo é ignorada e todos os desenrolamentos que não sejam de epílogo são considerados completos.  
...
#### <a name="shrink-wrapping"></a>Shrink-wrapping  
 É um caso especial mais complexo de fragmentos de função *shrink-wrapping*, uma técnica de adiamento register salva desde o início da função para mais tarde na função, otimizar para casos simples que não exigem o salvamento do registro. Esse procedimento pode ser descrito como uma região externa que aloca o espaço de pilha, mas salva um conjunto mínimo de registros e uma região interna que salva e restaura registros adicionais.  
...
```  
 ...
 O essencial aqui é que em cada limite de instrução, a pilha seja completamente consistente com os códigos de desenrolamento da região. Se ocorrer um desenrolamento antes do envio por push interno neste exemplo, ele será considerado parte da região A e apenas o prólogo da região A será desenrolado. Se o desenrolamento após o envio por push interno, ele é considerado parte da região B, que não tem nenhum prólogo, mas tem desenrolamento códigos que descrevem o envio por push interno e o prólogo original da região lógica semelhante A. mantém o pop interna.  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x000535F8 (= 0x004535F8 0x00400000)  
  
-   Palavra 1  
  
    -   *Sinalizador* = 1, indicando os formatos de prólogo e epílogo canônicos  
  
    -   *Função comprimento* = 0x31 (= 0x62/2)  
  
    -   *RET* = 1, que indica uma retorno de ramificação de 16 bits  
  
    -   *H* = 0, indicando que os parâmetros não foram localizadas  
  
    -   *R*= 0 e *Reg* = 1, indicando que o envio por push/pop de r5 r4  
  
    -   *L* = 0, indicando que nenhum LR salvar/restaurar  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x000533AC (= 0x004533AC-0x00400000)  
...
-   Palavra 1  
    ...
    -   *RET* = 0, indicando um pop {pc} retorno  
  
    -   *H* = 0, indicando que os parâmetros não foram localizadas  
  
    -   *R*= 0 e *Reg* = 3, que indica por push/pop de r7 r4  
  
    -   *L* = 1, indicando LR foi salvo/restaurado  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x00053988 (= 0x00453988 0x00400000)  
...
-   Palavra 1  
    ...
    -   *RET* = 0, indicando um pop {pc}-estilo de retorno (nesse caso um pc ldr, [sp] #0x14 retornar)  
  
    -   *H* = 1, indicando que os parâmetros foram adaptadores de rede  
  
    -   *R*= 0 e *Reg* = 2, que indica por push/pop de r6 r4  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x000592F4 (= 0x004592F4 0x00400000)  
  
-   Palavra 1  
  
    -   *Sinalizador* = 0, indicando que o registro .xdata (necessário devido a vários epilogues)  
  
    -   *endereço de .xdata* -0x00400000  
...
-   Palavra 0  
    ...
    -   *Drivers* = 0, indicando que a primeira versão do xdata  
  
    -   *X* = 0, indicando que nenhum dado de exceção  
  
    -   *E* = 0, indicando uma lista de escopos de epílogo  
  
    -   *F* = 0, indicando uma descrição da função completo, incluindo prólogo  
  
    -   *Contagem de epílogo* = 0x04, indicando os escopos de epílogo total 4  
  
    -   *Código palavras* = 0x01, indicando uma palavra de 32 bits dos códigos de liberação  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x00085A20 (= 0x00485A20 0x00400000)  
...
-   Palavra 0  
    ...
    -   *Contagem de epílogo* = 0x001, indicando que o escopo de epílogo total 1  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x00088C24 (= 0x00488C24 0x00400000)  
...
-   Palavra 0  
    ...
    -   *X* = 1, indicando que os dados da exceção presentes  
  
    -   *E* = 1, indicando que um único epílogo  
  
    -   *F* = 0, indicando uma descrição da função completo, incluindo prólogo  
  
    -   *Contagem de epílogo* = 0x00, que indica os códigos de desenrolamento de epílogo começam no deslocamento 0x00  
  
    -   *Código palavras* = 0x02, indicando que duas palavras de 32 bits dos códigos de liberação  
...
-   Word 3 especifica um manipulador de exceção = 0x0019A7ED (= 0x0059A7ED - 0x00400000)  
...
-   Palavra 0  
    ...
    -   *Iniciar função RVA* = 0x00088C72 (= 0x00488C72 0x00400000)  
...
-   Palavra 1  
    ...
    -   *R*= 0 e *Reg* = 7, indicando que não há registros foram salvos/restaurado  
