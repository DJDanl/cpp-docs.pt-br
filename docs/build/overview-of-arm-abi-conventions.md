...
A interface binária do aplicativo \(ABI\) de código compilado para Windows em processadores ARM é baseada na EABI padrão do ARM.  Este artigo destaca diferenças chave entre Windows em ARM e o padrão.  Para obter mais informações sobre a EABI padrão do ARM, consulte [Interface Binária do Aplicativo \(ABI\) da Arquitetura ARM](http://infocenter.arm.com/help/index.jsp?topic=/com.arm.doc.subset.swdev.abi/index.html).  
...
### Estágio B—Pré\-enchimento e Extensão de Argumentos  
...
## Consulte também  
 ...
 [Tratamento de exceção ARM](../build/arm-exception-handling.md)