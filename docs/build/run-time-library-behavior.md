...
A biblioteca VCRuntime fornece uma função de ponto de entrada `_DllMainCRTStartup` chamada para lidar com as operações de inicialização e término padrão. Na anexação de processo `_DllMainCRTStartup` , a função configura verificações de segurança de buffer, INICIALIZA o CRT e outras bibliotecas, inicializa informações de tipo em tempo de execução, inicializa e chama construtores para dados estáticos e não locais, inicializa o armazenamento local de thread, incrementa um contador estático interno para cada anexo e, em seguida, chama um `DllMain`usuário ou uma biblioteca fornecida. Na desanexação do processo, a função passa por essas etapas na ordem inversa. Ele chama `DllMain`, Decrementa o contador interno, chama destruidores, chama funções de terminação CRT e `atexit` funções registradas e notifica quaisquer outras bibliotecas de encerramento. Quando o contador de anexos chega a zero, a função `FALSE` retorna para indicar ao Windows que a dll pode ser descarregada. A `_DllMainCRTStartup` função também é chamada durante anexação de thread e desanexação de thread. Nesses casos, o código VCRuntime não faz inicialização ou encerramento adicional por conta própria e apenas chamadas `DllMain` para passar a mensagem. Se `DllMain` retornar `FALSE` da anexação de processo, sinalizar `_DllMainCRTStartup` falha `DllMain` , chamar novamente `DLL_PROCESS_DETACH` e passar como o argumento de *motivo* , passará pelo restante do processo de encerramento.
...
Algumas bibliotecas encapsulam `DllMain` a função para você. Por exemplo, em uma DLL do MFC regular, implemente `CWinApp` as `InitInstance` funções `ExitInstance` do objeto e do membro para executar a inicialização e o encerramento exigidos por sua dll. Para obter mais detalhes, consulte a seção [inicializar DLLs regulares do MFC](#initializing-regular-dlls) .
...
Como as DLLs do MFC regulares `CWinApp` têm um objeto, elas devem executar suas tarefas de inicialização e encerramento no mesmo local que um aplicativo do MFC `InitInstance` : `ExitInstance` nas funções de membro e da `CWinApp`classe derivada da dll. Como o MFC fornece `DllMain` uma função que é chamada `_DllMainCRTStartup` pelo `DLL_PROCESS_ATTACH` para `DLL_PROCESS_DETACH`e, você não deve escrever sua `DllMain` própria função. A função fornecida `DllMain` pelo MFC chama `InitInstance` quando sua dll é carregada e ela chama `ExitInstance` antes que a dll seja descarregada.
...
No `DllMain` que o assistente de DLL do MFC gera, a chamada `AfxInitExtensionModule` para captura as classes de tempo de execução do módulo`CRuntimeClass` (estruturas), bem como suas fábricas de`COleObjectFactory` objeto (objetos) para uso `CDynLinkLibrary` quando o objeto é criado. Você deve verificar o valor de retorno `AfxInitExtensionModule`de; se um valor zero for retornado de `AfxInitExtensionModule`, retorna zero de sua `DllMain` função.
