...
-   Compilador e opções do vinculador  
  
    -   Otimizações Guiadas por Perfil  
  
    -   O nível de otimização devo usar?  
  
    -   Opções de ponto de flutuante  
...
### <a name="profile-guided-optimization"></a>Otimizações Guiadas por Perfil  
 Otimização Guiada por perfil (PGO) de linguagem do Visual C++. Essa otimização usa dados de perfil de últimas execuções de uma versão instrumentada de um aplicativo para otimização posterior do aplicativo da unidade. Usar PGO pode ser demorado, portanto, não pode ser algo que cada desenvolvedor usa, mas é recomendável usar PGO para a compilação de lançamento final de um produto. Para obter mais informações, consulte [Profile-Guided otimizações](../../build/reference/profile-guided-optimizations.md).  
  
 Além disso, a otimização de programa total (também conhecidos como geração de código de tempo de vinculação) e o **/O1** e **/O2** otimizações foi aprimorado. Em geral, um aplicativo compilado com uma dessas opções será mais rápido do que o mesmo aplicativo compilado com um compilador anterior.  
...
### <a name="which-level-of-optimization-should-i-use"></a>O nível de otimização devo usar?  
 ...
 O **/Gy** também é muito útil. Ele gera um COMDAT separado para cada função, fornecendo o vinculador mais flexibilidade quando se trata de remoção não referenciada COMDATs e COMDAT dobra. A única desvantagem de usar **/Gy** é que ele pode ter um efeito secundário no momento da compilação. Portanto, geralmente é recomendável usá-lo. Para obter mais informações, consulte [/Gy (habilitar vinculação em nível de função)](../../build/reference/gy-enable-function-level-linking.md).  
...
### <a name="floating-point-switches"></a>Opções de ponto de flutuante  
...
## <a name="intrinsic-support"></a>Suporte intrínseco  
 Intrínsecos são função chama em que o compilador não tem conhecimento intrínseco sobre a chamada e, em vez de chamar uma função em uma biblioteca, ele emite código para essa função. O intrin.h do arquivo de cabeçalho localizado em < Installation_Directory > \VC\include\intrin.h contém todos os disponíveis intrínsecos para cada uma das três plataformas com suporte (x x86, x64 e ARM).  
...
1.  Seu código é mais portáteis. Vários os intrínsecos estão disponíveis em várias arquiteturas de CPU.  
  
2.  Seu código é mais fácil de ler, pois o código ainda está escrito em C/C++.  
  
3.  O código obtém o benefício de otimizações do compilador. Como o compilador fica melhor, melhora a geração de código para os intrínsecos.  
