...
**/OT** (favorecer código rápido) maximiza a velocidade de EXEs e DLLs ao instruir o compilador a favorecer velocidade sobre tamanho. (Esse é o padrão). O compilador pode reduzir muitas construções de C e C++ em sequências funcionalmente semelhantes do código de máquina. Ocasionalmente, essas diferenças oferecem vantagens e desvantagens de tamanho e velocidade. A opção /Ot indicada pelo maximizar velocidade ([/O2](o1-o2-minimize-size-maximize-speed.md)) opção. O **/O2** opção combina várias opções para produzir um código muito rápido.
