...
![Visual Studio C++ propriedades do projeto](../media/cpp-property-page-2017.png)
...
1. **Regra:**  Regra geralmente é o nó raiz no arquivo xml; ele pode ter muitos atributos:
   ...
   a. **Nome:** O atributo de nome é uma id para a regra. Precisa ser exclusivo entre todos os arquivos XML da página de propriedades de um projeto.
   ...
   c. **SwitchPrefix:** Isso é o prefixo usado na linha de comando para as opções. Um valor igual a "/" resulta em opções semelhantes a /ZI, /nologo, /W3, etc.

   d. **Ordem:** Isso é uma sugestão para um potencial cliente de interface do usuário sobre o local relativo desta regra em comparação comparada todas as outras regras no sistema.

   e. **xmlns:** Este é um elemento XAML padrão. É possível ver três namespaces listados. Eles correspondem aos namespaces para as classes de desserialização de XAML, o esquema XAML e o namespace do sistema, respectivamente.
   ...
   g. **DataSource:** Esta é uma propriedade muito importante que informa ao sistema de projeto, o local do qual o valor da propriedade deve ser lida e gravada para e seu agrupamento (explicado abaixo). Para cl.xml, estes valores são:
...
1. **Propriedades:** Essa é a parte principal do arquivo xml e contém a lista de todas as propriedades nesta regra. Cada propriedade pode ter um dos cinco tipos possíveis mostrados no esqueleto de XAML acima. Naturalmente, você pode ter apenas alguns desses tipos em seu arquivo. Uma propriedade tem diversos atributos que permitem que ela seja descrita de maneira sofisticada. Explicaremos apenas a **StringProperty** aqui. O restante é muito parecido.
   ...
   b. **Categoria:** Isso declara a categoria na qual essa propriedade cai. Tente encontrar essa propriedade na categoria **Arquivos de Saída** na interface do usuário.

   c. **Comutador:** Quando uma regra representa uma ferramenta – como a ferramenta de compilador nesse caso, a maioria das propriedades da regra são passados como comutadores para a ferramenta executável durante o tempo de compilação. O valor desse atributo indica o literal de opção a ser usado. A propriedade acima especifica que sua opção deve ser **Fo**. Combinada com o atributo **SwitchPrefix** na Rule pai, essa propriedade é passada para o executável como **/Fo"Debug\"** (visível na linha de comando para o C/C++ na interface do usuário da página de propriedades).

   Outros atributos de propriedade incluem:

   d. **Visível:** Se por algum motivo, você não deseja que a propriedade aparecem nas páginas de propriedades (mas provavelmente ainda está disponível durante o tempo de compilação), defina esse atributo como false.

   e. **ReadOnly:** Se você quiser fornecer uma exibição somente leitura do valor desta propriedade nas páginas de propriedades, defina esse atributo como true.

   f. **IncludeInCommandLine:** Algumas propriedades talvez não precise ser passados para uma ferramenta durante o tempo de compilação. A definição desse atributo como falso impedirá que ele seja passado.
