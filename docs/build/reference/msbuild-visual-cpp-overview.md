...
- % VSINSTALLDIR% MSBuild\\*versão*\\do Microsoft\\vc\\VCTargets\\
...
- % VSINSTALLDIR% MSBuild\\*versão* do Microsoft\\VC\\\\plataformas VCTargets\\\\*plataforma*\\
...
- % VSINSTALLDIR% MSBuild\\Microsoft\\VC\\*versão*\\plataformas VCTargets\\\\*plataforma*\\PlatformToolsets\\*conjunto de ferramentas*\\
...
- % VSINSTALLDIR% Common7\\IDE\\VC\\\\plataformas\\*plataforma*\\
...
- *unidade*:\\arquivos *de programas (x86)* \\MSBuild\\Microsoft. Cpp (x86)\\v 4.0\\*versão*\\
...
- *unidade*:\\arquivos *de programas (x86)* \\MSBuild\\plataformas Microsoft. Cpp\\v 4.0\\\\*plataforma*\\PlatformToolsets\\*conjunto de ferramentas*\\
...
Há centenas de destinos nos arquivos de suporte do Visual Studio. No entanto, a maioria são destinos orientados ao sistema que o usuário pode ignorar. A maioria dos destinos de sistema são prefixados por um sublinhado (`_`), ou ter um nome que começa com "PrepareFor", "Compute", "antes", "After", "pre" ou "post".
...
| Recriar | Limpa e, em seguida, compila o projeto. |
...
\ de [referência de tarefa do MSBuild](/visualstudio/msbuild/msbuild-task-reference)
\ [tarefa BSCMAKE](/visualstudio/msbuild/bscmake-task)
\ de [tarefas do CL](/visualstudio/msbuild/cl-task)
\ [Tarefa CPPClean](/visualstudio/msbuild/cppclean-task)
\ de [tarefas lib](/visualstudio/msbuild/lib-task)
[Vincular\ tarefa](/visualstudio/msbuild/link-task)
\ [tarefa MIDL](/visualstudio/msbuild/midl-task)
\ de [tarefas MT](/visualstudio/msbuild/mt-task)
\ de [tarefas RC](/visualstudio/msbuild/rc-task)
