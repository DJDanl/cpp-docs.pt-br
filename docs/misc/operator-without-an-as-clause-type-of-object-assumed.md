...
Um procedimento de operador não especifica um `As` cláusula.  
 ...
 Um `As` cláusula identifica um tipo de dados a ser associado um elemento de programação. Em um [Instrução Operator](../Topic/Operator%20Statement.md), ela especifica o tipo de dados do valor que o procedimento de operador retorna para o código de chamada. Se você não incluir um `As` cláusula de `Operator` tipo de instrução, os dados de retorno padrão é `Object`.  
