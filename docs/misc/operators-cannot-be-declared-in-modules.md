...
Um [Instrução Operator](../Topic/Operator%20Statement.md) aparece em uma definição de módulo.  
...
-   Se você solicitar uma operação no módulo, use um [Instrução Function](../Topic/Function%20Statement%20\(Visual%20Basic\).md) para definir uma `Function` procedimento que executa a operação.  
