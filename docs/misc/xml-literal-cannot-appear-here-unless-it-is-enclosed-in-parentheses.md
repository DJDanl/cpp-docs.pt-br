...
Uma declaração literal XML é usada em uma expressão em um local que é ambíguo para o [!INCLUDE[vbprvb](../Token/vbprvb_md.md)] compilador. Ou seja, o [!INCLUDE[vbprvb](../Token/vbprvb_md.md)] compilador não pode determinar se o menor\-que caractere \(\<\) serve como um operador de comparação ou o início de um literal XML. O código a seguir fornece um exemplo:  
