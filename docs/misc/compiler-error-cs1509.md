...
Arquivo referenciado 'arquivo' não é um assembly. Use ' \/ \/addmodule ' opção  
 ...
 Um arquivo de saída \(arquivo de saída 1\), produzido em uma compilação usada [\/target: Module](../Topic/-target:module%20\(C%23%20Compiler%20Options\).md) \(não tem um manifesto de assembly\), foi especificado para [\/Reference](../Topic/-reference%20\(C%23%20Compiler%20Options\).md). Portanto, em vez de acrescentar um assembly para o assembly para o programa atual, as informações de metadados no arquivo de saída 1 serão adicionadas ao assembly para o programa atual.