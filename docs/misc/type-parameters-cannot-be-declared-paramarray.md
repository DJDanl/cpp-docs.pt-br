...
Uma definição de um delegado, um evento ou um operador declara um [ParamArray](../Topic/ParamArray%20\(Visual%20Basic\).md) parâmetro.  
...
-   Se você estiver definindo um evento ou delegado, você deve refazer a lógica geral dessa parte do seu aplicativo. Você não pode usar [Opcional](../Topic/Optional%20\(Visual%20Basic\).md) ou `ParamArray` parâmetros ou versões sobrecarregadas, nos parâmetros do delegado ou evento.  
