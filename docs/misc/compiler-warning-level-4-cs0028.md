...
'declaração de função ' tem a assinatura incorreta para um ponto de entrada  
 ...
 A declaração de método para `Main` era inválido: foi declarado com uma assinatura inválida.`Main` deve ser declarado como static e ele devem retornar [int](../Topic/int%20\(C%23%20Reference\).md) ou [void](../Topic/void%20\(C%23%20Reference\).md). Para obter mais informações, consulte [Main\(\) e argumentos de linha de comando](../Topic/Main\(\)%20and%20Command-Line%20Arguments%20\(C%23%20Programming%20Guide\).md).  
