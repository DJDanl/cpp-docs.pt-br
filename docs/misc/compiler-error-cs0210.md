...
Você deve fornecer um inicializador em uma declaração de instrução fixed ou using  
 ...
 Você deve declarar e inicializar a variável em uma [instrução fixa](../Topic/fixed%20Statement%20\(C%23%20Reference\).md). Para obter mais informações, consulte [Código não seguro e ponteiros](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md).  
...
```  
 ...
 O exemplo a seguir também gera CS0210 porque o [usando a instrução](../Topic/using%20Statement%20\(C%23%20Reference\).md) não tem nenhum inicializador.  
