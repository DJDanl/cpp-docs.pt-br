...
Não há suporte para a propriedade, indexador ou evento 'property' de linguagem. Tente chamar diretamente o acessador métodos 'acessador set' ou 'acessador get'  
 ...
 O código está consumindo um objeto que tem um não\-padrão [indexador](../Topic/Indexers%20\(C%23%20Programming%20Guide\).md) e tentou usar a sintaxe indexada. Para resolver esse erro, chame a propriedade `get` ou `set` método do acessador.  
