...
O primeiro operando de um "é" ou "como" operador não pode ser uma expressão lambda ou um método anônimo.  
 ...
 Métodos anônimos e expressões lambda não podem ser usados no lado esquerdo da [é](../Topic/is%20\(C%23%20Reference\).md) ou [como](../Topic/as%20\(C%23%20Reference\).md).  
