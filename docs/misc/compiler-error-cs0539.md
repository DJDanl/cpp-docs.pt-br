...
membro na declaração de interface explícita não é um membro de interface  
 ...
 Foi feita uma tentativa para declarar explicitamente uma [interface](../Topic/interface%20\(C%23%20Reference\).md) membro que não existe. Você deve excluir a declaração ou alterá\-lo para que ele se refere a um membro de interface válido.  
