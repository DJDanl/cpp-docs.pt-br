...
Um <xref:System.ServiceModel.Security.MessageSecurityException> exceção é lançada quando [!INCLUDE[vsindigo](../misc/includes/vsindigo_md.md)] determina que uma mensagem não está protegida corretamente ou foi violada. O erro ocorre com mais freqüência quando as seguintes condições forem verdadeiras:  
