...
Campos de somente leitura e estático 'nome do campo' não podem ser passados como ref ou out \(exceto em um construtor estático\)  
 ...
 Um [readonly](../Topic/readonly%20\(C%23%20Reference\).md) variável deve ter o mesmo [estático](../Topic/static%20\(C%23%20Reference\).md) uso do construtor no qual você deseja passar como um [ref](../Topic/ref%20\(C%23%20Reference\).md) ou [out](../Topic/out%20\(C%23%20Reference\).md) parâmetro. Para obter mais informações, consulte [Passando parâmetros](../Topic/Passing%20Parameters%20\(C%23%20Programming%20Guide\).md).  
