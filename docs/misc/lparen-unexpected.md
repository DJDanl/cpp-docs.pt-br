...
'\(' Unexpected. Matrizes de tipos genéricos sem instâncias não são permitidas.  
 ...
 [!INCLUDE[vbprvb](../Token/vbprvb_md.md)] não é possível compilar uma matriz, a menos que se trata de um tipo de dados específico. Você não pode usar um parâmetro de tipo de dados de um tipo genérico como o tipo de dados de uma matriz.  
