...
## Exemplo  
 O código a seguir gera CS1955 porque o código está tentando invocar um campo e uma propriedade usando o operador de chamada de método [\(\)](../Topic/\(\)%20Operator%20\(C%23%20Reference\).md). Você não pode chamar uma propriedade ou campo, mas você pode acessar o valor que ele armazena usando o operador de acesso de membro \( [.](../Topic/.%20Operator%20\(C%23%20Reference\).md) \).  
