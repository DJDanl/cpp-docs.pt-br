...
Um erro ou mensagem de aviso será exibida se uma ferramenta retorna um erro ou um aviso ao processar um arquivo. resx.  Como parte do processo de compilação, o sistema de projeto transforma cada arquivo. resx em um arquivo binário compreendido pelo [!INCLUDE[dnprdnshort](../Token/dnprdnshort_md.md)] Gerenciador de recursos.  Essa transformação é feita usando ferramentas no processo.  
