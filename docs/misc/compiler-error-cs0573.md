...
declaração de campo: não é possível ter inicializadores de campo de instância em structs  
 ...
 Você não pode inicializar um campo de instância de um [struct](../Topic/struct%20\(C%23%20Reference\).md). Campos de tipos de valor serão inicializados com seus valores padrão e campos de tipo de referência serão inicializados com `null`.  
