...
Um [Instrução Operator](../Topic/Operator%20Statement.md) Especifica o [Implements](../Topic/Implements%20Clause%20\(Visual%20Basic\).md) palavra\-chave.  
 ...
 Uma `Operator` procedimento requer a `Public` e `Shared` palavras\-chave e um operador de conversão requer o `Widening` ou o `Narrowing` palavra\-chave. Para obter mais informações, consulte [Procedimentos do operador](../Topic/Operator%20Procedures%20\(Visual%20Basic\).md).  
