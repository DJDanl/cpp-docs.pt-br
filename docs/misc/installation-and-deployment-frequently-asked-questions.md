...
Este tópico aborda questões da [!INCLUDE[vsipsdk](../mfc/includes/vsipsdk_md.md)] a comunidade de usuários sobre a instalação e implantação.  O tópico continuará a ser atualizado com novo conteúdo da comunidade.  
 ...
 **P:**  há uma maneira para determinar programaticamente o LCID de um [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] instalação?  
  
 **R:** <xref:Microsoft.VisualStudio.Shell.Interop.IUIHostLocale2.GetUILocale%2A> ou <xref:Microsoft.VisualStudio.Shell.Interop.IUIHostLocale.GetUILocale%2A>retornará o LCID da [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] em uso no momento.  
