...
Não é possível carregar os assemblies referenciados.  
 ...
 Esse erro ocorre se a ferramenta de análise é não é possível carregar os assemblies referenciados pelo assembly na análise.  Como resultado, este erro pode causar CA0001 em outros locais porque muitos estados inesperados podem ocorrer se isso ocorre.  Se você estiver usando a ferramenta de análise de código gerenciado de dentro de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)], estes são alguns dos motivos que podem causar CA0058:  
  
-   O assembly de entrada foi preterido e agora pontos em um assembly que faça referência a outros assemblies fora da lista de referências no arquivo de projeto de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)].  
...
-   Em C\+\+, é possível fazer referência aos assemblies que usam ForcedUsing em VCPROJ, substituindo passar de linha de comando\/ForcedUsing ou adicionando Some.dll \#using\> \<somente na origem.  
 ...
 Em todos os casos, a resolução para esse problema é adicionar a referência ausentes ao projeto de [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] própria.  
