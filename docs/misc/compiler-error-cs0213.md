...
Você não pode usar a instrução fixed para obter o endereço de uma expressão fixed  
 ...
 Uma variável local em um [unsafe](../Topic/unsafe%20\(C%23%20Reference\).md) método ou um parâmetro já é fixa \(na pilha\), portanto você não pode obter o endereço de qualquer uma destas duas variáveis em uma [fixo](../Topic/fixed%20Statement%20\(C%23%20Reference\).md) expressão. Para obter mais informações, consulte [Código não seguro e ponteiros](../Topic/Unsafe%20Code%20and%20Pointers%20\(C%23%20Programming%20Guide\).md).  
