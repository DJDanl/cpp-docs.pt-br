...
Esse erro ocorre quando o \/AssociateFiles de opção de linha de comando devenv é usado, mas os direitos de usuário atuais não permitem o acesso à seção HKEY\_CLASSES\_ROOT do registro. Quando você executar [!INCLUDE[vsprvs](../assembler/masm/includes/vsprvs_md.md)] em [!INCLUDE[wiprlhext](../c-runtime-library/reference/includes/wiprlhext_md.md)], você deve executar devenv como um administrador para usar a opção \/AssociateFiles \(devenv.exe\).  
