...
Passando 'argumento' como ref ou out ou a obtenção de seu endereço pode gerar uma exceção de tempo de execução porque ele é um campo de uma classe de marshaling por referência  
 ...
 Qualquer classe derivada, direta ou indiretamente, de <xref:System.MarshalByRefObject> é uma classe de marshaling por referência. Essa classe pode ser empacotada por referência entre limites de processo e de máquina. Portanto, as instâncias dessa classe pode ser proxies para objetos remotos. Você não pode passar um campo de um objeto proxy como [ref](../Topic/ref%20\(C%23%20Reference\).md) ou [out](../Topic/out%20\(C%23%20Reference\).md). Portanto, não é possível passar campos dessa classe como `ref` ou `out`, a menos que a instância é [isso](../Topic/this%20\(C%23%20Reference\).md), que não pode ser um objeto proxy.  
