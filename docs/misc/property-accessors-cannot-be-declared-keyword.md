...
Um `Get` ou `Set` declaração de procedimento inclui uma palavra\-chave não é válida em um procedimento de propriedade.  
 ...
 O [Instrução Get](../Topic/Get%20Statement.md) e [Instrução Set](../Topic/Set%20Statement%20\(Visual%20Basic\).md) Permitir um modificador de acesso \(`Public`, `Protected`, `Friend`, `Protected Friend`, `Private`\).  
