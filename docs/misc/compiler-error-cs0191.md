...
Propriedade ou indexador 'name' não pode ser atribuído a \-\-é somente leitura  
 ...
 Um [readonly](../Topic/readonly%20\(C%23%20Reference\).md) campo só pode ter uma atribuição em um construtor ou declaração. Para obter mais informações, consulte [Construtores](../Topic/Constructors%20\(C%23%20Programming%20Guide\).md).  
  
 CS0191 também ocorrerá se o `readonly` campo é [estático](../Topic/static%20\(C%23%20Reference\).md) e não está marcado como o construtor `static`.  
