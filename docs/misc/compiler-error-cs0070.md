...
O evento 'event' só pode aparecer à esquerda de \+ \= ou \-\= \(exceto quando usado de dentro do tipo 'type'\)  
 ...
 Fora da classe é definida em, uma [evento](../Topic/event%20\(C%23%20Reference\).md) só pode adicionar ou subtrair referências. Para obter mais informações, consulte [Eventos](../Topic/Events%20\(C%23%20Programming%20Guide\).md).  
