...
'member' não pode ser extern e declarar um corpo  
 ...
 Quando um membro de classe é marcado [extern](../Topic/extern%20\(C%23%20Reference\).md), isso significa que a definição do membro está localizada em outro arquivo. Portanto, um membro da classe marcada como **extern** não pode ser definido na classe. Remova o `extern` palavra\-chave ou excluir a definição. Para obter mais informações, consulte [Métodos](../Topic/Methods%20\(C%23%20Programming%20Guide\).md).  
