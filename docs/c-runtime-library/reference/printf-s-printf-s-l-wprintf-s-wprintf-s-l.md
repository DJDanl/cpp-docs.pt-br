...
O **printf_s** função formata e imprime uma série de caracteres e valores para o fluxo de saída padrão, **stdout**. Se argumentos siga o *formato* cadeia de caracteres, o *formato* cadeia de caracteres deve conter especificações que determinam o formato de saída para os argumentos.

A principal diferença entre **printf_s** e **printf** é que **printf_s** verifica a cadeia de caracteres de formato para formatação de caracteres válida, enquanto **printf**  verifica apenas se a cadeia de caracteres de formato é um ponteiro nulo. Se uma das verificações falhar, o manipulador de parâmetro inválido será invocado, conforme descrito em [Validação de parâmetro](../../c-runtime-library/parameter-validation.md). Se a execução é permitida para continuar, a função retornará -1 e conjuntos de **errno** para **EINVAL**.
...
**printf_s** e **fprintf_s** tenham comportamento idêntico, exceto que **printf_s** grava a saída **stdout** em vez de um destino de tipo **Arquivo**. Para obter mais informações, consulte [fprintf_s, _fprintf_s_l, fwprintf_s, _fwprintf_s_l](fprintf-s-fprintf-s-l-fwprintf-s-fwprintf-s-l.md).

**wprintf_s** é uma versão de caractere largo de **printf_s**; *formato* é uma cadeia de caracteres largos. **wprintf_s** e **printf_s** tenham comportamento idêntico, se o fluxo está aberto no modo de ANSI. **printf_s** atualmente não suportam a saída em um fluxo UNICODE.
