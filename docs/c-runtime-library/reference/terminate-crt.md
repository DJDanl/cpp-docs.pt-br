...
Chamadas `abort` ou uma função que você especifica usando `set_terminate`.  
...
## Comentários  
 A função de `terminate` é usada com manipulação de exceção C\+\+ e chamar nos seguintes casos:  
  
-   Um manipulador de correspondência de captura não pode ser localizado de uma exceção lançada C\+\+.  
  
-   Uma exceção é gerada por uma função de destruidor durante a pilha desenrola.  
  
-   A pilha está corrompida após lançada uma exceção.  
  
 chamadas `abort` de`terminate` por padrão.  Você pode alterar esse padrão escrevendo sua própria função de término e chamando `set_terminate` com o nome da função como seu argumento.  `terminate` chama a função last especificada como um argumento para `set_terminate`.  Para obter mais informações, consulte [Exceções sem\-tratamento C\+\+](../../cpp/unhandled-cpp-exceptions.md).  
...
```  
  ...
  **term\_func\(\) foi chamado por terminate\(\).**   
