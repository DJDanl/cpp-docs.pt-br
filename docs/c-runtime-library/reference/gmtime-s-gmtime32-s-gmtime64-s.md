...
ms.locfileid: "51332459"
---
# <a name="gmtimes-gmtime32s-gmtime64s"></a>gmtime_s, _gmtime32_s, _gmtime64_s

Converte um valor de tempo em um **tm** estrutura. Estas são versões de [_gmtime32, _gmtime64](gmtime-gmtime32-gmtime64.md) com melhorias de segurança como descrito em [Recursos de segurança no CRT](../../c-runtime-library/security-features-in-the-crt.md).
...
*tmDest*<br/>
Ponteiro para um [tm](../../c-runtime-library/standard-types.md) estrutura. Os campos da estrutura retornada contêm o valor avaliado do *temporizador* argumento em UTC em vez de no horário local.
...
|Não **nulo** (aponta para a memória válida)|**NULL**|**EINVAL**|Todos os campos definidos como -1.|
...
O **_gmtime32_s** função divide a *sourceTime* valor e o armazena em uma estrutura do tipo **tm**, definida em time. h. O endereço da estrutura é passado *tmDest*. O valor de *sourceTime* geralmente é obtido de uma chamada para o [tempo](time-time32-time64.md) função.
...
**_gmtime64_s**, que usa o **__time64_t** estrutura, permite que as datas sejam expressas até 23:59:59, 31 de dezembro de 3000, a UTC; enquanto **gmtime32_s** representa apenas datas até 23:59:59 18 de janeiro de 2038, UTC. Meia-noite de 1º de janeiro de 1970 é o limite inferior do intervalo de datas para essas duas funções.

**gmtime_s** é uma função embutida que é avaliada como **_gmtime64_s** e **time_t** é equivalente a **__time64_t**. Se você precisar forçar o compilador a interpretar **time_t** como o antigo 32-bit **time_t**, você pode definir **_USE_32BIT_TIME_T**. Essa ação fará **gmtime_s** seja embutido em **_gmtime32_s**. Isso não é recomendado, pois seu aplicativo poderá falhar após 18 de janeiro de 2038 e isso não é permitido em plataformas de 64 bits.
