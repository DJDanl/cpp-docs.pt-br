...
**%**[[*bandeiras]*](#flags) [[*largura*](#width)] [. [*precisão*](#precision)] [[*tamanho*](#size)] [*tipo*](#type)
...
> [!NOTE]
> No `printf` Visual Studio 2015 A família de `scanf` funções foi declarada como **inline** e movida para os `<stdio.h>` cabeçalhos. `<conio.h>` Se você estiver migrando código mais antigo, você pode ver *lNK2019* em conexão com essas funções. Para obter mais informações, consulte [Visual C++ change history 2003 - 2015](../porting/visual-cpp-change-history-2003-2015.md#stdio_and_conio).
...
|**Eu**|Integer|Inteiro decimal assinado.|
...
|**Um**|Ponto flutuante|Valor de ponto flutuante hexadecimal de dupla precisão assinado que tem a forma [-]0x*h.hhhh*__p±__*dd*, onde *h.hhhh* são os dígitos hexágonos (usando letras minúsculas) da mantissa, e *dd* são um ou mais dígitos para o expoente. A precisão especifica o número de dígitos após a vírgula.|
|**A**|Ponto flutuante|Valor de ponto flutuante hexadecimal de dupla precisão assinado que tem a forma [-]0X*h.hhhh*__P±__*dd*, onde *h.hhhh* são os dígitos hexa (usando letras maiúsculas) da mantissa, e *dd* são um ou mais dígitos para o expoente. A precisão especifica o número de dígitos após a vírgula.|
...
||Quando é usado com o **formato e**, **E**, **f** **#** , **F,** **a**, ou **A,** a bandeira força o valor de saída para conter um ponto decimal.|O ponto decimal será exibido somente se os dígitos o seguirem.|
...
Ao contrário da especificação de largura, a especificação de precisão pode causar o truncamento do valor de saída ou arredondamento de um valor de ponto flutuante. Se a *precisão* for especificada como 0 e o valor a ser convertido for 0, o resultado não será a saída de caracteres, como mostrado neste exemplo:
...
|----------|-------------|-------------|
|**a**, **A**|A precisão especifica o número de dígitos após a vírgula.|A precisão padrão é 13. Se a precisão for 0, nenhum ponto **#** decimal será impresso a menos que a bandeira seja usada.|
...
Um especificador de tipo **hc** ou **hC** é sinônimo de **c** em funções `printf` e de **C** em funções `wprintf`. Um especificador do tipo **lc,** **lC,** **wc**ou **wC** é sinônimo `wprintf` de **C** em `printf` funções e com **c** em funções. Um especificador de tipo **hs** ou **hS** é sinônimo de **s** em funções `printf` e de **S** em funções `wprintf`. Um especificador de tipo **ls**, **lS**, **ws** ou **wS** é sinônimo de **S** em funções `printf` e de **s** em funções `wprintf`.

> [!NOTE]
> **Específico da Microsoft:** Os prefixos **modificadores i** (maiúsculas i), **I32,** **I64**e **w** argument size são extensões da Microsoft e não são compatíveis com ISO C. O prefixo **h** quando é usado com o tipo de dados `char` e o prefixo **l** (L minúsculo) quando usado com o tipo de dados `double` são extensões da Microsoft.
