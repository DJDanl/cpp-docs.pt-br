...
> [!NOTE]
> Este artigo aborda o tipo de linguagem ISO Standard C++ **`enum`** e o tipo de **classe de enumeração** com escopo definido (ou fortemente tipado), que é introduzido no C++ 11. Para obter informações sobre a **classe de enumeração pública** ou tipos de **classe de enumeração privada** em c++/CLI e c++/CX, consulte [enum Class](../extensions/enum-class-cpp-component-extensions.md).
