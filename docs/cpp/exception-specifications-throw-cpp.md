...
```
informa o compilador que a função não lança exceções. However, in **/std:c++14** mode this could lead to undefined behavior if the function does throw an exception. Therefore we recommend using the [noexcept](../cpp/noexcept-cpp.md) operator instead of the one above:
...
As especificações explícitas de exceção não são permitidas em funções C. A C function is assumed not to throw exceptions under **/EHsc**, and may throw structured exceptions under **/EHs**, **/EHa**, or **/EHac**.
...
|C++ function with `noexcept`, `noexcept(true)`, or `throw()` exception specification|Não|Não|Sim|Sim|
|C++ function with `noexcept(false)`, `throw(...)`, or `throw(type)` exception specification|Sim|Sim|Sim|Sim|
