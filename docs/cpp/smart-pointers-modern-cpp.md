...
ms.locfileid: "81365546"
---
# <a name="smart-pointers-modern-c"></a>Ponteiros inteligentes (C++ moderno)

Na programação C++ moderna, a Biblioteca Padrão inclui *ponteiros inteligentes,* que são usados para ajudar a garantir que os programas estejam livres de vazamentos de memória e recursos e sejam seguros para exceções.

## <a name="uses-for-smart-pointers"></a>Uso de ponteiros inteligentes

Ponteiros inteligentes são `std` definidos no namespace no [ \<](../standard-library/memory.md) arquivo de cabeçalho>memória. Eles são cruciais para o idioma de programação de programação [raii](objects-own-resources-raii.md) ou *de aquisição de recursos.* O objetivo principal dessa linguagem é garantir que a aquisição de recursos ocorra ao mesmo tempo em que o objeto é inicializado, de forma que todos os recursos do objeto sejam criados e preparados em uma linha de código. Em termos práticos, o princípio fundamental da linguagem RAII é fornecer a propriedade de qualquer recurso alocado a heap, por exemplo, memória alocada dinamicamente ou identificadores de objetos do sistema, a um objeto alocado em pilha cujo destruidor contenha o código para excluir ou liberar o recurso e também qualquer código de limpeza associado.
...
O exemplo a `unique_ptr` seguir mostra como um tipo de ponteiro inteligente da Biblioteca Padrão C++ pode ser usado para encapsular um ponteiro para um objeto grande.
...
1. Declare o ponteiro inteligente como uma variável automática (local). (Não use **new** o `malloc` novo ou expressão no ponteiro inteligente em si.)

1. No parâmetro de tipo, especifique o tipo apontado do ponteiro encapsulado.

1. Passe um ponteiro bruto para um **novo**objeto no construtor de ponteirointeligente. (Algumas funções do utilitário ou construtores de ponteiro inteligente fazem isso para você.)

1. Use os operadores `->` e `*` sobrecarregados para acessar o objeto.

1. Deixe o ponteiro inteligente excluir o objeto.

Ponteiros inteligentes são criados para terem a maior eficiência possível em termos de memória e de desempenho. Por exemplo, o único membro de dados em `unique_ptr` é o ponteiro encapsulado. Isso significa que `unique_ptr` é exatamente do mesmo tamanho que o ponteiro, com quatro bytes ou com oito bytes. Acessar o ponteiro encapsulado usando o ponteiro inteligente sobrecarregado * e -> operadores não é significativamente mais lento do que acessar os ponteiros brutos diretamente.

Os ponteiros inteligentes têm suas próprias funções de membro, que são acessadas usando notação "ponto". Por exemplo, alguns ponteiros inteligentes c++ Standard Library têm uma função de membro de reset que libera a propriedade do ponteiro. Isso é útil quando você deseja liberar a memória possuída pelo ponteiro inteligente antes que o ponteiro inteligente saia do escopo, como mostrado no exemplo a seguir.

[!code-cpp[smart_pointers_intro#3](codesnippet/CPP/smart-pointers-modern-cpp_3.cpp)]

Os ponteiros inteligentes geralmente oferecem uma maneira de acessar diretamente seu ponteiro bruto. Os ponteiros inteligentes c++ Standard Library têm uma `get` função de membro para este fim e `CComPtr` tem um membro de classe pública. `p` Fornecendo acesso direto ao ponteiro subjacente, você pode usar o ponteiro inteligente para gerenciar a memória em seu próprio código e ainda passar o ponteiro bruto para o código que não oferece suporte a ponteiros inteligentes.
...
Use isso a menos que você não possa usar a ATL. Executa a contagem de referência usando os métodos `AddRef` e `Release`. Para obter mais informações, [consulte Como criar e usar instâncias CComPtr e CComQIPtr](how-to-create-and-use-ccomptr-and-ccomqiptr-instances.md).

[Classe CComQIPtr](../atl/reference/ccomqiptr-class.md)<br/>
É semelhante a `CComPtr` mas também fornece a sintaxe simplificada para chamar `QueryInterface` em objetos COM. Para obter mais informações, [consulte Como criar e usar instâncias CComPtr e CComQIPtr](how-to-create-and-use-ccomptr-and-ccomqiptr-instances.md).
...
Além de ponteiros inteligentes para objetos COM, o ATL também define ponteiros inteligentes e coleções de ponteiros inteligentes, para objetos C++ simples e antigos (POCO). Na programação clássica do Windows, esses tipos são alternativas úteis para as coleções da C++ Standard Library, especialmente quando a portabilidade de código não é necessária ou quando você não quer misturar os modelos de programação da C++ Standard Library e ATL.
...
Ponteiro inteligente para objetos que são alocados usando a função [C malloc.](../c-runtime-library/reference/malloc.md)
