...
Os programas C e C++ podem usar o mecanismo de manipulação de exceção estruturada (SEH) no sistema operacional Windows. Os conceitos em SEH são semelhantes àqueles em exceções do C++, exceto que o SEH usa as construções **__try**, **`__except`** , e **`__finally`** em vez de **`try`** e **`catch`** . No compilador do Microsoft C++ (MSVC), as exceções do C++ são implementadas para SEH. No entanto, ao escrever código C++, use a sintaxe de exceção C++.
