...
ms.locfileid: "81353727"
---
# <a name="multiple-base-classes"></a>Várias classes base

Uma classe pode ser derivada de mais de uma classe base. Em um modelo de herança múltipla (onde as classes são derivadas de mais de uma classe base), as classes base são especificadas usando o elemento de gramática *da lista base.* Por exemplo, a declaração de classe de `CollectionOfBook`, derivada de `Collection` e de `Book`, pode ser especificada:
...
Ao especificar a *lista base,* não é possível especificar o mesmo nome de classe mais de uma vez. No entanto, uma classe pode ser uma base indireta para uma classe derivada mais de uma vez.
...
Ao declarar uma classe base virtual, a palavra-chave **virtual** aparece nas listas base das classes derivadas.

Considere a hierarquia de classes na figura a seguir, que ilustra uma linha simulada de almoço.

![Gráfico da linha de almoço simulada](../cpp/media/vc38xp1.gif "Gráfico da linha de almoço simulada") <br/>
...
![Objeto de linha de&#45;de almoço simulado](../cpp/media/vc38xp2.gif "Objeto de linha de&#45;de almoço simulado") <br/>
Objeto simulado da linha de almoço
...
A palavra-chave **virtual** garante que apenas `Queue` uma cópia do subobjeto esteja incluída (veja a figura a seguir).

![Almoço simulado&#45;objeto de linha, classes de base virtuais](../cpp/media/vc38xp3.gif "Almoço simulado&#45;objeto de linha, classes de base virtuais") <br/>
Objeto simulado de linha de almoço com classes de base virtuais
...
![& virtuais não&#45;herança virtual no layout do objeto](../cpp/media/vc38xp5.gif "& virtuais não&#45;herança virtual no layout do objeto") <br/>
...
Se uma classe derivada substitui uma função virtual que herda de uma classe base virtual e, se um construtor ou um destruidor para a classe base derivada chamar essa função usando um ponteiro para a classe base virtual, o compilador virtual poderá inserir campos “vtordisp” adicionais ocultos nas classes com bases virtuais. A `/vd0` opção compilador suprime a adição do membro de deslocamento do construtor/destructor vtordisp oculto. A `/vd1` opção compilador, o padrão, permite-os onde forem necessários. Desative vtordisps apenas se você tiver certeza de que todos os destruidores e construtores da classe chamam funções virtuais virtualmente.

A `/vd` opção compilador afeta todo um módulo de compilação. Use `vtordisp` o pragma para suprimir `vtordisp` e, em seguida, reativar campos em uma base de classe por classe:
...
1. Se o acesso ao nome viola a permissão de acesso a membros, será gerada uma mensagem de erro. (Para obter mais informações, consulte [Controle de Acesso ao Membro](../cpp/member-access-control-cpp.md).)
...
- O efeito da aplicação do endereço**&** do operador ( ) a esse objeto. Observe que o operador address-of sempre fornece o endereço base do objeto.
...
### <a name="ambiguities-and-virtual-base-classes"></a>Ambiguidades e classes de base virtuais
...
![Derivação virtual e derivação virtual não&#45;](../cpp/media/vc38xr1.gif "Derivação virtual e derivação virtual não&#45;") <br/>
Derivação virtual vs. não virtual
