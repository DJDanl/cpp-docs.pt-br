...
As seções a seguir descrevem como transportar exceções usando o tipo de `exception_ptr` e as funções `current_exception`, `rethrow_exception`e `make_exception_ptr`.
...
Chame a função `current_exception` em um bloco **Catch** . Se uma exceção estiver em trânsito e o bloco **Catch** puder capturar a exceção, a função `current_exception` retornará um objeto `exception_ptr` que faz referência à exceção. Caso contrário, a função retornará um objeto `exception_ptr` nulo.
...
Se você usar a opção de compilador **/EHA** , poderá capturar uma exceção SEH em um C++ bloco **Catch** . A função `current_exception` retorna um objeto `exception_ptr` que faz referência à exceção SEH. E a função `rethrow_exception` lançará a exceção SEH se você chamá-la com o objeto de `exception_ptr` transportado como seu argumento.

A função `current_exception` retornará um `exception_ptr` nulo se você chamá-lo em um manipulador de terminação de **__FINALLY** Seh, um manipulador de exceção **__except** ou a expressão de filtro **__except** .
...
Chamar a função `make_exception_ptr` é equivalente a lançar uma C++ exceção, capturá-la em um bloco **Catch** e, em seguida, chamar a função `current_exception` para retornar um objeto `exception_ptr` que faz referência à exceção. A implementação da função `make_exception_ptr` da Microsoft é mais eficiente do que lançar e depois capturar uma exceção.
