...
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "declarando funções, sobrecarga"
  - "sobrecarga de função"
  - "sobrecarga de função, sobre sobrecarga de função"
...
|**const** ou `volatile` \(veja abaixo\)|Sim|  
 ...
 Embora as funções podem ser diferentes com base no tipo de retorno, eles não podem ser sobrecarregados dessa forma.  `Const` ou `volatile` só são usados como base para a sobrecarga se eles forem usados em uma classe para aplicar o **isso** ponteiro para a classe, não tipo de retorno da função.  Em outras palavras, a sobrecarga se aplica apenas se a palavra\-chave **const** ou `volatile` seguir a lista de argumentos da função na declaração.  
...
```  
 ...
 Para restrições de sobrecarga e obter informações sobre como a sobrecarga afeta outros elementos C\+\+, consulte [Sobrecarga](../misc/overloading-cpp.md).  
  
## Correspondência de argumento  
 As funções sobrecarregadas são selecionadas para corresponder melhor declarações de função no escopo atual aos argumentos fornecidos na chamada de função.  Se uma função apropriada for encontrada, essa função é chamada. "  Apropriada"neste contexto significa uma das seguintes opções:  
...
|-----------------------------------------------------------------------------|----------------------------------------------------------------------------|  
|2 Variant \(`int` pode ser convertido em `long` usando uma conversão padrão\)|Variant 1 \(`int` pode ser convertida em `long` usando uma conversão padrão\)|  
  
 Observe que a interseção entre esses dois conjuntos está vazia.  Em virtude disso, o compilador gera uma mensagem de erro.  
  
 Para a correspondência do argumento, uma função com *n*argumentos padrão é tratada como funções *n*\+1 separadas, cada uma com um número diferente de argumentos.  
...
## Diferenças de tipo de argumento  
 ...
 Entretanto, o mecanismo de sobrecarga de função pode distinguir entre as referências que estão qualificadas por **const** e `volatile` e referências ao tipo de base.  Isso torna o código como o seguinte possível:  
...
```  
 ...
 Os ponteiros para objetos **const** e `volatile` também são considerados diferentes dos ponteiros para o tipo de base para fins de sobrecarga.  
  
## Correspondência de argumento e conversões  
...
-   As sequências de conversões que podem ser encurtadas removendo as conversões intermediárias não são consideradas.  
 ...
 A sequência de conversões resultante, se houver, será considerada a melhor sequência de correspondência.  Há várias maneiras para converter um objeto do tipo `int` digitar `unsigned long`usando conversões padrão \(descrito no [conversões padrão](../cpp/standard-conversions.md)\):  
...
1.  Correspondência exata.  Uma correspondência exata entre os tipos com que a função é chamada e os tipos declarados no protótipo da função sempre é a melhor correspondência.  As sequências de conversões triviais são classificadas como correspondências exatas.  No entanto, as sequências que não fazem qualquer uma dessas conversões são consideradas melhor do que as sequências que convertem:  
    ...
    -   De ponteiro, em ponteiro em **const** \(`type` **\*** em **const** `type` **\***\).  
  
    -   De ponteiro, em ponteiro em `volatile` \(`type` **\*** em `volatile` `type` **\***\).  
...
3.  Correspondência usando conversões padrão.  Qualquer sequência não classificada como correspondência exata ou correspondência usando promoções que contém somente conversões padrão e conversões triviais é classificada como correspondência usando conversões padrão.  Nessa categoria, as seguintes regras são aplicadas:  
    ...
    -   A conversão de um ponteiro em uma classe derivada, em um ponteiro em uma classe base direta ou indireta é preferível à conversão em **void \*** ou **const void \***.  
 ...
 ![De PreferredConversionsGraph](../cpp/media/vc391t1.png "vc391T1")  
Gráfico ilustrando as conversões preferidas  
 ...
 ![De MultipleInheritanceGraph](../Image/vc391T2.gif "vc391T2")  
...
```  
 ...
 No exemplo anterior, a conversão definida pelo usuário, **operator long**, é chamada para converter `udc` no tipo **long**.  Se nenhuma conversão definida pelo usuário no tipo **long** for definida, a conversão continuaria da seguinte maneira: tipo `UDC` convertido no tipo `int` usando a conversão definida pelo usuário.  Em seguida, a conversão padrão do tipo `int` no tipo **long** seria aplicada para corresponder ao argumento na declaração.  
...
## Correspondência de argumento e este ponteiro  
 ...
 Quando o `– >` operador de seleção de membro é usado para acessar uma função de membro, o `this` tem o tipo de argumento de ponteiro `class-name` `* const`.  Se os membros são declarados como `const` ou `volatile`, os tipos são `const` `class-name``* const` e `volatile` `class-name` `* const`, respectivamente.  
...
```  
 ...
 O operando da esquerda a `–>*` e `.*` operadores \(ponteiro para membro\) são tratados da mesma forma que o `.` e `–>` operadores \(seleção de membro\) em relação à correspondência de argumento.  
...
-   Funções sobrecarregadas com listas de argumentos dos mesmos tipos, com base apenas no tipo de retorno, são um erro.  
 ...
 Você pode sobrecarregar **novo operador** somente de acordo com o tipo de retorno — especificamente, com base no modificador de modelo de memória especificado.  
