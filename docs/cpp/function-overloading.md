...
Por exemplo, uma função `print` que usa um argumento `std::string` pode executar tarefas muito diferentes de uma que usa um argumento do tipo **Double**. A sobrecarga evita que você precise usar nomes como `print_string` ou `print_double`. No momento da compilação, o compilador escolhe qual sobrecarga usar com base no tipo de argumentos passado pelo chamador.  Se você chamar `print(42.0)`, a função `void print(double d)` será invocada. Se você chamar `print("hello world")`, a sobrecarga de `void print(std::string)` será invocada.
...
|Uso de nomes de **typedef**|Não|
|Limites de matriz não especificados|Não|
|**const** ou **volatile**|Sim, quando aplicado à função inteira|
...
|Conjunto 1: Funções candidatas que têm o primeiro argumento do tipo fração|Conjunto 2: Funções candidatas cujo segundo argumento pode ser convertido para o tipo **int**|
|--------------------------------------------------------------------------|-----------------------------------------------------------------------------------|
|Variant 1|A variante 1 (**int** pode ser convertida para **longa** utilização de uma conversão padrão)|
|Variante 3||
...
|---------------------------------------------------------------------|----------------------------------------------------------------------|
|A variante 2 (**int** pode ser convertida para **longa** utilização de uma conversão padrão)|A variante 1 (**int** pode ser convertida para **longa** utilização de uma conversão padrão)|
...
Pelo mesmo motivo, os argumentos de função de um tipo modificado por **const** ou **volátil** não são tratados de maneira diferente do tipo base para fins de sobrecarga.
...
Ponteiros para objetos **const** e **volatile** também são considerados diferentes de ponteiros para o tipo base para fins de sobrecarga.
...
A sequência de conversões resultante, se houver, será considerada a melhor sequência de correspondência. Há várias maneiras de converter um objeto do tipo **int** para tipo **sem sinal** usando conversões padrão (descritas em [conversões padrão](../cpp/standard-conversions.md)):

- Converta de **int** para **longa** e, em seguida, de **longo** para **não assinado longo**.

- Converta de **int** para **sem sinal**.
...
1. Correspondência exata. Uma correspondência exata entre os tipos com que a função é chamada e os tipos declarados no protótipo da função sempre é a melhor correspondência. As sequências de conversões triviais são classificadas como correspondências exatas. No entanto, as sequências que não fazem nenhuma dessas conversões são consideradas melhores do que as sequências que são convertidas:
   ...
   - De ponteiro, para ponteiro para **const** (`type` <strong>\*</strong> a **const** `type` <strong>\*</strong>).

   - De ponteiro, para ponteiro para **volátil** (`type` <strong>\*</strong> para **volátil** `type` <strong>\*</strong>).

   - De referência, para referência a **const** (`type` **&** a **const** `type` **&** ).

   - De referência, para fazer referência a **volátil** (`type` **&** como **volátil** `type` **&** ).

1. Correspondência usando promoções. Qualquer sequência não classificada como uma correspondência exata que contenha apenas promoções integrais, conversões de **float** para **Double**e conversões triviais são classificadas como uma correspondência usando promoções. Embora não seja tão boa quanto a correspondência exata, a correspondência usando promoções é melhor do que a correspondência usando conversões padrão.
...
A conversão do tipo `C*` no tipo `B*` é preferível à conversão do tipo `C*` no tipo `A*`. A razão é que eles estão no mesmo caminho, e `B*` é mais próximo. No entanto, a conversão do tipo `C*` para o tipo `D*` não é preferível à conversão para o tipo `A*`; Não há preferência porque as conversões seguem caminhos diferentes.
...
As conversões disponíveis definidas pelo usuário para a classe `UDC` são do tipo **int** e do tipo **Long**. Portanto, o compilador considera conversões para o tipo do objeto que está sendo correspondido: `UDC`. Existe uma conversão para **int** e ela está selecionada.
...
No exemplo anterior, a conversão definida pelo usuário, **operador Long**, é invocada para converter `udc` para o tipo **Long**. Se nenhuma conversão definida pelo usuário para o tipo **Long** tiver sido definida, a conversão teria prosseguido da seguinte maneira: O tipo `UDC` teria sido convertido para o tipo **int** usando a conversão definida pelo usuário. Em seguida, a conversão padrão do tipo **int** para o tipo **Long** teria sido aplicada para corresponder ao argumento na declaração.
...
- Converta de tipo **int** para tipo `UDC1` (uma conversão definida pelo usuário).

- Converter do tipo **int** para o tipo **Long**; em seguida, converta para o tipo `UDC2` (uma conversão em duas etapas).
...
As funções de membro de classe são tratadas de forma diferente, dependendo se elas são declaradas como **estáticas**. Como as funções não estáticas têm um argumento implícito que fornece **esse** ponteiro, as funções não estáticas são consideradas para ter mais um argumento do que funções estáticas; caso contrário, eles serão declarados de forma idêntica.

Essas funções de membro não estáticos exigem que o **ponteiro tenha** o mesmo tipo de objeto por meio do qual a função está sendo chamada ou, para operadores sobrecarregados, eles exigem que o primeiro argumento corresponda ao objeto no qual o operador está sendo aplicado. (Para obter mais informações sobre operadores sobrecarregados, consulte [operadores sobrecarregados](../cpp/operator-overloading.md).)

Ao contrário de outros argumentos em funções sobrecarregadas, nenhum objeto temporário é introduzido e nenhuma tentativa de conversões é feita ao tentar corresponder ao argumento **de ponteiro.**

Quando o operador de seleção de membro `->` é usado para acessar uma função de membro da classe `class_name`, **esse** argumento de ponteiro tem um tipo de `class_name * const`. Se os membros forem declarados como **const** ou **voláteis**, os tipos serão `const class_name * const` e `volatile class_name * const`, respectivamente.
...
Os qualificadores de referência possibilitam a sobrecarga de uma função de membro com base em se o objeto apontado por **isso** é um Rvalue ou um lvalue.  Esse recurso pode ser usado para evitar operações de cópia desnecessárias em cenários em que você opta por não fornecer acesso de ponteiro aos dados. Por exemplo, suponha que a classe `C` inicialize alguns dados em seu construtor e retorne uma cópia desses dados na `get_data()`de função de membro. Se um objeto do tipo `C` for um Rvalue que está prestes a ser destruído, o compilador escolherá a sobrecarga `get_data() &&`, que moverá os dados em vez de copiá-los.
...
- as declarações de **typedef** não definem novos tipos; Eles apresentam sinônimos para tipos existentes. Eles não afetam o mecanismo de sobrecarga. Considere o código a seguir:
...
O código anterior mostra duas definições da função `func`. A definição que usa um argumento do tipo `char *` é local para `main` devido à instrução **externa** . Portanto, a definição que usa um argumento do tipo **int** é ocultada e a primeira chamada para `func` está em erro.
