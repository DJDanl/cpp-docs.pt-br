...
**Visual Studio 2017 versão 15.5**: o compilador do Visual C++ dá suporte a cerca de 75% dos recursos do C++17, incluindo associações estruturadas, lambdas `constexpr`, `if constexpr`, variáveis embutidas, expressões de dobra e adição de `noexcept` ao tipo sistema. Eles estão disponíveis na opção /std:c++17. O modo de conformidade /permissive- inclui suporte parcial para pesquisa de nome em duas fases. Para obter mais informações, consulte [Melhorias na conformidade do C++ no Visual Studio 2017](cpp-conformance-improvements-2017.md). 
...
* basic_string\<char16_t> agora emprega otimizações memcmp/memcpy/etc. que basic_string\<wchar_t> emprega.
* Uma limitação do otimizador que impedia que os ponteiros de função fossem embutidos e que foi exposta pelo nosso trabalho “evitar a cópia de funções” no Visual Studio 2015 Atualização 3 foi contornada, restaurando o desempenho de lower_bound(iter, iter, function pointer).
* A sobrecarga da verificação da ordem de depuração do iterador de entradas para inclusões, set_difference, set_symmetric_difference e set_union foi reduzida pelos iteradores de decodificação antes de verificar a ordem.
* std::inplace_merge agora ignora elementos que já estão no lugar.
* Construir std::random_device não constrói e, em seguida, destrói um std::string.
* std::equal e std::partition tinham uma passagem de otimização de jump-threading que salva uma comparação de iterador.
* Quando std::reverse é passado e aponta para T facilmente copiado, ele agora enviará uma implementação vetorizada manuscrita.
* std::fill, std::equal e std::lexicographical_compare aprenderam como enviar para memset / memcmp para std::byte e gsl::byte (e outras enumerações e classes de enumerações com caracteres). Observe que std::copy realiza o envio usando is_trivially_copyable e, portanto, não precisa mudar.
* A STL não contém destruidores com chaves vazias cujo único comportamento era tornar os tipos em não facilmente destrutíveis.

#### <a name="correctness-fixes"></a>Correções de exatidão
* std::partition agora chama o predicado vezes e não N + 1 vezes, como o padrão exige.
* As tentativas de evitar estáticas mágicas no 15.3 foram reparadas no 15.5.
...
* __declspec(allocator) agora é protegido apenas para C1XX a fim de evitar avisos de Clang que não entendem esse declspec.
* basic_string::npos agora está disponível como uma constante de tempo de compilação.
* std::allocator agora manipula corretamente a alocação de tipos excessivamente alinhados – aqueles cujo alinhamento é superior a max_align_t – no modo C++17 (a menos que desabilitado por /Zc:alignedNew-).  Por exemplo, vetores de objetos com alinhamento de 16 ou 32 bytes agora serão corretamente alinhados para instruções SSE / AVX.
...
**Visual Studio 2017 versão 15.5**: diversas operações C++ que usam o mecanismo IntelliSense para refatoração e navegação de código são executadas muito mais rápido. Os números a seguir se baseiam na solução do Visual Studio Chromium com 3500 projetos: 
|||
|-|-|
...
O C++ agora dá suporte a Ctrl + Clique em Ir para a Definição, facilitando a navegação do mouse para definições. O Visualizador de Estrutura do pacote de Ferramentas Avançadas de Produtividade agora também está incluído no produto por padrão.
...
**Visual Studio 2017 versão 15.3**: suporte adicionado para o gerador Ninja do CMake. Para saber mais, veja [Projetos CMake no Visual C++](ide/cmake-tools-for-visual-cpp.md).  

**Visual Studio 2017 versão 15.5**: suporte adicionado para importação de caches de CMake existentes. Para saber mais, veja [Projetos CMake no Visual C++](ide/cmake-tools-for-visual-cpp.md).  
...
**Visual Studio 2017 versão 15.5**:
1. A carga de trabalho do Linux adicionou suporte a rsync como uma alternativa ao sftp para sincronização de arquivos para computadores Linux remotos.  
2. Foi adicionado suporte compilação cruzada direcionada a microcontroladores ARM. Para habilitar isso na instalação, escolha o desenvolvimento do Linux com carga de trabalho C++ e selecione a opção para Desenvolvimento IoT e Inserido. Isso adiciona as ferramentas de compilação cruzada do ARM GCC e Make à sua instalação. Para obter mais informações, consulte [Compilação cruzada do ARM GCC no Visual Studio](https://blogs.msdn.microsoft.com/vcblog/2017/10/23/arm-gcc-cross-compilation-in-visual-studio/).
3. Foi adicionado o suporte a CMake. Agora, você pode trabalhar em sua base de código existente do CMake sem precisar convertê-la em um projeto do Visual Studio. Para obter mais informações, consulte [Configurar um projeto do Linux CMake](linux/cmake-linux-project.md).
4. Foi adicionado suporte à execução de tarefas remotas. Essa capacidade permite executar qualquer comando em um sistema remoto definido no Gerenciador de Conexões do Visual Studio. As tarefas remotas também fornecem a capacidade de copiar arquivos para o sistema remoto. 
...
**Visual Studio 2017 versão 15.5**  
Foi adicionado um modelo de projeto do **Projeto de Empacotamento de Aplicativo do Windows**, que dá suporte ao empacotamento de aplicativos de área de trabalho usando a Ponte de Desktop. Ele está disponível em **Arquivo | Novo | Projeto | Instalado | Visual C++ | Plataforma Universal do Windows**.
...
**Visual Studio 2017 versão 15.5**: o Google Test Adapter e o Boost.Test Adapter agora estão disponíveis como componentes do **Desenvolvimento de Área de Trabalho com C++** e são integrados ao **Gerenciador de Testes**. Foi adicionado o suporte a CTest para projetos Cmake (usando Abrir Pasta), embora a integração total com o **Gerenciador de Testes** ainda não esteja disponível. Para obter mais informações, consulte [Escrevendo testes de unidade para C/C++](/visualstudio/test/writing-unit-tests-for-c-cpp).
