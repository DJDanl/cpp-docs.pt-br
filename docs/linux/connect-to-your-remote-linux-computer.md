...
description: Como se conectar a um computador Linux remoto ou a um subsistema do Windows para Linux de dentro de um projeto do Visual Studio C++.
ms.date: 01/17/2020
...
Ao usar uma conexão remota, o Visual Studio compila projetos C++ do Linux no computador remoto. Não importa se é uma máquina física, uma VM na nuvem ou WSL.
...
Para configurar a instalação do WSL para funcionar com o Visual Studio, você precisará das seguintes ferramentas instaladas: gcc ou Clang, gdb, Make, ninja-Build (necessário somente para projetos CMake usando o Visual Studio 2019 versão 16,6 ou posterior), rsync e zip. Você pode instalá-los em distribuições que usam a apt usando esse comando, que também instala o compilador g + +:
