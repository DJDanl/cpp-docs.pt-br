---
title: Configurar o desenvolvimento Linux remoto seguro em conformidade com FIPS
...
---
# <a name="set-up-fips-compliant-secure-remote-linux-development"></a>Configurar o desenvolvimento Linux remoto seguro em conformidade com FIPS
...
1. Abra */etc/ssh/sshd_config* como raiz. Edite (ou adicione, se eles não existirem) as seguintes linhas:
   ...
   > [!NOTE]
   > SSH-RSA é o único algoritmo de chave de host compatível com FIPS versus suporta. Os algoritmos de\*-CTR do AES também são compatíveis com FIPS, mas a implementação no Visual Studio não é aprovada. Os algoritmos de troca de chave ECDH-\* são compatíveis com FIPS, mas o Visual Studio não dá suporte a eles.
