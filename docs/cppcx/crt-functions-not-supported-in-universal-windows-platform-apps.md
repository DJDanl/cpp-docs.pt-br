...
Muitas funções de tempo de execução C (CRT) não estão disponíveis quando você cria aplicativos Plataforma Universal do Windows (UWP). Às vezes, as soluções alternativas estão disponíveis — por exemplo, você pode usar APIs do Windows Runtime ou do Win32. Em outros casos, as funções de CRT foram banidas porque os recursos correspondentes ou as APIs de suporte não são aplicáveis aos aplicativos UWP. Para procurar um método alternativo com suporte para o Windows Runtime, consulte [alternativas para APIs do Windows em aplicativos UWP](/uwp/win32-and-com/alternatives-to-windows-apis-uwp).

A tabela a seguir lista as funções do CRT que não estão disponíveis quando você cria aplicativos UWP. Ele indica as soluções alternativas que se aplicam.
...
|`getpid` `_getpid` | Essas funções estão obsoletas.|Use a API do Win32 `GetCurrentProcessId`.|
