...
2.  Escolha a versão **Prompt de Comando do Desenvolvedor** que corresponde à versão do Visual Studio que você está usando quando ela aparecer na lista de correspondências.  
  
3.  No console **Prompt de Comando do Desenvolvedor**, digite o comando `cl /Bv /CLR`.  
...
2.  Localize a linha que contém o nome do arquivo de código-fonte em que o problema ocorre. A linha abaixo dela contém o comando cl.exe correspondente e seus argumentos.  
...
### <a name="the-repro"></a>A reprodução  
 Precisamos de uma reprodução — um exemplo de código-fonte independente que demonstra o problema encontrado — para que podemos pode reproduzir o erro em nossos computadores. O tipo de problema encontrado determinará o tipo de reprodução que você deverá incluir em seu relatório. Sem uma reprodução apropriada, não temos nada para investigar.  
...
#### <a name="frontend-parser-crash"></a>Falha de front-end (analisador)  
 ...
 Para esse tipo de falha, forneça uma [Reprodução pré-processada](#preprocessed_repro).  
...
####  <a name="backend_crash"></a> Falha de back-end (geração de código)  
 ...
 Aqui está a saída do compilador de exemplo para esse tipo de falha em que a LTCG **não** é usada. Se a saída do compilador tiver esta aparência, você deverá fornecer uma [Reprodução pré-processada](#preprocessed_repro).  
...
>  Se a saída menciona C1001 ou envolve a Geração de Código Durante o Tempo de Vinculação, consulte [Falha de back-end (geração de código)](#backend_crash) em vez disso para obter mais informações.  
 ...
 Para esse tipo de falha, forneça uma [Reprodução de vinculação](#link_repro).  
...
### <a name="file-a-bug-on-microsoft-connect"></a>Registrar um bug no Microsoft Connect  
 O Microsoft Connect ([connect.microsoft.com](http://connect.microsoft.com)) é uma maneira de nossa comunidade de usuários se conectar diretamente às equipes que criam os produtos da Microsoft. No Connect, você pode registrar novos bugs e fazer solicitações de recursos, exibir outros bugs e solicitações feitas pela comunidade e dizer quais são mais importantes para você.  
  
 Relatar o problema por meio do Connect é melhor quando você deseja compartilhar seu relatório com a comunidade do Visual Studio e acompanhar seu progresso publicamente. Ao compartilhar seu relatório, outras pessoas, às vezes, podem fornecer soluções alternativas ou informações adicionais que podem nos ajudar a diagnosticar o problema. Você ainda pode usar o Connect se quiser manter o relatório privado (por exemplo, quando você não pode compartilhar o código em seu relatório), apenas certifique-se de definir a visibilidade do relatório como Particular antes de enviar o formulário.  
  
-   [Microsoft Connect: Report a problem with Visual Studio 2015 Update 3](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6493) (Microsoft Connect: relatar um problema com o Visual Studio 2015 atualização 3)  
  
-   [Microsoft Connect: Report a problem with Visual Studio 2015 Update 2](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6406) (Microsoft Connect: relatar um problema com o Visual Studio 2015 atualização 2)  
  
-   [Microsoft Connect: Report a problem with Visual Studio 2015 Update 1](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6326) (Microsoft Connect: relatar um problema com o Visual Studio 2015 atualização 1)  
  
-   [Microsoft Connect: Report a problem with Visual Studio 2015](https://connect.microsoft.com/VisualStudio/Feedback/LoadSubmitFeedbackForm?FormID=6240) (Microsoft Connect: relatar um problema com o Visual Studio 2015)  
  
 Você pode relatar um problema com outros produtos do Visual Studio e do .Net Framework localizando o produto na lista suspensa na página do Connect [Visual Studio and .NET Framework feedback](https://connect.microsoft.com/VisualStudio/feedback/LoadSubmitFeedbackForm) (Comentários sobre o Visual Studio e .NET Framework).  
...
### <a name="send-an-email"></a>Enviar um email  
 ...
 Relatar o problema por email não tem a vasta comunidade do Microsoft Connect, mas às vezes pode ser melhor para reproduções grandes. Ele também pode ser a melhor ou a única opção se o seu ambiente de trabalho não estiver conectado à Internet ou você não puder usar o Microsoft Connect.  
...
3.  Na janela do console **Prompt de Comando do Desenvolvedor**, insira o comando `cl /P argumentsfilename.cpp`.  
...
3.  Na janela do console **Prompt de Comando do Desenvolvedor**, insira o comando `cl arguments /TP filename.i`.  
  
4.  Confirme que o problema é reproduzido.  
...
2.  Defina a variável de ambiente link_repro para o diretório que você acabou de criar. Digite o comando `set link_repro=directory`.  
  
3.  Se você quiser executar o build de dentro do Visual Studio, inicie-o do prompt de comando inserindo o comando `devenv`. Isso garante que o valor da variável de ambiente link_repro esteja visível para o Visual Studio.  
  
4.  Compile seu aplicativo e confirme se o problema esperado ocorreu.  
  
5.  Feche o Visual Studio agora se você o iniciou na etapa 3.  
  
6.  Limpe a variável de ambiente link_repro. Digite o comando `set link_repro=`  
