...
ms.openlocfilehash: 2ee2aba1e0fee2e5fe05563bec5e005693f4390a
...
```  
 ...
 A tabela a seguir resume as conversões de tipos integrais com sinal. Ela pressupõe que, por padrão, o tipo `char` tem sinal. Se você usar uma opção de tempo de compilação para alterar o padrão do tipo `char` para sem sinal, as conversões fornecidas na tabela [Conversões de tipos integrais sem sinal](../c-language/conversions-from-unsigned-integral-types.md) para o tipo `unsigned char` serão aplicadas no lugar das conversões indicadas na tabela a seguir (Conversões de tipos integrais com sinal).  
