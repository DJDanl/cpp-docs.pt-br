...
|[const_iterator](#const_iterator)|Um tipo que fornece um iterador bidirecional que pode ler um **`const`** elemento no `hash_multiset` .|
|[const_pointer](#const_pointer)|Um tipo que fornece um ponteiro para um **`const`** elemento em um `hash_multiset` .|
|[const_reference](#const_reference)|Um tipo que fornece uma referência a um **`const`** elemento armazenado em um `hash_multiset` para leitura e execução de **`const`** operações.|
|[const_reverse_iterator](#const_reverse_iterator)|Um tipo que fornece um iterador bidirecional que pode ler qualquer **`const`** elemento no `hash_multiset` .|
...
|[repeti](#iterator)|Um tipo que fornece um iterador bidirecional que pode ler ou modificar qualquer elemento em um `hash_multiset`.|
...
|[refere](#pointer)|Um tipo que fornece um ponteiro para um elemento em um `hash_multiset`.|
...
|-|-|
|[Comece](#begin)|Retorna um iterador que trata do primeiro elemento no `hash_multiset`.|
...
|[formatação](#clear)|Apaga todos os elementos de um `hash_multiset`.|
...
|[completo](#end)|Retorna um iterador que trata o local após o último elemento em um `hash_multiset`.|
...
|[permuta](#swap)|Troca os elementos de dois `hash_multiset`s.|
...
Na maioria dos casos, um [const_iterator](#const_iterator) deve ser usado para acessar os elementos em um **`const`** objeto hash_multiset.
...
Um tipo que fornece uma referência a um **`const`** elemento armazenado em um hash_multiset para leitura e execução de **`const`** operações.
...
Um tipo que fornece um iterador bidirecional que pode ler qualquer **`const`** elemento na hash_multiset.
...
## <a name="hash_multisetfind"></a><a name="find"></a>hash_multiset:: localizar
...
A função membro retorna um iterador que resolve um elemento no hash_multiset cuja chave de classificação é `equivalent` para a chave de argumento em um predicado binário que induzi uma ordenação com base em uma relação comparação menor que.
...
Os três primeiros construtores especificam um inicial vazio `hash_multiset` , o segundo que especifica o tipo de função de comparação (*comp*) a ser usado no estabelecimento da ordem dos elementos e da terceira especificação explícita do tipo de alocador (*Al*) a ser usado. A palavra-chave **`explicit`** suprime determinados tipos de conversão automática de tipo.
...
## <a name="hash_multisetiterator"></a><a name="iterator"></a>hash_multiset:: iterador
...
Depois de apagar todos os elementos existentes em um `hash_multiset` , `operator=` o copia ou move o conteúdo do *lado* para o `hash_multiset` .
...
Observe que ambos os [key_compare](#key_compare) e `value_compare` são sinônimos para as *características*do parâmetro de modelo. Os dois tipos são fornecidos para as classes set e multiset, em que são idênticos, para compatibilidade com as classes map e multimap, em que são diferentes.
