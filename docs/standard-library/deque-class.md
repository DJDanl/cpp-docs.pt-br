...
*Alocador*\
O tipo que representa o objeto alocador armazenado que encapsula detalhes sobre a alocação e desalocação de memória do deque. Esse argumento é opcional e o valor padrão é **alocador \<Type> **.
...
|||
|-|-|
|[deque](#deque)|Constrói um `deque`. Vários construtores são fornecidos para configurar o conteúdo do novo de `deque` maneiras diferentes: vazio; carregado com um número especificado de elementos vazios; conteúdo movido ou copiado de outro `deque` ; conteúdo copiado ou movido usando um iterador; e um elemento copiado para os `deque` `count` horários. Alguns dos construtores habilitam usando um `allocator` personalizado para criar elementos.|
...
|[const_iterator](#const_iterator)|Um tipo que fornece um iterador de acesso aleatório que pode acessar e ler elementos no `deque` as**`const`**|
...
|[const_reverse_iterator](#const_reverse_iterator)|Um tipo que fornece um iterador de acesso aleatório que pode acessar e ler elementos no `deque` as **`const`** . O deque é exibido na ordem inversa. Para obter mais informações, consulte [Classe reverse_iterator](../standard-library/reverse-iterator-class.md)|
...
|[repeti](#iterator)|Um tipo que fornece um iterador de acesso aleatório que pode ler ou modificar qualquer elemento em um `deque`.|
|[refere](#pointer)|Um tipo que fornece um ponteiro para um elemento em um `deque`.|
...
|[Voltar](#back)|Retorna uma referência para o último elemento do `deque`.|
|[Comece](#begin)|Retorna um iterador de acesso aleatório que trata o primeiro elemento no `deque`.|
|[cbegin](#cbegin)|Retorna um iterador const para o primeiro elemento no `deque`.|
|[cend](#cend)|Retorna um iterador de acesso aleatório **`const`** que aponta muito além do fim do `deque` .|
|[formatação](#clear)|Apaga todos os elementos de um `deque`.|
...
|[empty](#empty)|Retorna **`true`** se o `deque` contém zero elementos e **`false`** se ele contém um ou mais elementos.|
|[completo](#end)|Retorna um iterador de acesso aleatório que aponta para imediatamente após o fim do `deque`.|
|[erase](#erase)|Remove um elemento ou um intervalo de elementos em um `deque` das posições especificadas.|
|[dianteiro](#front)|Retorna uma referência para o primeiro elemento em um `deque`.|
...
|[operador =](#op_eq)|Substitui os elementos do `deque` por uma cópia de outro `deque`.|
...
Se o *PDV* for maior que o tamanho do deque, o `at` lançará uma exceção.

### <a name="return-value"></a>Valor retornado

Se o valor retornado `at` for atribuído a um `const_reference`, o objeto do deque não poderá ser modificado. Se o valor retornado `at` for atribuído a um `reference`, o objeto do deque poderá ser modificado.
...
Se o valor retornado `back` for atribuído a um `const_reference`, o objeto do deque não poderá ser modificado. Se o valor retornado `back` for atribuído a um `reference`, o objeto do deque poderá ser modificado.

Quando compilado usando [_ITERATOR_DEBUG_LEVEL](../standard-library/iterator-debug-level.md) definido como 1 ou 2, um erro de runtime ocorrerá se você tentar acessar um elemento em um deque vazio.  Consulte [Iteradores Verificados](../standard-library/checked-iterators.md) para obter mais informações.
...
Se o valor retornado `begin` for atribuído a um `const_iterator`, o objeto do deque não poderá ser modificado. Se o valor de retorno de `begin` for atribuído a um `iterator` , o objeto deque poderá ser modificado.
...
Um tipo que fornece um iterador de acesso aleatório que pode acessar e ler um **`const`** elemento no deque.
...
Um tipo que fornece uma referência a um **`const`** elemento armazenado em um deque para leitura e execução de **`const`** operações.
...
Um tipo que fornece um iterador de acesso aleatório que pode ler qualquer **`const`** elemento no deque.
...
*Contar*\
O número de elementos no deque construído.

*Val*\
O valor dos elementos no deque construído.

*Certo*\
O deque dos quais o deque construído deve ser uma cópia.
...
Todos os construtores armazenam um objeto de alocador (*Al*) e inicializam o deque.
...
O terceiro construtor Especifica uma repetição de um número especificado ( `count` ) de elementos do valor padrão para classe `Type` .

Os quarto e quinto construtores especificam uma repetição de valores (*Count*) de valor `val` .

O sexto Construtor especifica uma cópia do *direito*deque.

Os sétimo e oitavo construtores copiam o intervalo `[First, Last)` de um deque.

O sétimo Construtor move o deque para a *direita*.
...
*_Where*\
A posição no [deque](../standard-library/deque-class.md) em que o primeiro elemento é inserido.
...
*Val*\
O elemento adicionado ao final do [deque](../standard-library/deque-class.md).
...
*Val*\
O elemento adicionado ao início do [deque](../standard-library/deque-class.md).
...
`end`é usado para testar se um iterador atingiu o final de seu deque.
...
Se o valor retornado `front` for atribuído a um `const_reference`, o objeto do deque não poderá ser modificado. Se o valor retornado `front` for atribuído a um `reference`, o objeto do deque poderá ser modificado.
...
*Posição*\
A posição no deque de destino em que o primeiro elemento é inserido.

*Val*\
O valor do elemento que está sendo inserido no deque.

*Contar*\
O número de elementos que estão sendo inseridos no deque.

*Primeiro*\
A posição do primeiro elemento no intervalo de elementos no deque de argumentos a ser copiada.

*Última*\
A posição do primeiro elemento após o intervalo de elementos no deque de argumentos a ser copiada.

*IList*\
A initializer_list de elementos a ser inserida.
...
Se o valor retornado `operator[]` for atribuído a um `const_reference`, o objeto do deque não poderá ser modificado. Se o valor retornado `operator[]` for atribuído a um `reference`, o objeto do deque poderá ser modificado.

Quando compilado usando [_ITERATOR_DEBUG_LEVEL](../standard-library/iterator-debug-level.md) definido como 1 ou 2, um erro de runtime ocorrerá se você tentar acessar um elemento fora dos limites do deque.  Consulte [Iteradores Verificados](../standard-library/checked-iterators.md) para obter mais informações.
...
*Certo*\
O deque que fornece o novo conteúdo.

### <a name="remarks"></a>Comentários

A primeira substituição copia os elementos para esse deque da *direita*, a origem da atribuição. A segunda substituição move os elementos para esse deque da *direita*.
...
Se o tamanho do deque for menor que o tamanho solicitado, *_Newsize*, os elementos serão adicionados ao deque até atingir o tamanho solicitado.

Se o tamanho do deque for maior que o tamanho solicitado, os elementos mais próximos ao final do deque serão excluídos até que deque atinja o tamanho *_Newsize*.
...
*mantida*\
Um deque cujos elementos devem ser trocados pelos do deque *direito*.
