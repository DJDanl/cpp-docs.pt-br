...
O objeto ordena a sequência que ele controla chamando dois objetos armazenados, um objeto de função de comparação do tipo [unordered_multimap::key_equal](#key_equal) e um objeto de função de hash do tipo [unordered_multimap::hasher](#hasher). Você acessa o primeiro objeto armazenado chamando a função de membro [unordered_multimap::key_eq](#key_eq)`()`; e você acessa o segundo objeto armazenado chamando a função de membro [unordered_multimap::hash_function](#hash)`()`. Especificamente, para todos os valores `X` e `Y` do tipo `Key`, a chamada `key_eq()(X, Y)` retornará true somente se os dois valores de argumento tiverem ordem equivalente; a chamada `hash_function()(keyval)` resulta em uma distribuição de valores do tipo `size_t`. Ao contrário do modelo de `unordered_multimap` classe unordered_map `key_eq()(X, Y)` [Classe](../standard-library/unordered-map-class.md), um objeto de tipo não garante que seja sempre falso para qualquer dois elementos da seqüência controlada. (As chaves não precisam ser exclusivas.)
...
## <a name="unordered_multimapbegin"></a><a name="begin"></a>unordered_multimap::começar
...
## <a name="unordered_multimapbucket"></a><a name="bucket"></a>unordered_multimap::balde
...
## <a name="unordered_multimapcount"></a><a name="count"></a>unordered_multimap::contagem
...
## <a name="unordered_multimapempty"></a><a name="empty"></a>unordered_multimap::vazio
...
## <a name="unordered_multimapend"></a><a name="end"></a>unordered_multimap::fim
...
## <a name="unordered_multimaperase"></a><a name="erase"></a>unordered_multimap::apagar
...
## <a name="unordered_multimapinsert"></a><a name="insert"></a>unordered_multimap::inserir
...
|*Valty*|Parâmetro de modelo que especifica o tipo de argumento que o unordered_multimap pode usar para construir um elemento de [value_type](../standard-library/map-class.md#value_type), e aperfeiçoa *Val* como argumento.|
...
A função membro de intervalo (5) insere a seqüência de valores do elemento `[First, Last)`em um unordered_multimap que corresponde a cada elemento abordado por um iterizador na faixa; portanto, *o último* não é inserido. A função de membro do contêiner `end()` refere-se à posição logo após o último elemento no contêiner, por exemplo, a instrução `m.insert(v.begin(), v.end());` insere todos os elementos de `v` em `m`.
...
A função `(float)`membro retorna [unordered_multimap::tamanho](#size)`() / (float)`[unordered_multimap:bucket_count](#bucket_count)`()`, o número médio de elementos por balde.
...
|Parâmetro|Descrição|
|-|-|
|*Certo*|Unordered_multimap que está sendo copiado no unordered_multimap.|

### <a name="remarks"></a>Comentários

Depois de apagar quaisquer elementos `operator=` existentes em uma unordered_multimap, copia ou move o conteúdo da *direita para* o unordered_multimap.
...
## <a name="unordered_multimapreference"></a><a name="reference"></a>unordered_multimap::referência
...
## <a name="unordered_multimapsize"></a><a name="size"></a>unordered_multimap::tamanho
...
A função do membro troca `*this` as seqüências controladas entre e *a direita*. Se [unordered_multimap::get_allocator](#get_allocator)`() == right.get_allocator()`, ele o faz em tempo constante, ele lança uma exceção `Tr`apenas como resultado da cópia dos traços armazenados objeto do tipo , e invalida nenhuma referência, ponteiros ou iteradores que designem elementos nas duas seqüências controladas. Caso contrário, executará uma série de atribuições de elemento e de chamadas do construtor proporcional ao número de elementos nas duas sequências controladas.
...
|Parâmetro|Descrição|
|-|-|
|*Inseriterador*|O tipo de iterador.|
