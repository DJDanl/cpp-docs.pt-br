...
`Container` O tipo de contêiner na parte traseira de quais elementos serão inseridos por um `back_insert_iterator`.
...
`_Cont` O contêiner que o `back_insert_iterator` é um elemento em insert.
