...
O *argumento* é o ângulo que o vetor complexo faz com o eixo real positivo no plano complexo. Para um número complexo *a + bi,* o argumento é igual a arctan(b/a ).*b/a* O ângulo tem valor positivo quando medido no sentido anti-horário do eixo real positivo e valor negativo quando medido no sentido horário. Os valores principais são maiores que -pi e menores ou iguais a +pi.
...
cos *(z*) = cosh *(a + bi*) = cosh *(a)* cos *(b)*+ isinh *(a*) pecado *(b*)
...
## <a name="polar"></a><a name="polar"></a>Polar
...
A forma polar de um número complexo fornece o módulo *r* e o argumento *p,* onde esses parâmetros estão relacionados com \* os componentes cartesianos reais e imaginários *a* e *b* pelas equações *a* = r cos *p* e *b* = *r* \* sin *p*.
...
pecado *(z*) = (1/2 *i*)\*( exp (*iz*) - exp (- *iz*) )

pecado *(z*) = pecado *(a + bi*) = pecado *(a)* cosh *(b)*+ icos *(a*) sinh *(b*)
...
sinh *(z*) = sinh *(a + bi*) = sinh *(a)* cos *(b)*+ *i*cosh *(a*) pecado *(b)*
...
tan (*z*) = pecado *(z*) / cos (*z*) = ( exp (*iz*) - exp (- *iz*) ) / *i*( exp (*iz*) + exp (- *iz*) ) ) ) )
