...
|[basic_istringstream](../standard-library/basic-istringstream-class.md)|Descreve um objeto que controla a extração de elementos e objetos codificados de um buffer [](../standard-library/basic-stringbuf-class.md)<de fluxo da classe basic_stringbuf `Alloc`**elem**, TR, >, `Elem`com elementos do tipo, cujo caractere as características são determinadas pela classe `Tr`e cujos elementos são alocados por um alocador de classe. `Alloc`|
