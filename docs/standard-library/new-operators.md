...
`ptr` O ponteiro cujo valor é a ser renderizado inválido pela exclusão.
...
`count` O número de bytes de armazenamento a ser alocada.
...
`count` O número de bytes de armazenamento a ser alocada para o objeto de matriz.
