...
`F` Uma função definida pelo aplicativo a ser executado pelo thread.

`A` Uma lista de argumentos a serem passados para `F`.

`Other` Um existente `thread` objeto.
