...
-  Se *contagem* < 1,  **n**  = 1 e como tal, é equivalente ao construtor padrão, gerando sempre zero.  
-  Se *contagem* > 0,  **n**   =  *contagem*. Fornecido **d** = (*alta* - *baixa*) /  **n**  é maior que zero, usando **d** subintervalos uniformes, cada peso atribuído da seguinte maneira: `weight[k] = weightfunc(x)`, onde **x** = *baixa* + **k**  *  **d** + **d** / 2, para **k** = 0,...,  **n**  - 1.  
