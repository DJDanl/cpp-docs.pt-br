...
#### Parâmetros  
 `RealType`  
 O tipo de resultado de ponto flutuante, o padrão é `double`. Para os tipos possíveis, consulte [\<random\>](../standard-library/random.md).  
  
## Comentários  
 A classe de modelo descreve uma distribuição que produz valores de uma integral especificado pelo usuário tipo ou tipo `double` se nenhum for fornecido, distribuído de acordo com o aluno *t*\-distribuição. A seguinte tabela contém links para artigos sobre membros individuais.  
...
|`student_t_distribution::operator()`||[student\_t\_distribution::param\_type](../Topic/student_t_distribution::param_type.md)|  
 ...
 Para obter informações detalhadas sobre o aluno *t*\-distribuição, consulte o artigo de Wolfram MathWorld [t\-distribuição de alunos](http://go.microsoft.com/fwlink/?LinkId=401094).  
