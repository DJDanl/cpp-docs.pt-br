---
title: Funções &lt;iomanip&gt; | Microsoft Docs
ms.custom: ''
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
_Amount valor monetário extraído.

_Intl se `true`, use o formato internacional. O valor padrão é `false`.

### <a name="remarks"></a>Comentários

O manipulador retorna um objeto que, quando extraído do fluxo `str`, se comporta como um `formatted input function` que chama a função membro `get` para a faceta de localidade `money_get` associada a `str`, usando `_Intl` para indicar o formato internacional. Se for bem-sucedida, a chamada armazena em `_Amount` o valor monetário extraído. O manipulador, então, retorna `str`.
...
`_Tptr` A hora na forma de uma estrutura de hora.

`_Fmt` O formato desejado a ser usado para obter o valor de tempo.
...
`_Amount` O valor monetário inserir em fluxo.

`_Intl` Definido como `true` se manipulator devem usar o formato internacional, `false` se não for.
...
O manipulador retorna um objeto que, quando inserido no fluxo `str`, se comporta como uma função de saída formatada que chama a função membro `put` para a faceta de localidade `money_put` associada a `str`. Se for bem-sucedida, a chamada insere `amount` formatado adequadamente, usando `_Intl` para indicar o formato internacional e `str.fill()` como o elemento de preenchimento. O manipulador, então, retorna `str`.
...
`_Tptr` O valor de tempo para gravar o fluxo fornecido em uma estrutura de tempo.

`_Fmt` O formato desejado para gravar o valor de tempo.
...
`str` Um std::string, char *, cadeia de caracteres literal ou raw cadeia de caracteres literal ou uma versão ampla de qualquer uma dessas (por exemplo, std:: wstring, wchar_t\*).

`delimiter` Um caractere especificado pelo usuário, ou o caractere largo, para usar como o delimitador para o início e fim da cadeia de caracteres.

`escape` Um caractere especificado pelo usuário, ou o caractere largo, para usar como o caractere de escape para sequências de escape na cadeia de caracteres.
...
`Mask` Os sinalizadores para limpar.
...
`Ch` O caractere que será usado para preencher os espaços em uma exibição justificado à direita.
...
`Mask` Os sinalizadores para definir.
...
`Prec` A precisão para valores de ponto flutuante.
...
`Wide` A largura do campo de exibição.
