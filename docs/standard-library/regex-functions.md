...
|[permuta](#swap)|Troca dois `basic_regex` ou `match_results` objetos.|
...
*Bidirecional*\
O tipo de iterador para subcorrespondências. Para casos comuns, este é um dos `string::const_iterator` , `wstring::const_iterator` `const char*` ou `const wchar_t*` .

*Alocação*\
A classe alocadora de resultados de correspondência.

*Elem*\
O tipo de elemento a ser correspondido. Para casos comuns, isso `string` é `wstring` , `char*` ou `wchar_t*` .
...
*outra*\
A expressão regular para fazer a correspondência. Digite `regex` para `string` e `char*` , ou `wregex` para `wstring` e `wchar_t*` .
...
A primeira função constrói um objeto de [classe regex_iterator](../standard-library/regex-iterator-class.md) `iter(first, last, re, flags)` e o usa para dividir seu intervalo de entrada `[first, last)` em uma série de subsequências `T0 M0 T1 M1...TN-1 MN-1 TN` , em que `Mn` é a enésimo correspondência detectada pelo iterador. Se nenhuma correspondência for encontrada, `T0` é todo o intervalo de entrada e `N` é zero. Se apenas a primeira correspondência `(flags & format_first_only) != 0` for usada, `T1` é todo o texto de entrada após a correspondência e `N` é 1. Para cada um `i` no intervalo `[0, N)` , se `(flags & format_no_copy) == 0` ele copiar o texto do intervalo `Ti` para o iterador *out*. Em seguida, ele chama `m.format(out, fmt, flags)` , em que `m` é o `match_results` objeto retornado pelo objeto iterador `iter` para a subsequência `Mi` . Por fim, se `(flags & format_no_copy) == 0` ele copiar o texto do intervalo `TN` para o iterador. *out* A *função retorna.*
