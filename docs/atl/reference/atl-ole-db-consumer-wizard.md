 ...
 **conexão**  
  ...
  Para dados do SQL:  
    ...
    **Nome de fonte de dados de uso:** essa lista suspensa exibe fontes de dados registrados no seu computador. Você pode configurar fontes de dados antes do tempo usando o administrador de fonte de dados ODBC- ou -**usar a cadeia de caracteres de conexão:** insira uma cadeia de caracteres de conexão já tenham obtido, ou clique no **Build** botão; o **Selecionar fonte de dados** caixa de diálogo é exibida. Selecione uma fonte de dados de arquivo ou de máquina e clique em **Okey**.  
 ...
 **Atribuído**  
 ...
 -   Se você selecionar um consumidor **tipo** da tabela, o assistente usa o `db_source` e `db_table` atributos para criar a tabela e o acessador de tabela de declarações de classe e usa `db_column` para criar o mapa de coluna, por exemplo:  
...
```
   ...
   em vez de usar o `CTable` classe de modelo para declarar a tabela e a classe de acessador de tabela e as macros BEGIN_COLUMN_MAP e END_COLUMN_MAP para criar o mapa de coluna, por exemplo:  
...
-   Se você selecionar um consumidor **tipo** do comando, o assistente usa o `db_source` e `db_command` atributos e usa `db_column` para criar o mapa de coluna, por exemplo:  
