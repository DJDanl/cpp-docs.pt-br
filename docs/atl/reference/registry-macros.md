...
Se você quiser que a ATL substitua valores de substituição em tempo de execução, não especifique o DECLARE_REGISTRY_RESOURCE ou DECLARE_REGISTRY_RESOURCEID Macro. Em vez disso, crie uma matriz de estruturas `_ATL_REGMAP_ENTRIES`, em que cada entrada contém um espaço reservado variável emparelhado com um valor para substituir o espaço reservado em tempo de execução. Em seguida, chame [CAtlModule:: UpdateRegistryFromResourceD](catlmodule-class.md#updateregistryfromresourced) ou [CAtlModule:: UpdateRegistryFromResourceS](catlmodule-class.md#updateregistryfromresources), passando a matriz. Isso adiciona todos os valores de substituição nas estruturas de `_ATL_REGMAP_ENTRIES` ao mapa de substituição do registrador.
