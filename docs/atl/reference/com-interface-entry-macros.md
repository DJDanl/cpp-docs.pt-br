...
ms.locfileid: "88834694"
---
# <a name="com_interface_entry-macros"></a>COM_INTERFACE_ENTRY macros

Essas macros inserem as interfaces de um objeto em seu mapa de COM para que possam ser acessadas pelo `QueryInterface` . A ordem das entradas no mapa COM é que as interfaces de ordem serão verificadas em busca de um IID correspondente durante `QueryInterface` .
...
|[COM_INTERFACE_ENTRY_FUNC](#com_interface_entry_func)|Um mecanismo geral para conectar-se à lógica da ATL `QueryInterface` .|
...
Por exemplo, se você derivar o objeto de classe de duas interfaces duplas, exporá o `IDispatch` uso de COM_INTERFACE_ENTRY2, uma vez que o `IDispatch` pode ser obtido em qualquer uma das interfaces.
...
O parâmetro *punk* é considerado para apontar para o desconhecido interno de uma agregação ou para NULL; nesse caso, a entrada é ignorada. Normalmente, você seria `CoCreate` a agregação em `FinalConstruct` .
...
A interface IID será construída acrescentando *x* a `IID_` . Por exemplo, se *x* for `IPersistStorage` , o IID será `IID_IPersistStorage` .
...
Um mecanismo geral para conectar-se à lógica da ATL `QueryInterface` .
