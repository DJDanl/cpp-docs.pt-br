---
title: CComBSTR-classe | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CComBSTR::AppendBSTR](#appendbstr)|Acrescenta uma `BSTR` para `m_str`.|  
|[CComBSTR::AppendBytes](#appendbytes)|Acrescenta um número especificado de bytes a serem `m_str`.|  
|[CComBSTR::ArrayToBSTR](#arraytobstr)|Cria um `BSTR` partir do primeiro caractere de cada elemento no safearray e anexa-o para o `CComBSTR` objeto.|  
...
|Nome|Descrição|  
|----------|-----------------|  
|[CComBSTR::operator BSTR](#operator_bstr)|Conversões de uma `CComBSTR` o objeto para um `BSTR`.|  
...
## <a name="remarks"></a>Comentários  
 ...
 A [BSTR](http://msdn.microsoft.com/en-us/1b2d7d2c-47af-4389-a6b6-b01b7e915228) é terminada em nulo após a última contados caractere, mas também pode conter caracteres nulos incorporados a cadeia de caracteres. O comprimento da cadeia de caracteres é determinado pela contagem de caracteres, não o primeiro caractere nulo.  
  
> [!NOTE]
>  O `CComBSTR` classe fornece um número de membros (construtores, operadores de atribuição e operadores de comparação) que usem cadeias de caracteres ANSI ou Unicode como argumentos. As versões ANSI dessas funções são menos eficientes do que suas contrapartes de Unicode porque temporárias cadeias de caracteres Unicode são geralmente criadas internamente. Para eficiência, usa as versões Unicode onde possível.  
  
> [!NOTE]
>  Devido ao comportamento de pesquisa aprimorado implementado no Visual Studio .NET, como código `bstr = L"String2" + bstr;`, que talvez tenham sido compilados em versões anteriores, em vez disso, devem ser implementadas como `bstr = CStringW(L"String2") + bstr`.  
  
 Para obter uma lista de cuidados ao usar `CComBSTR`, consulte [Programando com o CComBSTR](../../atl/programming-with-ccombstr-atl.md).  
...
### <a name="parameters"></a>Parâmetros  
 `bstrSrc`  
 [in] Um `CComBSTR` objeto acrescentar.  
  
 *CH*  
 [in] Um caractere para acrescentar.  
  
 `lpsz`  
 [in] Uma cadeia de caracteres terminada em zero para acrescentar. Você pode passar uma cadeia de caracteres Unicode por meio de **LPCOLESTR** sobrecarga ou uma cadeia de caracteres ANSI via o `LPCSTR` versão.  
  
 `nLen`  
 [in] O número de caracteres `lpsz` acrescentar.  
...
### <a name="remarks"></a>Comentários  
 Não transmita uma cadeia de caracteres largos comum para esse método. O compilador não pode capturar o erro e ocorrerão erros de tempo de execução.  
...
### <a name="parameters"></a>Parâmetros  
 `pSrc`  
 [in] Safearray contendo os elementos usados para criar a cadeia de caracteres.  
...
### <a name="parameters"></a>Parâmetros  
 `bstrSrc`  
 [in] A `BSTR` para atribuir a atual `CComBSTR` objeto.  
...
### <a name="parameters"></a>Parâmetros  
 *src*  
 [in] O `BSTR` anexar ao objeto.  
  
### <a name="remarks"></a>Comentários  
 Não transmita uma cadeia de caracteres largos comum para esse método. O compilador não pode capturar o erro e ocorrerão erros de tempo de execução.  
  
> [!NOTE]
>  Esse método declarará se `m_str` não **nulo**.  
...
### <a name="parameters"></a>Parâmetros  
 ...
 `sz`  
 [in] Uma cadeia de caracteres para copiar. Especifica a versão Unicode um **LPCOLESTR**; a versão ANSI Especifica um `LPCSTR`.  
...
### <a name="remarks"></a>Comentários  
 Os conjuntos de construtor de cópia `m_str` para uma cópia do `BSTR` membro do *src*. O **REFGUID** construtor converte o **GUID** uma cadeia de caracteres usando **StringFromGUID2** e armazena o resultado.  
  
 Conjunto de construtores `m_str` para uma cópia da cadeia de caracteres especificada. Se você passar um valor `nSize`, então somente `nSize` caracteres serão copiados, seguido por um caractere nulo de terminação.  
  
 `CComBSTR`oferece suporte à semântica de movimentação. Você pode usar o construtor de movimentação (o construtor que usa uma referência de rvalue ( `&&`) para criar um novo objeto que usa os mesmos dados subjacentes, conforme você passa como um argumento, sem a sobrecarga de copiar o objeto do objeto antigo.  
...
### <a name="parameters"></a>Parâmetros  
 *pbstr*  
 [out] O endereço de um `BSTR` para retornar a cadeia de caracteres alocada por esse método.  
  
 `pvarDest`  
 [out] O endereço de uma **VARIANT** para retornar a cadeia de caracteres alocada por esse método.  
  
### <a name="return-value"></a>Valor de retorno  
 Um padrão `HRESULT` valor que indica o êxito ou falha da cópia.  
  
### <a name="remarks"></a>Comentários  
 Depois de chamar esse método, o **VARIANT** apontada por `pvarDest` será do tipo `VT_BSTR`.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna **true** se a cadeia de caracteres for com êxito carregado; caso contrário, retornará **false**.  
  
### <a name="remarks"></a>Comentários  
 A primeira função carrega o recurso do módulo identificado por você por meio de `hInst` parâmetro. A segunda função carrega o recurso do módulo de recursos associado a [CComModule](../../atl/reference/ccommodule-class.md)-objeto usada neste projeto derivado.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna **true** se o [m_str](#m_str) é membro **nulo**; caso contrário, **false**.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna **true** se o item que está sendo comparado não for igual do `CComBSTR` objeto; caso contrário, retornará **false**.  
...
### <a name="remarks"></a>Comentários  
 `CComBstr operator &`uma asserção especial associado a ela para ajudar a identificar vazamentos de memória. O programa declarará quando o `m_str` membro é inicializado. Esta declaração foi criada para identificar situações em que um programador usa o `& operator` para atribuir um novo valor para `m_str` membro sem liberar a primeira alocação de `m_str`. Se `m_str` é igual a NULL, o programa assumirá que m_str não foi alocada ainda. Nesse caso, o programa não declarará.  
  
 Esta declaração não é habilitada por padrão. Definir `ATL_CCOMBSTR_ADDRESS_OF_ASSERT` para habilitar essa afirmação.  
...
### <a name="remarks"></a>Comentários  
 `CComBSTR`s são comparados textualmente no contexto da localidade padrão do usuário. O **LPCOLESTR** comparação é feita usando `memcmp` nos dados brutos de cada cadeia de caracteres. O `LPCSTR` comparação é realizada da mesma forma quando um temporário Unicode cópia de `pszSrc` foi criado. O operador de comparação final apenas compara a cadeia de caracteres independente **nulo**.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna **true** se o item que está sendo comparado é igual do `CComBSTR` objeto; caso contrário, retornará **false**.  
...
### <a name="remarks"></a>Comentários  
 **ReadToStream** requer que o conteúdo de fluxo na posição atual para ser compatível com o formato de dados gravado por uma chamada a [WriteToStream](#writetostream).  
...
## <a name="see-also"></a>Consulte também  
 ...
 [Macros de conversão de cadeia de caracteres MFC e ATL](http://msdn.microsoft.com/library/8f53659e-0464-4424-97db-6b8453c49863)
