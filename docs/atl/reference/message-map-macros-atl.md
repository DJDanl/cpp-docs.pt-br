...
Quando um `CMyExtWindow` objeto recebe uma mensagem de WM_PAINT, a mensagem é direcionada para `CMyExtWindow::OnPaint` o processamento real. Se `OnPaint` o indicar que a mensagem requer processamento adicional, a mensagem será direcionada para o mapa de mensagens padrão no `CMyBaseWindow` .
...
- Se um procedimento de janela estiver usando o `CMyClass` mapa de mensagens padrão e não `OnPaint` tratar uma mensagem, a mensagem será direcionada para o `CMyBaseClass` mapa de mensagens padrão do para processamento.

- Se um procedimento de janela estiver usando o primeiro mapa de mensagem alternativo no `CMyClass` , todas as mensagens serão direcionadas para o `CMyBaseClass` mapa de mensagens padrão do.

- Se um procedimento de janela estiver usando o `CMyClass` segundo mapa de mensagem alternativo e não `OnChar` tratar uma mensagem, a mensagem será direcionada para o mapa de mensagem alternativo especificado no `CMyBaseClass` . `CMyBaseClass` deve ter declarado este mapa de mensagem com ALT_MSG_MAP (1).
...
- Se um procedimento de janela estiver usando o `CMyClass` mapa de mensagens padrão e não `OnPaint` tratar uma mensagem, a mensagem será direcionada para o `m_obj` mapa de mensagens padrão do para processamento.

- Se um procedimento de janela estiver usando o primeiro mapa de mensagem alternativo no `CMyClass` , todas as mensagens serão direcionadas para o `m_obj` mapa de mensagens padrão do.

- Se um procedimento de janela estiver usando o `CMyClass` segundo mapa de mensagem alternativo e não `OnChar` tratar uma mensagem, a mensagem será direcionada para o mapa de mensagem alternativo especificado de `m_obj` . `CMyContainedClass`A classe deve ter declarado este mapa de mensagem com ALT_MSG_MAP (1).
...
O mapa de mensagens `bHandled` é definido como verdadeiro antes de `CommandHandler` ser chamado. Se o não `CommandHandler` tratar totalmente a mensagem, ele deverá definir `bHandled` como false para indicar que a mensagem precisa de processamento adicional.
...
Além de COMMAND_HANDLER, você pode usar [MESSAGE_HANDLER](#message_handler) para mapear uma mensagem de WM_COMMAND sem considerar um identificador ou código. Nesse caso, o `MESSAGE_HANDLER(WM_COMMAND, OnHandlerFunction)` direcionará todas as mensagens de WM_COMMAND para `OnHandlerFunction` .
...
O mapa de mensagens `bHandled` é definido como verdadeiro antes de `MessageHandler` ser chamado. Se o não `MessageHandler` tratar totalmente a mensagem, ele deverá definir `bHandled` como false para indicar que a mensagem precisa de processamento adicional.
...
O mapa de mensagens `bHandled` é definido como verdadeiro antes de `NotifyHandler` ser chamado. Se o não `NotifyHandler` tratar totalmente a mensagem, ele deverá definir `bHandled` como false para indicar que a mensagem precisa de processamento adicional.

> [!NOTE]
> Sempre inicie um mapa de mensagens com [BEGIN_MSG_MAP](#begin_msg_map). Em seguida, você pode declarar mapas de mensagem alternativos subsequentes com [ALT_MSG_MAP](#alt_msg_map). A macro [END_MSG_MAP](#end_msg_map) marca o final do mapa de mensagens. Cada mapa de mensagens deve ter exatamente uma instância de BEGIN_MSG_MAP e END_MSG_MAP.

Além de NOTIFY_HANDLER, você pode usar [MESSAGE_HANDLER](#message_handler) para mapear uma mensagem de WM_NOTIFY sem considerar um identificador ou código. Nesse caso, o `MESSAGE_HANDLER(WM_NOTIFY, OnHandlerFunction)` direcionará todas as mensagens de WM_NOTIFY para `OnHandlerFunction` .
