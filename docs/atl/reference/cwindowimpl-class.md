...
`CWindowImpl::Create` Cria uma janela com base nas informações de classe de janela gerenciadas pelo [CWndClassInfo](../../atl/reference/cwndclassinfo-class.md). `CWindowImpl` contém a macro [DECLARE_WND_CLASS](window-class-macros.md#declare_wnd_class) , que significa `CWndClassInfo` registrar uma nova classe de janela. Se você quiser superclassear uma classe de janela existente, derive sua classe de `CWindowImpl` e inclua a macro [DECLARE_WND_SUPERCLASS](window-class-macros.md#declare_wnd_superclass) . Nesse caso, o `CWndClassInfo` registra uma classe de janela baseada em uma classe existente, mas usa `CWindowImpl::WindowProc` . Por exemplo:

[!code-cpp[NVC_ATL_Windowing#43](../../atl/codesnippet/cpp/cwindowimpl-class_1.h)]

> [!NOTE]
> Como `CWndClassInfo` o gerencia as informações de apenas uma classe de janela, cada janela criada por meio de uma instância do `CWindowImpl` é baseada na mesma classe de janela.

`CWindowImpl` também dá suporte à subclasse Window. O `SubclassWindow` método anexa uma janela existente ao `CWindowImpl` objeto e altera o procedimento de janela para `CWindowImpl::WindowProc` . Cada instância do `CWindowImpl` pode subclasse de uma janela diferente.

> [!NOTE]
> Para qualquer `CWindowImpl` objeto específico, chame `Create` ou `SubclassWindow` . Não invoque os dois métodos no mesmo objeto.
...
`CWindowImpl` deriva de, que deriva de `CWindowImplBaseT` `CWindowImplRoot` , que deriva de `TBase` e [CMessageMap](../../atl/reference/cmessagemap-class.md).

|Para saber mais sobre|Consulte|
|--------------------------------|---------|
|Criando controles|[Tutorial da ATL](../../atl/active-template-library-atl-tutorial.md)|
|Usando o Windows na ATL|[Classes de janela do ATL](../../atl/atl-window-classes.md)|
...
*Rect*<br/>
no Uma estrutura [Rect](/windows/win32/api/windef/ns-windef-rect) que especifica a posição da janela. O `RECT` pode ser passado por ponteiro ou por referência.
