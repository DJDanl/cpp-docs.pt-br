...
|[OnFinalMessage](#onfinalmessage)|Chamado após a última mensagem ser recebida (normalmente WM_NCDESTROY).|
...
Você pode usar `CWindowImpl` para criar uma janela ou uma subclasse de uma janela existente. o `CWindowImpl` procedimento de janela usa um mapa de mensagens para direcionar mensagens para os manipuladores apropriados.

`CWindowImpl::Create`Cria uma janela com base nas informações de classe de janela gerenciadas pelo [CWndClassInfo](../../atl/reference/cwndclassinfo-class.md). `CWindowImpl`contém a macro [DECLARE_WND_CLASS](window-class-macros.md#declare_wnd_class) , que significa `CWndClassInfo` registrar uma nova classe de janela. Se você quiser superclassear uma classe de janela existente, derive `CWindowImpl` sua classe de e inclua a macro [DECLARE_WND_SUPERCLASS](window-class-macros.md#declare_wnd_superclass) . Nesse caso, `CWndClassInfo` o registra uma classe de janela baseada em uma classe existente, mas usa `CWindowImpl::WindowProc`. Por exemplo:

[!code-cpp[NVC_ATL_Windowing#43](../../atl/codesnippet/cpp/cwindowimpl-class_1.h)]

> [!NOTE]
>  Como `CWndClassInfo` o gerencia as informações de apenas uma classe de janela, cada janela criada por meio `CWindowImpl` de uma instância do é baseada na mesma classe de janela.

`CWindowImpl`também dá suporte à subclasse Window. O `SubclassWindow` método anexa uma janela existente `CWindowImpl` ao objeto e altera o procedimento de janela para `CWindowImpl::WindowProc`. Cada instância do `CWindowImpl` pode subclasse de uma janela diferente.

> [!NOTE]
>  Para qualquer objeto `CWindowImpl` específico, `Create` chame ou `SubclassWindow`. Não invoque os dois métodos no mesmo objeto.

Além disso, `CWindowImpl`a ATL fornece [CContainedWindow](../../atl/reference/ccontainedwindowt-class.md) para criar uma janela que está contida em outro objeto.

O destruidor de classe base ( `CWindowImplRoot`~) garante que a janela seja desapareceda antes que o objeto seja destruído.

`CWindowImpl`deriva de `CWindowImplRoot` `TBase` , que deriva de, que deriva de e [CMessageMap.](../../atl/reference/cmessagemap-class.md) `CWindowImplBaseT`
...
`Create`o primeiro registra a classe de janela se ela ainda não tiver sido registrada. A janela recém-criada é anexada automaticamente ao `CWindowImpl` objeto.

> [!NOTE]
>  Não chame `Create` se você já chamou [SubclassWindow](#subclasswindow).

Para usar uma classe de janela baseada em uma classe de janela existente, derive sua classe `CWindowImpl` de e inclua a macro [DECLARE_WND_SUPERCLASS](window-class-macros.md#declare_wnd_superclass) . O procedimento de janela da classe de janela existente é salvo em [m_pfnSuperWindowProc](#m_pfnsuperwindowproc). Para obter mais informações, consulte a visão geral do [CWindowImpl](../../atl/reference/cwindowimpl-class.md) .
...
Retorna a mensagem atual, empacotada na `MSG` estrutura.
...
Por padrão, `CWindowImpl` o obtém esse método por meio da macro [DECLARE_WND_CLASS](window-class-macros.md#declare_wnd_class) , que especifica uma nova classe de janela.

Para supergravar uma classe de janela existente, derive `CWindowImpl` sua classe de e inclua a macro `GetWndClassInfo` [DECLARE_WND_SUPERCLASS](window-class-macros.md#declare_wnd_superclass) para substituir. Para obter mais informações, consulte a visão geral do [CWindowImpl](../../atl/reference/cwindowimpl-class.md) .

Além de usar as macros DECLARE_WND_CLASS e DECLARE_WND_SUPERCLASS, você `GetWndClassInfo` pode substituir pela sua própria implementação.
...
[CWindowImpl::D efwindowproc](#defwindowproc) envia informações da mensagem para o procedimento de janela `m_pfnSuperWindowProc`salvo em.
...
Subclasses a janela identificada por *HWND* e a anexa ao `CWindowImpl` objeto.
...
Desanexa a janela de subclasse do `CWindowImpl` objeto e restaura o procedimento de janela original, salvo em [m_pfnSuperWindowProc](#m_pfnsuperwindowproc).
