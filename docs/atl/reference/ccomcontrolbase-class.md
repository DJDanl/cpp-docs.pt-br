...
|Nome|DESCRIÇÃO|
|----------|-----------------|
|[CComControlBase:: AppearanceType](#appearancetype)|Substituir se sua propriedade `m_nAppearance` Stock não for do tipo **Short**.|
...
|[CComControlBase::D oesVerbActivate](#doesverbactivate)|Verifica se o parâmetro *iVerb* usado pelo `IOleObjectImpl::DoVerb` ativa a interface do usuário do controle (*iVerb* Equals OLEIVERB_UIACTIVATE), define a ação tomada quando o usuário clica duas vezes no controle (*iVerb* é igual a OLEIVERB_PRIMARY), exibe o controle (*iVerb* é igual a OLEIVERB_SHOW) ou ativa o controle (*iVerb* é igual a OLEIVERB_INPLACEACTIVATE).|
|[CComControlBase::D oesVerbUIActivate](#doesverbuiactivate)|Verifica se o parâmetro *iVerb* usado por `IOleObjectImpl::DoVerb` faz com que a interface do usuário do controle seja ativada e retorne true.|
...
|[CComControlBase::GetAmbientFont](#getambientfont)|Recupera um ponteiro para a interface de `IFont` de ambiente do contêiner.|
|[CComControlBase::GetAmbientFontDisp](#getambientfontdisp)|Recupera um ponteiro para a interface de expedição `IFontDisp` ambiente do contêiner.|
...
|[CComControlBase:: GetDirty](#getdirty)|Retorna o valor do `m_bRequiresSave`de membros de dados.|
...
|[CComControlBase::OnDrawAdvanced](#ondrawadvanced)|O `OnDrawAdvanced` padrão prepara um contexto de dispositivo normalizado para desenho e, em seguida, chama o método `OnDraw` da classe de controle.|
...
|[CComControlBase:: OnPaint](#onpaint)|Prepara o contêiner para pintura, obtém a área do cliente do controle e, em seguida, chama o método de `OnDraw` da classe de controle.|
...
|[CComControlBase:: m_bDrawFromNatural](#m_bdrawfromnatural)|Sinalizador que indica que `IDataObjectImpl::GetData` e `CComControlBase::GetZoomInfo` devem definir o tamanho do controle de `m_sizeNatural` em vez de `m_sizeExtent`.|
|[CComControlBase:: m_bDrawGetDataInHimetric](#m_bdrawgetdatainhimetric)|Sinalizador que indica que `IDataObjectImpl::GetData` deve usar unidades HIMETRIC e não pixels ao desenhar.|
|[CComControlBase:: m_bInPlaceActive](#m_binplaceactive)|Sinalizador que indica que o controle está ativo no local.|
|[CComControlBase:: m_bInPlaceSiteEx](#m_binplacesiteex)|Sinalizador indicando que o contêiner dá suporte à interface `IOleInPlaceSiteEx` e aos recursos de controle OCX96, como controles sem janelas e de cintilação.|
...
|[CComControlBase:: m_spAmbientDispatch](#m_spambientdispatch)|Um objeto `CComDispatchDriver` que permite recuperar e definir as propriedades do contêiner por meio de um ponteiro de `IDispatch`.|
...
Essa classe fornece métodos para criar e gerenciar controles ATL. A [classe CComControl](../../atl/reference/ccomcontrol-class.md) deriva de `CComControlBase`. Quando você cria um controle padrão ou um controle DHTML usando o assistente de controle ATL, o assistente irá derivar automaticamente a classe de `CComControlBase`.
...
Substituir se sua propriedade `m_nAppearance` Stock não for do tipo **Short**.
...
O assistente de controle ATL adiciona `m_nAppearance` Propriedade Stock do tipo short. Substitua `AppearanceType` se você usar um tipo de dados diferente.
...
Inicializa o tamanho do controle para 5080X5080 HIMETRIC unidades (2 "X2") e inicializa o `CComControlBase` valores de membro de dados como nulo ou falso.
...
Se o controle estiver em janela, `~CComControlBase` destrói-o chamando [DestroyWindow](/windows/win32/api/winuser/nf-winuser-destroywindow).
...
Recupera um ponteiro para a interface de `IFont` de ambiente do contêiner.
...
##  <a name="getambientproperty"></a>CComControlBase:: getambienteproperty
...
Retorna o valor do `m_bRequiresSave`de membros de dados.
...
Esse método também recupera um ponteiro de interface `IOleInPlaceSite`, `IOleInPlaceSiteEx`ou `IOleInPlaceSiteWindowless` para o controle e o armazena no membro de dados da classe de controle [CComControlBase:: m_spInPlaceSite](#m_spinplacesite). Os membros de dados da classe de controle [CComControlBase:: m_bInPlaceSiteEx](#m_binplacesiteex), [CComControlBase:: m_bWndLess](#m_bwndless), [CComControlBase:: m_bWasOnceWindowless](#m_bwasoncewindowless)e [CComControlBase:: m_bNegotiatedWnd](#m_bnegotiatedwnd) são definidos como true conforme apropriado.
...
Isso significa que o controle está visível e sua janela, se houver, está visível, mas seus menus e barras de ferramentas podem não estar ativos. O sinalizador `m_bUIActive` indica que a interface do usuário do controle, como menus, também está ativa.
...
Sinalizador indicando que o contêiner dá suporte à interface `IOleInPlaceSiteEx` e aos recursos de controle OCX96, como controles sem janelas e de cintilação.
...
O membro de dados `m_spInPlaceSite` aponta para uma interface [IOleInPlaceSite](/windows/win32/api/oleidl/nn-oleidl-ioleinplacesite), [IOleInPlaceSiteEx](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesiteex)ou [IOleInPlaceSiteWindowless](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesitewindowless) , dependendo do valor dos sinalizadores `m_bWndLess` e `m_bInPlaceSiteEx`. (O membro de dados `m_bNegotiatedWnd` deve ser TRUE para que o ponteiro de `m_spInPlaceSite` seja válido.)

Se `m_bWndLess` for FALSE e `m_bInPlaceSiteEx` for TRUE, `m_spInPlaceSite` será um ponteiro de interface `IOleInPlaceSiteEx`. Consulte [m_spInPlaceSite](#m_spinplacesite) para uma tabela que mostra a relação entre esses três membros de dados.
...
O sinalizador de `m_bNegotiatedWnd` deve ser TRUE para que o ponteiro de `m_spInPlaceSite` seja válido.
...
Esse sinalizador é verificado por `IOleObjectImpl::SetExtent` e, se for TRUE, o tamanho passado para `SetExtent` é atribuído a `m_sizeNatural`.
...
O sinalizador `m_bInPlaceActive` indica que o controle está ativo, mas não que sua interface do usuário está ativa.
...
O membro de dados `m_spInPlaceSite` aponta para uma interface [IOleInPlaceSite](/windows/win32/api/oleidl/nn-oleidl-ioleinplacesite), [IOleInPlaceSiteEx](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesiteex)ou [IOleInPlaceSiteWindowless](/windows/win32/api/ocidl/nn-ocidl-ioleinplacesitewindowless) , dependendo do valor dos sinalizadores `m_bWndLess` e [CComControlBase:: m_bInPlaceSiteEx](#m_binplacesiteex) . (O membro de dados [CComControlBase:: m_bNegotiatedWnd](#m_bnegotiatedwnd) deve ser true para que o ponteiro [CComControlBase:: m_spInPlaceSite](#m_spinplacesite) seja válido.)

Se `m_bWndLess` for TRUE, `m_spInPlaceSite` será um ponteiro de interface `IOleInPlaceSiteWindowless`. Consulte [CComControlBase:: m_spInPlaceSite](#m_spinplacesite) para uma tabela que mostra a relação completa entre esses membros de dados.
...
Esse tamanho é dimensionado pela exibição. O tamanho físico do controle é especificado no membro de dados `m_sizeNatural` e é fixo.
...
Um objeto `CComDispatchDriver` que permite recuperar e definir as propriedades de um objeto por meio de um ponteiro de `IDispatch`.
...
O ponteiro de `m_spInPlaceSite` só será válido se o sinalizador de [m_bNegotiatedWnd](#m_bnegotiatedwnd) for true.

A tabela a seguir mostra como o tipo de ponteiro `m_spInPlaceSite` depende dos sinalizadores de membro de dados [m_bWndLess](#m_bwndless) e [m_bInPlaceSiteEx](#m_binplacesiteex) :
...
A interface `m_spOleAdviseHolder` implementa os métodos [IOleObject:: Advise](/windows/win32/api/oleidl/nf-oleidl-ioleobject-advise) e [IOleObject:: Unadvise](/windows/win32/api/oleidl/nf-oleidl-ioleobject-unadvise) para estabelecer e excluir as conexões de consultoria para o contêiner. O contêiner do controle deve implementar um coletor de aviso dando suporte à interface [IAdviseSink](/windows/win32/api/objidl/nn-objidl-iadvisesink) .
...
Um método `OnDraw` é adicionado automaticamente à sua classe de controle quando você cria seu controle com o assistente de controle do ATL. O padrão do assistente `OnDraw` desenha um retângulo com o rótulo "ATL 8,0".
...
O `OnDrawAdvanced` padrão prepara um contexto de dispositivo normalizado para desenho e, em seguida, chama o método `OnDraw` da classe de controle.
...
Prepara o contêiner para pintura, obtém a área do cliente do controle e, em seguida, chama o método de `OnDrawAdvanced` da classe de controle.
...
Se *wParam* não for NULL, `OnPaint` pressupõe que ele contém um HDC válido e o usa em vez de [CComControlBase:: m_hWndCD](#m_hwndcd).
