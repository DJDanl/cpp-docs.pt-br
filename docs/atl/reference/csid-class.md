---
title: Classe CSid | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
|[CSid::Domain](#domain)|Retorna o nome do domínio associado a `CSid` objeto.|  
...
|[CSid::LoadAccount](#loadaccount)|Atualizações de `CSid` objeto dado o nome da conta e o domínio ou um existente `SID` estrutura.|  
|[CSid::Sid](#sid)|Retorna a cadeia de caracteres de identificação.|  
...
|[operador const SID *](#operator_const_sid__star)|Conversões de uma `CSid` objeto em um ponteiro para um `SID` estrutura.|  
  
### <a name="global-operators"></a>Operadores globais  
  
|||  
|-|-|  
|[operador = =](#operator_eq_eq)|Testes de dois objetos de descritor de segurança de igualdade|  
|[operador! =](#operator_neq)|Testes de dois objetos de descritor de segurança para desigualdade|  
...
## <a name="remarks"></a>Comentários  
 ...
 Aplicativos não devem modificar o `SID` estrutura diretamente, mas em vez disso, use os métodos fornecidos nessa classe de wrapper. Consulte também [AtlGetOwnerSid](http://msdn.microsoft.com/library/0e3a2606-74b8-4412-9803-bb437e22da85), [AtlSetGroupSid](http://msdn.microsoft.com/library/83531d32-11ab-4a68-a3c6-1bfa54ab8dfa), [AtlGetGroupSid](http://msdn.microsoft.com/library/8e7ec6b9-15c8-4a8a-977e-1e4c853d0be7), e [AtlSetOwnerSid](http://msdn.microsoft.com/library/3a8abb76-1d2c-465d-a5e8-62a12a3c37f3).  
...
### <a name="remarks"></a>Comentários  
 Esse método tenta encontrar um nome especificado `SID` (identificador de segurança). Para obter detalhes completos, consulte [LookupAccountSid](http://msdn.microsoft.com/library/windows/desktop/aa379166).  
...
### <a name="parameters"></a>Parâmetros  
 `rhs`  
 Existente `CSid` objeto ou `SID` estrutura (identificador de segurança).  
...
### <a name="remarks"></a>Comentários  
 O construtor inicializa o `CSid` objeto, defina um membro de dados internos como *SidTypeInvalid*, ou copiando as configurações de uma já existente `CSid`, `SID`, ou conta existente.  
...
##  <a name="domain"></a>CSid::Domain  
 Retorna o nome do domínio associado a `CSid` objeto.  
...
### <a name="remarks"></a>Comentários  
 Se o `CSid` estrutura não é válida, o valor de retorno é indefinido. Antes de chamar `GetLength`, use o [CSid::IsValid](#isvalid) a função de membro para verificar se `CSid` é válido.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna o endereço do `CSid` base do objeto `SID` estrutura.  
...
### <a name="return-value"></a>Valor de retorno  
 Se o método for bem-sucedido, ele retorna o endereço do **SID_IDENTIFIER_AUTHORITY** estrutura. Se ele falhar, o valor de retorno será indefinido. Falha pode ocorrer se o `CSid` objeto não é válido, caso em que o [CSid::IsValid](#isvalid) método retorna **false**. A função `GetLastError` pode ser chamado para obter informações de erro estendidas.  
...
### <a name="remarks"></a>Comentários  
 O *nSubAuthority* parâmetro especifica um valor de índice que identifica o elemento da matriz subautoridade o método retornará. O método não realiza nenhum teste de validação nesse valor. Um aplicativo pode chamar [CSid::GetSubAuthorityCount](#getsubauthoritycount) para descobrir o intervalo de valores aceitáveis.  
...
### <a name="return-value"></a>Valor de retorno  
 ...
 Se o método falhar, o valor de retorno será indefinido. O método falhará se o `CSid` objeto é inválido. Para obter outras informações sobre o erro, chame `GetLastError`.  
...
##  <a name="loadaccount"></a>CSid::LoadAccount  
 Atualizações de `CSid` objeto dado o nome da conta e o domínio ou uma estrutura de SID (identificador de segurança) existente.  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna **true** com êxito, **false** em caso de falha. Para obter outras informações sobre o erro, chame `GetLastError`.  
...
##  <a name="operator_eq_eq"></a>CSid::operator = =  
 Testa dois objetos de descritor de segurança de igualdade.  
...
##  <a name="operator_const_sid__star"></a>SID const CSid::operator *  
 Conversões de uma `CSid` objeto em um ponteiro para um `SID` estrutura (identificador de segurança).  
...
### <a name="return-value"></a>Valor de retorno  
 Retorna o `SID` estrutura como uma cadeia de caracteres em um formato adequado para exibição, armazenamento e transmissão. Equivalente a [ConvertSidToStringSid](http://msdn.microsoft.com/library/windows/desktop/aa376399), embora essa função só está disponível no Windows 2000 ou posterior e, portanto, é emulada para sistemas operacionais anteriores.  
...
|SidTypeWellKnownGroup|Indica um `SID` para um grupo conhecido.|  
|SidTypeDeletedAccount|Indica um `SID` para uma conta excluída.|  
|SidTypeInvalid|Indica um inválido `SID`.|  
|SidTypeUnknown|Indica um desconhecido `SID` tipo.|  
|SidTypeComputer|Indica um `SID` para um computador.|  
  
### <a name="remarks"></a>Comentários  
 Chamar [CSid::LoadAccount](#loadaccount) para atualizar o `CSid` objeto antes de chamar `SidNameUse` para retornar seu estado. `SidNameUse`não altera o estado do objeto (chamando a **LookupAccountName** ou **LookupAccountSid**), mas retorna apenas o estado atual.  
