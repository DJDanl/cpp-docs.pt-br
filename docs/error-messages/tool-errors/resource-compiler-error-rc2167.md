---
title: RC2167 de erro do compilador de recurso | Documentos do Microsoft
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology:
- devlang-cpp
ms.tgt_pltfrm: 
ms.topic: error-reference
f1_keywords:
- RC2167
dev_langs:
- C++
helpviewer_keywords:
- RC2167
ms.assetid: 9b24a8eb-367b-4ff6-8266-b0c215f32d6a
caps.latest.revision: 6
author: corob-msft
ms.author: corob
manager: ghogen
translation.priority.ht:
- cs-cz
- de-de
- es-es
- fr-fr
- it-it
- ja-jp
- ko-kr
- pl-pl
- pt-br
- ru-ru
- tr-tr
- zh-cn
- zh-tw
translationtype: Machine Translation
ms.sourcegitcommit: 3168772cbb7e8127523bc2fc2da5cc9b4f59beb8
ms.openlocfilehash: a5455945f5e0d50482fed0afb75b26c86fd7d89e
ms.lasthandoff: 02/25/2017

---
# <a name="resource-compiler-error-rc2167"></a>Erro RC2167 (compilador de recurso)
campo VERSIONINFO não reconhecido. BEGIN ou vírgula esperada  
  
 Um campo não reconhecido foi encontrado no **fixo** faz parte de um **VERSIONINFO** declaração de estrutura. A **VERSIONINFO** campo deve ser DWORDS separados por uma vírgula.
