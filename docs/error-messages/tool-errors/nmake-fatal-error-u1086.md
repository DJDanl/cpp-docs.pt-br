---
title: NMAKE Erro Fatal U1086 | Microsoft Docs
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
ms.technology: cpp-tools
ms.tgt_pltfrm: 
ms.topic: error-reference
f1_keywords: U1086
dev_langs: C++
helpviewer_keywords: U1086
ms.assetid: 6d3cd68a-ead6-4a6d-a205-01324785de7e
caps.latest.revision: "5"
author: corob-msft
ms.author: corob
manager: ghogen
ms.workload: cplusplus
ms.openlocfilehash: b4d4f77f5de7d89c68804458103de624f913ff26
ms.sourcegitcommit: 8fa8fdf0fbb4f57950f1e8f4f9b81b4d39ec7d7a
ms.translationtype: MT
ms.contentlocale: pt-BR
ms.lasthandoff: 12/21/2017
---
# <a name="nmake-fatal-error-u1086"></a>Erro fatal U1086 (NMAKE)
**regra de inferência não pode ter dependentes**  
  
 Os dois-pontos (**:**) em uma inferência regra deve ser seguida por uma destas opções:  
  
-   Caractere de nova linha  
  
-   -E-vírgula (**;**), que pode ser seguido por um comando  
  
-   Sinal de número (**#**), que pode ser seguido de um comentário