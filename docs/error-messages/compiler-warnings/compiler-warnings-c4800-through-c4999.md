...
|Aviso|Mensagem|
|-------------|-------------|
|[Aviso do compilador (nível 3) C4800](compiler-warning-level-3-c4800.md)|'type': forçando valor para bool 'true' ou 'false' (aviso de desempenho)|
|[Aviso do compilador (nível 1) C4803](compiler-warning-level-1-c4803.md)|'method': o método raise possui uma classe de armazenamento diferente do evento, 'event'|
|[Aviso do compilador (nível 1) C4804](compiler-warning-level-1-c4804.md)|'operação de ': uso não seguro de tipo 'bool' em operação|
|[Aviso do compilador (nível 1) C4805](compiler-warning-level-1-c4805.md)|'operação de ': mistura não segura de tipo 'type' e tipo 'type' em operação|
...
|[Aviso do compilador (nível 4) C4820](compiler-warning-level-4-c4820.md)|preenchimento de bytes 'bytes' adicionado após construção de 'member_name'|
...
|[Aviso do compilador (nível 3) C4823](compiler-warning-level-3-c4823.md)|'function': usa ponteiros de fixação mas desenrolar semântica não está habilitada. Considere o uso de /EHa|
...
|[Compilador C4867 de aviso (erro)](compiler-warning-c4867.md)|'function': chamada de função faltando lista de argumentos; Use 'chamar' para criar um ponteiro para membro|
...
|[Aviso do compilador (nível 1) C4917](compiler-warning-level-1-c4917.md)|'Declarador de ': um GUID só pode ser associado uma classe, interface ou namespace|
...
|[Aviso do compilador (nível 1) C4929](compiler-warning-level-1-c4929.md)|'arquivo': biblioteca de tipos contém uma união; Ignorando o qualificador 'embedded_idl'|
...
|[Aviso do compilador (nível 1) C4945](compiler-warning-level-1-c4945.md)|'symbol': não é possível importar símbolo de 'assembly1': como 'symbol' já foi importado de outro assembly 'assembly2'|
...
|Aviso do compilador (nível 1) C4951|'function' foi editado após os dados de perfil de função não usados coleta dados de perfil|
|Aviso do compilador (nível 1) C4952|'function': nenhum dado de perfil encontrado no banco de dados do programa 'pgd_file'|
|Aviso do compilador (nível 1) C4953|Inline 'function' foi editado após os dados de perfil não usados coleta dados de perfil|
...
|Aviso do compilador (nível 1, erro) C4956|'type': este tipo não é verificável|
|Aviso do compilador (nível 1, erro) C4957|'cast': conversão explícita de 'conversão de' para 'cast_to' não é verificável|
|Aviso do compilador (nível 1, erro) C4958|'operação de ': aritmética de ponteiro não é verificável|
|Aviso do compilador (nível 1, erro) C4959|não é possível definir 'type' do tipo não gerenciado em /CLR: safe porque o acesso aos seus membros produz código não verificável|
|Aviso do compilador (nível 4) C4960|'function' é muito grande para ser analisado|
...
|Aviso do compilador (nível 4) C4962|'function': otimizações guiadas por perfil desabilitadas porque otimizações fizeram dados de perfil se tornar inconsistente|
...
|[Aviso do compilador (nível 3) C4995](compiler-warning-level-3-c4995.md)|'function': nome foi marcado como #pragma preterido|
