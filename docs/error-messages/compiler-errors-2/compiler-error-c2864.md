...
Para inicializar um **`static`** membro de dados que é definido como **`volatile`** **`const`** um tipo, não, ou não integral, use uma instrução de definição de membro. Eles não podem ser inicializados em uma declaração.
