...
Não é possível restringir um tipo genérico a ambos **`value class`** e **`ref class`** .

Consulte [restrições em parâmetros de tipo genérico (C++/CLI)](../../extensions/constraints-on-generic-type-parameters-cpp-cli.md) para obter mais informações.
