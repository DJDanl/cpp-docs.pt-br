...
Para lidar com o `IColumnsInfo::GetColumnsInfo` chamar, exclua o PROVIDER_COLUMN_MAP, que define uma função `GetColumnInfo`, da `CAgentMan` registro do usuário no *personalizado*RS.h e substituí-lo com a definição para seu próprio `GetColumnInfo` função:  
