...
*piid*<br/>
Um ponteiro de ID de interface ( `IID*` ) para uma `IRowsetNotify` interface de ponto de conexão. O valor padrão é `&__uuidof(IRowsetNotify)`.

*DynamicUnkArray*<br/>
Uma matriz do tipo [CComDynamicUnkArray](../../atl/reference/ccomdynamicunkarray-class.md), que é uma matriz de ponteiros alocada dinamicamente `IUnknown` para as interfaces do coletor do cliente.
...
Transmite um evento [OnRowChange](/previous-versions/windows/desktop/ms722694(v=vs.85)) para todos os ouvintes no ponto de conexão `IID_IRowsetNotify` para notificar os consumidores de uma alteração que afeta as linhas.
...
Transmite um evento [OnRowsetChange](/previous-versions/windows/desktop/ms722669(v=vs.85)) para todos os ouvintes no ponto de conexão `IID_IRowsetNotify` para notificar os consumidores de uma alteração que afeta todo o conjunto de linhas.
