...
Todos os componentes COM derivam de `CComObjectRootEx` e `CComCoClass` . `CComObjectRootEx` fornece toda a implementação para a `IUnknown` interface. Ele pode lidar com qualquer modelo de Threading. `CComCoClass` manipula qualquer suporte de erro necessário. Se você quiser enviar informações de erro mais avançadas para o cliente, poderá usar algumas das APIs de erro no `CComCoClass` .

O objeto de fonte de dados também é herdado de várias classes ' impl '. Cada classe fornece a implementação para uma interface. O objeto de fonte de dados implementa as `IPersist` `IDBProperties` interfaces,, `IDBInitialize` e `IDBCreateSession` . Cada interface é exigida pelo OLE DB para implementar o objeto de fonte de dados. Você pode optar por dar suporte ou não a uma funcionalidade específica, herdando ou não herdando de uma dessas classes ' impl '. Se você quiser dar suporte à `IDBDataSourceAdmin` interface, herdará da `IDBDataSourceAdminImpl` classe para obter a funcionalidade necessária.

## <a name="com-map"></a>Mapa COM

Sempre que o cliente chama `QueryInterface` uma interface na fonte de dados, ele passa pelo seguinte mapa com:
...
As macros COM_INTERFACE_ENTRY são da ATL e dizem a implementação de `QueryInterface` em `CComObjectRootEx` para retornar as interfaces apropriadas.
...
Visual C++ oferece suporte a conjuntos de propriedades definidas pelo usuário. Você não precisa substituir `GetProperties` ou `GetPropertyInfo` . Em vez disso, os modelos detectam qualquer conjunto de propriedades definidas pelo usuário e o adicionam ao objeto apropriado.
...
[Arquivos gerados pelo assistente de provedor](../../data/oledb/provider-wizard-generated-files.md)<br/>
